...
- **`argv[0] == "autter"`** --> `commands::autter_handlers::handle_autter()` -- direct subcommands (checkpoint, blame, diff, status, search, etc.)
- **Debug-only shortcut**: When `cfg!(debug_assertions)` and `AUTTER=git` env var is set, forces git proxy mode regardless of binary name. This is how integration tests invoke the binary as a git proxy without symlinking.

### Core data flow: checkpoint --> working log --> authorship note

1. **Checkpoint**: An AI coding agent calls `autter checkpoint <agent>` with hook input (typically JSON via stdin) before AND after it edits a file. The corresponding agent preset (`src/commands/checkpoint_agent/agent_presets.rs`) extracts edited file paths, transcript, and model info. The checkpoint processor diffs the file against HEAD's version or the last-checkpointed value of that file and compute character-level attributions. The combination of pre and post file edit checkpoints is what allows us to know exactly what the AI changed (since we can compare the before and after). There are 3 main types of checkpoints in autter:
    * Plain or legacy `human`: only due to legacy, it's still called `human` as it used to mean "human" edited files, but since we migrated to an explicit Human checkpoint (now called `known_human`), this checkpoint represents 'untracked' changes. This is the checkpoint that AI agent presets invoke to take the before edit snapshots. Changes caught by these checkpoints do get explicit attestations in the final authorship notes (they are basically holes in the data) and stats recognize them as untracked. For testing, invoke by calling `autter checkpoint human` (for unscoped) or `autter checkpoint human /path/to/file` (for scoped).
    * Known human (`known_human`) checkpoints: this is the 'real' Human checkpoint. These are never called by the AI agent presets and are only invoked by our IDE/editor extensions that recognize when a change has actually been made by the human by typing, etc. For testing, invoke via `autter checkpoint mock_known_human` (for unscoped) or `autter checkpoint mock_known_human /path/to/file` (for scoped).
    * AI checkpoint (`ai_agent`) checkpoints: this is the AI checkpoint that explicitly associates the captured changes with the particular AI agent and session. This is the checkpoint taht AI agent presets invoke to take the after edit snapshots. For testing, invoke via `autter checkpoint mock_ai` (for unscoped) or `autter checkpoint mock_ai /path/to/file` (for scoped).
...
`Config` is a global `OnceLock` singleton accessed via `Config::get()`. It reads from `~/.autter/config.json`. In tests, `AUTTER_TEST_CONFIG_PATCH` env var allows overriding specific config fields without a real config file. Feature flags follow precedence: environment vars (`AUTTER_*` prefix via `envy`) > config file > defaults.
...
`AutterError` enum in `src/error.rs` -- not `thiserror`-based, uses manual `Display`/`From` impls. Variants: `GitCliError` (captures exit code + stderr + args), `IoError`, `JsonError`, `SqliteError`, `PresetError`, `Generic`, `GixError`.
...
- **`tests/repos/test_repo.rs`** -- `TestRepo` struct: creates temp git repos, runs autter commands as subprocess. Uses `get_binary_path()` which auto-compiles the binary via a `OnceLock`. Tests invoke the binary with `AUTTER=git` env var to trigger git proxy mode.
...
";
    ...
    repo.autter(&["checkpoint", "mock_known_human", "example.md"])
        .unwrap();
...
";
    ...
    repo.autter(&["checkpoint", "mock_ai", "example.md"])
        .unwrap();
    // Example of a completely untracked edit where we didn't fire a checkpoint call at all
...
";
    ...
    repo.autter(&["checkpoint", "human", "example.md"])
        .unwrap();
...
- Each `TestRepo` gets a random temp directory and a separate `AUTTER_TEST_DB_PATH`.
- `AUTTER_TEST_CONFIG_PATCH` env var passes `ConfigPatch` JSON to override config in subprocess.
- Background flush is skipped when `AUTTER_TEST_DB_PATH` is set (prevents race conditions on temp dir cleanup).
...
- **`debug_log()`** for conditional debug output: prints `[autter]` prefixed messages to stderr when `cfg!(debug_assertions)` or `AUTTER_DEBUG=1`. Set `AUTTER_DEBUG=0` to suppress in debug builds.
- **`AUTTER_DEBUG_PERFORMANCE=1`** (or `=2` for JSON) enables performance timing output.
...
- **`AUTTER_VERSION` constant** changes between debug/release/test modes via `cfg` attributes in `authorship_log_serialization.rs`.
...
- **Test binary auto-compilation**: Integration tests trigger `cargo build --bin autter` on first test run via `OnceLock`. If you change code and run tests, the test harness recompiles. This can cause confusion if you're debugging -- the test binary is always a debug build at `target/debug/autter`.

- **argv[0] dispatch is load-bearing**: The binary's behavior is entirely determined by how it's invoked. In production, symlinking as `git` makes it a proxy. In tests, `AUTTER=git` env var forces proxy mode (debug builds only). Breaking this dispatch breaks everything.
...
- **Working log base commit**: Working logs are keyed by the HEAD commit at checkpoint time (`.git/ai/working_logs/<sha>/`). Autter must ensure that HEAD changes update/copy over the working log accordingly.
