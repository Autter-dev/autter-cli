...
### Modified files:
- `src/commands/autter_handlers.rs` — rewrite `handle_checkpoint` to ~50 lines, update `synthesize_hook_input_from_cli_args`, delete ~600 lines of helpers
...
fn resolve_file_context(path: &Path) -> Result<(PathBuf, BaseCommit), AutterError> {
...
fn build_checkpoint_files(file_paths: &[PathBuf]) -> Result<Vec<CheckpointFile>, AutterError> {
...
```rust
fn execute_pre_file_edit(e: PreFileEdit) -> Result<CheckpointRequest, AutterError> {
...
fn execute_post_file_edit(
    e: PostFileEdit,
    preset_name: &str,
) -> Result<CheckpointRequest, AutterError> {
    let files = build_checkpoint_files(&e.file_paths)?;
...
```rust
fn execute_known_human_edit(e: KnownHumanEdit) -> Result<CheckpointRequest, AutterError> {
    let files = build_checkpoint_files(&e.file_paths)?;
...
```rust
fn execute_untracked_edit(e: UntrackedEdit) -> Result<CheckpointRequest, AutterError> {
    let files = build_checkpoint_files(&e.file_paths)?;
...
```rust
fn execute_pre_bash_call(_e: PreBashCall) -> Result<Option<CheckpointRequest>, AutterError> {
    // TODO: Task 8 will rewrite this to send BashSessionStart to daemon
    Ok(None)
}

fn execute_post_bash_call(e: PostBashCall) -> Result<CheckpointRequest, AutterError> {
...
Expect errors only from downstream consumers of `CheckpointRequest` (autter_handlers.rs, daemon.rs, checkpoint.rs), not from orchestrator.rs itself.
...
**Files:**
- Modify: `src/commands/autter_handlers.rs:294-870+`

- [ ] **Step 1: Update synthesize_hook_input_from_cli_args to resolve absolute paths**

In `src/commands/autter_handlers.rs`, find `synthesize_hook_input_from_cli_args` (line 1719). Update the `"human" | "mock_ai" | "mock_known_human"` arm to resolve relative paths to absolute:
...
fn handle_checkpoint(args: &[String]) {
    ...
    let daemon_config = if is_test
        && (std::env::var_os("AUTTER_DAEMON_HOME").is_some()
            || std::env::var_os("AUTTER_DAEMON_CONTROL_SOCKET").is_some())
    {
...
Delete these functions from `autter_handlers.rs` (search for `fn` to find exact locations):
...
- [ ] **Step 4: Verify compilation of autter_handlers.rs**
...
pub fn handle_bash_pre_tool_use_with_context(
...
) -> Result<BashToolResult, AutterError> {
    ...
    let daemon_config = crate::daemon::DaemonConfig::from_env_or_default_paths()
        .map_err(|e| AutterError::Generic(e))?;
...
HookEvent::PostToolUse => {
    ...
    let pre_snapshot = if let Some(data) = response.data {
        let snap_response: BashSnapshotQueryResponse =
            serde_json::from_value(data).map_err(AutterError::JsonError)?;
...
```rust
fn execute_pre_bash_call(e: PreBashCall) -> Result<Option<CheckpointRequest>, AutterError> {
