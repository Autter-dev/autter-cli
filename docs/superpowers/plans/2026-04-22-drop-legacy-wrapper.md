...
- **Keep `log_performance_for_checkpoint`**: This function in `wrapper_performance_targets.rs` is used by `autter_handlers.rs` for checkpoint timing. Only remove `log_performance_target_if_violated` and `BenchmarkResult`.
...
Remove `async_mode` assertions from `test_default_feature_flags`, remove `async_mode` fields from all test structs (e.g. `test_from_env_and_file_file_overrides`, `test_serialization`, `test_clone_trait`), and remove `AUTTER_ASYNC_MODE` env var cleanup from serial tests.
...
In the `proxy_to_git` function, remove the `env_remove("AUTTER_ASYNC_MODE")` calls (lines 858 and 892) since the env var no longer exists.
...
### Task 3: Remove `async_mode` checks from `autter_handlers.rs`, `install_hooks.rs`, `utils.rs`
...
- [ ] **Step 1: Simplify `autter_handlers.rs` — always init daemon telemetry**
...
In `checkpoint_delegation_enabled()` (lines 55-67), remove the `async_mode` check. The function now just checks the `AUTTER_DAEMON_CHECKPOINT_DELEGATE` env var fallback. Or if the intent is "always true when async_mode is on", simplify to always return true with the env var as an override.
...
```bash
git add src/commands/autter_handlers.rs src/commands/install_hooks.rs src/utils.rs
git commit -m "refactor: remove async_mode conditionals from autter_handlers, install_hooks, utils"
```
...
Since this is no longer wrapper-specific, rename from `wrapper_performance_targets.rs` to `performance_targets.rs`. Update the `pub mod` in `src/observability/mod.rs` and all import sites (`src/commands/autter_handlers.rs` line 20, `tests/integration/repos/test_repo.rs` line 14, `tests/integration/performance.rs` line 12).
...
The first test `async_mode_wrapper_commit_passthrough_skips_autter_side_effects` is specifically testing the old wrapper+async_mode behavior — this test is entirely dead and should be deleted.
...
Remove any `async_mode` fields from `ConfigPatch`, `FileConfig`, and related serialization/deserialization. Remove the `AUTTER_ASYNC_MODE` env var handling anywhere it appears.
