...
pub fn resolve_preset(name: &str) -> Result<Box<dyn AgentPreset>, AutterError> {
    match name {
...
impl AgentPreset for ClaudePreset {
    fn parse(&self, _hook_input: &str, _trace_id: &str) -> Result<Vec<ParsedHookEvent>, AutterError> {
        Err(AutterError::PresetError("Not yet implemented".to_string()))
    }
}
```
...
pub fn required_str<'a>(data: &'a Value, key: &str) -> Result<&'a str, AutterError> {
    data.get(key)
        .and_then(|v| v.as_str())
        .ok_or_else(|| AutterError::PresetError(format!("{} not found in hook_input", key)))
}
...
pub fn required_file_stem(data: &Value, path_key: &str) -> Result<String, AutterError> {
    ...
    Path::new(path_str)
        ...
        .map(|s| s.to_string())
        .ok_or_else(|| {
            AutterError::PresetError(format!("Could not extract file stem from {}", path_key))
        })
}
...
pub fn execute_preset_checkpoint(
    preset_name: &str,
    hook_input: &str,
) -> Result<Vec<CheckpointResult>, AutterError> {
...
fn resolve_repo_working_dir_from_file_paths(file_paths: &[PathBuf]) -> Result<PathBuf, AutterError> {
    let first_path = file_paths.first().ok_or_else(|| {
        AutterError::PresetError("No file paths provided for repo discovery".to_string())
    })?;
    let repo = find_repository_for_file(&first_path.to_string_lossy(), None)?;
    repo.workdir()
        .map(|p| PathBuf::from(p))
        .map_err(|e| AutterError::Generic(format!("Failed to get workdir: {}", e)))
}

fn resolve_repo_working_dir_from_cwd(cwd: &Path) -> Result<PathBuf, AutterError> {
...
fn execute_event(event: ParsedHookEvent) -> Result<CheckpointResult, AutterError> {
    match event {
...
fn execute_pre_file_edit(e: PreFileEdit) -> Result<CheckpointResult, AutterError> {
...
fn execute_post_file_edit(e: PostFileEdit) -> Result<CheckpointResult, AutterError> {
    let repo_working_dir = if !e.file_paths.is_empty() {
        resolve_repo_working_dir_from_file_paths(&e.file_paths)?
    } else {
        resolve_repo_working_dir_from_cwd(&e.context.cwd)?
...
fn execute_pre_bash_call(e: PreBashCall) -> Result<CheckpointResult, AutterError> {
...
fn execute_post_bash_call(e: PostBashCall) -> Result<CheckpointResult, AutterError> {
    let repo_working_dir = resolve_repo_working_dir_from_cwd(&e.context.cwd)?;
...
pub fn read_transcript(source: &TranscriptSource) -> Result<(AiTranscript, Option<String>), AutterError> {
    match source {
...
fn read_from_path(path: &Path, format: TranscriptFormat) -> Result<(AiTranscript, Option<String>), AutterError> {
    match format {
...
impl AgentPreset for ClaudePreset {
    fn parse(&self, hook_input: &str, trace_id: &str) -> Result<Vec<ParsedHookEvent>, AutterError> {
        ...
        if Self::is_vscode_copilot_hook_payload(&data) {
            return Err(AutterError::PresetError(
...
impl CodexPreset {
    fn session_id_from_hook_data(data: &serde_json::Value) -> Result<String, AutterError> {
        parse::optional_str_multi(data, &["session_id", "thread_id"])
            .map(|s| s.to_string())
            .ok_or_else(|| {
                AutterError::PresetError(
...
impl AgentPreset for CodexPreset {
    fn parse(&self, hook_input: &str, trace_id: &str) -> Result<Vec<ParsedHookEvent>, AutterError> {
        ...
        if !is_bash {
            return Err(AutterError::PresetError(format!(
...
impl AgentPreset for WindsurfPreset {
    fn parse(&self, hook_input: &str, trace_id: &str) -> Result<Vec<ParsedHookEvent>, AutterError> {
        ...
        let cwd = tool_info
            ...
            .ok_or_else(|| AutterError::PresetError("cwd not found in hook_input".to_string()))?;
...
impl GithubCopilotPreset {
    fn parse_legacy(
        ...
        trace_id: &str,
    ) -> Result<Vec<ParsedHookEvent>, AutterError> {
        ...
        let cwd = parse::optional_str_multi(data, &["workspace_folder", "workspaceFolder"])
            .ok_or_else(|| {
                AutterError::PresetError("workspace_folder not found in hook_input".to_string())
            })?;
    ...
    fn parse_vscode_native(
        ...
        let cwd = parse::optional_str_multi(data, &["cwd", "workspace_folder", "workspaceFolder"])
            .ok_or_else(|| {
                AutterError::PresetError("cwd not found in hook_input".to_string())
            })?;
...
impl AgentPreset for AgentV1Preset {
    fn parse(&self, hook_input: &str, trace_id: &str) -> Result<Vec<ParsedHookEvent>, AutterError> {
        let payload: AgentV1Payload = serde_json::from_str(hook_input)
            .map_err(|e| AutterError::PresetError(format!("Invalid agent-v1 JSON: {}", e)))?;
...
**Files:**
- Modify: `src/commands/autter_handlers.rs`

- [ ] **Step 1: Replace the 14-arm preset dispatch with orchestrator call**

In `src/commands/autter_handlers.rs`, replace the match block at lines ~414-634 (where each preset is individually matched and `.run()` is called) with:
...
```bash
git add src/commands/autter_handlers.rs
git add src/commands/checkpoint_agent/orchestrator.rs
...
pub fn run(
    ...
    is_pre_commit: bool,
) -> Result<(usize, usize, usize), AutterError> {
```
...
- `autter_handlers.rs`: `run_checkpoint_via_daemon_or_local()` passes `CheckpointResult` instead of `AgentRunResult`
...
```bash
git add src/daemon/control_api.rs src/daemon.rs src/commands/checkpoint.rs src/commands/autter_handlers.rs
...
) -> Result<BashPreHookResult, AutterError> {
...
- `src/commands/autter_handlers.rs` imports
...
```rust
fn build_scoped_human_checkpoint_result(&self) -> Result<Option<CheckpointResult>, AutterError> {
