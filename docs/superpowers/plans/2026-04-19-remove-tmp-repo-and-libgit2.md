...
**Architecture:** Each `#[cfg(test)]` module that uses TmpRepo gets migrated to a new integration test file. Tests that call internal functions directly will be rewritten to exercise the same behavior through the autter CLI/wrapper using TestRepo's `git()`, `autter()`, `commit()`, and file manipulation APIs. After all tests are migrated, TmpRepo and all git2/libgit2 references are deleted.
...
| `repo.trigger_checkpoint_with_author("user")` | `repo.autter(&["checkpoint", "mock_known_human", "f.txt"])` |
| `repo.trigger_checkpoint_with_ai("Claude", ...)` | `repo.autter(&["checkpoint", "mock_ai", "f.txt"])` |
...
| `repo.blame_for_file(&f, None)` | `repo.autter(&["blame", "f.txt"])` then parse output |
| `repo.stage_file("f.txt")` | `repo.git(&["add", "f.txt"])` |
| `repo.current_branch()` | `repo.current_branch()` |
| `repo.autter_repo()` | Not available — test through CLI instead |
| `repo.path()` | `repo.path()` |
...
For tests that call internal Rust functions (e.g., `post_commit()`, `checkpoint()`, `refs::notes_add()`), rewrite them to achieve the same effect through autter CLI commands and verify outcomes through autter's external interfaces (blame output, authorship notes, stats).
...
fn test_pre_commit_checkpoint_context_uses_inflight_bash_agent_context() {
    ...
    repo.git(&["add", "test.txt"]).unwrap();
    repo.autter(&["checkpoint", "mock_ai", "test.txt"]).unwrap();
...
fn test_save_stash_note_roundtrip() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "file.txt"]).unwrap();
...
fn test_save_stash_note_large_content() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "large.txt"]).unwrap();
...
These tests verify `autter status` output with various pathspec and ignore pattern configurations.
...
fn test_get_working_dir_diff_stats_post_filter_equivalence() {
    ...
    let output = repo.autter(&["status"]).unwrap();
...
fn test_get_working_dir_diff_stats_respects_ignore_patterns() {
    ...
    repo.patch_autter_config(|config| {
...
fn test_count_ai_lines_from_initial_respects_ignore_patterns() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "src/main.rs"]).unwrap();

    repo.filename("Cargo.lock").set_contents(vec!["lock", "ai_lock_line"]);
    repo.git(&["add", "Cargo.lock"]).unwrap();
    repo.autter(&["checkpoint", "mock_ai", "Cargo.lock"]).unwrap();
...
These tests verify git notes operations (add, show, batch, grep, merge, copy). Through TestRepo, we create commits, write authorship notes via `autter checkpoint` + `commit`, and verify them via `read_authorship_note()` or git-notes CLI.
...
Range authorship is exercised through `autter stats --range` or CI commands. Tests verify stats output for commit ranges.
...
These tests verify that `.gitattributes` and `.autter-ignore` file parsing works correctly. Through TestRepo, create files and verify ignore behavior through stats output.
...
Prompt finding is exercised through `autter show-prompt` command.
...
Checkpoint tests exercise `autter checkpoint` CLI command and verify working logs and authorship notes.
...
```toml
# Before:
autter = { path = ".", features = ["test-support"] }
...
```rust
#[cfg(feature = "test-support")]
impl From<git2::Error> for AutterError {
    fn from(err: git2::Error) -> Self {
        AutterError::GitError(err)
    }
}
```
