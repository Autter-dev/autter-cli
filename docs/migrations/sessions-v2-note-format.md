...
This document describes several major, though strictly non-breaking, changes to the `refs/notes/ai` attribution note format for the Autter community landing in v1.4.0.

## Preamble

We intend for the notes format to be as stable as possible and we do not take these changes lightly. We understand that many in the community rely on Autter data, and stability is a huge part of our contract with the community.
...
Existing parsers will NOT error out and the Autter CLI will continue to support the old format indefinitely.

However, existing parsers should be updated as soon as practical to ensure that they are able to understand the new properties and data that Autter will report.
