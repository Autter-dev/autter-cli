{
  description = "autter - AI-powered Git tracking and intelligence for code repositories";
  ...
  outputs = { self, nixpkgs, rust-overlay, flake-utils }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # Build the autter binary using the pinned Rust toolchain
        autter-unwrapped = rustPlatform.buildRustPackage {
          pname = "autter";
          version = "1.5.9";
        ...
        # Wrapped version that sets up the autter environment properly
        autter-wrapped = pkgs.writeShellScriptBin "autter" ''
          # Ensure config directory exists
          mkdir -p "$HOME/.autter"

          # Create config.json if it doesn't exist
          if [ ! -f "$HOME/.autter/config.json" ]; then
            # Find the system git (not our wrapper)
            GIT_PATH="${pkgs.git}/bin/git"
            cat > "$HOME/.autter/config.json" <<EOF
          {
            "git_path": "$GIT_PATH"
          }
          EOF
          ...
          # Execute autter with all arguments
          exec ${autter-unwrapped}/bin/autter "$@"
        '';

        # Wrapper for git command that preserves argv[0] as "git"
        # This is critical: when symlinked as "git", the wrapper must set argv[0]
        # to "git" so the Rust binary routes to handle_git() instead of handle_autter()
        git-wrapper = pkgs.writeShellScriptBin "git" ''
          ...
          # Execute autter with argv[0] set to "git" to trigger passthrough mode
          # The -a flag ensures argv[0] is "git" regardless of the actual binary path
          exec -a git ${autter-unwrapped}/bin/autter "$@"
        '';

        # Create git-og wrapper that bypasses autter and calls real git directly
        ...
        autter-minimal = pkgs.symlinkJoin {
          name = "autter-minimal-${autter-unwrapped.version}";
          paths = [ autter-wrapped autter-unwrapped git-og ];
          ...
          meta = autter-unwrapped.meta // {
            description = autter-unwrapped.meta.description + " (without git wrapper)";
          };
        };
        ...
        autter-package = pkgs.symlinkJoin {
          name = "autter-${autter-unwrapped.version}";
          paths = [ autter-wrapped git-wrapper autter-unwrapped git-og ];
          ...
          meta = autter-unwrapped.meta // {
            description = autter-unwrapped.meta.description + " (with git wrapper)";
          };
        };
      ...
      in
      {
        ...
        devShells.default = pkgs.mkShell {
          packages = [
            ...
            # shellHook creates wrapper scripts (git, autter, git-og) that
            # point to the locally-built target/debug/autter binary, so that
            # development builds are tested directly. Use `git-og` to bypass
            # autter and call real git.
            sqlite
          ...
          shellHook = ''
            ...
            # Set up development autter wrappers for nix develop (Nix-specific; non-Nix devs use scripts/dev.sh)
            BUILD_TYPE="''${AUTTER_BUILD_TYPE:-debug}"
            GITWRAP_DIR="$HOME/.autter-local-dev/gitwrap/bin"
            TARGET_DIR="''${CARGO_TARGET_DIR:-$(pwd)/target}"
            BINARY="$TARGET_DIR/$BUILD_TYPE/autter"
...
#!/bin/bash
if [ ! -x "$BINARY" ]; then
  echo "autter: dev binary not found at $BINARY" >&2
...
GITEOF
            ...
            # Create autter wrapper
            cat > "$GITWRAP_DIR/autter" <<AUTTEREOF
#!/bin/bash
if [ ! -x "$BINARY" ]; then
  echo "autter: dev binary not found at $BINARY" >&2
  echo "Run 'cargo build' first, then retry." >&2
...
exec "$BINARY" "\$@"
AUTTEREOF
            chmod +x "$GITWRAP_DIR/autter"

            # Create git-og wrapper (bypasses autter, calls real git directly)
...
GITOGEOF
            ...
            if [ -x "$BINARY" ]; then
              "$GITWRAP_DIR/autter" install-hooks 2>/dev/null || true
            fi

            # Install lefthook git hooks (use real git, not the autter wrapper,
            ...
            echo "autter development environment"
            ...
            echo ""
            echo "git, autter, git-og -> wrappers in $GITWRAP_DIR"
            echo "Set AUTTER_BUILD_TYPE=release for release builds."
          '';
        };

        # Main packages
        packages = {
          # Unwrapped binary (just the autter executable)
          unwrapped = autter-unwrapped;

          # Wrapped version with helper scripts
          wrapped = autter-wrapped;

          # Minimal package without git symlink (for Home Manager/environments with existing git)
          minimal = autter-minimal;

          # Complete package with git/git-og symlinks (for standalone use)
          default = autter-package;

          # Alias for clarity
          autter = autter-package;
        };
        ...
        checks =
          let
            ...
            mkCheck = attrs: rustPlatform.buildRustPackage ({
              version = autter-unwrapped.version;
              src = ./.;
      ...
      overlays.default = final: prev: {
        autter = self.packages.${prev.stdenv.hostPlatform.system}.default;
        autter-unwrapped = self.packages.${prev.stdenv.hostPlatform.system}.unwrapped;
      };
      ...
      nixosModules.default = { config, lib, pkgs, ... }:
        ...
        let
          ...
          configJsonFile = jsonFormat.generate "autter-config.json" configFile;
        in
        {
          options.programs.autter = {
            enable = mkEnableOption "autter - AI-powered Git tracking";

            package = mkOption {
              type = types.package;
              default = self.packages.${pkgs.stdenv.hostPlatform.system}.default;
              defaultText = literalExpression "inputs.autter.packages.\${pkgs.system}.default";
              description = "The autter package to use.";
            };
            ...
            installHooks = mkOption {
              ...
              default = true;
              description = ''
                Whether to run 'autter install-hooks' on system activation.
            ...
            setGitAlias = mkOption {
              ...
              default = true;
              description = ''
                Whether to make 'git' command use autter wrapper.
                When enabled, autter is placed before regular git in PATH.
            ...
            settings = {
              ...
              apiKey = mkOption {
                ...
                default = null;
                description = ''
                  The autter API key as a plain string. Sets AUTTER_API_KEY at
              ...
              apiKeyFile = mkOption {
                type = types.nullOr types.str;
                default = null;
                example = "$HOME/.secrets/autter-api-key";
                description = ''
                  Path to a file containing the autter API key. The file is read
              ...
              apiKeyCommand = mkOption {
                type = types.nullOr types.str;
                default = null;
                example = "pass show autter/api-key";
                description = ''
                  Shell command that prints the autter API key to stdout. Run at
                  shell startup to set AUTTER_API_KEY. The command string is stored
              ...
              apiBaseUrl = mkOption {
                ...
                default = null;
                description = ''
                  API base URL for autter services.
                  Defaults to "https://autter.dev" if not specified.
                '';
              };
              ...
              excludeRepositories = mkOption {
                ...
                example = [ "https://github.com/private/*" ];
                description = ''
                  List of repository URL patterns (globs) to exclude from autter tracking.
              ...
              featureFlags = {
                ...
                gitHooksEnabled = mkOption {
                  ...
                  default = null;
                  description = ''
                    Enable git hooks integration for autter tracking.
                  '';
                };
                ...
                gitHooksExternallyManaged = mkOption {
                  ...
                  description = ''
                    ...
                    (e.g., by lefthook or husky). When enabled, autter will not
          ...
          config = mkIf cfg.enable {
            ...
            # Set AUTTER_API_KEY at shell startup. Precedence: command > file > key.
            environment.interactiveShellInit =
              let
                snippet =
                  if cfg.settings.apiKeyCommand != null then
                    ''export AUTTER_API_KEY="$(${cfg.settings.apiKeyCommand})"''
                  else if cfg.settings.apiKeyFile != null then
                    ''export AUTTER_API_KEY="$(cat "${cfg.settings.apiKeyFile}")"''
                  else if cfg.settings.apiKey != null then
                    ''export AUTTER_API_KEY="${cfg.settings.apiKey}"''
                  else "";
              in
              mkIf (snippet != "") snippet;

            # Set up system-wide configuration on activation
            system.activationScripts.autter = mkIf cfg.installHooks (
              stringAfter [ "users" ] ''
                ...
                for user_home in /home/* /Users/* /root; do
                  if [ -d "$user_home" ]; then
                    ...
                    # Create config directory
                    mkdir -p "$user_home/.autter"
                    chown "$user" "$user_home/.autter" 2>/dev/null || true
                    ...
                    if [ ! -f "$user_home/.autter/config.json" ] || [ -L "$user_home/.autter/config.json" ]; then
                      cp -f ${configJsonFile} "$user_home/.autter/config.json"
                      chmod 644 "$user_home/.autter/config.json"
                      chown "$user" "$user_home/.autter/config.json" 2>/dev/null || true
                    fi

                    # Install hooks (run as user if possible)
                    if command -v sudo >/dev/null 2>&1 && [ "$user" != "root" ]; then
                      sudo -u "$user" ${cfg.package}/bin/autter install-hooks 2>/dev/null || true
                    else
                      ${cfg.package}/bin/autter install-hooks 2>/dev/null || true
                    fi
                  fi
                done
              ''
        ...
        in
        {
          options.programs.autter = {
            ...
            installHooks = mkOption {
              ...
              default = true;
              description = ''
                Whether to run 'autter install-hooks' on activation.
                This sets up IDE and agent integration hooks.
              '';
            };
          ...
          config = mkIf cfg.enable {
            # Add autter to user packages
            home.packages = [ cfg.package ];

            # Config file contains no secrets; served directly from the Nix store.
            home.file.".autter/config.json" = {
              source = jsonFormat.generate "autter-config.json" configFile;
            };
            ...
            home.activation.autter-install-hooks = mkIf cfg.installHooks (
              lib.hm.dag.entryAfter [ "writeBoundary" ] ''
                $DRY_RUN_CMD ${cfg.package}/bin/autter install-hooks || true
              ''
            );
          };
        };
