...
class TelemetryService : Disposable {
    ...
    companion object {
        ...
        private const val PLUGIN_ID = "com.useautter.plugins.jetbrains"
        ...
        private const val PLUGIN_EVENT_TAG = "autter_plugin_event"
    ...
    private fun isOssTelemetryDisabled(): Boolean {
        return try {
            ...
            val configFile = File(homeDir, ".autter/config.json")
    ...
    private fun readDistinctId(): String? {
        return try {
            val homeDir = System.getProperty("user.home")
            val distinctIdFile = File(homeDir, ".autter/internal/distinct_id")
    ...
    /**
     * Reports that autter CLI was not found (simple version for backwards compatibility).
     */
    fun reportAutterNotFound() {
        reportAutterNotFound(null, null, emptyList(), null)
    }

    /**
     * Reports that autter CLI was not found with detailed context.
     */
    fun reportAutterNotFound(
        exitCode: Int?,
        output: String?,
        ...
        try {
            val context = mutableMapOf<String, Any>(
                "error_type" to "autter_not_found"
            )
            ...
            captureEvent("autter_error", getCommonProperties() + context)

            val message = buildString {
                append("autter CLI not found")
    ...
    /**
     * Reports a version mismatch where autter is below minimum required version.
     */
    fun reportVersionMismatch(foundVersion: String, requiredVersion: String) {
        try {
            ...
            captureSentryMessage(
                "autter version mismatch: found $foundVersion, required $requiredVersion",
                SentryLevel.WARNING,
                context
            )
        } catch (_: Throwable) {
    ...
    fun reportCheckpointFailure(exitCode: Int, output: String) {
        try {
            ...
            captureEvent("autter_error", getCommonProperties() + context)
            captureSentryMessage(
                "autter checkpoint failed with exit code $exitCode",
                SentryLevel.ERROR,
                context
            )
        } catch (_: Throwable) {
    ...
    fun reportCheckpointTimeout() {
        try {
            ...
            captureEvent("autter_error", getCommonProperties() + context)
            captureSentryMessage("autter checkpoint timed out after 30 seconds", SentryLevel.ERROR, context)
        } catch (_: Throwable) {
            // Non-critical – never surface to user
        }
    }
    ...
    fun captureError(throwable: Throwable, context: Map<String, String> = emptyMap()) {
        try {
            ...
            captureEvent("autter_error", getCommonProperties() + eventContext)
