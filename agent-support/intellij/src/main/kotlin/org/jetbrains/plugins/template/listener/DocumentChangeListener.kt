...
import org.jetbrains.plugins.template.services.AutterService
...
/**
 * Listens for document changes and triggers autter checkpoints when AI agents
...
class DocumentChangeListener(
    ...
    private fun triggerBeforeEditCheckpoint(
        ...
        // Convert absolute path to relative path for autter
        ...
        scheduler.execute {
            AutterService.getInstance().checkpoint(input, workspaceRoot)
        }
    }
    ...
    private fun executeAfterEditCheckpoint(filePath: String) {
        ...
        val input = AgentV1Input.AiAgent(
            ...
            conversationId = AutterService.getInstance().sessionId,
        ...
        AutterService.getInstance().checkpoint(input, pending.workspaceRoot)
    }
