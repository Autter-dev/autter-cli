...
export class AITabEditManager {
  ...
  enableIfSupported(): boolean {
    if (this.isSupportedIDEHost()) {
      console.log(`[autter] Enabling AI tab detection for ${this.ideHostConfig.kind}`);
      this.registration = this.registerOverride();
      return true;
    }
    console.log(`[autter] AI tab detection not supported for ${this.ideHostConfig.kind}`);
    return false;
  }

  handleDocumentContentChangeEvent(event: vscode.TextDocumentChangeEvent): void {
    console.log('[autter] Document content change event', event);
  ...
  beforeHook(args: any[]) {
    ...
    console.log('[autter] before ai tab completion accepted', args);
  ...
  async afterHook(result: unknown) {
    console.log('[autter] after ai tab completion accepted', result);
    const last = this.lastDocumentChangeEvent;
    if (!last) {
      console.log('[autter] No last document change event to inspect');
      return;
    }
    if (!this.beforeCompletionFileStates) {
      console.log('[autter] No before completion file states to inspect');
      return;
    }
    ...
    if (!beforeContent) {
      console.log('[autter] No before content found for', last.document.uri.fsPath);
      return;
    }
