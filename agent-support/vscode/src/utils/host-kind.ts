...
function loadVscodeIdeNames(): string[] {
  try {
    const filePath = path.join(os.homedir(), ".autter", "VSCODE_IDE_NAMES");
...
export function detectIDEHost(): IDEHostConfiguration {
  ...
  if (kind === "unknown") {
    ...
    if (vscodeIdeNames.length > 0) {
      console.log('[autter] Checking for VSCode-like IDE: ', vscodeIdeNames);
      if (vscodeIdeNames.some(has)) {
        kind = "vscode";
        console.log('[autter] Found VSCode-like IDE: ', kind);
      }
    }
  } else {
    console.log('[autter] Recognized IDE: ', kind);
  }
