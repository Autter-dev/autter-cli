...
function getDistinctId(): string {
  try {
    const idPath = path.join(os.homedir(), ".autter", "internal", "distinct_id");
...
function isTelemetryOssDisabled(): boolean {
  try {
    const configPath = path.join(os.homedir(), ".autter", "config.json");
...
export function activate(context: vscode.ExtensionContext) {
  ...
  // In dev mode, resolve autter binary via login shell (debug host has stripped PATH)
  ...
  if (Config.isAiTabTrackingEnabled()) {
    ...
    if (aiTabTrackingEnabled) {
      console.log('[autter] Tracking document content changes for AI tab completion detection');
      vscode.window.showInformationMessage('autter: AI tab tracking is enabled (experimental)');
      context.subscriptions.push(
  ...
  if (ideHostCfg.kind === IDEHostKindVSCode) {
    if (aiEditManager.areLegacyCopilotHooksEnabled()) {
      console.log('[autter] Using VS Code/Copilot legacy extension detection strategy');
...
export function deactivate() {
  console.log('[autter] extension deactivated');
}
