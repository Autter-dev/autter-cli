...
export class KnownHumanCheckpointManager {
  ...
  public handleSaveEvent(doc: vscode.TextDocument): void {
    ...
    if (this.isInternalVSCodePath(filePath)) {
      console.log("[autter] KnownHumanCheckpointManager: Ignoring internal VSCode file:", filePath);
      return;
    }

    const repoRoot = getGitRepoRoot(doc.uri);
    if (!repoRoot) {
      console.log("[autter] KnownHumanCheckpointManager: No git repo found for", filePath, "- skipping");
      return;
    }
    ...
    const timer = setTimeout(() => {
      this.executeCheckpoint(repoRoot).catch((err) =>
        console.error("[autter] KnownHumanCheckpointManager: Checkpoint error:", err)
      );
    }, this.debounceMs);

    this.pendingTimers.set(repoRoot, timer);
    console.log("[autter] KnownHumanCheckpointManager: Save queued for", filePath);
  }
  ...
  private async executeCheckpoint(repoRoot: string): Promise<void> {
    ...
    for (const absolutePath of snapshot) {
      ...
      if (doc) {
        ...
        try {
          ...
          console.error("[autter] KnownHumanCheckpointManager: Failed to read file", absolutePath, err);
        }
      }
    ...
    console.log("[autter] KnownHumanCheckpointManager: Firing known_human checkpoint for", editedFilepaths);

    const proc = spawn(getAutterBinary(), ["checkpoint", "known_human", "--hook-input", "stdin"], {
      cwd: repoRoot,
    });
    ...
    proc.on("close", (code) => {
      if (code !== 0) {
        console.error("[autter] KnownHumanCheckpointManager: Checkpoint exited with code", code, stdout, stderr);
      } else {
        console.log("[autter] KnownHumanCheckpointManager: Checkpoint succeeded", stdout.trim());
      }
    });
