...
import { resolveAutterBinary } from "./utils/binary-path";
...
export class BlameLensManager {
  ...
  private static readonly VIRTUAL_SCHEME = 'autter-blame';
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    this.statusBarItem.name = 'autter';
    this.statusBarItem.command = 'autter.toggleAICode';
  ...
  public activate(): void {
    ...
    this.context.subscriptions.push(
      vscode.commands.registerCommand('autter.showCommitDiff', async (commitSha: string, workspacePath: string) => {
    ...
    this.context.subscriptions.push(
      vscode.commands.registerCommand('autter.toggleAICode', () => {
    ...
    this.context.subscriptions.push(
      vscode.workspace.onDidChangeConfiguration((event) => {
        ...
        if (event.affectsConfiguration('workbench.colorCustomizations')) {
          console.log('[autter] Color customizations changed, rebuilding color decorations');
          this.rebuildColorDecorations();
        }
      })
    );
    ...
    this.context.subscriptions.push(
      vscode.window.onDidChangeActiveColorTheme(() => {
        console.log('[autter] Theme changed, rebuilding color decorations');
        this.rebuildColorDecorations();
      })
    );
    ...
    console.log('[autter] BlameLensManager activated');

    // Resolve autter binary path early (uses login shell to get full user PATH)
    resolveAutterBinary().then((path) => {
      if (path) {
        const { execFile } = require('child_process');
        execFile(path, ['--version'], (err: Error | null, stdout: string) => {
          if (!err) {
            console.log('[autter] Version:', stdout.trim());
          }
        });
      }
    });
  ...
  private handleDocumentSave(document: vscode.TextDocument): void {
    ...
    console.log('[autter] Document saved, invalidated blame cache for:', document.uri.fsPath);
  }
  ...
  private handleDocumentChange(event: vscode.TextDocumentChangeEvent): void {
    ...
    // Debounce the refresh to avoid hammering autter on every keystroke
  ...
  private handleDocumentClose(document: vscode.TextDocument): void {
    ...
    console.log('[autter] Document closed, cancelled blame for:', document.uri.fsPath);
  }
  ...
  private async showBlameModeQuickPick(): Promise<void> {
    ...
    const selected = await vscode.window.showQuickPick(items, {
      placeHolder: 'Select Autter Blame Mode',
      title: 'Autter Blame Mode',
    });
  ...
  private async applyBlameMode(newMode: BlameMode): Promise<void> {
    ...
    console.log(`[autter] Blame mode changed to: ${newMode}`);
  }
  ...
  private handleBlameModeChange(): void {
    ...
    console.log(`[autter] Blame mode changed to: ${newMode} via settings`);
  }
  ...
  private async requestBlameForFullFile(editor: vscode.TextEditor): Promise<void> {
    ...
    try {
      ...
      console.error('[autter] Blame request failed:', error);
      this.pendingBlameRequest = null;
    }
  }
    ...
    if (this.currentDocumentUri !== documentUri || !this.currentBlameResult) {
      ...
      if (!this.pendingBlameRequest) {
        ...
        this.pendingBlameRequest.then(result => {
          ...
          console.error('[autter] Failed to get blame for status bar:', error);
          this.pendingBlameRequest = null;
        });
      }
  ...
  private rebuildColorDecorations(): void {
    ...
    if (this.filteredColors.length === 0) {
      console.log('[autter] All colors filtered out by contrast check, using full palette');
      this.filteredColors = [...this.HUNK_COLORS];
    } else {
      console.log(`[autter] Filtered colors: ${this.filteredColors.length}/${this.HUNK_COLORS.length} colors have sufficient contrast`);
    }
  ...
  private buildHoverContent(lineInfo: LineBlameInfo | undefined, documentUri?: vscode.Uri, blameResult?: BlameResult): vscode.MarkdownString {
    ...
    // ═══════════════════════════════════════════════════════════════
    if (modelDisplay) {
      md.appendMarkdown(`<span style="color:#a78bfa;">**${humanName}**</span> with <span style="color:#60a5fa;">**${modelDisplay}**</span> in <span style="color:#f472b6;">**${toolCapitalized}**</span> · <span style="color:#94a3b8;">*powered by Autter*</span>\n\n`);
    } else {
      md.appendMarkdown(`<span style="color:#a78bfa;">**${humanName}**</span> with <span style="color:#f472b6;">**${toolCapitalized}**</span> · <span style="color:#94a3b8;">*powered by Autter*</span>\n\n`);
    }
    md.appendMarkdown(`---\n\n`);
    ...
    if (!hasMessages) {
      ...
      if (hasMessagesUrl) {
        ...
        if (isDifferentUser) {
          ...
          md.appendCodeblock('autter login', 'bash');
        } else {
    ...
    if (commits && commits.length > 0) {
      ...
      for (const commitSha of commits) {
        ...
        if (workspaceFolder) {
          ...
          md.appendMarkdown(`- \`${shortSha}\` [view diff](command:autter.showCommitDiff?${commandArgs})\n`);
        } else {
  ...
  private async showCommitDiff(commitSha: string, workspacePath: string): Promise<void> {
    ...
    try {
      ...
      console.error('[autter] Failed to open commit diff:', error);
  ...
  /**
   * Get the workspace cwd for running autter commands against a document.
   */
...
export function registerBlameLensCommands(context: vscode.ExtensionContext): void {
  context.subscriptions.push(
    vscode.commands.registerCommand('autter.viewAuthor', (lineNumber: number) => {
