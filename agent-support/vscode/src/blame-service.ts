...
// JSON output structure from autter blame --json
...
/**
 * Service for executing autter blame and managing blame results.
...
export class BlameService {
  ...
  private autterAvailable: boolean | null = null;
   ...
   * Returns null if autter is not available, file is not in git, or an error occurs.
   */
  public async requestBlame(
    ...
    // Check if autter is available
    if (this.autterAvailable === false) {
      return null;
    }
    ...
    // Get current content for cache key and to pass to autter
  ...
  private async executeBlame(
    ...
    return new Promise((resolve, reject) => {
      ...
      // This allows autter to properly shift AI attributions for dirty files
      const args = ['blame', '--json', '--contents', '-', filePath];
      const binary = getAutterBinary();
      console.log('[autter] Spawning blame:', { binary, args, cwd });
      ...
      proc.on('error', (error: NodeJS.ErrnoException) => {
        ...
        if (error.code === 'ENOENT') {
          // autter not installed
          this.autterAvailable = false;
          this.showInstallMessage();
          reject(new Error('autter not installed'));
        } else {
          reject(error);
        }
      });
      ...
      proc.on('close', (code) => {
        ...
        if (code !== 0) {
          ...
          if (stderr.includes('not a git repository')) {
            ...
            console.error('[autter] blame error:', stderr);
            reject(new Error(`autter blame failed with code ${code}`));
          }
          return;
        }
        
        // Mark autter as available
        this.autterAvailable = true;
        
        try {
          console.log('[autter] Raw blame stdout (first 500 chars):', stdout.substring(0, 500));
          console.log('[autter] Raw blame stderr:', stderr);
          const jsonOutput = JSON.parse(stdout) as BlameJsonOutput;
          const result = this.parseBlameOutput(jsonOutput, document.lineCount);
          resolve(result);
        } catch (parseError) {
          console.error('[autter] Failed to parse blame JSON:', parseError);
  ...
  /**
   * Parse the JSON output from autter blame and expand line ranges.
   */
  private parseBlameOutput(output: BlameJsonOutput, totalLines: number): BlameResult {
    ...
    for (const [hash, record] of Object.entries(output.prompts || {})) {
      ...
      console.log(`[autter] parseBlameOutput prompt ${hash}: messages=${msgs.length}, hasText=${msgs.some(m => m.text)}, messages_url=${record.messages_url || 'none'}`);
  ...
  /**
   * Fetch prompt messages from CAS via `autter show-prompt`.
  ...
  public async fetchPromptFromCAS(
    ...
    return new Promise((resolve) => {
      ...
      const proc = spawn(getAutterBinary(), args, {
        cwd,
        timeout: 15000,
      });
  ...
  private showInstallMessage(): void {
    ...
    vscode.window.showInformationMessage(
      'autter is not installed. Install it to see AI authorship information.',
      'Learn More'
    ).then((choice) => {
      if (choice === 'Learn More') {
        vscode.env.openExternal(
          vscode.Uri.parse('https://github.com/autter-dev/autter-cli')
        );
      }
    });
  }
