...
export class AIEditManager {
  ...
  private autterVersion: string | null = null;
  private hasShownAutterErrorMessage = false;
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    if (!this.legacyCopilotHooksEnabled) {
      console.log(`[autter] AIEditManager: VS Code ${vscode.version} has native hooks; skipping legacy extension checkpoints`);
    }
    ...
    if (context.storageUri?.fsPath) {
      ...
      console.warn('[autter] No workspace storage URI available');
    }
  ...
  private cleanupOldCheckpointEntries(): void {
    ...
    if (entriesToDelete.length > 0) {
      console.log('[autter] AIEditManager: Cleaned up', entriesToDelete.length, 'old checkpoint entries');
    }
  }
  ...
  public handleSaveEvent(doc: vscode.TextDocument): void {
    ...
    console.log('[autter] AIEditManager: Save event tracked for', filePath);
  }
  ...
  public handleOpenEvent(doc: vscode.TextDocument): void {
    console.log('[autter] AIEditManager: Open event detected for', doc);
    ...
    if (doc.uri.scheme === "chat-editing-snapshot-text-model" || doc.uri.scheme === "chat-editing-text-model") {
      ...
      console.log('[autter] AIEditManager: Snapshot open event detected for', filePath, 'scheme:', doc.uri.scheme, 'seen count:', this.snapshotOpenEvents.get(filePath)?.count, '- triggering human checkpoint');
  ...
  public handleCloseEvent(doc: vscode.TextDocument): void {
    ...
    if (doc.uri.scheme === "chat-editing-snapshot-text-model" || doc.uri.scheme === "chat-editing-text-model") {
      console.log('[autter] AIEditManager: Snapshot close event detected for', doc);
      // console.log('[autter] AIEditManager: Snapshot close event detected, triggering human checkpoint');
      // const filePath = doc.uri.fsPath;
      // this.triggerHumanCheckpoint([filePath]);
    }
  }
  ...
  private evaluateSaveForCheckpoint(filePath: string): void {
    ...
    console.log('[autter] AIEditManager: Evaluating save for checkpoint for', filePath, '- snapshot info:', snapshotInfo);
    ...
    if (snapshotInfo && snapshotInfo.count >= 1 && snapshotInfo.uri?.query) {
      ...
      if (snapshotAge >= this.MAX_SNAPSHOT_AGE_MS) {
        console.log('[autter] AIEditManager: Snapshot is too old (' + Math.round(snapshotAge / 1000) + 's), skipping AI checkpoint for', filePath);
      } else {
        const storagePath = this.workspaceBaseStoragePath;
        if (!storagePath) {
          console.warn('[autter] AIEditManager: Missing workspace storage path, skipping AI checkpoint for', filePath);
        } else {
          try {
            const params = JSON.parse(snapshotInfo.uri.query);
            let sessionId = params.chatSessionId || params.sessionId;

            console.log("[autter] AIEditManager: Parsed snapshot params:", params);
            ...
            if (!sessionId && params.chatSessionResource) {
              ...
              console.log("[autter] AIEditManager: Detected chatSessionResource, attempting to parse sessionId");
              ...
              console.log("[autter] AIEditManager: Parsed sessionId from chatSessionResource:", sessionId);
            }
            ...
            if (!sessionId && params.session && params.session.path && params.session.path.startsWith && params.session.path.startsWith('/')) {
              ...
              console.log("[autter] AIEditManager: Detected session as object, decoding sessionId");
              sessionId = Buffer.from(params.session.path.slice(1), 'base64').toString('utf-8');
              console.log("[autter] AIEditManager: Parsed sessionId from Base64:", sessionId);
            }

            if (!sessionId) {
              console.warn('[autter] AIEditManager: Snapshot URI missing session id, skipping AI checkpoint for', filePath);
            } else {
              const workspaceFolder = vscode.workspace.getWorkspaceFolder(vscode.Uri.file(filePath));
              if (!workspaceFolder) {
                console.warn('[autter] AIEditManager: No workspace folder found for', filePath, '- skipping AI checkpoint');
              } else {
              ...
              console.log('[autter] AIEditManager: AI edit detected for', filePath, '- triggering AI checkpoint (sessionId:', sessionId, ', chatSessionPath:', chatSessionPath, ', workspaceFolder:', workspaceFolder.uri.fsPath, ')');
              
              // Get dirty files and ensure the saved file is included with its content from VS Code
              const dirtyFiles = this.getDirtyFiles();
              console.log('[autter] AIEditManager: Dirty files:', dirtyFiles);
              ...
              console.log('[autter] AIEditManager: Dirty files with saved file content:', dirtyFiles);
            ...
            console.error('[autter] AIEditManager: Unable to trigger AI checkpoint for', filePath, e);
          }
        }
      }
    }
    ...
    if (!checkpointTriggered) {
      console.log('[autter] AIEditManager: No AI pattern detected for', filePath, '- skipping checkpoint');
    }
  ...
  private triggerHumanCheckpoint(willEditFilepaths: string[]): void {
    if (!willEditFilepaths || willEditFilepaths.length === 0) {
      console.warn('[autter] AIEditManager: Cannot trigger human checkpoint without files');
      return;
    }
    ...
    const filesToCheckpoint = willEditFilepaths.filter(filePath => {
      ...
      if (lastCheckpoint && (now - lastCheckpoint) < this.HUMAN_CHECKPOINT_DEBOUNCE_MS) {
        console.log('[autter] AIEditManager: Skipping file due to debounce:', filePath);
        return false;
      }
      return true;
    });
    ...
    if (filesToCheckpoint.length === 0) {
      console.log('[autter] AIEditManager: All files were recently checkpointed, skipping');
      return;
    }
    ...
    if (!workspaceFolder) {
      console.warn('[autter] AIEditManager: No workspace folder found for human checkpoint');
      return;
    }

    console.log('[autter] AIEditManager: Triggering human checkpoint for files:', filesToCheckpoint);
  ...
  async checkpoint(author: "human" | "ai" | "ai_tab", hookInput: string): Promise<boolean> {
    if (author !== "ai_tab" && !this.legacyCopilotHooksEnabled) {
      console.log("[autter] AIEditManager: Skipping legacy human/ai checkpoint dispatch (native VS Code hooks active)");
      return true;
    }

    if (!(await this.checkAutter())) {
      return false;
    }
    ...
    return new Promise<boolean>((resolve) => {
      ...
      if (!workspaceRoot) {
        console.warn('[autter] AIEditManager: No workspace root found, skipping checkpoint');
        resolve(false);
        return;
      }
      ...
      console.log('[autter] AIEditManager: Spawning autter with args:', args);
      console.log('[autter] AIEditManager: Workspace root:', workspaceRoot);
      console.log('[autter] AIEditManager: Hook input:', hookInput);

      const proc = spawn(getAutterBinary(), args, { cwd: workspaceRoot });
      ...
      proc.on("error", (error) => {
        console.error('[autter] AIEditManager: Checkpoint error:', error, stdout, stderr);
        vscode.window.showErrorMessage(
          "autter checkpoint error: " + error.message + " - " + stdout + " - " + stderr
        );
        resolve(false);
      });
      ...
      proc.on("close", (code) => {
        if (code !== 0) {
          console.error('[autter] AIEditManager: Checkpoint exited with code:', code, stdout, stderr);
          vscode.window.showErrorMessage(
            "autter checkpoint error: exited with code " + code + " - " + stdout + " - " + stderr
          );
          resolve(false);
        } else {
          const config = vscode.workspace.getConfiguration("autter");
  ...
  async showAutterUpdateRequiredMsg(detectedVersion: string) {
    const url = vscode.Uri.parse(AUTTER_INSTALL_DOCS_URL);

    const choice = await vscode.window.showErrorMessage(
      `autter version ${detectedVersion} is no longer supported.`,
      'Update autter'
    );

    if (choice === 'Update autter') {
      await vscode.env.openExternal(url);
    }
  }

  async showAutterNotInstalledMsg() {
    const url = vscode.Uri.parse(AUTTER_INSTALL_DOCS_URL);

    const choice = await vscode.window.showInformationMessage(
      'autter is not installed.',
      'Install autter'
    );

    if (choice === 'Install autter') {
      await vscode.env.openExternal(url);
    }
  }

  async checkAutter(): Promise<boolean> {
      ...
      exec("autter --version", (error, stdout, stderr) => {
        if (error) {
          if (!this.hasShownAutterErrorMessage) {
            ...
            vscode.window.showInformationMessage(
              "autter not installed. Visit https://github.com/autter-dev/autter-cli to install it."
            );
            this.hasShownAutterErrorMessage = true;
          }
          ...
          if (!isVersionSatisfied(detectedVersion, MIN_AUTTER_VERSION)) {
            if (!this.hasShownAutterErrorMessage) {
              this.showAutterUpdateRequiredMsg(detectedVersion);
              this.hasShownAutterErrorMessage = true;
            }
            resolve(false);
            return;
          ...
          this.autterVersion = detectedVersion;
          resolve(true);
        }
      });
    });
