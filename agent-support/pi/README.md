...
`autter install-hooks` installs a managed Pi extension at:

- `~/.pi/agent/extensions/autter.ts`

That extension emits canonical Pi `before_edit` / `after_edit` checkpoint payloads and runs:

```bash
autter checkpoint pi --hook-input stdin
```
...
```bash
grep -n "checkpoint', 'pi'\|autter.override.json" ~/.pi/agent/extensions/autter.ts
```
