...
namespace AutterVS.Listeners
{
    ...
    public sealed class DocumentSaveListener : IDisposable
    {
        ...
        public void OnDocumentSaved(string filePath)
        {
            ...
            {
                Trace.WriteLine($"[autter] [SAVE] Skipping internal path: {filePath}");
                return;
            }
            ...
            Trace.WriteLine($"[autter] [SAVE] Document saved: {Path.GetFileName(filePath)}");
        ...
        private void ExecuteCheckpoint(string workspaceRoot)
        {
            ...
            foreach (var absolutePath in paths)
            {
                ...
                {
                    Trace.WriteLine($"[autter] [SAVE] Could not read file: {absolutePath}");
                }
            }

            if (editedPaths.Count == 0) return;

            Trace.WriteLine($"[autter] [SAVE] Firing known_human checkpoint for {editedPaths.Count} file(s)");
