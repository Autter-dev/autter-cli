# autter Visual Studio Extension: Technical Design

## 1. Overview

This document describes the design of a Visual Studio (VSIX) extension that detects AI-generated code edits (primarily GitHub Copilot) and records them via the `autter` CLI. The extension follows the same architectural patterns as the existing IntelliJ plugin.

### Goals

- Detect when GitHub Copilot (inline completions or chat edits) modifies code in Visual Studio
- Record AI-authored edits by calling `autter checkpoint agent-v1 --hook-input stdin`
- Record human edits as `known_human` checkpoints so autter can distinguish the before/after boundary
- Auto-install via `autter install-hooks`
...
│                                                         │
│  ┌──────────────────────┐                               │
│  │   AutterPackage       │  (AsyncPackage entry point)   │
│  │   ├── BinaryResolver │  Locate autter binary         │
│  │   └── Registers:     │                               │
│  │       ├── TextBufferListener   ──┐                   │
...
│  ┌──────────────────────────────────────────────┐       │
│  │  CheckpointService                           │       │
│  │  Spawns: autter checkpoint agent-v1          │       │
│  │          --hook-input stdin                   │       │
...
### 4.1 AutterPackage (entry point)

**File**: `src/AutterVS/AutterPackage.cs`

The `AsyncPackage` subclass that Visual Studio loads on startup. Responsibilities:

- Resolve the autter binary path (via `BinaryResolver`)
...
- Display info bar if autter is not installed (currently logs only, UI planned)
...
**File**: `src/AutterVS/Services/BinaryResolver.cs`

Locates the `autter` (or `autter.exe`) binary. Search order:

1. `%USERPROFILE%\.autter\bin\autter.exe` (standard install location on Windows)
2. `%USERPROFILE%\.autter-local-dev\gitwrap\bin\autter.exe` (nix dev path)
3. `PATH` lookup via `where autter` (Windows) or `which autter` (Mac/Linux)

After finding the binary, runs `autter version` to verify it meets the minimum version requirement (currently `1.0.23`, matching IntelliJ). Caches the resolved path for the session. Logs structured error messages when the binary is not found or version is too old, matching IntelliJ's `AutterService.findAutterBinary()` pattern.

### 4.3 GitRepoResolver

**File**: `src/AutterVS/Services/GitRepoResolver.cs`
...
**File**: `src/AutterVS/Detection/CopilotEditDetector.cs`
...
**File**: `src/AutterVS/Listeners/TextBufferListener.cs`
...
**File**: `src/AutterVS/Listeners/DocumentSaveListener.cs`
...
**File**: `src/AutterVS/Services/CheckpointService.cs`

Spawns the `autter` CLI process and writes JSON to stdin. Modeled after IntelliJ's `AutterService.checkpoint()`:
...
**File**: `src/AutterVS/Models/CheckpointInput.cs`
...
├── AutterVS.sln                        # Solution file
└── src/
    ├── AutterVS/
    │   ├── AutterVS.csproj             # Project file (targets net48, VS 2022+)
...
- Run `autter status` to confirm attribution is recorded
- Run `autter log` after committing to verify notes are attached
...
- **Info bar notification**: `IVsInfoBarUIFactory` to show a visible UI notification when autter is not installed.
