#!/usr/bin/env python3
"""
Run a curated subset of the Git test suite against autter.

This harness clones the Git repository (or reuses a local clone), runs the
selected tests with GIT_TEST_INSTALLED pointing at a autter wrapper, and
...
def make_isolated_env(isolated_home: str) -> dict:
    ...
    and a autter config optimised for compatibility testing:

    - _AUTTER_INTERNAL_DISABLE_WRAPPER_DAEMON_AUTOSPAWN=1 : disables daemon auto-spawn entirely
    - git_path           : hardcoded real-git path so autter never probes on
                           every invocation
    - allow_repositories : non-empty sentinel so no compat-test repo (which has
                           no remotes) ever matches → skip_hooks=true → autter
                           acts as a pure passthrough proxy for every command.
                           Without this, autter runs its full hook machinery
    ...
    - Reading/writing the developer's ~/.autter/config.json or ~/.claude/
    ...
    # Sanitize PATH first so shutil.which finds the real git, not a autter wrapper.
    sanitized = []
    for entry in env.get("PATH", "").split(os.pathsep):
        ...
        if os.path.isfile(git_bin) or os.path.islink(git_bin):
            try:
                ...
                if "autter" in real:
                    continue  # skip autter wrapper directories
            except OSError:
                pass
    ...
    # Write autter config.
    autter_dir = os.path.join(isolated_home, ".autter")
    os.makedirs(autter_dir, exist_ok=True)
    with open(os.path.join(autter_dir, "config.json"), "w") as f:
        json.dump(
            {
                "git_path": real_git,
                ...
                # False → skip_hooks=True → autter proxies without running hooks.
                "allow_repositories": ["AUTTER_COMPAT_TEST_SENTINEL_NEVER_MATCHES"],
            },
            f,
        )

    # Suppress daemon auto-spawn via env var.  Some git test scripts temporarily
    # change HOME inside subshells (e.g. HOME=$(pwd)/alias-config).  Any autter
    ...
    # _AUTTER_INTERNAL_DISABLE_WRAPPER_DAEMON_AUTOSPAWN is checked in ensure_daemon_running() before spawning.
    env["_AUTTER_INTERNAL_DISABLE_WRAPPER_DAEMON_AUTOSPAWN"] = "1"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run core Git tests against autter.")
    ...
    parser.add_argument("--autter-bin", type=Path, default=REPO_ROOT / "target" / "release" / "autter")
    args = parser.parse_args()

    tests = read_tests_list(args.tests_file)

    if not args.autter_bin.exists():
        raise FileNotFoundError(
            f"autter binary not found at {args.autter_bin}. Build it with `cargo build --release`."
        )

    whitelist = load_whitelist(args.whitelist)

    # Wrap the entire test run (including git clone/build) in an isolated HOME so
    # that the release autter binary cannot read or write the developer's real
    # ~/.autter/config.json, ~/.claude/settings.json, etc.  The isolated env
    # sets _AUTTER_INTERNAL_DISABLE_WRAPPER_DAEMON_AUTOSPAWN=1 to prevent daemon
    # auto-spawn and the resulting 2-second-per-git-command timeout.
    with tempfile.TemporaryDirectory(prefix="autter-compat-home-") as isolated_home:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # exec -a) and we can ensure _AUTTER_INTERNAL_DISABLE_WRAPPER_DAEMON_AUTOSPAWN
            # is set on every invocation.  The _AUTTER prefix (not GIT_) is intentional:
            ...
            git_wrapper.write_text(
                f"#!/bin/bash\n"
                f"_AUTTER_INTERNAL_DISABLE_WRAPPER_DAEMON_AUTOSPAWN=1\n"
                f"export _AUTTER_INTERNAL_DISABLE_WRAPPER_DAEMON_AUTOSPAWN\n"
                # exec -a git sets argv[0] to "git" so autter's binary-name check
                # routes to handle_git() instead of handle_autter() (help text).
                # bash is required for exec -a; /bin/sh (dash) does not support it.
                f'exec -a git "{args.autter_bin}" "$@"\n'
            )
            git_wrapper.chmod(0o755)
            (wrapper_dir / "autter").symlink_to(args.autter_bin)
