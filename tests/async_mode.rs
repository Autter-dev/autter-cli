...
use autter::daemon::control_api::CasSyncPayload;
use autter::daemon::{
...
fn configure_test_daemon_env(command: &mut Command, repo: &TestRepo) {
    ...
    command.env("AUTTER_DAEMON_TRACE_SOCKET", daemon_trace_socket_path(repo));
}

fn write_daemon_config(repo: &TestRepo) {
    let config_dir = repo.test_home_path().join(".autter");
...
fn autter_with_daemon_env(repo: &TestRepo, args: &[&str]) -> Result<String, String> {
    ...
    let envs = [
        ("AUTTER_DAEMON_HOME", daemon_home.as_str()),
        ("AUTTER_DAEMON_CONTROL_SOCKET", control_socket.as_str()),
        ("AUTTER_DAEMON_TRACE_SOCKET", trace_socket.as_str()),
    ];
    repo.autter_with_env(args, &envs)
}
...
fn daemon_command_output(repo: &TestRepo, args: &[&str], cwd: &Path) -> Output {
    ...
    command
        .output()
        .expect("failed to invoke autter daemon command")
}
...
fn install_hooks_async_mode_trace2_target_routes_real_git_trace_to_daemon() {
    ...
    git_command.args(["config", "--local", "autter.tracing-test", "1"]);
...
fn async_mode_checkpoint_starts_daemon_when_down() {
    ...
    let output = repo
        .autter(&["checkpoint", "mock_ai", "async-checkpoint.txt"])
...
fn async_commit_with_ai_content(extra_envs: &[(&str, &str)]) -> (TestRepo, String) {
    ...
    // adds its own sync + note check). We pass AUTTER_TEST_FORCE_TTY so
    // the wrapper treats this pipe as an interactive terminal.
    repo.git(&["add", "-A"]).expect("add should succeed");
    let mut envs: Vec<(&str, &str)> = vec![("AUTTER_TEST_FORCE_TTY", "1")];
...
fn async_mode_post_commit_quiet_flag_suppresses_stats() {
    ...
    let output = repo
        .git_with_env(
            &["commit", "-q", "-m", "AI quiet"],
            &[("AUTTER_TEST_FORCE_TTY", "1")],
            None,
        )
        .expect("commit should succeed");

    // With -q the wrapper should suppress all autter post-commit output.
    assert!(
        !output.contains("you") && !output.contains("[autter]"),
...
fn async_mode_post_commit_non_interactive_suppresses_stats() {
    ...
    // Commit WITHOUT AUTTER_TEST_FORCE_TTY – the pipe means non-interactive.
    let output = repo
