...
use autter::streams::agents::GeminiAgent;
use autter::streams::watermark::ByteOffsetWatermark;
use serde_json::json;
use std::fs;

fn parse_gemini(hook_input: &str) -> Result<Vec<ParsedHookEvent>, AutterError> {
...
fn test_gemini_e2e_with_attribution() {
    ...
    repo.autter(&["checkpoint", "gemini", "--hook-input", &hook_input])
        .unwrap();
...
fn test_gemini_preset_bash_tool_aftertool_detects_changes() {
    ...
    .to_string();
    repo.autter(&["checkpoint", "gemini", "--hook-input", &pre_hook_input])
        .unwrap();
    ...
    .to_string();
    repo.autter(&["checkpoint", "gemini", "--hook-input", &post_hook_input])
        .unwrap();
