...
fn status_json(repo: &TestRepo) -> StatusOutput {
    let raw = repo
        .autter(&["status", "--json"])
        .expect("autter status --json should succeed");
...
fn test_working_dir_diff_stats_all_files_checkpointed() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "a.txt", "b.txt"])
        .unwrap();
...
fn test_working_dir_diff_stats_with_rename() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "new_name.txt"])
        .unwrap();
...
fn test_working_dir_diff_stats_respects_ignore_patterns() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "src/lib.rs", "Cargo.lock"])
        .unwrap();
