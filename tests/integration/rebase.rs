...
fn test_rebase_preserves_prompt_only_commit_note_metadata() {
    ...
    notes_add(&autter_repo, &prod_commit.commit_sha, &mutated_source_note)
...
fn test_rebase_squash_preserves_human_attribution() {
    ...
    repo.autter(&["checkpoint", "human", "handler.go"]).unwrap();
    let ai_content_1 = "\
func handleOrder() {
    validate()
    process()
...
";
    ...
    repo.autter(&["checkpoint", "mock_ai", "handler.go"])
        .unwrap();
    repo.stage_all_and_commit("AI commit 1").unwrap();
...
";
    ...
    repo.autter(&["checkpoint", "mock_known_human", "handler.go"])
        .unwrap();
...
fn test_rebase_squash_preserves_session_attribution() {
    ...
    repo.autter(&["checkpoint", "human", "service.go"]).unwrap();
    let ai_content_1 = "\
func serve() {
    listen()
    handle()
...
";
    std::fs::write(&file_path, ai_content_1).unwrap();
    repo.autter(&["checkpoint", "mock_ai", "service.go"])
        .unwrap();
    repo.stage_all_and_commit("AI commit 1").unwrap();
