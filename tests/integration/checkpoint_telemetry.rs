...
fn test_claude_file_edit_checkpoint_propagates_tool_use_id_to_metadata() {
    ...
    repo.autter(&["checkpoint", "claude", "--hook-input", &pre_hook])
    ...
    repo.autter(&["checkpoint", "claude", "--hook-input", &post_hook])
    ...
    let ai_checkpoint = checkpoints
        .iter()
        .find(|c| c.kind == autter::authorship::working_log::CheckpointKind::AiAgent)
...
fn test_gemini_file_edit_checkpoint_propagates_tool_use_id_to_metadata() {
    ...
    repo.autter(&["checkpoint", "gemini", "--hook-input", &pre_hook])
    ...
    repo.autter(&["checkpoint", "gemini", "--hook-input", &post_hook])
