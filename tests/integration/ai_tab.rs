...
fn parse_ai_tab(hook_input: &str) -> Result<Vec<ParsedHookEvent>, AutterError> {
...
fn run_ai_tab_checkpoint(repo: &TestRepo, hook_payload: serde_json::Value) {
    ...
    match repo.autter(&args) {
        Ok(output) => {
            println!("autter checkpoint output: {}", output);
        }
        Err(err) => {
...
fn test_ai_tab_rejects_invalid_hook_event() {
    ...
    match result {
        Err(AutterError::PresetError(message)) => {
            assert!(
