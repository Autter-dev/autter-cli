...
use autter::daemon::checkpoint::build_agent_usage_attrs;
...
fn test_agent_usage_attrs_include_repo_url_when_remote_exists() {
    ...
    let attrs = build_agent_usage_attrs(Some(&autter_repo), &agent_id);
...
fn test_agent_usage_attrs_local_path_remote_no_repo_url() {
    ...
    let autter_repo = AutterRepository::find_repository_in_path(mirror.path().to_str().unwrap())
        .expect("should open repo");
