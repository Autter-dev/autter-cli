...
use autter::git::status::MAX_PATHSPEC_ARGS;
...
fn test_status_post_filter_excludes_unmatched_files() {
    ...
    let result = autter_repo.status(Some(&subset), true).unwrap();
...
fn test_status_post_filter_rename_matched_by_new_path() {
    ...
    let result = autter_repo.status(Some(&pathspecs), true).unwrap();
...
fn test_diff_added_lines_post_filter_equivalence() {
    ...
    let autter_repo = find_repository_in_path(repo.path().to_str().unwrap()).unwrap();
    let sha1 = autter_repo.head().unwrap().target().unwrap();
    ...
    let autter_repo = find_repository_in_path(repo.path().to_str().unwrap()).unwrap();
    let sha2 = autter_repo.head().unwrap().target().unwrap();
...
fn test_diff_workdir_insertions_post_filter_equivalence() {
    ...
    // Small pathspec
    let small: HashSet<String> = filenames.iter().cloned().collect();
    let (all_small, ins_small) = autter_repo
    ...
    let refs: Vec<&str> = filenames.iter().map(|s| s.as_str()).collect();
    let large = padded_pathspecs(&refs);
    let (all_large, ins_large) = autter_repo
...
fn test_diff_workdir_insertions_both_maps_filtered() {
    ...
    let pathspecs = padded_pathspecs(&["a.txt"]);
    let (all_added, pure_insertions) = autter_repo
...
fn test_diff_tree_to_tree_post_filter_equivalence() {
    ...
    // Get trees
    let commit1 = autter_repo.find_commit(sha1).unwrap();
    let tree1 = commit1.tree().unwrap();
    let commit2 = autter_repo.find_commit(sha2).unwrap();
...
fn test_threshold_boundary_1000_vs_1001() {
    ...
    let result_1000 = autter_repo
        .list_commit_files(&head_sha, Some(&exactly_1000))
        .unwrap();
    let result_1001 = autter_repo
