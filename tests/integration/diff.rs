...
use autter::authorship::transcript::{AiTranscript, Message};
...
fn diff_json(repo: &TestRepo, args: &[&str]) -> Value {
    let output = repo.autter(args).expect("autter diff should succeed");
...
fn test_diff_single_commit() {
    ...
    // Run autter diff on the second commit
    let output = repo
        .autter(&["diff", &second.commit_sha])
        .expect("autter diff should succeed");
...
fn test_diff_commit_range() {
    ...
    // Run autter diff with range
    let range = format!("{}..{}", first.commit_sha, third.commit_sha);
    let output = repo
        .autter(&["diff", &range])
        .expect("autter diff range should succeed");
...
fn test_diff_two_positional_revisions_uses_git_range_semantics() {
    ...
    let autter_diff = repo
        .autter(&["diff", &from.commit_sha, &to.commit_sha])
        .expect("autter diff should support two positional revisions");

    assert!(
        autter_diff.contains("+AI line 1") && autter_diff.contains("+AI line 2"),
        "autter diff should include net additions between from/to commits:\n{}",
        autter_diff
    );
    assert!(
        !autter_diff.contains("new file mode") && !autter_diff.contains("--- /dev/null"),
        "autter diff should not fallback to from^..from behavior:\n{}",
        autter_diff
    );
}
...
fn test_diff_initial_commit() {
    ...
    let output = repo
        .autter(&["diff", &commit.commit_sha])
        .expect("autter diff on initial commit should succeed");
...
fn test_diff_with_head_ref() {
    ...
    let output = repo
        .autter(&["diff", "HEAD"])
        .expect("autter diff HEAD should succeed");
...
fn test_diff_error_on_no_args() {
    ...
    let result = repo.autter(&["diff"]);

    // Should fail with error
    assert!(result.is_err(), "autter diff without arguments should fail");
}
...
fn test_diff_json_output_with_escaped_newlines() {
    ...
    // Run autter diff with --json flag
    let output = repo
        .autter(&["diff", &commit.commit_sha, "--json"])
        .expect("autter diff --json should succeed");
...
fn test_diff_json_include_stats_rejects_commit_ranges() {
    ...
    let result = repo.autter(&["diff", &range, "--json", "--include-stats"]);
    assert!(
        result.is_err(),
...
fn test_diff_range_multiple_commits() {
    ...
    let output = repo
        .autter(&["diff", &range])
        .expect("autter diff multi-commit range should succeed");
...
fn test_diff_ignores_repo_external_diff_helper_but_proxy_uses_it() {
    ...
    let autter_diff = repo
        ...
        .expect("autter diff should succeed");
    assert!(
        !autter_diff.contains(&marker),
        "autter diff should not use external diff helper output, got:\n{}",
        autter_diff
    );
    assert!(
        autter_diff.contains("diff --git"),
        "autter diff should emit standard unified diff output, got:\n{}",
        autter_diff
    );
    assert!(
        autter_diff.contains("@@"),
        "autter diff should include hunk headers, got:\n{}",
        autter_diff
    );
}
...
fn test_diff_parsing_is_stable_under_hostile_diff_config() {
    ...
    let autter_diff = repo
        .autter(&["diff", "HEAD"])
        .expect("autter diff should succeed");
    assert!(autter_diff.contains("diff --git"));
    assert!(autter_diff.contains("@@"));
    assert!(autter_diff.contains("+line two"));
    assert!(autter_diff.contains("+line three"));
}
...
fn test_checkpoint_and_commit_ignore_repo_external_diff_helper() {
    ...
    repo.autter(&["checkpoint", "mock_ai"])
...
fn test_diff_ignores_git_external_diff_env_but_proxy_uses_it() {
    ...
    let ai_diff = repo
        ...
        .expect("autter diff should succeed with GIT_EXTERNAL_DIFF set");
    assert!(
        !ai_diff.contains(marker),
        "autter diff should ignore GIT_EXTERNAL_DIFF for internal diff calls, got:\n{}",
        ai_diff
    );
    assert!(
        ai_diff.contains("diff --git"),
        "autter diff should still emit normal unified diff output, got:\n{}",
        ai_diff
    );
}
...
fn test_diff_ignores_git_diff_opts_env_for_internal_diff() {
    ...
    // autter diff should ignore GIT_DIFF_OPTS and keep normal context behavior.
    let ai_diff = repo
        .autter_with_env(
            &["diff", &commit.commit_sha],
            &[("GIT_DIFF_OPTS", "--unified=0")],
        )
        .expect("autter diff should succeed with GIT_DIFF_OPTS set");
    ...
    assert!(
        ai_context_count >= 2,
        "autter diff should ignore GIT_DIFF_OPTS and preserve context lines, got:\n{}",
        ai_diff
    );
}
...
fn test_diff_respects_effective_ignore_patterns() {
    let repo = TestRepo::new();
    let ignore_file_path = repo.path().join(".autter-ignore");
    fs::write(&ignore_file_path, "ignored/**\n").expect("should write .autter-ignore");
    ...
    let terminal_output = repo
        .autter(&["diff", &change_commit.commit_sha])
        .expect("autter diff should succeed");
    assert!(
    ...
    let json_output = repo
        .autter(&["diff", &change_commit.commit_sha, "--json"])
        .expect("autter diff --json should succeed");
...
fn test_diff_blame_deletions_terminal_annotations() {
    ...
    let without_flag = repo
        .autter(&["diff", &deletion_commit.commit_sha])
    ...
    let with_flag = repo
        .autter(&["diff", &deletion_commit.commit_sha, "--blame-deletions"])
...
fn test_diff_json_deleted_hunks_exact_replacement_from_known_origin_commit() {
    ...
    let output = repo
        .autter(&["diff", &commit_b.commit_sha, "--json", "--blame-deletions"])
        .expect("diff --json --blame-deletions should succeed");
    let json: Value = serde_json::from_str(&output).expect("diff JSON should parse");
...
fn test_diff_json_deleted_hunks_strict_mixed_origins_and_contiguous_segments() {
    ...
    let output = repo
        .autter(&["diff", &commit_c.commit_sha, "--json", "--blame-deletions"])
        .expect("diff --json --blame-deletions should succeed");
    let json: Value = serde_json::from_str(&output).expect("diff JSON should parse");
...
/// the authorship note covers those lines and autter diff shows them as AI.
#[test]
...
fn func_one() {
    ...
    // Run autter diff and check attributions.
    let output = repo
        .autter(&["diff", &commit_b.commit_sha])
        .expect("autter diff should succeed");
...
"##;
    ...
    repo.autter(&["checkpoint", "mock_ai", file_path])
    ...
    // Step 4: run autter diff and verify ALL added lines are attributed to AI
