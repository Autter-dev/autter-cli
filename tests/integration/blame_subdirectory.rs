...
fn test_blame_from_subdirectory_with_relative_path() {
    ...
    let output = repo
        .autter_from_working_dir(&subdir, &["blame", "main.rs"])
...
fn test_blame_from_nested_subdirectory_with_relative_path() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "src/lib/utils/helper.rs"])
        .unwrap();
    repo.stage_all_and_commit("Add helper").unwrap();

    let output = repo
        .autter_from_working_dir(&nested_dir, &["blame", "helper.rs"])
...
fn test_blame_from_subdirectory_with_subpath() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "src/lib/mod.rs"])
        .unwrap();
    repo.stage_all_and_commit("Add mod").unwrap();

    let output = repo
        .autter_from_working_dir(&src_dir, &["blame", "lib/mod.rs"])
...
fn test_blame_from_repo_root_with_subdir_path() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "src/app.rs"])
        .unwrap();
    repo.stage_all_and_commit("Add app").unwrap();

    let output = repo
        .autter(&["blame", "src/app.rs"])
...
fn test_blame_from_subdirectory_preserves_ai_authorship() {
    ...
    let root_output = repo
        .autter(&["blame", "src/code.rs"])
        .expect("blame from root should work");

    let subdir_output = repo
        .autter_from_working_dir(&subdir, &["blame", "code.rs"])
...
fn test_blame_from_subdirectory_nonexistent_file_errors() {
    ...
    let result = repo.autter_from_working_dir(&subdir, &["blame", "nonexistent.rs"]);
...
fn test_blame_from_subdirectory_with_line_range() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "src/multi.rs"])
        .unwrap();
    repo.stage_all_and_commit("Add multi-line file").unwrap();

    let output = repo
        .autter_from_working_dir(&subdir, &["blame", "-L", "2,4", "multi.rs"])
...
fn test_blame_from_deep_subdir_dotdot_into_sibling_dir() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "x/y/target.rs"])
        .unwrap();
    repo.stage_all_and_commit("Add target file").unwrap();

    let output = repo
        .autter_from_working_dir(&dir_a, &["blame", "../../../x/y/target.rs"])
...
fn test_blame_from_deep_subdir_dotdot_up_one_level() {
    ...
    let output = repo
        .autter_from_working_dir(&child_dir, &["blame", "../lib.rs"])
...
fn test_blame_from_deep_subdir_dotdot_multiple_levels() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "a/root_level.rs"])
        .unwrap();
    repo.stage_all_and_commit("Add root level file").unwrap();

    let output = repo
        .autter_from_working_dir(&deep_dir, &["blame", "../../../root_level.rs"])
...
fn test_blame_from_deep_subdir_file_in_repo_root() {
    ...
    let output = repo
        .autter_from_working_dir(&deep_dir, &["blame", "../../../root.txt"])
...
fn test_blame_from_subdir_dotdot_into_different_subtree() {
    ...
    repo.git(&["add", "packages/backend/src/server.rs"])
        .unwrap();
    repo.autter(&["checkpoint", "mock_ai", "packages/backend/src/server.rs"])
        .unwrap();
    repo.stage_all_and_commit("Add server").unwrap();

    let output = repo
        .autter_from_working_dir(&frontend_dir, &["blame", "../../backend/src/server.rs"])
...
fn test_blame_from_deep_subdir_preserves_ai_authorship_with_dotdot() {
    ...
    let root_output = repo
        .autter(&["blame", "src/utils/helpers.rs"])
        .expect("blame from root should work");

    let subdir_output = repo
        .autter_from_working_dir(&deep_dir, &["blame", "../../utils/helpers.rs"])
