...
fn test_config_get_str_simple_value() {
    ...
    let autter_repo =
        AutterRepository::find_repository_in_path(repo.path().to_str().unwrap()).unwrap();
...
fn test_config_get_str_missing_key_returns_none() {
    ...
    let result = autter_repo.config_get_str("nonexistent.key").unwrap();
...
fn test_config_get_regexp_with_subsections() {
    ...
    let result = autter_repo.config_get_regexp(r"^remote\..*\.url$").unwrap();
...
fn test_config_get_regexp_case_insensitive_keys() {
    ...
    let result = autter_repo.config_get_regexp(r"^core\.autocrlf$").unwrap();
    assert!(
...
fn test_config_falls_back_to_global() {
    ...
    let test_key = "autterci.globalcheck";
    ...
    let autter_repo =
        AutterRepository::find_repository_in_path(repo.path().to_str().unwrap()).unwrap();
    let result = autter_repo.config_get_str(test_key).unwrap();
...
fn test_config_local_overrides_global() {
    ...
    let autter_repo =
        AutterRepository::find_repository_in_path(repo.path().to_str().unwrap()).unwrap();
    let result = autter_repo.config_get_str("user.name").unwrap();
...
fn test_config_get_str_bare_repo() {
    ...
    let autter_repo = AutterRepository::from_bare_repository(repo.path()).unwrap();
    let result = autter_repo.config_get_str(key).unwrap();
...
fn test_config_get_regexp_bare_repo() {
    ...
    let autter_repo = AutterRepository::from_bare_repository(repo.path()).unwrap();
    let result = autter_repo.config_get_regexp(r"^baretest\.").unwrap();
