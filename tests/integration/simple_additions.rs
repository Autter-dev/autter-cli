...
use autter::config::AuthorConfig;
use std::fs;
...
";
    ...
    repo.autter(&["checkpoint", "human", "example.md"]).unwrap();
...
fn test_mock_ai_with_pathspecs() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "file1.txt"])
        .unwrap();
...
fn test_with_duplicate_lines() {
    ...
    .unwrap();
    repo.autter(&["checkpoint", "mock_ai"]).unwrap();
...
fn test_ai_deletion_with_human_checkpoint_in_same_commit() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "data.txt"]).unwrap();
    ...
    repo.autter(&["checkpoint", "mock_known_human", "data.txt"])
        .unwrap();
    ...
    println!("file: {:?}", repo.autter(&["blame", "data.txt"]).unwrap());
    ...
    let stats_output = repo.autter(&["stats", "HEAD", "--json"]).unwrap();
...
fn test_large_ai_readme_rewrite_with_no_data_bug() {
    ...
    fs::write(
        &file_path,
        "## A quick demo of Autter Rewrites\n\ndasdas\n\nHUMAN",
    )
    .unwrap();
...
"
    ...
    repo.autter(&["checkpoint", "mock_ai", "Readme.md"])
        .unwrap();
...
fn test_deletion_within_a_single_line_attribution() {
    ...
    let repo = TestRepo::new();
    let file_path = repo.path().join("autter-integration-service.ts");

    // Initial commit: File with old constructor signature (all human)
    fs::write(
        &file_path,
        "/**\n * Service for integrating autter hooks into the hook system.\n */\nexport class AutterIntegrationService {\n  private readonly commandPath: string;\n  private registered = false;\n\n  constructor(_config: Config, enabled: boolean = true) {\n    this.enabled = enabled;\n    this.commandPath = 'autter';\n  }\n}\n",
    )
    .unwrap();
    ...
    repo.autter(&["checkpoint", "mock_ai", "autter-integration-service.ts"])
        .unwrap();
    ...
    let mut file = repo.filename("autter-integration-service.ts");
    file.assert_lines_and_blame(crate::lines![
        "/**".human(),
        " * Service for integrating autter hooks into the hook system.".human(),
        " */".human(),
        "export class AutterIntegrationService {".human(),
...
/// See: https://github.com/autter-dev/autter-cli/issues/356
#[test]
...
fn test_ai_edits_file_with_spaces_in_filename() {
    ...
    // Mark the AI-authored content with mock_ai checkpoint
    repo.autter(&["checkpoint", "mock_ai", "my test file.txt"])
        .unwrap();
...
fn test_ai_generated_file_then_human_full_rewrite() {
    ...
    let checkpoints_jsonl = format!(
        r#"{{"kind":"AiAgent","diff":"fake_diff_sha","author":"Test User","entries":[{{"file":"jokes-cli.ts","blob_sha":"{ai_sha}","attributions":[],"line_attributions":[{{"start_line":1,"end_line":6,"author_id":"{agent_author_id}","overrode":null}}]}}],"timestamp":1000,"transcript":{{"messages":[]}},"agent_id":{{"tool":"mock_ai","id":"test_session","model":"test"}},"agent_metadata":null,"line_stats":{{"additions":6,"deletions":0,"additions_sloc":5,"deletions_sloc":0}},"api_version":"checkpoint/1.0.0","autter_version":"development:1.1.23"}}
{{"kind":"Human","diff":"fake_diff_sha2","author":"Test User","entries":[{{"file":"jokes-cli.ts","blob_sha":"{human_sha}","attributions":[{{"start":0,"end":0,"author_id":"human","ts":2000}},{{"start":0,"end":{human_len},"author_id":"human","ts":2000}}],"line_attributions":[]}}],"timestamp":2000,"transcript":null,"agent_id":null,"agent_metadata":null,"line_stats":{{"additions":2,"deletions":6,"additions_sloc":2,"deletions_sloc":5}},"api_version":"checkpoint/1.0.0","autter_version":"development:1.1.23"}}"#
    );
    fs::write(
...
fn test_known_human_record_includes_email() {
    ...
    repo.autter(&["checkpoint", "human", "app.go"]).unwrap();
    fs::write(&file_path, "func main() {}\n").unwrap();
    repo.autter(&["checkpoint", "mock_ai", "app.go"]).unwrap();
    repo.stage_all_and_commit("AI commit").unwrap();

    // Human edits the file
    fs::write(&file_path, "func main() {}\nfunc helper() {}\n").unwrap();
    repo.autter(&["checkpoint", "mock_known_human", "app.go"])
        .unwrap();
...
fn test_session_record_human_author_includes_email() {
    ...
    repo.autter(&["checkpoint", "human", "main.rs"]).unwrap();
    fs::write(&file_path, "fn main() {}\n").unwrap();
    repo.autter(&["checkpoint", "mock_ai", "main.rs"]).unwrap();
    repo.stage_all_and_commit("AI commit").unwrap();
...
fn test_author_config_cli_set_get_unset() {
    ...
    repo.autter(&["config", "set", "author.name", "Config User"])
        .unwrap();
    repo.autter(&["config", "set", "author.email", "config@example.com"])
        .unwrap();

    let name = repo.autter(&["config", "author.name"]).unwrap();
    assert_eq!(name.trim(), "\"Config User\"");

    let author = repo.autter(&["config", "author"]).unwrap();
    ...
    repo.autter(&["config", "unset", "author.name"]).unwrap();
    let author = repo.autter(&["config", "author"]).unwrap();
    let value: serde_json::Value =
        serde_json::from_str(author.trim()).expect("author config should be JSON");
    ...
    repo.autter(&["config", "unset", "author"]).unwrap();
    let author = repo.autter(&["config", "author"]).unwrap();
    let value: serde_json::Value =
        serde_json::from_str(author.trim()).expect("author config should be JSON");
...
fn test_author_config_overrides_session_and_known_human_records() {
    ...
    repo.autter(&["checkpoint", "human", "author_config.rs"])
        .unwrap();
    fs::write(&file_path, "fn ai() {}\n").unwrap();
    repo.autter(&["checkpoint", "mock_ai", "author_config.rs"])
        .unwrap();
    ...
    fs::write(&file_path, "fn ai() {}\nfn human() {}\n").unwrap();
    repo.autter(&["checkpoint", "mock_known_human", "author_config.rs"])
        .unwrap();
...
fn test_author_config_partial_overrides_fall_back_to_git_committer_identity() {
    ...
    name_repo.patch_autter_config(|patch| {
        patch.author = Some(AuthorConfig {
    ...
    name_repo
        .autter(&["checkpoint", "human", "partial_name.rs"])
        .unwrap();
    fs::write(&file_path, "fn ai() {}\n").unwrap();
    name_repo
        .autter(&["checkpoint", "mock_ai", "partial_name.rs"])
        .unwrap();
    name_repo
    ...
    email_repo.patch_autter_config(|patch| {
        patch.author = Some(AuthorConfig {
            name: None,
    ...
    email_repo
        .autter(&["checkpoint", "human", "partial_email.rs"])
        .unwrap();
    fs::write(&file_path, "fn ai() {}\n").unwrap();
    email_repo
        .autter(&["checkpoint", "mock_ai", "partial_email.rs"])
        .unwrap();
    email_repo
...
fn test_checkout_carryover_preserves_author_email_in_session() {
    ...
    repo.autter(&["checkpoint", "human", "work.txt"]).unwrap();
    fs::write(&file_path, "AI line\n").unwrap();
    repo.autter(&["checkpoint", "mock_ai", "work.txt"]).unwrap();
...
fn test_rebase_rewrite_preserves_author_email_in_session() {
    ...
    // AI commit on top
    repo.autter(&["checkpoint", "human", "code.rs"]).unwrap();
    fs::write(&file_path, "fn base() {}\nfn ai() {}\n").unwrap();
    repo.autter(&["checkpoint", "mock_ai", "code.rs"]).unwrap();
    repo.stage_all_and_commit("ai commit").unwrap();
...
fn test_rebase_rewrite_preserves_author_email_in_human_record() {
    ...
    fs::write(&file_path, "fn base() {}\nfn human() {}\n").unwrap();
    repo.autter(&["checkpoint", "mock_known_human", "code.rs"])
        .unwrap();
    repo.stage_all_and_commit("human commit").unwrap();
...
/// Verify that `autter status` implicit checkpoint flows through to email in SessionRecord.
...
fn test_status_checkpoint_preserves_author_email_in_session() {
    ...
    // AI edits
    repo.autter(&["checkpoint", "human", "app.py"]).unwrap();
    fs::write(&file_path, "print('hello')\nprint('ai')\n").unwrap();
    repo.autter(&["checkpoint", "mock_ai", "app.py"]).unwrap();

    // Run autter status (triggers implicit human checkpoint internally)
    let _ = repo.autter(&["status", "--json"]);
