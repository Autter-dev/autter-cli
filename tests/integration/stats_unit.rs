...
use autter::authorship::authorship_log_serialization::AttestationEntry;
use autter::authorship::authorship_log_serialization::AuthorshipLog;
use autter::authorship::authorship_log_serialization::FileAttestation;
use autter::authorship::authorship_log_serialization::generate_short_hash;
use autter::authorship::stats::*;
...
fn test_stats_for_simple_ai_commit() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "test.txt"])
        .unwrap();
    ...
    let stats = stats_for_commit_stats(&autter_repo, &head_sha, &[]).unwrap();
...
fn test_stats_ignores_single_lockfile() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "src/main.rs"])
        .unwrap();
    repo.stage_all_and_commit("Initial commit").unwrap();
    ...
    repo.autter(&["checkpoint", "mock_ai", "src/main.rs"])
        .unwrap();
    ...
    let stats_with_lockfile = stats_for_commit_stats(&autter_repo, &head_sha, &[]).unwrap();
    ...
    let stats_without_lockfile =
        stats_for_commit_stats(&autter_repo, &head_sha, &ignore_patterns).unwrap();
...
fn test_stats_ignores_multiple_lockfiles() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "README.md"])
        .unwrap();
    repo.stage_all_and_commit("Initial commit").unwrap();
    ...
    let stats_all = stats_for_commit_stats(&autter_repo, &head_sha, &[]).unwrap();
    ...
    let stats_filtered = stats_for_commit_stats(&autter_repo, &head_sha, &ignore_patterns).unwrap();
...
fn test_stats_with_lockfile_only_commit() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "src/lib.rs"])
        .unwrap();
    repo.stage_all_and_commit("Initial commit").unwrap();
    ...
    repo.git(&["add", "Cargo.lock"]).unwrap();
    repo.autter(&["checkpoint", "mock_known_human", "Cargo.lock"])
        .unwrap();
    ...
    let stats_with = stats_for_commit_stats(&autter_repo, &head_sha, &[]).unwrap();
    assert_eq!(stats_with.git_diff_added_lines, 2000);

    // Test WITH ignore - shows 0 lines (lockfile-only commit)
    let ignore_patterns = vec!["Cargo.lock".to_string()];
    let stats_without = stats_for_commit_stats(&autter_repo, &head_sha, &ignore_patterns).unwrap();
...
fn test_stats_with_glob_patterns() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "src/lib.rs"])
        .unwrap();
    ...
    let stats_filtered = stats_for_commit_stats(&autter_repo, &head_sha, &glob_patterns).unwrap();
    assert_eq!(stats_filtered.git_diff_added_lines, 1);
...
fn test_stats_for_merge_commit_skips_ai_acceptance() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "main.txt"])
        .unwrap();
    ...
    let autter_repo = find_repository_in_path(repo.path().to_str().unwrap()).unwrap();
    let stats = stats_for_commit_stats(&autter_repo, &merge_sha, &[]).unwrap();
...
fn test_stats_command_with_json_output() {
    ...
    let result = stats_command(&autter_repo, Some(&head_sha), true, &[]);
    assert!(result.is_ok());
}
...
fn test_stats_command_default_to_head() {
    ...
    let result = stats_command(&autter_repo, None, false, &[]);
    assert!(result.is_ok());
}
...
fn test_get_git_diff_stats_binary_files() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "text.txt"])
        .unwrap();
    ...
    let result = get_git_diff_stats(&autter_repo, &head_sha, &[]);
    assert!(result.is_ok());
}
