...
fn benchmark_checkpoint_wrapper(iterations: usize) -> DurationStats {
    ...
    repo.autter(&["checkpoint", "mock_ai", "warmup.txt"])
        .unwrap();
    ...
    for i in 0..iterations {
        ...
        repo.autter(&["checkpoint", "mock_ai", &fname]).unwrap();
...
fn bench_checkpoint_multi_file_wrapper() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "w.txt"]).unwrap();
...
fn bench_checkpoint_correctness_after_optimization() {
    ...
    let start = Instant::now();
    repo.autter(&["checkpoint", "mock_ai", "verified.txt"])
        .unwrap();
