...
fn log_default_shows_stats_without_raw_note() {
    ...
    let output = repo
        .autter(&["log", "--no-pager", "-n", "1"])
        .expect("autter log should succeed");

    assert!(output.contains("feat: add ai line"), "output:\n{}", output);
    assert!(output.contains("Autter stats:"), "output:\n{}", output);
...
fn log_raw_shows_authorship_note() {
    ...
    let output = repo
        .autter(&["log", "--no-pager", "--raw", "-n", "1"])
        .expect("autter log --raw should succeed");
...
fn log_plain_proxies_git_notes_backend() {
    ...
    let output = repo
        .autter(&["log", "--no-pager", "--plain", "-n", "1"])
        .expect("autter log --plain should proxy git log");
    ...
    assert!(
        !output.contains("Autter stats:"),
        "plain output should not use autter's renderer:\n{}",
        output
    );
}
...
fn log_multiple_commits_parse_record_boundaries() {
    ...
    let output = repo
        .autter(&["log", "--no-pager", "-n", "2"])
        .expect("autter log should parse multiple commits");

    assert!(output.contains("feat: second"), "output:\n{}", output);
    assert!(output.contains("feat: first"), "output:\n{}", output);
    assert_eq!(
        output.matches("Autter stats:").count(),
        2,
        "output:\n{}",
        output
    );
...
fn log_commit_body_is_separated_from_subject() {
    ...
    let output = repo
        .autter(&["log", "--no-pager", "-n", "1"])
        .expect("autter log should render commit body");
...
fn log_http_backend_reads_notes_db_without_git_notes_ref() {
    ...
    let output = repo
        .autter_with_env(
            ...
            &[("AUTTER_TEST_NOTES_DB_PATH", notes_db_path_string.as_str())],
        )
        .expect("autter log should read HTTP notes cache");
