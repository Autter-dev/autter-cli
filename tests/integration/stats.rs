...
fn test_authorship_log_stats() {
    ...
    let raw = repo.autter(&["stats", "--json"]).unwrap();
    let json = extract_json_object(&raw);
...
fn test_stats_cli_range() {
    ...
    let raw = repo
        ...
        .expect("autter stats range should succeed");

    let output = extract_json_object(&raw);
    let stats: autter::authorship::range_authorship::RangeAuthorshipStats =
...
fn test_stats_cli_range_ignores_repo_external_diff_helper() {
    ...
    let raw = repo
        .autter(&["stats", &range, "--json"])
        .expect("autter stats range should succeed with external diff configured");
    assert!(
        !raw.contains(&marker),
        "autter stats output should not include external helper output, got:\n{}",
        raw
    );
...
fn test_stats_cli_range_with_hostile_diff_config() {
    ...
    let raw = repo
        .autter(&["stats", &range, "--json"])
        .expect("autter stats range should succeed with hostile diff config");
    let output = extract_json_object(&raw);
    let stats: autter::authorship::range_authorship::RangeAuthorshipStats =
        serde_json::from_str(&output).unwrap();
...
fn test_stats_cli_empty_tree_range() {
    ...
    let raw = repo
        .autter(&["stats", &range, "--json"])
        .expect("autter stats empty-tree range should succeed");
...
fn test_markdown_stats_deletion_only() {
    use autter::authorship::stats::write_stats_to_markdown;
...
fn test_markdown_stats_formatting() {
    use autter::authorship::stats::{ToolModelHeadlineStats, write_stats_to_markdown};
    use std::collections::BTreeMap;
...
fn test_stats_in_bare_clone_uses_root_gitattributes_linguist_generated() {
    ...
    let output = Command::new(crate::repos::test_repo::get_binary_path())
        ...
        .output()
        .expect("autter stats should run in bare repo");

    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        output.status.success(),
        "autter stats failed in bare clone:\nstdout: {}\nstderr: {}",
        stdout,
        stderr
    );
...
fn test_stats_range_uses_default_ignores() {
    ...
    let raw = repo
        ...
        .expect("autter stats range should succeed");
    let json = extract_json_object(&raw);
    let range_stats: autter::authorship::range_authorship::RangeAuthorshipStats =
...
fn test_post_commit_large_ignored_files_do_not_trigger_skip_warning() {
    ...
    assert!(
        !commit
            .stdout
            .contains("Skipped autter stats for large commit"),
...
fn test_stats_ignores_renamed_files() {
    ...
    // Verify that autter diff recognizes this as a rename
    let diff_output = repo.autter(&["diff", "HEAD"]).unwrap();
    assert!(
        diff_output.contains("similarity index 100%") || diff_output.contains("rename from"),
        "autter diff should recognize pure renames"
    );
