...
use autter::commands::checkpoint_agent::bash_tool;
use autter::daemon::control_api::ControlRequest;
use autter::daemon::send_control_request_with_timeout;
use std::fs;
...
/// Spawns an isolated `autter bg run` daemon for benchmarking and kills it on
/// drop.  Sets `AUTTER_DAEMON_CONTROL_SOCKET` in the current process so that
/// `query_daemon_watermarks` inside `handle_bash_tool` connects to this daemon
/// instead of the system-wide one.
struct BenchDaemon {
    child: Child,
    control_socket: PathBuf,
    /// Saved value of AUTTER_DAEMON_CONTROL_SOCKET before we overwrote it.
...
impl BenchDaemon {
    fn start(repo_root: &Path, daemon_home: &Path) -> Self {
        ...
        let binary = {
            ...
            let rel = manifest.join("target/release/autter");
            let dbg = manifest.join("target/debug/autter");
        ...
        let child = Command::new(&binary)
            ...
            .env("AUTTER_DAEMON_HOME", daemon_home)
            .env("AUTTER_DAEMON_CONTROL_SOCKET", &control_socket)
            .env("AUTTER_DAEMON_TRACE_SOCKET", &trace_socket)
            .env("AUTTER_TEST_DB_PATH", &test_db)
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
        ...
        let prev_socket_env = std::env::var("AUTTER_DAEMON_CONTROL_SOCKET").ok();
        // SAFETY: benchmark tests run single-threaded with #[ignore]; no other
        // threads read this env var concurrently during the test.
        unsafe { std::env::set_var("AUTTER_DAEMON_CONTROL_SOCKET", &control_socket) };
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            match &self.prev_socket_env {
                Some(v) => std::env::set_var("AUTTER_DAEMON_CONTROL_SOCKET", v),
                None => std::env::remove_var("AUTTER_DAEMON_CONTROL_SOCKET"),
            }
        }
