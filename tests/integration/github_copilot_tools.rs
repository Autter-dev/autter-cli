...
/// an object with a `path` field. Diffs should still expose the Copilot CLI tool.
#[test]
fn test_copilot_cli_claude_format_edit_update_attribution_and_diff_json() {
    let repo = TestRepo::new();
    ...
    let diff_output = repo
        .autter(&["diff", &commit_sha, "--json", "--include-stats"])
        .expect("diff json should succeed");
    let diff: Value = serde_json::from_str(&diff_output).expect("diff output should be JSON");

    let sessions = diff["sessions"]
        .as_object()
        .expect("diff should include sessions");
    assert!(
        sessions
            .values()
            .any(|session| session["agent_id"]["tool"] == "github-copilot-cli"),
        "diff sessions should include github-copilot-cli, got: {sessions:#?}"
    );

    let breakdown = diff["commit_stats"]["tool_model_breakdown"]
        .as_object()
