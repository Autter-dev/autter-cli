...
fn status_from_args(repo: &TestRepo, args: &[&str]) -> StatusOutput {
    let raw = repo.autter(args).expect("autter status should succeed");
...
#[test]
fn test_checkpoint_honors_autter_ignore_file() {
    let repo = TestRepo::new();
    ...
    assert!(
        ...
        "Expected .autter-ignore pattern to filter out docs/guide.md"
    );
}

#[test]
fn test_status_honors_autter_ignore_file() {
    let repo = TestRepo::new();
    ...
    repo.autter(&["checkpoint", "mock_ai", "src/app.ts", "docs/api.md"])
        .unwrap();
...
#[test]
fn test_status_autter_ignore_union_with_gitattributes() {
    let repo = TestRepo::new();
    ...
    // docs/api.md ignored by .autter-ignore
    assert_eq!(status.stats.git_diff_added_lines, 1);
    assert_eq!(status.stats.git_diff_deleted_lines, 0);
    assert_eq!(status.stats.ai_accepted, 1);
}
...
crate::reuse_tests_in_worktree!(
    ...
    test_checkpoint_honors_autter_ignore_file,
    test_status_honors_autter_ignore_file,
    test_status_autter_ignore_union_with_gitattributes,
