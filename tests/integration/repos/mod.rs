                ...
                impl TestRepoWithCFlag {
                    ...
                    fn git_from_working_dir(
                        ...
                        if mode.uses_daemon() {
                            ...
                            let nesting = std::env::var("AUTTER_TEST_TRACE2_NESTING")
                                .unwrap_or_else(|_| "10".to_string());
                            command.env(
                                "GIT_TRACE2_EVENT",
                                autter::daemon::DaemonConfig::trace2_event_target_for_path(
                                    &trace_socket,
                                ),
                                );
                        ...
                        command.env("AUTTER_TEST_DB_PATH", self.inner.test_db_path().to_str().unwrap());
                        command.env("AUTTER_TEST_DB_PATH", self.inner.test_db_path().to_str().unwrap());
