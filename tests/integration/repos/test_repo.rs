...
use autter::authorship::stats::CommitStats;
use autter::config::ConfigPatch;
use autter::daemon::{
    ControlRequest, DaemonConfig, local_socket_connects_with_timeout, send_control_request,
    send_control_request_with_timeout,
};
use autter::feature_flags::FeatureFlags;
use autter::git::cli_parser::{ParsedGitInvocation, extract_clone_target_directory};
use autter::git::repo_storage::PersistedWorkingLog;
use autter::git::repository as AutterRepository;
...
impl GitTestMode {
    pub fn from_env() -> Self {
        let mode = std::env::var("AUTTER_TEST_GIT_MODE")
...
impl DaemonProcess {
    ...
    fn start_with_env(
        ...
        let mut child = command
            .spawn()
            .expect("failed to spawn autter subprocess for test mode");
        let pid = child.id();
    ...
    fn wait_until_trace_pipeline_ready(
        ...
        command
            ...
            .args(["config", "--local", "autter.test-readiness-probe", "1"])
            .env(
                "GIT_TRACE2_EVENT",
...
fn configure_test_home_env(command: &mut Command, test_home: &Path) {
    ...
    // Sanitize PATH: remove any directories that contain a autter wrapper.
    // Without this, git internals (which call `git` sub-processes via PATH) will
    // hit the installed release autter binary, which spawns a background daemon
        ...
        let sanitized: Vec<&str> = path
            ...
            .filter(|dir| {
                ...
                if git_path.is_file() || git_path.is_symlink() {
                    // Shell-script wrapper containing "autter"
                    if let Ok(contents) = fs::read_to_string(&git_path)
                        && contents.contains("autter")
                    {
                        return false;
                    }
                    // Symlink whose target contains "autter"
                    if let Ok(target) = std::fs::read_link(&git_path)
                        && target.to_string_lossy().contains("autter")
                    {
                        return false;
                    }
                    // Canonical path contains "autter"
                    if let Ok(canonical) = git_path.canonicalize()
                        && canonical.to_string_lossy().contains("autter")
                    {
                        return false;
                    }
                }
...
fn shared_daemon_pool_size() -> usize {
    std::env::var("AUTTER_TEST_SHARED_DAEMON_POOL_SIZE")
        .ok()
...
fn start_shared_daemon_process(repo_path: &Path, shard: Option<usize>) -> DaemonProcess {
    ...
    let shard_suffix = shard
        ...
        .unwrap_or_default();
    let daemon_home = base.join(format!("autter-shared-daemon-{}{}-home", n, shard_suffix));
    let test_db_path = base.join(format!("autter-shared-daemon-{}{}-db", n, shard_suffix));
...
fn resolve_test_db_path(
    ...
    if git_mode.uses_hooks() {
        test_home.join(".autter").join("internal").join("db")
    } else {
...
fn autter_primary_command<'a>(args: &'a [&'a str]) -> Option<&'a str> {
...
fn normalize_test_autter_checkpoint_args(args: &[&str]) -> Vec<String> {
    let original = args
        .iter()
        .map(|arg| (*arg).to_string())
        .collect::<Vec<_>>();
    if autter_primary_command(args) != Some("checkpoint") || args.len() <= 1 {
        return original;
    }
...
fn autter_command_requires_daemon_sync(args: &[&str]) -> bool {
    matches!(
        autter_primary_command(args),
...
impl TestRepo {
    fn parsed_git_invocation_for_tracking(
        ...
        autter::daemon::test_sync::tracked_parsed_git_invocation_for_test_sync(&argv, cwd)
    }
    ...
    pub(crate) fn git_command_affects_daemon_for_tracking(
        ...
        autter::daemon::test_sync::tracks_parsed_git_invocation_for_test_sync(&parsed)
    }
    ...
    fn apply_default_config_patch(&mut self) {
        self.patch_autter_config(|patch| {
    ...
    pub fn new_with_mode_and_daemon_scope(
        ...
        // Isolate this test binary's HOME before any git or autter subprocess is spawned.
    ...
    pub(crate) fn set_daemon_env_for_in_process(&self) {
        unsafe {
            std::env::set_var("AUTTER_DAEMON_HOME", self.daemon_home_path());
            std::env::set_var(
                "AUTTER_DAEMON_CONTROL_SOCKET",
    ...
    fn trace2_nesting_value() -> String {
        std::env::var("AUTTER_TEST_TRACE2_NESTING").unwrap_or_else(|_| "10".to_string())
    }
    ...
    fn daemon_family_key_for_repo_path(&self, repo_path: &Path) -> String {
        let repo = AutterRepository::find_repository_in_path(repo_path.to_str().unwrap())
            .unwrap_or_else(|e| {
                panic!(
    ...
    fn maybe_daemon_family_key_for_repo_path(&self, repo_path: &Path) -> Option<String> {
        ...
        let repo = AutterRepository::find_repository_in_path(lookup_path.to_str()?).ok()?;
        let common_dir = repo
            .common_dir()
            .canonicalize()
            .unwrap_or_else(|_| repo.common_dir().to_path_buf());
    ...
    pub(crate) fn append_daemon_test_sync_session_args(
        ...
        args.push(format!(
            "{}={}",
            autter::daemon::test_sync::TEST_SYNC_SESSION_CONFIG_KEY,
            session
        ));
    }

    fn checkpoint_path_args<'a>(&self, args: &'a [&'a str]) -> Vec<&'a str> {
        if autter_primary_command(args) != Some("checkpoint") {
            return Vec::new();
        }
    ...
    fn setup_git_hooks_mode(&self) {
        ...
        let output = command
            .output()
            .expect("failed to run autter git-hooks ensure in test setup");
        if !output.status.success() {
            panic!(
                "autter git-hooks ensure failed during test setup:\nstdout: {}\nstderr: {}",
                String::from_utf8_lossy(&output.stdout),
                String::from_utf8_lossy(&output.stderr),
            );
        }
    ...
    fn configure_command_env(&self, command: &mut Command) {
        ...
        if self.git_mode.uses_hooks() {
            command.env("AUTTER_GLOBAL_GIT_HOOKS", "true");
        }

        if self.git_mode.uses_wrapper() {
            command.env("AUTTER", "git");
        }
        ...
        if self.git_mode == GitTestMode::WrapperDaemon {
            command.env("AUTTER_DAEMON_HOME", self.daemon_home_path());
            command.env(
                "AUTTER_DAEMON_CONTROL_SOCKET",
                self.daemon_control_socket_path(),
            );
    ...
    /// Patch the autter config for this test repo
    /// Allows overriding specific config properties like ignore_prompts, telemetry settings, etc.
    /// The patch is applied via environment variable when running autter commands
    ///
    /// # Example
    /// ```ignore
    ...
    /// });
    /// ```
    pub fn patch_autter_config<F>(&mut self, f: F)
    where
    ...
    pub fn stats(&self) -> Result<CommitStats, String> {
        let output = self.autter(&["stats", "--json"])?;
        let start = output
            .find('{')
    ...
    pub fn autter(&self, args: &[&str]) -> Result<String, String> {
        self.autter_with_env(args, &[])
    }
    ...
    /// This tests that autter correctly finds the repository root when run from a subdirectory
    ...
    /// Run a raw git command (bypassing autter hooks) with custom environment variables.
    ...
    pub fn git_og_with_env(&self, args: &[&str], envs: &[(&str, &str)]) -> Result<String, String> {
        ...
        let command_affects_daemon = env_explicitly_enables_trace2(envs)
            && autter::daemon::test_sync::tracks_parsed_git_invocation_for_test_sync(
                &tracked_invocation,
            );
    ...
    pub fn benchmark_git(&self, args: &[&str]) -> Result<BenchmarkResult, String> {
        let output = self.git_with_env(args, &[("AUTTER_DEBUG_PERFORMANCE", "2")], None)?;

        println!("output: {}", output);
        Self::parse_benchmark_result(&output)
    }

    pub fn benchmark_autter(&self, args: &[&str]) -> Result<BenchmarkResult, String> {
        let output = self.autter_with_env(args, &[("AUTTER_DEBUG_PERFORMANCE", "2")])?;
    ...
    fn parse_benchmark_result(output: &str) -> Result<BenchmarkResult, String> {
        ...
        for line in output.lines() {
            if line.contains("[autter (perf-json)]") {
    ...
    pub fn autter_from_working_dir(
        &self,
        working_dir: &std::path::Path,
        args: &[&str],
    ) -> Result<String, String> {
    ...
    pub fn autter_with_env(&self, args: &[&str], envs: &[(&str, &str)]) -> Result<String, String> {
        ...
        if output.status.success() {
            ...
            // Combine stdout and stderr since autter often writes to stderr
            let combined = if stdout.is_empty() {
                stderr
            } else if stderr.is_empty() {
                stdout
    ...
    /// Run a autter command with data provided on stdin
    pub fn autter_with_stdin(&self, args: &[&str], stdin_data: &[u8]) -> Result<String, String> {
        use std::io::Write;
        ...
        let mut child = command
            .spawn()
            .unwrap_or_else(|_| panic!("Failed to spawn autter command: {:?}", args));
        ...
        let output = child
            .wait_with_output()
            .unwrap_or_else(|_| panic!("Failed to wait for autter command: {:?}", args));
    ...
    pub fn current_working_logs(&self) -> PersistedWorkingLog {
        ...
        let repo = AutterRepository::find_repository_in_path(self.path.to_str().unwrap())
    ...
    /// This tests that autter correctly handles commits when run from a subdirectory
    /// The working_dir will be canonicalized to ensure it's an absolute path
    ...
    pub fn commit_with_env(
        ...
        match output {
            Ok(combined) => {
                ...
                let mut content = autter::git::refs::show_authorship_note(&repo, &head_commit);
                if content.is_none() && self.git_mode.uses_daemon() {
                    for _ in 0..10 {
                        thread::sleep(Duration::from_millis(50));
                        content = autter::git::refs::show_authorship_note(&repo, &head_commit);
                        if content.is_some() {
                            break;
                        }
                    }
...
impl Drop for TestRepo {
    fn drop(&mut self) {
        if std::env::var("AUTTER_TEST_KEEP_REPOS")
            .map(|v| v == "1")
            .unwrap_or(false)
        {
            return;
...
fn find_real_git_by_probe() -> String {
    ...
    if !local_git.is_empty() {
        ...
        if autter::config::is_real_git_candidate(p) {
            return local_git;
        }
    }
    ...
    // Last resort: rely on PATH (will fail if only autter is on PATH, but
...
fn ensure_isolated_process_home() {
    ...
    PROCESS_HOME.get_or_init(|| {
        ...
        // Minimal ~/.autter/config.json: real git_path
        let autter_dir = home.join(".autter");
        fs::create_dir_all(&autter_dir).expect("create .autter dir");
        // Escape backslashes for JSON (relevant on Windows)
        let real_git_json = real_git.replace('\\', "\\\\");
        fs::write(
            autter_dir.join("config.json"),
            format!(r#"{{"git_path":"{real_git_json}"}}"#),
        )
        .expect("write test autter config");
        ...
        unsafe {
            ...
            // Sanitize the process-level PATH to remove autter wrapper directories.
            ...
            // git internals from resolving `git` via PATH to the installed autter
...
pub(crate) fn real_git_executable() -> &'static str {
    ...
    ensure_isolated_process_home();
    autter::config::Config::get().git_cmd()
}
...
fn init_template_repo() -> PathBuf {
    let path = std::env::temp_dir().join(format!("autter-test-template-{}", std::process::id()));
...
fn init_bare_template_repo() -> PathBuf {
    let path =
        std::env::temp_dir().join(format!("autter-test-template-bare-{}", std::process::id()));
    let _ = fs::remove_dir_all(&path);
...
fn compile_binary() -> PathBuf {
    if let Ok(override_path) = std::env::var("AUTTER_TEST_BINARY_PATH") {
        let path = PathBuf::from(override_path);
        if path.is_file() {
            println!(
                "Using prebuilt autter test binary from AUTTER_TEST_BINARY_PATH: {}",
                path.display()
            );
            return path;
        }
        panic!(
            "AUTTER_TEST_BINARY_PATH does not point to a file: {}",
            path.display()
        );
    }

    println!("Compiling autter binary for tests...");

    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let output = Command::new("cargo")
        .args(["build", "--bin", "autter", "--features", "test-support"])
        .current_dir(manifest_dir)
        .output()
        .expect("Failed to compile autter binary");

    if !output.status.success() {
        panic!(
            "Failed to compile autter:\nstdout: {}\nstderr: {}",
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        );
    }
    ...
    #[cfg(windows)]
    let binary_path = PathBuf::from(&target_dir).join("debug/autter.exe");
    #[cfg(not(windows))]
    let binary_path = PathBuf::from(&target_dir).join("debug/autter");
...
mod tests {
    ...
    #[test]
    fn test_normalize_test_autter_checkpoint_args_inserts_separator_for_direct_file() {
        assert_eq!(
            normalize_test_autter_checkpoint_args(&["checkpoint", "src/lib.rs"]),
            vec!["checkpoint", "--", "src/lib.rs"]
        );
    }

    #[test]
    fn test_normalize_test_autter_checkpoint_args_preserves_known_presets_and_separator() {
        assert_eq!(
            normalize_test_autter_checkpoint_args(&["checkpoint", "mock_ai", "src/lib.rs"]),
    ...
    #[test]
    fn test_normalize_test_autter_checkpoint_args_handles_hook_input_before_pathspecs() {
        assert_eq!(
            normalize_test_autter_checkpoint_args(&[
                "checkpoint",
                "--hook-input",
                "{\"cwd\":\"/tmp/repo\"}",
                "src/lib.rs",
    ...
    #[test]
    fn test_isolated_process_home_controls_autter_internal_dir() {
        ensure_isolated_process_home();
        ...
        #[cfg(windows)]
        {
            ...
            assert_eq!(
                ...
                "",
                "HOMEDRIVE should not point autter back at the real user profile"
            );
            assert_eq!(
                std::env::var("HOMEPATH").unwrap_or_default(),
                "",
                "HOMEPATH should not point autter back at the real user profile"
            );
        }

        assert_eq!(
            autter::config::internal_dir_path().expect("internal dir should resolve"),
            home.join(".autter").join("internal"),
            "in-process autter config lookup must use the isolated test home"
        );
    }
}
