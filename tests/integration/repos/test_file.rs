...
impl<'a> TestFile<'a> {
    ...
    pub fn from_existing_file(file_path: PathBuf, repo: &'a super::test_repo::TestRepo) -> Self {
        ...
        let blame_result = repo.autter(&["blame", filename]);
    ...
    pub fn assert_blame_snapshot(&self) {
        ...
        let blame_output = self
            .repo
            .autter(&["blame", filename])
            .expect("autter blame should succeed");
    ...
    /// Parse autter blame output and extract the author for each line
    /// Format: sha (author date line_num) content
    ...
    fn run_checkpoint_for_author_type(&self, author_type: &AuthorType) {
        ...
        let result = match author_type {
            AuthorType::Ai => self
                .repo
                .autter(&["checkpoint", "mock_ai", relative_path.as_str()]),
            AuthorType::Human => {
                self.repo
                    .autter(&["checkpoint", "mock_known_human", relative_path.as_str()])
            }
            AuthorType::UnattributedHuman => {
                self.repo
                    .autter(&["checkpoint", "--", relative_path.as_str()])
            }
        };
        result.unwrap();
    }
