...
use autter::streams::agents::AmpAgent;
use autter::streams::watermark::RecordIndexWatermark;
use serde_json::json;
use std::fs;
...
fn test_amp_preset_pretooluse_returns_human_checkpoint() {
    unsafe {
        std::env::set_var("AUTTER_AMP_THREADS_PATH", amp_threads_fixture_path());
    }
    ...
    unsafe {
        std::env::remove_var("AUTTER_AMP_THREADS_PATH");
    }
...
fn test_amp_e2e_checkpoint_and_commit() {
    ...
    unsafe {
        std::env::set_var("AUTTER_AMP_THREADS_PATH", temp_threads.path());
    }
    ...
    .to_string();
    repo.autter(&["checkpoint", "amp", "--hook-input", &pre_hook_input])
        .unwrap();
    ...
    .to_string();
    repo.autter(&["checkpoint", "amp", "--hook-input", &post_hook_input])
        .unwrap();
