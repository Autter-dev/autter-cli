...
use autter::authorship::attribution_tracker::{
...
use autter::authorship::rebase_authorship::{
...
use autter::authorship::virtual_attribution::VirtualAttributions;
use autter::authorship::working_log::{AgentId, Checkpoint, CheckpointKind};
use autter::error::AutterError;
use autter::git::refs::{notes_add, show_authorship_note};
use autter::git::repository::find_repository_in_path;
use autter::git::rewrite_log::{RebaseCompleteEvent, RewriteLogEvent};
use std::collections::{HashMap, HashSet};

fn try_fast_path_rebase_note_remap(
    repo: &autter::git::repository::Repository,
...
fn walk_commits_to_base_linear_history_is_bounded_and_ordered() {
    ...
    let autter_repo = find_repository_in_path(repo.path().to_str().unwrap()).unwrap();
    let base = repo
    ...
    let commits = walk_commits_to_base(&autter_repo, &head, &base).expect("walk should succeed");
...
fn walk_commits_to_base_merge_history_includes_both_sides_without_full_dag_walk() {
    ...
    let commits =
        walk_commits_to_base(&autter_repo, &merge_head, &base).expect("walk should succeed");
...
fn walk_commits_to_base_rejects_non_ancestor_base() {
    ...
    let err = walk_commits_to_base(&autter_repo, &middle, &top).expect_err("should fail");
...
fn get_pathspecs_from_commits_keeps_hex_filenames() {
    ...
    let paths = get_pathspecs_from_commits(&autter_repo, &[commit_sha])
...
fn collect_changed_file_contents_from_diff_handles_add_modify_delete_and_filtering() {
    ...
    let head_sha = repo
        ...
        .trim()
        .to_string();
    let head = autter_repo.find_commit(head_sha).expect("head commit");
    ...
    let (changed, contents) =
        collect_changed_file_contents_from_diff(&autter_repo, &diff, &tracked_all)
    ...
    let (subset_changed, subset_contents) =
        collect_changed_file_contents_from_diff(&autter_repo, &diff, &tracked_subset)
...
fn fast_path_rebase_note_remap_copies_logs_when_tracked_blobs_match() {
    ...
    let original_commit = repo
        ...
        .trim()
        .to_string();
    write_minimal_authorship_note(&autter_repo, &original_commit, "ai.txt", "mock_ai");
    ...
    let remapped_note_raw =
        show_authorship_note(&autter_repo, &new_commit).expect("new note content");
    let remapped =
...
fn rebase_complete_merges_initial_when_both_working_logs_exist() {
    ...
    assert!(
        !autter_repo.storage.has_working_log(&original_head),
...
fn regression_initial_preserved_through_checkpoint_commit_rebase() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "app.py", "utils.py"])
...
fn diff_based_transfer_equal_content() {
    use autter::authorship::rebase_authorship::diff_based_line_attribution_transfer;
...
fn flatten_prompts_picks_per_commit_record_for_same_session_multi_commit() {
    ...
    // Note for commit A: 5 AI lines (feature.txt lines 3-7)
    {
        ...
        notes_add(&autter_repo, &sha_a, &note).expect("write note A");
    }
    ...
    // Note for commit B: 10 AI lines (other.txt lines 1-10)
    {
        ...
        notes_add(&autter_repo, &sha_b, &note).expect("write note B");
    }
    ...
    // Verify new_A note
    {
        let note_raw = show_authorship_note(&autter_repo, &new_a).expect("read new_A note");
    ...
    // Verify new_B note
    {
        let note_raw = show_authorship_note(&autter_repo, &new_b).expect("read new_B note");
