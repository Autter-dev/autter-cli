...
/// When autter runs inside a no-hooks background agent (simulated via
/// `AUTTER_CLOUD_AGENT=1` on the daemon), commits should be attributed wholly
/// to the detected AI tool even though no checkpoints were fired.
#[test]
fn test_no_hooks_agent_all_lines_attributed() {
    let repo = TestRepo::new_with_daemon_env(&[("AUTTER_CLOUD_AGENT", "1")]);
