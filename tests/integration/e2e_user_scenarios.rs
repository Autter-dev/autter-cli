...
fn test_basic_workflow_mixed_authorship() {
    ...
    repo.autter(&["checkpoint", "mock_known_human"]).unwrap();
    ...
    .unwrap();
    repo.autter(&["checkpoint", "mock_ai", "example.py"])
        .unwrap();
    ...
    let blame_output = repo.autter(&["blame", "example.py"]).unwrap();
    assert!(
...
fn test_checkpoint_mock_ai_with_file_path() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "ai_file.txt"])
        .unwrap();
}
...
fn test_blame_shows_correct_attribution() {
    ...
    let blame_output = repo.autter(&["blame", "test.txt"]).unwrap();
...
fn test_stats_shows_ai_contribution() {
    ...
    fs::write(&file_path, "user line\nAI line 1\nAI line 2\nAI line 3\n").unwrap();
    repo.autter(&["checkpoint", "mock_ai", "stats_test.txt"])
        .unwrap();
...
";
    ...
    repo.autter(&["checkpoint", "mock_ai", "code.py"]).unwrap();
...
";
    ...
    repo.autter(&["checkpoint", "mock_ai", "calculator.py"])
        .unwrap();
...
";
    fs::write(&file_path, ai_code).unwrap();
    repo.autter(&["checkpoint", "mock_ai", "new_module.py"])
        .unwrap();
...
";
    ...
    repo.autter(&["checkpoint", "mock_known_human", "example.txt"])
        .unwrap();
...
";
    ...
    let blame_after = repo.autter(&["blame", "example.txt"]).unwrap();
...
// ---------------------------------------------------------------------------
#[test]
#[ignore = "https://github.com/autter-dev/autter-cli/issues/162"]
...
// ---------------------------------------------------------------------------
#[test]
#[ignore = "https://github.com/autter-dev/autter-cli/issues/169"]
...
";
    ...
    repo.autter(&["checkpoint", "mock_ai", "feature.py"])
        .unwrap();
...
";
    fs::write(&file_path, ai_edit).unwrap();
    repo.autter(&["checkpoint", "mock_ai", "shared.py"])
        .unwrap();
    repo.git(&["add", "shared.py"]).unwrap();
...
// ---------------------------------------------------------------------------
// Test 16: autter stats range command
// ---------------------------------------------------------------------------
#[test]
...
";
    ...
    let raw_single = repo
        .autter(&["stats", &range_single, "--json"])
        .expect("stats range should succeed");
...
";
    ...
    repo.autter(&["checkpoint", "mock_ai", "api_handler.py"])
        .unwrap();
...
";
    ...
    let blame_after = repo.autter(&["blame", "api_handler.py"]).unwrap();
    assert!(blame_after.contains("mock_ai"));
    assert!(blame_after.contains("Test User"));
...
//
// Scenario from https://github.com/autter-dev/autter-cli/issues/394:
...
fn test_issue_394_multiuser_reformat_ai_attribution() {
    ...
    repo.autter(&["checkpoint", "human", "hello.js"]).unwrap();
...
";
    ...
    repo.autter(&["checkpoint", "mock_ai", "hello.js"]).unwrap();
    ...
    let blame_output = repo.autter(&["blame", "hello.js"]).unwrap();
    eprintln!("=== autter blame output (issue #394) ===\n{blame_output}");
