...
use autter::mdm::jetbrains::download::{
...
fn create_test_plugin_zip() -> Vec<u8> {
    let mut buffer = Vec::new();
    {
        ...
        zip.start_file("autter-plugin/plugin.xml", options).unwrap();
        zip.write_all(b"<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<idea-plugin></idea-plugin>")
            .unwrap();

        // Add a lib directory
        zip.add_directory("autter-plugin/lib/", options).unwrap();

        // Add a jar file
        zip.start_file("autter-plugin/lib/plugin.jar", options)
            .unwrap();
...
fn create_test_plugin_zip_with_executable() -> Vec<u8> {
    let mut buffer = Vec::new();
    {
        ...
        zip.start_file("autter-plugin/bin/plugin-launcher.sh", options)
            .unwrap();
        zip.write_all(b"#!/bin/bash\necho 'test'").unwrap();

        // Add regular file
        let regular_options: FileOptions<()> = FileOptions::default();
        zip.start_file("autter-plugin/README.md", regular_options)
            .unwrap();
...
fn test_install_plugin_extracts_files() {
    ...
    let plugin_xml = plugin_dir.join("autter-plugin/plugin.xml");
    assert!(plugin_xml.exists(), "plugin.xml should be extracted");

    let jar_file = plugin_dir.join("autter-plugin/lib/plugin.jar");
...
fn test_install_plugin_creates_nested_directories() {
    ...
    let lib_dir = plugin_dir.join("autter-plugin/lib");
...
fn test_install_plugin_preserves_executable_permissions() {
    ...
    let script_path = plugin_dir.join("autter-plugin/bin/plugin-launcher.sh");
...
fn test_install_plugin_handles_directory_entries() {
    ...
    let mut buffer = Vec::new();
    {
        ...
        zip.add_directory("autter-plugin/", options).unwrap();
        zip.add_directory("autter-plugin/resources/", options)
            .unwrap();

        // Add file in directory
        zip.start_file("autter-plugin/resources/config.json", options)
            .unwrap();
    ...
    let resources_dir = plugin_dir.join("autter-plugin/resources");
...
fn test_install_plugin_via_cli_paths_and_args() {
    ...
    let plugin_id = "com.useautter.plugins.jetbrains";
...
fn test_install_plugin_with_special_characters_in_filename() {
    ...
    let mut buffer = Vec::new();
    {
        ...
        zip.start_file("autter-plugin/resources/strings_en.xml", options)
            .unwrap();
        zip.write_all(b"<strings></strings>").unwrap();

        zip.start_file("autter-plugin/resources/strings_中文.xml", options)
            .unwrap();
        zip.write_all(b"<strings></strings>").unwrap();
    ...
    let en_file = plugin_dir.join("autter-plugin/resources/strings_en.xml");
    assert!(en_file.exists(), "English strings file should exist");

    let zh_file = plugin_dir.join("autter-plugin/resources/strings_中文.xml");
...
fn test_install_plugin_with_deep_nesting() {
    ...
    let mut buffer = Vec::new();
    {
        ...
        let deep_path = "autter-plugin/src/main/java/com/useautter/plugin/actions/DeepFile.java";
        zip.start_file(deep_path, options).unwrap();
        zip.write_all(b"package com.useautter.plugin.actions;")
            .unwrap();
    ...
    let deep_file =
        plugin_dir.join("autter-plugin/src/main/java/com/useautter/plugin/actions/DeepFile.java");
...
fn test_install_plugin_overwrites_existing_files() {
    ...
    // Create initial file
    let file_path = plugin_dir.join("autter-plugin/plugin.xml");
...
fn test_install_plugin_with_large_files() {
    ...
    let mut buffer = Vec::new();
    {
        ...
        zip.start_file("autter-plugin/large-library.jar", options)
            .unwrap();
    ...
    let large_file = plugin_dir.join("autter-plugin/large-library.jar");
