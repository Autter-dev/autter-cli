...
use autter::streams::agents::ContinueAgent;
use autter::streams::watermark::RecordIndexWatermark;
use serde_json::json;
use std::fs;

fn parse_continue(hook_input: &str) -> Result<Vec<ParsedHookEvent>, autter::error::AutterError> {
...
fn test_continue_cli_e2e_with_attribution() {
    ...
    let result = repo
        .autter(&["checkpoint", "continue-cli", "--hook-input", &hook_input])
        .unwrap();
...
fn test_continue_cli_e2e_multiple_tool_calls() {
    ...
    repo.autter(&["checkpoint", "continue-cli", "--hook-input", &hook_input])
        .unwrap();
