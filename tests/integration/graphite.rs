/// Graphite (`gt` CLI) test suite for autter attribution preservation.
///
/// These tests verify that autter attribution (line-level blame tracking of AI vs human authorship)
...
///
/// autter's wrapper-mode `update-ref` post-hook intercepts the ref move, detects the
...
/// real git binary or the autter wrapper depending on the test mode.
static GT_GIT_SHIM_DIR: OnceLock<PathBuf> = OnceLock::new();

fn gt_git_shim_dir() -> &'static PathBuf {
    GT_GIT_SHIM_DIR.get_or_init(|| {
        let shim_binary = PathBuf::from(env!("CARGO_BIN_EXE_autter-test-git-shim"));
        let shim_dir =
            std::env::temp_dir().join(format!("autter-gt-git-shim-{}", std::process::id()));
...
fn new_gt_started_log_path() -> PathBuf {
    std::env::temp_dir().join(format!(
        "autter-gt-started-{}-{}.jsonl",
        std::process::id(),
        autter::uuid::generate_v4()
    ))
}
...
/// pointing to the autter binary, all of `gt`'s git operations flow through
/// the autter wrapper. This ensures attribution notes are properly created
...
fn gt(repo: &TestRepo, args: &[&str]) -> Result<String, String> {
    ...
    command.env("AUTTER_TEST_GIT_SHIM_TARGET", gt_git_target(repo));
    command.env(
        "AUTTER_TEST_GIT_SHIM_FALLBACK_TARGET",
        real_git_executable(),
    );
    if repo.mode().uses_wrapper() {
        command.env("AUTTER_TEST_GIT_SHIM_TARGET_USE_AUTTER", "1");
    }
    if let Some(started_log_path) = started_log_path.as_ref() {
        command.env("AUTTER_TEST_SYNC_START_LOG", started_log_path);
    }
    ...
    if repo.mode().uses_daemon() {
        ...
        command.env(
            "GIT_TRACE2_EVENT",
            autter::daemon::DaemonConfig::trace2_event_target_for_path(&trace_socket),
        );
        command.env("GIT_TRACE2_EVENT_NESTING", nesting);
    }

    // In WrapperDaemon mode, the shim's target (autter wrapper) needs daemon
    ...
    if repo.mode() == GitTestMode::WrapperDaemon {
        command.env("AUTTER_DAEMON_HOME", repo.daemon_home_path());
        command.env(
    ...
    command.env("AUTTER_TEST_DB_PATH", repo.test_db_path().to_str().unwrap());
    command.env("AUTTER_TEST_DB_PATH", repo.test_db_path().to_str().unwrap());

    // Pass config patch (needed for wrapper-daemon mode).
    if let Some(patch) = repo.config_patch_json() {
        command.env("AUTTER_TEST_CONFIG_PATCH", patch);
    }
...
/// Install autter hooks in a test repo so that `gt` rebase operations
/// trigger autter's post-rewrite hook for attribution note copying.
...
fn install_hooks(repo: &TestRepo) {
    ...
    let output = command
        .output()
        .expect("failed to run autter git-hooks ensure");
    if !output.status.success() {
        panic!(
            "autter git-hooks ensure failed:\nstdout: {}\nstderr: {}",
...
/// Also installs autter hooks so that rebase-based gt operations
...
/// instead of `git rebase`, bypassing autter attribution tracking entirely.
#[test]
#[ignore]
...
/// to amend earlier commits, bypassing autter attribution tracking entirely.
#[test]
#[ignore]
