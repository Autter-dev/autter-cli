...
impl GitHubTestRepo {
    ...
    pub fn install_github_ci_workflow(&self) -> Result<(), String> {
        // Use autter to install the workflow
        let output = self
            .repo
            .autter(&["ci", "github", "install"])
        ...
        self.repo
            .git(&["add", ".github/workflows/autter.yaml"])
            .map_err(|e| format!("Failed to add workflow file: {}", e))?;

        self.repo
            .git(&["commit", "-m", "Add autter CI workflow"])
...
impl Drop for GitHubTestRepo {
    fn drop(&mut self) {
        if std::env::var("AUTTER_TEST_NO_CLEANUP").is_ok() {
            eprintln!(
...
fn generate_repo_name(test_name: &str) -> String {
    ...
    format!("autter-{}-{}", truncated_name, timestamp)
}
