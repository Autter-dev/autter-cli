...
# It searches for repositories matching the pattern 'autter-test-*' and deletes them.
...
echo "🔍 Searching for test repositories (autter-test-*)..."
echo ""

# Get list of repositories matching the pattern
REPOS=$(gh repo list "$GITHUB_USER" --json name --jq '.[] | select(.name | startswith("autter-test-")) | .name')
