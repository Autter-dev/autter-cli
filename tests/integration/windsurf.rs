...
use autter::streams::agents::WindsurfAgent;
...
fn parse_windsurf(hook_input: &str) -> Result<Vec<ParsedHookEvent>, AutterError> {
...
fn test_windsurf_e2e_with_attribution() {
    ...
    repo.autter(&["checkpoint", "windsurf", "--hook-input", &hook_input])
        .unwrap();
...
fn test_windsurf_preset_post_run_command_detects_changed_files() {
    ...
    repo.autter(&["checkpoint", "windsurf", "--hook-input", &pre_hook_input])
        .unwrap();
    ...
    repo.autter(&["checkpoint", "windsurf", "--hook-input", &post_hook_input])
        .unwrap();
...
fn test_windsurf_preset_post_run_command_without_snapshot_falls_back_gracefully() {
    ...
    let result = repo.autter(&["checkpoint", "windsurf", "--hook-input", &hook_input]);
    assert!(
        result.is_ok(),
...
fn test_windsurf_e2e_run_command_attribution() {
    ...
    .to_string();
    repo.autter(&["checkpoint", "windsurf", "--hook-input", &pre_hook])
        .unwrap();
    ...
    .to_string();
    repo.autter(&["checkpoint", "windsurf", "--hook-input", &post_hook])
        .unwrap();
...
fn test_windsurf_known_human_suppressed_during_pending_ai_edit() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "race.txt"])
        .unwrap();
...
fn test_known_human_not_suppressed_without_pending_ai_edit() {
    ...
    fs::write(&file_path, "first line\nhuman added line\n").unwrap();
    repo.autter(&["checkpoint", "mock_known_human", "human_edit.txt"])
        .unwrap();
