...
use autter::authorship::range_authorship::{EMPTY_TREE_HASH, range_authorship, should_ignore_file};
use autter::git::repository::{CommitRange, find_repository_in_path};
...
fn test_range_authorship_empty_tree_with_multiple_files() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "file1.txt"])
        .unwrap();
    repo.autter(&["checkpoint", "mock_ai", "file2.txt"])
        .unwrap();
...
fn test_range_authorship_lockfile_only_commit() {
    ...
    repo.git(&["add", "package-lock.json", "yarn.lock"])
        .unwrap();
    repo.autter(&["checkpoint", "mock_known_human", "package-lock.json"])
        .unwrap();
