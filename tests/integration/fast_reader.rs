...
fn test_fast_head_matches_git_cli() {
    ...
    let reader = autter::git::fast_reader::FastRefReader::new(&git_dir, &git_dir);
    ...
    match fast_result {
        autter::git::fast_reader::HeadKind::Symbolic(refname) => {
            assert_eq!(refname, git_result);
        }
        autter::git::fast_reader::HeadKind::Detached(_) => {
...
fn test_fast_detached_head_matches_git_cli() {
    ...
    match fast_head {
        autter::git::fast_reader::HeadKind::Detached(oid) => {
            assert_eq!(oid, sha);
        }
        autter::git::fast_reader::HeadKind::Symbolic(_) => {
...
fn test_fast_read_blob_matches_git_cli() {
    ...
    let git_dir = repo.path().join(".git");
    let reader = autter::git::fast_reader::FastObjectReader::new(&git_dir);
...
fn test_fast_reader_worktree_refs_in_common_dir() {
    ...
    let reader = autter::git::fast_reader::FastRefReader::new(&wt_git_dir, &common_dir);
