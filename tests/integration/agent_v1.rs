...
fn parse_agent_v1(hook_input: &str) -> Result<Vec<ParsedHookEvent>, autter::error::AutterError> {
...
fn test_agent_v1_relative_dirty_files_e2e_attribution() {
    ...
    .to_string();
    repo.autter(&["checkpoint", "agent-v1", "--hook-input", &human_payload])
        .unwrap();
    ...
    .to_string();
    repo.autter(&["checkpoint", "agent-v1", "--hook-input", &ai_payload])
        .unwrap();
