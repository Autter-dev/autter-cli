...
use autter::authorship::prompt_utils::{
    find_prompt, find_prompt_in_commit, find_prompt_in_history,
};
use autter::git::refs::get_authorship;
...
fn test_find_prompt_in_commit_integration() {
    ...
    let authorship = get_authorship(&autter_repo, &head_sha).unwrap();
    let prompt_id = authorship.metadata.prompts.keys().next().unwrap().clone();

    // Test finding the prompt
    let result = find_prompt_in_commit(&autter_repo, &prompt_id, "HEAD");
    assert!(result.is_ok());
...
fn test_find_prompt_in_commit_not_found() {
    ...
    let result = find_prompt_in_commit(&autter_repo, "nonexistent-prompt", "HEAD");
...
fn test_find_prompt_in_commit_invalid_revision() {
    ...
    let result = find_prompt_in_commit(&autter_repo, "any-prompt", "invalid-revision");
    assert!(result.is_err());
}
...
fn test_find_prompt_in_history_basic() {
    ...
    let result = find_prompt_in_history(&autter_repo, &prompt_id, 0);
    assert!(result.is_ok());
...
fn test_find_prompt_in_history_with_offset() {
    ...
    let result = find_prompt_in_history(&autter_repo, &prompt_id, 1);
    assert!(result.is_err());
    assert!(
        result
            .unwrap_err()
...
fn test_find_prompt_in_history_not_found() {
    ...
    let result = find_prompt_in_history(&autter_repo, "nonexistent-prompt", 0);
    assert!(result.is_err());
    assert!(
        result
            .unwrap_err()
...
fn test_find_prompt_delegates_to_commit() {
    ...
    let result = find_prompt(&autter_repo, &prompt_id, Some("HEAD"), 0);
    assert!(result.is_ok());
    let (_sha, prompt) = result.unwrap();
    assert_eq!(prompt.agent_id.tool, "mock_ai");
    // Note: agent_id.id is auto-generated by mock_ai, not fixed
...
fn test_find_prompt_delegates_to_history() {
    ...
    let result = find_prompt(&autter_repo, &prompt_id, None, 0);
    assert!(result.is_ok());
    let (_sha, prompt) = result.unwrap();
    assert_eq!(prompt.agent_id.tool, "mock_ai");
    // Note: agent_id.id is auto-generated by mock_ai, not fixed
...
fn test_find_prompt_in_history_empty_repo() {
    ...
    let result = find_prompt_in_history(&autter_repo, "any-prompt", 0);
    assert!(result.is_err());
    assert!(
        result
            .unwrap_err()
...
fn test_find_prompt_prompt_not_in_commit() {
    ...
    let result = find_prompt_in_commit(&autter_repo, "wrong-prompt-id", "HEAD");
    assert!(result.is_err());
    assert!(
        result
            .unwrap_err()
