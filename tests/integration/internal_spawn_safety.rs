    ...
    for file in files {
        ...
        for pattern in &disallowed_patterns {
            assert!(
                ...
                "direct internal background spawn found in {}: must use spawn_internal_autter_subcommand()",
                file.display()
            );
        }
    }
...
fn critical_background_spawners_call_spawn_helper() {
    ...
    for file in files {
        ...
        assert!(
            content.contains("spawn_internal_autter_subcommand("),
            "{} must call spawn_internal_autter_subcommand()",
            file.display()
        );
    }
}
...
fn internal_spawn_helper_calls_must_provide_guard_env() {
    ...
    let disallowed = Regex::new(
        r#"spawn_internal_autter_subcommand\(\s*"[^"]+"\s*,\s*&\[[^\]]*\]\s*,\s*None\s*,"#,
    )
    .unwrap();
    ...
    for file in files {
        ...
        assert!(
            ...
            "guardless spawn_internal_autter_subcommand call found in {}",
            file.display()
        );
    }
}
