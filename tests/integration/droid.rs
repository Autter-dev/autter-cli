...
use autter::streams::agents::DroidAgent;
use autter::streams::watermark::HybridWatermark;
use serde_json::json;
use std::fs;
use std::io::Write;
...
fn parse_droid(hook_input: &str) -> Result<Vec<ParsedHookEvent>, AutterError> {
...
fn test_droid_e2e_prefers_latest_checkpoint_for_prompts() {
    ...
    fs::write(&file_path, "// initial\n// ai line one\n").unwrap();
    repo.autter(&["checkpoint", "droid", "--hook-input", &hook_input])
        .unwrap();
