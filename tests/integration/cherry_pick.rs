...
fn test_single_commit_cherry_pick() {
    ...
    let log = autter::git::refs::get_reference_as_authorship_log_v3(
        &autter::git::find_repository_in_path(repo.path().to_str().unwrap()).unwrap(),
        &head_commit,
    )
    .unwrap();
...
///
/// Bug: autter pre-hook writes a CherryPickStart with empty source_commits when given
...
/// user runs `cherry-pick --skip`, autter failed to remove the skipped commit from the
