...
use autter::git::refs::get_reference_as_authorship_log_v3;
...
fn test_ci_squash_merge_basic() {
    ...
    // Get the Autter repository instance
    let autter_repo = AutterRepository::find_repository_in_path(repo.path().to_str().unwrap())
        .expect("Failed to find repository");

    // Call the CI rewrite function
    use autter::authorship::rebase_authorship::rewrite_authorship_after_squash_or_rebase;
...
fn test_ci_squash_merge_mixed_content() {
    ...
    let merge_log = get_reference_as_authorship_log_v3(&autter_repo, &merge_sha).unwrap();
    assert!(
...
fn test_ci_squash_merge_empty_notes_preserved() {
    ...
    let authorship_log = get_reference_as_authorship_log_v3(&autter_repo, &merge_sha).unwrap();
    assert!(
...
fn test_ci_squash_merge_no_notes_no_authorship_created() {
    ...
    assert!(
        get_reference_as_authorship_log_v3(&autter_repo, &merge_sha).is_err(),
...
fn test_ci_rebase_merge_commit_order_pairing() {
    ...
    use autter::ci::ci_context::{CiContext, CiEvent, CiRunOptions};
    ...
    let ctx = CiContext::with_repository(autter_repo, event);
...
/// Verify that `autter ci local merge` correctly pairs original commits with
...
/// `autter ci local merge` exactly as CI would and checks that:
///
...
/// `autter ci local merge`, every rebased commit must carry the note for its
...
/// Verify that `autter ci local sync` preserves authorship when an open PR
...
/// Verify that `autter ci local sync` handles GitHub's conflict-free
...
/// If the client-side autter CLI already handled the local rebase and pushed or
...
fn test_ci_local_sync_skips_when_current_rebased_commit_already_has_note() {
    ...
    let autter_repo =
        AutterRepository::find_repository_in_path(repo.path().to_str().expect("repo path"))
            .expect("autter repo");
    let existing_note = "client-side-note-that-ci-must-not-overwrite";
    notes_add(&autter_repo, &current_head_sha, existing_note).expect("add existing current note");
...
/// Verify that `autter ci local sync` can run for every PR synchronize event
...
/// Production-path (`autter ci local merge`) variant of the #1473 regression.
///
...
/// abbreviated SHA (as a human might via `autter ci local merge`).
///
