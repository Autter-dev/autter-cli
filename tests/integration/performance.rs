...
fn setup() {
    autter::config::Config::clear_test_feature_flags();
...
mod tests {
    ...
    fn test_ai_and_human_edits(#[case] repo_name: &str) {
        ...
        let result = sampler.sample(test_repo, |repo| {
            for file_path in &files_to_edit {
                ...
                // Step 2: Run autter checkpoint
                repo.autter(&["checkpoint", file_path])
    ...
    fn test_large_checkpoints(#[case] repo_name: &str) {
        ...
        let result = sampler.sample(test_repo, |repo| {
            ...
            // Step 2: Run autter checkpoint mock_ai -- <all pathspecs>
            ...
            repo.benchmark_autter(&final_checkpoint_args)
