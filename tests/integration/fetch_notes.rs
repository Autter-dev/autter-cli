...
fn test_fetch_notes_with_remote_flag() {
    ...
    let output = mirror
        .autter(&["fetch-notes", "--remote", "origin"])
...
fn test_fetch_notes_json_output_with_notes() {
    ...
    // Use -f in case autter hooks already created a note on this commit.
    mirror
        .git_og(&[
            "notes",
            "--ref=ai",
...
fn test_fetch_notes_unknown_option_fails() {
    ...
    let err = repo
        .autter(&["fetch-notes", "--invalid-flag"])
...
fn test_fetch_notes_duplicate_remote_fails() {
    ...
    let err = repo
        .autter(&["fetch-notes", "origin", "--remote", "upstream"])
...
fn test_fetch_notes_json_error_includes_remote() {
    ...
    let err = mirror
        .autter(&["fetch-notes", "--json", "--remote", "nonexistent"])
