...
use autter::ci::ci_context::{CiContext, CiEvent, CiRunResult};
use autter::git::notes_api::read_authorship_v3 as get_reference_as_authorship_log_v3;
...
///
/// Scenario:
/// 1. Contributor works in a fork with autter, creating AI-attributed code
...
fn test_ci_fork_squash_merge() {
    ...
    // Fork contributor adds AI code using autter
    ...
    let fork_repo = AutterRepository::find_repository_in_path(fork.path().to_str().unwrap())
    ...
    // Use git_og (raw git) to avoid autter auto-creating notes on this commit,
...
///
/// If a fork contributor doesn't use autter, there are no notes to fetch.
...
fn test_ci_fork_no_notes() {
    ...
    // Create fork WITHOUT autter (using git_og for all operations)
    let fork = TestRepo::new();
    let upstream_path = upstream.path().to_str().unwrap().to_string();
    fork.git_og(&["remote", "add", "upstream", &upstream_path])
        .unwrap();
    ...
    // Fork contributor adds code without autter
    let mut fork_file = fork.filename("feature.js");
    ...
    assert!(
        ...
        "Expected NoAuthorshipAvailable for fork with no autter notes, got {:?}",
        result
    );
}
...
fn test_ci_fork_merge_commit_no_notes_skips_without_push_error() {
    ...
    // Create fork WITHOUT autter notes
    let fork = TestRepo::new();
    let upstream_path = upstream.path().to_str().unwrap().to_string();
    fork.git_og(&["remote", "add", "upstream", &upstream_path])
        .unwrap();
