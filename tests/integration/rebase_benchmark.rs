...
///
/// Run with: cargo test --package autter --test integration rebase_benchmark -- --ignored --nocapture
#[test]
#[ignore]
...
fn benchmark_rebase_diff_based_large() {
    ...
    for commit_idx in 0..num_feature_commits {
        for file_idx in 0..num_ai_files {
            ...
            repo.autter(&["checkpoint", "mock_ai", &filename]).unwrap();
        }
    ...
    let result = repo.git_with_env(
        ...
        &[
            ("AUTTER_DEBUG_PERFORMANCE", "1"),
            ("AUTTER_REBASE_TIMING_FILE", &timing_path),
        ],
        None,
    );
    let rebase_duration = rebase_start.elapsed();
...
fn benchmark_blame_vs_diff() {
    ...
    let create_repo = |strip_notes: bool| -> (std::time::Duration, String) {
        ...
        if strip_notes {
            ...
            let _ = repo.git(&["update-ref", "-d", "refs/notes/autter-authorship"]);
        }
...
///
/// Run with: cargo test --package autter --test integration benchmark_rebase_heavy -- --ignored --nocapture
#[test]
#[ignore]
fn benchmark_rebase_heavy() {
    ...
    for commit_idx in 0..num_feature_commits {
        ...
        for i in 0..files_per_commit {
            ...
            repo.autter(&["checkpoint", "mock_ai_agent", &filename])
                .unwrap();
        }
        repo.git(&["add", "-A"]).unwrap();
        repo.stage_all_and_commit(&format!("AI feature commit {}", commit_idx))
...
fn benchmark_rebase_realistic_monorepo() {
    ...
    for commit_idx in 0..num_feature_commits {
        ...
        for &file_idx in &touched {
            ...
            fs::write(&path, &new_content).unwrap();
            repo.autter(&["checkpoint", "mock_ai", &spec.path]).unwrap();
        }
    ...
    if !restored_from_cache {
        ...
        for commit_idx in 0..num_feature_commits {
            ...
            for i in 0..files_this_commit.min(num_ai_files) {
                ...
                repo.autter(&["checkpoint", "mock_ai", &ai_file_paths[file_idx]])
                    .unwrap();
            }
...
///   3. `git update-ref` to advance the branch (triggers autter wrapper detection)
///
/// This models the actual Graphite CLI restack flow and tests whether autter's
...
fn benchmark_monorepo_graphite_rebase() {
    ...
    // new tip. autter's post_update_ref_hook sees the same N-commit rewrite
    ...
    // This is the single point where autter's wrapper detects a rewrite.
...
///
/// Run with: cargo test --package autter --test integration benchmark_large_scale_mixed -- --ignored --nocapture
#[test]
#[ignore]
