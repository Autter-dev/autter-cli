...
// https://github.com/autter-dev/autter-cli/issues/1204
//
// These tests verify:
...
fn test_cursor_shell_tool_full_cycle_produces_attribution() {
    ...
    repo.autter(&["checkpoint", "cursor", "--hook-input", &pre_hook_input])
    ...
    repo.autter(&["checkpoint", "cursor", "--hook-input", &post_hook_input])
...
fn test_multi_agent_codex_edit_not_stolen_by_claude_pre_hook() {
    ...
    repo.autter(&["checkpoint", "claude", "--hook-input", &claude_pre])
    ...
    repo.autter(&["checkpoint", "codex", "--hook-input", &codex_pre])
    ...
    repo.autter(&["checkpoint", "codex", "--hook-input", &codex_post])
    ...
    repo.autter(&["checkpoint", "claude", "--hook-input", &claude_post])
...
fn test_multi_agent_cursor_edit_not_stolen_by_claude_pre_hook() {
    ...
    repo.autter(&["checkpoint", "cursor", "--hook-input", &cursor_pre])
    ...
    repo.autter(&["checkpoint", "cursor", "--hook-input", &cursor_post])
