...
use autter::authorship::attribution_tracker::LineAttribution;
use autter::authorship::authorship_log::PromptRecord;
use autter::authorship::stats::CommitStats;
use autter::authorship::working_log::{AgentId, CheckpointKind};
...
fn stats_from_args(repo: &crate::repos::test_repo::TestRepo, args: &[&str]) -> CommitStats {
    let raw = repo.autter(args).expect("autter stats should succeed");
...
fn unique_worktree_path() -> PathBuf {
    ...
    std::env::temp_dir().join(format!("autter-worktree-{}", n))
}
...
crate::worktree_test_wrappers! {
    fn repository_paths_and_storage_are_worktree_aware() {
        ...
        let autter_repo = AutterRepository::find_repository_in_path(repo.path().to_str().unwrap())
            .expect("find autter repository");
        assert_eq!(
            autter_repo.workdir().unwrap().canonicalize().unwrap(),
            repo.path().canonicalize().unwrap(),
            "workdir should match linked worktree root"
        );
        assert_eq!(
            autter_repo.path().canonicalize().unwrap(),
        ...
        assert!(
            autter_repo.storage.working_logs.starts_with(&expected_prefix),
            "working logs should live under common-dir isolated storage: {}",
            autter_repo.storage.working_logs.display()
        );
    }
}
...
crate::worktree_test_wrappers! {
    fn checkpoint_and_blame_support_absolute_paths_in_worktree() {
        ...
        let output = repo
            .autter(&["blame", abs_path.to_str().unwrap()])
...
crate::worktree_test_wrappers! {
    fn blame_boundary_and_abbrev_match_git_in_worktree() {
        ...
        let autter_output = repo
            .autter(&["blame", "--abbrev", "12", "-b", "boundary.txt"])
            .expect("autter blame with boundary flags should succeed");

        assert_eq!(
            normalize_blame_for_format_parity(&autter_output),
            normalize_blame_for_format_parity(&git_output),
            "autter blame should match git formatting for boundary and abbrev in worktrees"
        );

        let git_root_output = repo
            .git(&["blame", "--abbrev=12", "--root", "boundary.txt"])
            .expect("git blame --root should succeed");
        let autter_root_output = repo
            .autter(&["blame", "--abbrev", "12", "--root", "boundary.txt"])
            .expect("autter blame --root should succeed");

        assert_eq!(
            normalize_blame_for_format_parity(&autter_root_output),
            normalize_blame_for_format_parity(&git_root_output),
            "autter blame should match git formatting for --root and abbrev in worktrees"
        );
    }
}
...
crate::worktree_test_wrappers! {
    fn diff_works_in_worktree_context() {
        ...
        let output = repo
            ...
            .expect("autter diff should succeed in worktree");
...
crate::worktree_test_wrappers! {
    fn multi_worktree_storage_isolation_prevents_cross_talk() {
        ...
        let repo_one = AutterRepository::find_repository_in_path(repo.path().to_str().unwrap())
            .expect("find first worktree repo");
        let repo_two =
            AutterRepository::find_repository_in_path(second_worktree.to_str().unwrap())
...
crate::worktree_test_wrappers! {
    fn worktree_initial_attributions_snapshot() {
        ...
        let blame_output = repo.autter(&["blame", "initial.txt"]).unwrap();
...
crate::worktree_test_wrappers! {
    fn stats_head_arg_uses_worktree_head() {
        // Regression test for issue #285: `autter stats head` in a worktree was
...
// Before the fix, autter would fail to store any checkpoint because:
...
// After the fix, autter detects that the edited file is outside the main
...
/// *different* directory (`file_path`).  Returns the autter stdout+stderr.
...
fn checkpoint_routes_to_linked_worktree_when_cwd_is_main_repo() {
    ...
    let wt_repo = AutterRepository::find_repository_in_path(linked_wt.to_str().unwrap())
...
fn human_checkpoint_routes_to_linked_worktree_when_cwd_is_main_repo() {
    ...
    // PreToolUse: informs autter that this file is *about to* be edited.
