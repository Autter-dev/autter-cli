...
use autter::commands::checkpoint_agent::presets::{ParsedHookEvent, resolve_preset};
use autter::error::AutterError;
use autter::streams::agent::Agent;
use autter::streams::agents::CodexAgent;
use autter::streams::watermark::ByteOffsetWatermark;
use serde_json::json;
use std::fs;

fn parse_codex(hook_input: &str) -> Result<Vec<ParsedHookEvent>, AutterError> {
...
fn test_codex_preset_structured_hook_input() {
    ...
    let hook_input = json!({
        ...
        "cwd": "/Users/test/projects/autter",
...
fn test_codex_read_only_bash_post_tool_use_before_edit_does_not_steal_commit_attribution() {
    ...
    .to_string();
    repo.autter(&["checkpoint", "codex", "--hook-input", &which_git_pre])
    ...
    .to_string();
    repo.autter(&["checkpoint", "codex", "--hook-input", &which_git_post])
    ...
    .to_string();
    repo.autter(&["checkpoint", "codex", "--hook-input", &commit_pre])
    ...
    .to_string();
    repo.autter(&["checkpoint", "codex", "--hook-input", &commit_post])
...
fn test_codex_e2e_bash_then_apply_patch_in_same_session() {
    ...
    repo.autter(&["checkpoint", "codex", "--hook-input", &bash_pre])
        .expect("bash pre-hook should succeed");
    ...
    repo.autter(&["checkpoint", "codex", "--hook-input", &bash_post])
        .expect("bash post-hook should succeed");
    ...
    repo.autter(&["checkpoint", "codex", "--hook-input", &patch_pre])
        .expect("apply_patch pre-hook should succeed");
    ...
    repo.autter(&["checkpoint", "codex", "--hook-input", &patch_post])
        .expect("apply_patch post-hook should succeed");
...
fn test_codex_e2e_bash_modifies_multiple_files_all_attributed() {
    ...
    repo.autter(&["checkpoint", "codex", "--hook-input", &pre_hook])
    ...
    repo.autter(&["checkpoint", "codex", "--hook-input", &post_hook])
...
fn test_codex_e2e_apply_patch_preserves_human_lines() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "config.toml"])
        .unwrap();
    repo.stage_all_and_commit("Initial commit").unwrap();
