...
fn test_ai_reflow_two_lines_to_one_attributed_to_ai() {
    ...
    // Use checkpoint + stage_all_and_commit so autter tracks authorship from the start.
    let initial_content = "\
fn ensure_isolated_process_home() {
    static PROCESS_HOME: OnceLock<std::path::PathBuf> = OnceLock::new();
    PROCESS_HOME.get_or_init(|| {
        let home = std::env::temp_dir()
            .join(format!(\"autter-test-home-{}\", std::process::id()));
...
";
    ...
    repo.autter(&["checkpoint", "--", "test_repo.rs"]).unwrap();
...
";
    ...
    repo.autter(&["checkpoint", "mock_ai", "test_repo.rs"])
        .unwrap();
    ...
    file.assert_lines_and_blame(crate::lines![
        ...
        "        let home = std::env::temp_dir().join(format!(\"autter-test-home-{}\", std::process::id()));".ai(),
...
fn test_human_reflow_of_ai_set_contents_retains_ai() {
    ...
    repo.autter(&["checkpoint", "--", "reflow.txt"]).unwrap();

    //   Phase B: AI checkpoint with real content
    let ai_content = "调用(参数一, 参数二, 参数三)";
    fs::write(&file_path, ai_content).unwrap();
    repo.git(&["add", "-A"]).unwrap();
    repo.autter(&["checkpoint", "mock_ai", "reflow.txt"])
        .unwrap();
...
fn test_ai_reflow_of_human_content_one_to_many_lines_attributed_to_ai() {
    ...
    repo.autter(&["checkpoint", "--", "reflow2.txt"]).unwrap();
    ...
    repo.autter(&["checkpoint", "mock_ai", "reflow2.txt"])
        .unwrap();
