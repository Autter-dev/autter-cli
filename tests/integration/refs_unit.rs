...
use autter::git::repository::{exec_git, find_repository_in_path};
use std::fs;
...
/// Helper: create a TestRepo and obtain a autter Repository handle.
fn repo_with_handle() -> (TestRepo, autter::git::repository::Repository) {
...
fn test_notes_add_and_show_authorship_note() {
    let (repo, autter_repo) = repo_with_handle();
    ...
    let non_existent_content =
        show_authorship_note(&autter_repo, "0000000000000000000000000000000000000000");
...
fn test_notes_add_batch_writes_multiple_notes() {
    ...
    notes_add_batch(&autter_repo, &entries).expect("batch notes add");

    let note_a = show_authorship_note(&autter_repo, &commit_a).expect("note A");
    let note_b = show_authorship_note(&autter_repo, &commit_b).expect("note B");
...
fn test_notes_add_blob_batch_reuses_existing_note_blob() {
    ...
    notes_add(&autter_repo, &commit_a, &note_content).expect("add note A");

    let blob_oids = note_blob_oids_for_commits(&autter_repo, std::slice::from_ref(&commit_a))
    ...
    notes_add_blob_batch(&autter_repo, std::slice::from_ref(&blob_entry))
        .expect("batch add blob-backed note");

    let raw_note_b = show_authorship_note(&autter_repo, &commit_b).expect("note B");
    assert_eq!(raw_note_b, note_content);

    let parsed_note_b =
        get_reference_as_authorship_log_v3(&autter_repo, &commit_b).expect("parse B");
...
fn test_copy_missing_notes_for_commits_from_ref_copies_only_requested_commits() {
    ...
    for (commit, note) in [(&commit_a, "fork-note-a"), (&commit_b, "fork-note-b")] {
        let mut args = autter_repo.global_args_for_exec();
    ...
    assert_eq!(copied, 1);
    assert_eq!(
        show_authorship_note(&autter_repo, &commit_a).as_deref(),
        Some("fork-note-a")
    );
    assert!(
        show_authorship_note(&autter_repo, &commit_b).is_none(),
...
fn test_copy_missing_notes_for_commits_from_ref_keeps_existing_local_note() {
    ...
    notes_add(&autter_repo, &commit_a, "local-note").expect("add local note");
...
fn test_ref_exists() {
    ...
    // HEAD should exist
    assert!(ref_exists(&autter_repo, "HEAD"));
    ...
    assert!(!ref_exists(&autter_repo, "refs/heads/nonexistent-branch"));
    assert!(!ref_exists(&autter_repo, "refs/notes/ai-test"));
}
...
fn test_merge_notes_from_ref() {
    ...
    let initial_note_c = show_authorship_note(&autter_repo, &commit_c);

    // Merge notes from refs/notes/test into refs/notes/ai
    merge_notes_from_ref(&autter_repo, "refs/notes/test").expect("merge notes");

    // After merge, commit C should have a note on refs/notes/ai
    let final_note_c = show_authorship_note(&autter_repo, &commit_c);
...
fn test_copy_ref() {
    ...
    notes_add(&autter_repo, &commit_sha, note_content).expect("add note");

    // refs/notes/ai should exist
    assert!(ref_exists(&autter_repo, "refs/notes/ai"));

    // refs/notes/ai-backup should not exist
    assert!(!ref_exists(&autter_repo, "refs/notes/ai-backup"));

    // Copy refs/notes/ai to refs/notes/ai-backup
    copy_ref(&autter_repo, "refs/notes/ai", "refs/notes/ai-backup").expect("copy ref");
    ...
    let note_from_ai = show_authorship_note(&autter_repo, &commit_sha).expect("note from ai");
...
fn test_grep_ai_notes_single_match() {
    ...
    notes_add(&autter_repo, &commit_sha, note).expect("add note");

    // Search for "cursor" should find the commit
    let results = grep_ai_notes(&autter_repo, "cursor").expect("grep");
...
fn test_grep_ai_notes_multiple_matches() {
    ...
    notes_add(&autter_repo, &commit_a, "{\"tool\":\"cursor\"}").expect("add note A");
    notes_add(&autter_repo, &commit_b, "{\"tool\":\"cursor\"}").expect("add note B");
    notes_add(&autter_repo, &commit_c, "{\"tool\":\"cursor\"}").expect("add note C");
...
fn test_grep_ai_notes_no_match() {
    ...
    let results = grep_ai_notes(&autter_repo, "vscode");
...
fn test_grep_ai_notes_no_notes() {
    ...
    let results = grep_ai_notes(&autter_repo, "cursor");
...
fn test_get_commits_with_notes_from_list() {
    ...
    let result = get_commits_with_notes_from_list(&autter_repo, &commit_list).expect("get commits");
...
fn test_note_blob_oids_for_commits_empty() {
    let (_repo, autter_repo) = repo_with_handle();

    // Empty list should return empty map
    let result = note_blob_oids_for_commits(&autter_repo, &[]).expect("empty list");
...
fn test_note_blob_oids_for_commits_no_notes() {
    ...
    let result = note_blob_oids_for_commits(&autter_repo, &[commit_sha]).expect("no notes");
    assert!(result.is_empty());
}
...
fn test_commits_with_authorship_notes() {
    ...
    notes_add(&autter_repo, &commit_a, "{\"test\":\"note\"}").expect("add note");

    let commits = vec![commit_a.clone(), commit_b.clone()];
    let result = commits_with_authorship_notes(&autter_repo, &commits).expect("check notes");
...
fn test_get_reference_as_working_log() {
    ...
    notes_add(&autter_repo, &commit_sha, working_log_json).expect("add note");

    let result = get_reference_as_working_log(&autter_repo, &commit_sha).expect("get working log");
...
fn test_get_reference_as_authorship_log_v3_version_mismatch() {
    ...
    let result = get_reference_as_authorship_log_v3(&autter_repo, &commit_sha);
    assert!(result.is_err());

    if let Err(AutterError::Generic(msg)) = result {
