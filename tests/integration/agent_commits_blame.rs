...
/// Extract the JSON object from autter output, stripping any trailing log/migration lines.
...
/// Write a file and commit it with a specific author identity, bypassing autter hooks.
...
fn test_agent_blame_cursor_email() {
    ...
    let output = repo.autter(&["blame", "test.rs"]).unwrap();
...
fn test_agent_blame_claude_email() {
    ...
    let output = repo.autter(&["blame", "test.py"]).unwrap();
...
fn test_agent_blame_codex_email() {
    ...
    let output = repo.autter(&["blame", "test.js"]).unwrap();
...
fn test_agent_blame_mixed_human_then_agent() {
    ...
    let output = repo.autter(&["blame", "mixed.rs"]).unwrap();
...
fn test_agent_blame_multiple_agents_same_file() {
    ...
    let output = repo.autter(&["blame", "multi.rs"]).unwrap();
    let blame_lines: Vec<&str> = output.lines().collect();
...
fn test_agent_blame_json_output() {
    ...
    let output = repo.autter(&["blame", "--json", "json_test.rs"]).unwrap();
...
fn test_agent_blame_json_mixed_human_agent() {
    ...
    let output = repo.autter(&["blame", "--json", "json_mixed.rs"]).unwrap();
...
fn test_agent_blame_porcelain_output() {
    ...
    let output = repo
        .autter(&["blame", "--porcelain", "porcelain_test.rs"])
        .unwrap();
...
fn test_agent_blame_line_porcelain_output() {
    ...
    let output = repo
        .autter(&["blame", "--line-porcelain", "line_porcelain_test.rs"])
        .unwrap();
...
fn test_agent_blame_human_email_not_detected_as_agent() {
    ...
    let output = repo.autter(&["blame", "human.rs"]).unwrap();
...
fn test_agent_email_with_authorship_note_uses_note() {
    ...
    let output = repo.autter(&["blame", "noted.rs"]).unwrap();
    assert!(
...
fn test_agent_blame_single_line_file() {
    ...
    let output = repo.autter(&["blame", "single.rs"]).unwrap();
    let blame_lines: Vec<&str> = output.lines().collect();
...
fn test_agent_blame_large_agent_commit() {
    ...
    let output = repo.autter(&["blame", "large.rs"]).unwrap();
    let blame_lines: Vec<&str> = output.lines().collect();
...
fn test_agent_blame_agent_modifies_human_lines() {
    ...
    let output = repo.autter(&["blame", "modify.rs"]).unwrap();
    let blame_lines: Vec<&str> = output.lines().collect();
...
fn test_agent_blame_with_line_range() {
    ...
    let output = repo.autter(&["blame", "-L", "2,4", "ranges.rs"]).unwrap();
    let blame_lines: Vec<&str> = output.lines().collect();
...
fn test_agent_blame_interleaved_agents_and_humans() {
    ...
    let output = repo.autter(&["blame", "interleaved.rs"]).unwrap();
    let blame_lines: Vec<&str> = output.lines().collect();
...
fn test_agent_blame_json_stats() {
    ...
    let output = repo.autter(&["blame", "--json", "stats_test.rs"]).unwrap();
    let json: serde_json::Value = serde_json::from_str(extract_json(&output)).expect("Valid JSON");
...
fn test_agent_blame_all_agents_in_separate_files() {
    ...
    for (filename, _, _, expected_tool) in &agents {
        let output = repo.autter(&["blame", filename]).unwrap();
        assert!(
...
fn test_agent_blame_incremental_output() {
    ...
    let output = repo
        .autter(&["blame", "--incremental", "incremental_test.rs"])
        .unwrap();
