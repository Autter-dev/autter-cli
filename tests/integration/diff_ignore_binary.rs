...
/// Helper: run autter diff --json and parse the result
...
/// Helper: run autter diff and return raw output (for non-JSON mode)
fn diff_raw(repo: &TestRepo, args: &[&str]) -> String {
    repo.autter(args).expect("autter diff should succeed")
}
...
fn test_diff_json_respects_gitattributes_linguist_generated() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "src/app.ts", "generated/schema.ts"])
        .unwrap();
    let commit = repo
...
#[test]
fn test_diff_json_respects_autter_ignore_file() {
    let repo = TestRepo::new();

    write_file(&repo, "src/main.rs", "fn main() {}\n");
    repo.stage_all_and_commit("initial").unwrap();

    // Set up .autter-ignore
    write_file(&repo, ".autter-ignore", "docs/**\n*.pdf\n");
    write_file(&repo, "src/main.rs", "fn main() {}\nfn added() {}\n");
    write_file(&repo, "docs/guide.md", "# Guide\nLine 1\nLine 2\n");

    repo.autter(&["checkpoint", "mock_ai", "src/main.rs", "docs/guide.md"])
        .unwrap();
    ...
    assert!(
        ...
        "docs/guide.md should be filtered by .autter-ignore"
    );
}
...
fn test_diff_json_respects_union_of_all_ignore_sources() {
    ...
    // Set up both .gitattributes and .autter-ignore
    write_file(
        &repo,
        ".gitattributes",
        "generated/** linguist-generated=true\n",
    ...
    assert!(
        ...
        "docs/api.md should be filtered by .autter-ignore"
    );
    assert!(
        !file_paths.contains(&"Cargo.lock"),
...
fn test_diff_terminal_respects_ignore_patterns() {
    ...
    assert!(
        ...
        "Terminal diff should not show .autter-ignore'd docs/guide.md"
    );
    assert!(
...
crate::reuse_tests_in_worktree!(
    ...
    test_diff_json_respects_autter_ignore_file,
