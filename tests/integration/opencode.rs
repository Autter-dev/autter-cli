...
fn parse_opencode(hook_input: &str) -> Result<Vec<ParsedHookEvent>, AutterError> {
...
fn test_opencode_preset_pretooluse_returns_human_checkpoint() {
    ...
    unsafe {
        std::env::remove_var("AUTTER_OPENCODE_STORAGE_PATH");
    }
...
fn test_opencode_e2e_checkpoint_and_commit() {
    ...
    repo.autter(&["checkpoint", "opencode", "--hook-input", &pre_hook_input])
        .unwrap();
    ...
    repo.autter(&["checkpoint", "opencode", "--hook-input", &post_hook_input])
        .unwrap();
