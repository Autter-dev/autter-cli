...
use autter::authorship::authorship_log::{LineRange, PromptRecord};
use autter::authorship::authorship_log_serialization::{
    AttestationEntry, AuthorshipLog, FileAttestation,
};
use autter::authorship::working_log::AgentId;
use autter::commands::blame::AutterBlameOptions;
...
fn test_blame_basic_format() {
    ...
    // Run git blame and autter blame
    let git_output = repo.git(&["blame", "test.txt"]).unwrap();
    let autter_output = repo.autter(&["blame", "test.txt"]).unwrap();

    // Compare normalized outputs
    let git_norm = normalize_for_snapshot(&git_output);
    let autter_norm = normalize_for_snapshot(&autter_output);
    println!("\n[DEBUG] Normalized git blame output:\n{}", git_norm);
    println!("\n[DEBUG] Normalized autter blame output:\n{}", autter_norm);
    assert_eq!(
        git_norm, autter_norm,
...
fn test_blame_line_range() {
    ...
    let autter_output = repo.autter(&["blame", "-L", "2,4", "test.txt"]).unwrap();
...
fn test_blame_multiple_line_ranges_default() {
    ...
    let autter_output = repo.autter(&args).unwrap();
...
fn test_blame_porcelain_multiple_hunks_same_commit_matches_git_filename_behavior() {
    ...
    let git_filename_count = git_output
        ...
        .count();
    let autter_filename_count = autter_output
        .lines()
        .filter(|line| line.starts_with("filename "))
        .count();
    assert_eq!(
        git_filename_count, autter_filename_count,
        "autter should emit filename lines in the same places as git"
    );
    assert_eq!(
        1, autter_filename_count,
...
fn test_blame_incremental_uses_real_commit_summaries() {
    ...
    let mut git_summaries: Vec<&str> = git_output
        ...
        .collect();
    let mut autter_summaries: Vec<&str> = autter_output
        .lines()
        .filter(|line| line.starts_with("summary "))
        .collect();
    git_summaries.sort_unstable();
    autter_summaries.sort_unstable();
    assert_eq!(
        git_summaries, autter_summaries,
        "autter incremental output should report the same commit summaries as git"
    );
    ...
    assert!(
        autter_output.contains("summary Update second line"),
        "autter incremental output should include real commit summaries"
    );
}
...
fn test_blame_porcelain_format() {
    ...
    let autter_output = repo.autter(&["blame", "--porcelain", "test.txt"]).unwrap();
...
fn test_blame_show_email() {
    ...
    let autter_output = repo.autter(&["blame", "-e", "test.txt"]).unwrap();
    ...
    assert!(
        autter_output.contains("@"),
        "Autter output should contain email"
    );
}
...
fn test_blame_show_name() {
    ...
    let autter_output = repo.autter(&["blame", "-f", "test.txt"]).unwrap();
    ...
    assert!(
        autter_output.contains("test.txt"),
        "Autter output should contain filename"
    );
}
...
fn test_blame_show_number() {
    ...
    let autter_output = repo.autter(&["blame", "-n", "test.txt"]).unwrap();
...
fn test_blame_suppress_author() {
    ...
    let autter_output = repo.autter(&["blame", "-s", "test.txt"]).unwrap();
    ...
    assert!(
        !autter_output.contains("Test User"),
        "Autter output should suppress author"
    );
}
...
fn test_blame_long_rev() {
    ...
    let autter_output = repo.autter(&["blame", "-l", "test.txt"]).unwrap();
    ...
    let git_sha_len = git_output
        ...
        .unwrap()
        .len();
    let autter_sha_len = autter_output
        .lines()
        .next()
        .unwrap()
        .split_whitespace()
    ...
    assert!(autter_sha_len > 8, "Autter should show long revision");
}
...
fn test_blame_raw_timestamp() {
    ...
    let autter_output = repo.autter(&["blame", "-t", "test.txt"]).unwrap();
    ...
    assert!(
        autter_output.chars().any(|c| c.is_numeric()),
        "Autter output should contain timestamps"
    );
}
...
fn test_blame_abbrev() {
    ...
    // Note: git requires --abbrev=4 format, autter accepts --abbrev 4
    let git_output = repo.git(&["blame", "--abbrev=4", "test.txt"]).unwrap();
    let autter_output = repo
        .autter(&["blame", "--abbrev", "4", "test.txt"])
        .unwrap();
...
fn test_blame_blank_boundary() {
    ...
    let autter_output = repo.autter(&["blame", "-b", "test.txt"]).unwrap();
...
fn test_blame_show_root() {
    ...
    let autter_output = repo.autter(&["blame", "--root", "test.txt"]).unwrap();
    ...
    assert!(
        autter_output.lines().count() > 0,
        "Autter should handle root commits"
    );
}
...
//     let autter_output = run_autter_blame(tmp_repo.path(), "test.txt", &["--show-stats"]);

//     let _comparison = create_blame_comparison(&git_output, &autter_output, "show_stats");
//     let git_norm = normalize_for_snapshot(&git_output);
//     let autter_norm = normalize_for_snapshot(&autter_output);
//     println!("\n[DEBUG] Normalized git blame output:\n{}", git_norm);
//     println!("\n[DEBUG] Normalized autter blame output:\n{}", autter_norm);
...
//     assert!(
//         autter_output.contains("%"),
//         "Autter output should contain statistics"
//     );
// }
#[test]
fn test_blame_date_format() {
    ...
    // Note: git requires --date=short format, autter accepts --date short
    let git_output = repo.git(&["blame", "--date=short", "test.txt"]).unwrap();
    let autter_output = repo
        .autter(&["blame", "--date", "short", "test.txt"])
        .unwrap();
    ...
    assert!(
        autter_output.contains("-"),
        "Autter output should contain date"
    );
}
...
fn test_blame_multiple_flags() {
    ...
    let git_output = repo
        ...
        .unwrap();
    let autter_output = repo
        .autter(&["blame", "-L", "2,4", "-e", "-n", "test.txt"])
        .unwrap();
    ...
    assert!(
        autter_output.lines().count() > 0,
        "Autter should handle multiple flags"
    );
...
fn test_blame_incremental_format() {
    ...
    let autter_output = repo
        .autter(&["blame", "--incremental", "test.txt"])
        .unwrap();
...
fn test_blame_line_porcelain() {
    ...
    let git_output = repo
        ...
        .unwrap();
    let autter_output = repo
        .autter(&["blame", "--line-porcelain", "test.txt"])
        .unwrap();
...
fn test_blame_with_ai_authorship() {
    ...
    let autter_authors = extract_authors(&autter_output);

    // Git should show the same author for all lines (the committer)
    // Autter should show different authors based on AI authorship
    assert_ne!(
        git_authors, autter_authors,
        "AI authorship should change the output"
    );

    // Verify autter shows AI authors where appropriate
    assert!(
        autter_authors
            .iter()
...
fn test_blame_contents_from_stdin() {
    ...
    // Run autter blame with --contents - (read from stdin)
    let autter_output = repo
        .autter_with_stdin(
    ...
    println!("\n[DEBUG] autter blame output:\n{}", autter_output);
    let lines = autter_output.lines().collect::<Vec<&str>>();
...
fn test_blame_mark_unknown_without_authorship_log() {
    ...
    // Create a commit WITHOUT using autter (bypassing hooks to avoid authorship log)
    file.set_contents(crate::lines!["Line from untracked commit"]);

    // Use git_og to bypass autter hooks
    ...
    let output_without_flag = repo.autter(&["blame", "test.txt"]).unwrap();
    ...
    let output_with_flag = repo
        .autter(&["blame", "--mark-unknown", "test.txt"])
        .unwrap();
...
fn test_blame_mark_unknown_mixed_commits() {
    ...
    // Create file2 WITH authorship log (through autter)
    ...
    let output1 = repo
        .autter(&["blame", "--mark-unknown", "untracked.txt"])
        .unwrap();
    ...
    let output2 = repo
        .autter(&["blame", "--mark-unknown", "tracked.txt"])
        .unwrap();
...
fn test_blame_mark_unknown_backward_compatible() {
    ...
    assert_eq!(
        git_norm, autter_norm,
        "Without --mark-unknown, autter blame should match git blame exactly"
    );
}
...
fn test_blame_auto_detects_git_blame_ignore_revs_file() {
    // Test that autter blame automatically uses .git-blame-ignore-revs when present
    let repo = TestRepo::new();
    let mut file = repo.filename("test.txt");
    ...
    // Run autter blame - it should auto-detect .git-blame-ignore-revs
    let autter_output = repo.autter(&["blame", "test.txt"]).unwrap();
    println!(
        "\n[DEBUG] autter blame with auto-detected ignore-revs:\n{}",
        autter_output
    );
    ...
    assert!(
        autter_output.contains(&initial_sha[..7]),
...
fn test_blame_no_ignore_revs_file_flag_disables_auto_detection() {
    ...
    let output_without_ignore = repo
        .autter(&["blame", "--no-ignore-revs-file", "test.txt"])
        .unwrap();
    println!(
        "\n[DEBUG] autter blame with --no-ignore-revs-file:\n{}",
...
fn test_blame_explicit_ignore_revs_file_takes_precedence() {
    ...
    let output = repo
        ...
        .unwrap();
    println!(
        "\n[DEBUG] autter blame with explicit --ignore-revs-file:\n{}",
        output
    );
...
fn test_blame_respects_git_config_blame_ignore_revs_file() {
    // Test that autter respects the blame.ignoreRevsFile git config
    let repo = TestRepo::new();
    let mut file = repo.filename("test.txt");
    ...
    // Run autter blame - should auto-detect from git config
    let output = repo.autter(&["blame", "test.txt"]).unwrap();
    println!(
        "\n[DEBUG] autter blame with blame.ignoreRevsFile config:\n{}",
        output
    );
...
fn test_blame_without_ignore_revs_file_works_normally() {
    ...
    // Run autter blame - should work normally
    let autter_output = repo.autter(&["blame", "test.txt"]).unwrap();
    let git_output = repo.git(&["blame", "test.txt"]).unwrap();
    ...
    assert!(
        autter_output.contains(&second_sha[..7]),
        "autter blame should show second commit when no ignore file exists. Output: {}",
        autter_output
    );
...
fn test_blame_ignore_revs_with_multiple_commits() {
    ...
    let output = repo.autter(&["blame", "test.txt"]).unwrap();
    println!(
        "\n[DEBUG] autter blame with multiple ignored commits:\n{}",
        output
    );
...
fn test_blame_ai_human_author() {
    ...
    let autter_repo = AutterRepository::find_repository_in_path(repo.path().to_str().unwrap())
        .expect("Failed to find repository");
    notes_add(&autter_repo, &initial_sha, &note_content).unwrap();

    // Call blame_hunks on the file
    let options = AutterBlameOptions::default();
    let hunks = autter_repo
