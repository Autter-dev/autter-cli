...
use autter::authorship::working_log::{AgentId, Checkpoint, CheckpointKind, WorkingLogEntry};
use autter::commands::checkpoint_agent::orchestrator::{
    BaseCommit, CheckpointFile, CheckpointRequest,
};
use autter::daemon::checkpoint::{
...
fn setup_repo_with_base_commit() -> (TestRepo, String, String) {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "lines.md"])
        .unwrap();
    repo.autter(&["checkpoint", "mock_known_human", "alphabet.md"])
        .unwrap();
...
fn test_checkpoint_with_staged_changes() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", &lines_file])
        .unwrap();
...
fn test_checkpoint_with_only_unstaged_changes_for_ai_without_pathspec() {
    ...
    repo.autter(&["checkpoint", "mock_ai", &lines_file])
        .unwrap();
...
fn test_checkpoint_with_paths_outside_repo() {
    ...
    let autter_repo = find_repository_in_path(repo.path().to_str().unwrap()).unwrap();
    let base_commit = autter_repo.head().unwrap().target().unwrap();
...
fn test_checkpoint_filters_external_paths_from_stored_checkpoints() {
    ...
    let result = repo.autter(&["checkpoint", "mock_known_human", &lines_file]);
...
fn test_known_human_checkpoint_without_ai_history_records_h_hash_attributions() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "simple.txt"])
        .unwrap();
...
fn test_checkpoint_skips_linguist_generated_files_from_root_gitattributes() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "main.rs"])
        .unwrap();
...
fn test_compute_line_stats_ignores_whitespace_only_lines() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "whitespace.txt"])
...
fn test_checkpoint_succeeds_with_legacy_initial_missing_blobs() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "file_a.txt"])
        .unwrap();
    repo.autter(&["checkpoint", "mock_known_human", "file_b.txt"])
        .unwrap();
    repo.stage_all_and_commit("initial commit").unwrap();
    ...
    let autter_repo = find_repository_in_path(repo.path().to_str().unwrap()).unwrap();
    let head_sha = autter_repo.head().unwrap().target().unwrap();
    let working_log = autter_repo
        .storage
...
fn test_scoped_checkpoint_records_file_deletion() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "to_delete.txt"])
        .unwrap();
    repo.stage_all_and_commit("initial commit").unwrap();

    // AI agent pre-edit snapshot (captures before state)
    repo.autter(&["checkpoint", "human", "to_delete.txt"])
        .unwrap();
    ...
    let checkpoint_result = repo.autter(&["checkpoint", "mock_ai", "to_delete.txt"]);
    assert!(
