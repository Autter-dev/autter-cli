...
fn test_ci_result_types_coverage() {
    ...
    use autter::ci::ci_context::CiRunResult;
...
fn test_ci_github_run_fetches_missing_previous_head_after_force_push() {
    ...
    let clone_path = ci_repo.path().join("autter-ci-clone");
...
fn test_ci_event_merge_structure() {
    use autter::ci::ci_context::CiEvent;
...
fn test_ci_context_with_temp_dir() {
    use autter::ci::ci_context::{CiContext, CiEvent};
...
fn test_github_workflow_file_creation() {
    ...
    let workflow_file = workflows_dir.join("autter-authorship.yml");

    // Write a minimal workflow
    fs::write(&workflow_file, "name: Autter Authorship\n").expect("Failed to write workflow");
...
fn test_github_workflow_path_structure() {
    ...
    assert!(
        expected_path
            .to_string_lossy()
            .contains("autter-authorship.yml")
    );
}
