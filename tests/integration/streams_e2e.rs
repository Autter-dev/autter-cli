...
use autter::streams::agents::{ClaudeAgent, CopilotAgent, OpenCodeAgent};
use autter::streams::watermark::{
...
fn test_subagent_session_record_has_parent_link() {
    use autter::streams::agents::claude::ClaudeAgent as ClaudeAgentImpl;
...
fn test_copilot_otel_events_use_otel_trace_event_type() {
    use autter::metrics::OtelTraceValues;
    use autter::metrics::events::otel_trace_pos;
    ...
    for raw_event in batch.events.iter().take(5) {
        ...
        let sparse = autter::metrics::PosEncoded::to_sparse(&values);
