...
use autter::authorship::diff_ai_accepted::diff_ai_accepted_stats;
use autter::authorship::post_commit::estimate_stats_cost_for_head;
use autter::authorship::stats::{get_git_diff_stats, stats_for_commit_stats};
...
fn autter_bin() -> String {
    std::env::var("CARGO_BIN_EXE_autter")
        .unwrap_or_else(|_| format!("{}/target/debug/autter", env!("CARGO_MANIFEST_DIR")))
}

fn benchmark_commit_with_autter(repo_path: &Path, message: &str) -> CommitPerfBreakdown {
    let output = Command::new(autter_bin())
        .arg("-C")
        .arg(repo_path)
        .arg("commit")
        .arg("-m")
        .arg(message)
        .env("AUTTER", "git")
        .env("AUTTER_DEBUG_PERFORMANCE", "2")
        .output()
        .expect("failed to execute autter commit");

    assert!(
        output.status.success(),
        "autter commit failed:\nstdout: {}\nstderr: {}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let perf_json_line = combined
        .lines()
        .find(|line| line.contains("[autter (perf-json)]"))
        .expect("missing perf-json output from autter commit");
...
fn benchmark_commit_post_command_hunk_density_hotspot() {
    ...
    let contiguous_perf = benchmark_commit_with_autter(contiguous_path, "contiguous");
    let scattered_perf = benchmark_commit_with_autter(scattered_path, "scattered");
...
fn benchmark_stats_thousands_changed_files_fast_path() {
    ...
    let file_count = std::env::var("AUTTER_BENCH_FILE_COUNT")
        .ok()
        .and_then(|v| v.parse::<usize>().ok())
        .unwrap_or(DEFAULT_FILE_COUNT);
    let runs_count = std::env::var("AUTTER_BENCH_RUNS")
        .ok()
        .and_then(|v| v.parse::<usize>().ok())
        .unwrap_or(DEFAULT_RUNS);

    let max_avg_ms = std::env::var("AUTTER_BENCH_MAX_AVG_MS")
        .ok()
...
/// causing a multi-second hang that users perceived as autter freezing.
#[test]
