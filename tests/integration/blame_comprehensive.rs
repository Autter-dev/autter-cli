...
fn test_blame_success_only_human_lines() {
    ...
    let output = repo.autter(&["blame", "human.txt"]).unwrap();
...
fn test_blame_success_only_ai_lines() {
    ...
    let output = repo.autter(&["blame", "ai.txt"]).unwrap();
...
fn test_blame_success_with_line_range() {
    ...
    let output = repo.autter(&["blame", "-L", "2,4", "ranges.txt"]).unwrap();
...
fn test_blame_success_with_newest_commit() {
    ...
    let options = AutterBlameOptions {
        newest_commit: Some(commit1.commit_sha.clone()),
        no_output: true,
        ..Default::default()
    };

    let (line_authors, _) = autter_repo.blame("versioned.txt", &options).unwrap();
...
fn test_blame_success_json_format() {
    ...
    let output = repo.autter(&["blame", "--json", "json_test.txt"]).unwrap();
...
fn test_blame_error_missing_file() {
    ...
    let result = repo.autter(&["blame", "nonexistent.txt"]);
...
fn test_blame_error_invalid_line_range_start_zero() {
    ...
    let result = repo.autter(&["blame", "-L", "0,1", "test.txt"]);
...
fn test_blame_error_invalid_line_range_end_zero() {
    ...
    let result = repo.autter(&["blame", "-L", "1,0", "test.txt"]);
...
fn test_blame_error_invalid_line_range_start_greater_than_end() {
    ...
    let result = repo.autter(&["blame", "-L", "3,1", "test.txt"]);
...
fn test_blame_error_invalid_line_range_beyond_file() {
    ...
    let result = repo.autter(&["blame", "-L", "1,100", "test.txt"]);
...
fn test_blame_error_invalid_commit_ref() {
    ...
    let result = repo.autter(&["blame", "invalid_sha_123", "test.txt"]);
...
fn test_blame_error_file_outside_repo() {
    ...
    let result = repo.autter(&["blame", outside_file.to_str().unwrap()]);
...
fn test_blame_error_directory_instead_of_file() {
    ...
    let result = repo.autter(&["blame", "src"]);
...
fn test_blame_edge_empty_file() {
    ...
    let result = repo.autter(&["blame", "empty.txt"]);
    assert!(
        result.is_err(),
...
fn test_blame_edge_single_line_file() {
    ...
    let output = repo.autter(&["blame", "single.txt"]).unwrap();
...
fn test_blame_edge_large_file() {
    ...
    let output = repo.autter(&["blame", "large.txt"]).unwrap();
...
fn test_blame_edge_file_with_unicode() {
    ...
    let output = repo.autter(&["blame", "unicode.txt"]).unwrap();
...
fn test_blame_edge_file_with_very_long_lines() {
    ...
    let output = repo.autter(&["blame", "longlines.txt"]).unwrap();
...
fn test_blame_edge_boundary_commit_flag() {
    ...
    let output = repo.autter(&["blame", "-b", "boundary.txt"]).unwrap();
...
fn test_blame_edge_renamed_file() {
    ...
    let output = repo.autter(&["blame", "renamed.txt"]).unwrap();
...
fn test_blame_edge_whitespace_only_lines() {
    ...
    let output = repo.autter(&["blame", "whitespace.txt"]).unwrap();
...
fn test_blame_format_porcelain_basic() {
    ...
    let output = repo.autter(&["blame", "--porcelain", "test.txt"]).unwrap();
...
fn test_blame_format_json_structure() {
    ...
    let output = repo.autter(&["blame", "--json", "test.txt"]).unwrap();
...
fn test_blame_format_default_with_flags() {
    ...
    let output = repo.autter(&["blame", "-e", "test.txt"]).unwrap();
    assert!(output.contains("@"));

    // Test with -n (show line numbers)
    let output = repo.autter(&["blame", "-n", "test.txt"]).unwrap();
    assert!(output.contains(" 1 "));
    assert!(output.contains(" 2 "));

    // Test with -f (show filename)
    let output = repo.autter(&["blame", "-f", "test.txt"]).unwrap();
    assert!(output.contains("test.txt"));

    // Test with -s (suppress author)
    let output = repo.autter(&["blame", "-s", "test.txt"]).unwrap();
    assert!(!output.contains("Test User"));
}
...
fn test_blame_ai_authorship_hunk_splitting() {
    ...
    let autter_repo = AutterRepository::find_repository_in_path(repo.path().to_str().unwrap())
        .expect("Failed to find repository");
    notes_add(&autter_repo, &commit_sha, &note_content).unwrap();
    ...
    let hunks = autter_repo.blame_hunks("test.txt", 1, 3, &options).unwrap();
...
fn test_blame_ai_authorship_no_splitting() {
    ...
    let hunks = autter_repo.blame_hunks("test.txt", 1, 2, &options).unwrap();
...
fn test_blame_ai_authorship_return_human_as_human() {
    ...
    let (line_authors, _) = autter_repo.blame("test.txt", &options).unwrap();
...
fn test_blame_commit_range_with_oldest_date() {
    ...
    let result = autter_repo.blame("test.txt", &options);
    assert!(result.is_ok());
}
...
fn test_blame_path_normalization_absolute() {
    ...
    let output = repo.autter(&["blame", abs_path.to_str().unwrap()]).unwrap();
...
fn test_blame_path_normalization_subdirectory() {
    ...
    let output = repo.autter(&["blame", "src/code.rs"]).unwrap();
...
fn test_blame_analysis_matches_blame_no_output_multi_ranges() {
    ...
    let (line_authors, prompt_records) = autter_repo.blame("test.txt", &options).unwrap();
    let analysis = autter_repo.blame_analysis("test.txt", &options).unwrap();
...
fn test_blame_ignore_whitespace() {
    ...
    let hunks = autter_repo.blame_hunks("test.txt", 1, 1, &options).unwrap();
...
fn test_blame_abbrev_custom_length() {
    ...
    let output = repo
        .autter(&["blame", "--abbrev", "10", "test.txt"])
        .unwrap();
...
fn test_blame_long_rev() {
    ...
    let output = repo.autter(&["blame", "-l", "test.txt"]).unwrap();
...
fn test_blame_stress_many_small_hunks() {
    ...
    let output = repo.autter(&["blame", "alternating.txt"]).unwrap();
...
fn test_blame_stress_deeply_nested_path() {
    ...
    let output = repo.autter(&["blame", "a/b/c/d/e/f/g/h/deep.txt"]).unwrap();
