...
fn init_test_git_config() {
    ...
    INIT.get_or_init(|| {
        let path = std::env::temp_dir().join("autter-test-global-gitconfig");
        let _ = fs::write(
            &path,
...
fn run_git(cwd: &Path, args: &[&str]) {
    ...
    let output = Command::new(autter::config::Config::get().git_cmd())
        .args(args)
        .current_dir(cwd)
        .output()
...
fn find_repository_in_path_supports_bare_repositories() {
    ...
    let discovered = autter::git::repository::discover_repository_in_path_no_git_exec(&bare)
...
fn find_repository_in_path_worktree_uses_common_dir_for_isolated_storage() {
    ...
    let discovered = autter::git::repository::discover_repository_in_path_no_git_exec(&worktree)
