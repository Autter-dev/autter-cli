...
//
// These tests verify that autter correctly handles:
...
{{
  ...
  "prompts": {{
    ...
    let autter_repo = autter::git::find_repository_in_path(repo.path().to_str().unwrap())
        .expect("find repository");
    notes_add(&autter_repo, base_sha, &old_note).expect("add old-format note");
...
{{
  ...
  "sessions": {{
    ...
    let autter_repo = autter::git::find_repository_in_path(repo.path().to_str().unwrap())
        .expect("find repository");
    notes_add(&autter_repo, &commit.commit_sha, &mixed_note).expect("add mixed-format note");
...
{{
  ...
  "prompts": {{
    ...
    let autter_repo = autter::git::find_repository_in_path(repo.path().to_str().unwrap())
        .expect("find repository");
    notes_add(&autter_repo, &commit_a.commit_sha, &old_note_a).expect("add old-format note A");
...
{{
  ...
  "prompts": {{
    ...
    let autter_repo = autter::git::find_repository_in_path(repo.path().to_str().unwrap())
        .expect("find repository");
    notes_add(&autter_repo, &source_commit.commit_sha, &old_note).expect("add old-format note");
...
{{
  ...
  "humans": {{
    ...
    let autter_repo = autter::git::find_repository_in_path(repo.path().to_str().unwrap())
        .expect("find repository");
    notes_add(&autter_repo, &ai_commit.commit_sha, &old_note).expect("add old-format note");
...
{{
  ...
  "prompts": {{
    ...
    let autter_repo = autter::git::find_repository_in_path(repo.path().to_str().unwrap())
        .expect("find repository");
    notes_add(&autter_repo, &commit.commit_sha, &old_note).expect("add old-format note");
...
// This simulates: user had autter old version, made a commit (old prompts note), then upgraded autter,
...
fn test_amend_old_prompts_commit_with_new_session_checkpoints() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "example.txt"])
        .unwrap();
    let commit = repo.stage_all_and_commit("Initial commit").unwrap();

    // Step 2: Replace the note with an old-format note (simulating pre-upgrade autter)
...
{{
  ...
  "prompts": {{
    ...
    let autter_repo = autter::git::find_repository_in_path(repo.path().to_str().unwrap())
        .expect("find repository");
    notes_add(&autter_repo, &commit.commit_sha, &old_note).expect("attach old-format note");
...
// This simulates: user upgrades autter mid-session. The working log has checkpoints from before
...
fn test_mixed_working_log_old_and_new_checkpoints_produce_both_prompts_and_sessions() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "mixed.txt"])
        .unwrap();
    ...
    repo.autter(&["checkpoint", "mock_ai", "mixed.txt"])
        .unwrap();
    ...
    for line in content.lines() {
                ...
                .is_some()
        {
            ...
            let agent_id_str = checkpoint
                ...
                .unwrap_or("");
            let old_author_id =
                autter::authorship::authorship_log_serialization::generate_short_hash(
                    agent_id_str,
                    agent_tool,
                );
...
fn test_reset_soft_old_note_then_new_session_checkpoints() {
    ...
    let base = "Base line\n";
    fs::write(&file_path, base).unwrap();
    repo.autter(&["checkpoint", "mock_known_human", "reset_test.txt"])
        .unwrap();
    repo.stage_all_and_commit("Base commit").unwrap();
    ...
    repo.autter(&["checkpoint", "mock_ai", "reset_test.txt"])
        .unwrap();
    let commit = repo.stage_all_and_commit("AI commit").unwrap();

    // Step 3: Replace the note with old-format (simulating pre-upgrade autter)
...
{{
  ...
  "prompts": {{
    ...
    let autter_repo = autter::git::find_repository_in_path(repo.path().to_str().unwrap())
        .expect("find repository");
    notes_add(&autter_repo, &commit_a.commit_sha, &old_note).expect("attach old-format note");
...
fn test_stash_pop_mixed_format_working_log() {
    ...
    let base = "Base line\n";
    fs::write(&file_path, base).unwrap();
    repo.autter(&["checkpoint", "mock_known_human", "stash_test.txt"])
        .unwrap();
    repo.stage_all_and_commit("Base commit").unwrap();
    ...
    let edit1 = "Base line\nAI old line\n";
    fs::write(&file_path, edit1).unwrap();
    repo.autter(&["checkpoint", "mock_ai", "stash_test.txt"])
        .unwrap();
...
{{
  ...
  "prompts": {{
    ...
    let autter_repo = autter::git::find_repository_in_path(repo.path().to_str().unwrap())
        .expect("find repository");
    notes_add(&autter_repo, &feature_commit.commit_sha, &old_note).expect("attach old-format note");
...
{{
  ...
  "prompts": {{
    ...
    let output = repo
        .autter(&["show-prompt", old_hash, "--commit", "HEAD"])
...
// Test 17: autter stats --json works correctly with old-format notes.
...
{{
  ...
  "prompts": {{
    ...
    // Run autter stats --json — should not crash on old-format notes
    let output = repo
        .autter(&["stats", "--json"])
...
// Test 18: autter diff --json --all-prompts with new-format (sessions-only) commit
...
fn test_diff_json_all_prompts_includes_sessions() {
    ...
    // Run autter diff --json --all-prompts
    let output = repo
        .autter(&["diff", "--json", "--all-prompts", &commit.commit_sha])
...
// Test 19: autter diff --json --all-prompts with old-format (prompts-only) commit
...
fn test_amend_old_prompts_delete_ai_line_then_add_new_session_line() {
    ...
    fs::write(&file_path, initial).unwrap();
    repo.autter(&["checkpoint", "mock_ai", "prune.txt"])
        .unwrap();
    let commit = repo.stage_all_and_commit("Initial commit").unwrap();
...
fn test_amend_old_prompts_keep_old_line_add_new_session_same_file() {
    ...
    let initial = "Human line\nOld AI line\n";
    fs::write(&file_path, initial).unwrap();
    repo.autter(&["checkpoint", "mock_ai", "keepold.txt"])
        .unwrap();
    let commit = repo.stage_all_and_commit("Initial commit").unwrap();
...
fn test_multiple_amends_mixed_format_accumulation() {
    ...
    let initial = "Line 1\nOld AI line\n";
    fs::write(&file_path, initial).unwrap();
    repo.autter(&["checkpoint", "mock_ai", "multi.txt"])
        .unwrap();
    let commit = repo.stage_all_and_commit("Initial commit").unwrap();
...
fn test_initial_from_old_note_plus_human_and_session_edits() {
    ...
    let base = "Line 1\n";
    fs::write(&file_path, base).unwrap();
    repo.autter(&["checkpoint", "mock_known_human", "triple.txt"])
        .unwrap();
    ...
    repo.autter(&["checkpoint", "mock_ai", "triple.txt"])
        .unwrap();
    let ai_commit = repo.stage_all_and_commit("AI commit").unwrap();
...
{{
  ...
  "prompts": {{
    ...
    let autter_repo = autter::git::find_repository_in_path(repo.path().to_str().unwrap())
        .expect("find repository");
    notes_add(&autter_repo, &ai_commit.commit_sha, &old_note).expect("attach old-format note");
...
fn test_amend_old_prompts_different_file_gets_session_edits() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "file_a.txt"])
        .unwrap();
    let commit = repo.stage_all_and_commit("Initial commit").unwrap();
...
{{
  ...
  "prompts": {{
    ...
    repo.autter(&["checkpoint", "mock_ai", "file_b.txt"])
        .unwrap();
...
// Test 25: autter status correctly counts AI lines from old-format INITIAL entries.
// When a reset brings old-format prompts into the INITIAL working log, `autter status`
...
fn test_status_counts_ai_lines_from_old_format_initial() {
    ...
    let base = "Base line\n";
    fs::write(&file_path, base).unwrap();
    repo.autter(&["checkpoint", "mock_known_human", "status_test.txt"])
        .unwrap();
    repo.stage_all_and_commit("Base").unwrap();
    ...
    fs::write(&file_path, ai_edit).unwrap();
    repo.autter(&["checkpoint", "mock_ai", "status_test.txt"])
        .unwrap();
    let ai_commit = repo.stage_all_and_commit("AI commit").unwrap();
...
{{
  ...
  "prompts": {{
    ...
    // Step 5: Run autter status --json and check it counts AI lines from old-format INITIAL
    let status_output = repo.autter(&["status", "--json"]);
    assert!(
        status_output.is_ok(),
        "autter status should work with old-format INITIAL"
    );
...
// Test 26: autter diff --json on a commit that has BOTH old prompts and new sessions
...
fn test_diff_json_mixed_format_commit_separates_prompts_and_sessions() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "mixed_diff.txt"])
        .unwrap();
    repo.stage_all_and_commit("Base commit").unwrap();

    // Step 2: Add AI content and commit
    fs::write(&file_path, "base line\nold ai line\n").unwrap();
    repo.autter(&["checkpoint", "mock_ai", "mixed_diff.txt"])
        .unwrap();
    let commit = repo.stage_all_and_commit("AI commit").unwrap();

    // Step 3: Replace note with old-format (simulating pre-upgrade autter)
...
{{
  ...
  "prompts": {{
    ...
    let sha = repo.git(&["rev-parse", "HEAD"]).unwrap().trim().to_string();
    let output = repo
        .autter(&["diff", &sha, "--json", "--include-stats"])
...
// Test 27: autter diff --json across a history that has both old-format and new-format commits.
...
fn test_diff_json_history_with_mixed_old_and_new_format_commits() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "range_mixed.txt"])
        .unwrap();
    let base = repo.stage_all_and_commit("Base").unwrap();

    // Step 2: Old-format AI commit
    fs::write(&file_path, "base\nold ai line\n").unwrap();
    repo.autter(&["checkpoint", "mock_ai", "range_mixed.txt"])
        .unwrap();
...
{{
  ...
  "prompts": {{
    ...
    let autter_repo = autter::git::find_repository_in_path(repo.path().to_str().unwrap())
        .expect("find repository");
    notes_add(&autter_repo, &old_commit.commit_sha, &old_note).expect("attach old-format note");
    ...
    let old_json: Value = {
        let output = repo
            .autter(&["diff", &old_commit.commit_sha, "--json", "--all-prompts"])
    ...
    let new_json: Value = {
        let output = repo
            .autter(&["diff", &new_commit.commit_sha, "--json", "--all-prompts"])
...
// Test 28: autter diff --json --include-stats on single commit with old-format note
...
fn test_diff_json_stats_with_old_format_note_only() {
    ...
    // Base commit
    fs::write(&file_path, "base\n").unwrap();
    repo.autter(&["checkpoint", "mock_known_human", "old_stats.txt"])
        .unwrap();
    let base = repo.stage_all_and_commit("Base").unwrap();

    // AI commit
    fs::write(&file_path, "base\nai one\nai two\n").unwrap();
    repo.autter(&["checkpoint", "mock_ai", "old_stats.txt"])
        .unwrap();
    let commit = repo.stage_all_and_commit("AI commit").unwrap();
...
{{
  ...
  "prompts": {{
    ...
    let output = repo
        .autter(&["diff", &commit.commit_sha, "--json", "--include-stats"])
        .expect("diff --json --include-stats should succeed");
    let json: Value = serde_json::from_str(output.trim()).expect("diff JSON should parse");
