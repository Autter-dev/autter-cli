...
fn superuser_guard_does_not_block_non_root_invocations() {
    ...
    let result = repo.autter(&["version"]);
...
fn superuser_guard_allow_env_var_is_respected() {
    let repo = TestRepo::new();
    let result = repo.autter_with_env(&["version"], &[("AUTTER_ALLOW_SUPERUSER", "1")]);
    assert!(
        result.is_ok(),
        "version should succeed with AUTTER_ALLOW_SUPERUSER=1"
    );
}
...
fn superuser_guard_warns_when_running_as_root_without_opt_in() {
    ...
    cmd.args(["status"]).env_remove("AUTTER_ALLOW_SUPERUSER");
...
fn remove_all_ci_env_vars(cmd: &mut Command) -> &mut Command {
    cmd.env_remove("CI")
        ...
        .env_remove("AUTTER_DAEMON_UPGRADE")
...
fn superuser_guard_allows_root_with_env_var_opt_in() {
    ...
    let binary_path = get_binary_path();
    let mut cmd = Command::new(binary_path);
    cmd.args(["status"]).env("AUTTER_ALLOW_SUPERUSER", "1");
    remove_all_ci_env_vars(&mut cmd);
    let output = cmd.output().expect("failed to execute binary");
    ...
    assert!(
        ...
        "should NOT show warning when AUTTER_ALLOW_SUPERUSER=1 is set, got: {stderr}"
    );
    assert!(
        stderr.contains("warning: running as superuser (AUTTER_ALLOW_SUPERUSER is set)"),
        "should show opt-in acknowledgment when running as root with AUTTER_ALLOW_SUPERUSER, got: {stderr}"
    );
}
...
fn superuser_guard_allows_root_in_ci_environment() {
    ...
    cmd.args(["status"])
        .env("CI", "true")
        .env_remove("AUTTER_ALLOW_SUPERUSER");
    remove_all_ci_env_vars(&mut cmd);
