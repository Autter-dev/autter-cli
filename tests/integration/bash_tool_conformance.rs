...
fn test_build_gitignore_parses_rules() {
    // build_gitignore covers autter-specific patterns only (defaults,
    // .autter-ignore, linguist-generated).  Standard .gitignore rules are
    ...
    // autter default patterns should be present (*.lock is in DEFAULT_IGNORE_PATTERNS)
    assert!(
        gitignore
            .matched(Path::new("Cargo.lock"), false)
            .is_ignore(),
        "Cargo.lock should match autter default patterns"
    );
    ...
    assert!(
        ...
        "*.rs should not match any autter default patterns"
    );
}
...
fn test_handle_bash_tool_detects_rename() {
    use autter::commands::checkpoint_agent::bash_tool::diff;
    let repo = TestRepo::new();
    let root = repo_root(&repo);
