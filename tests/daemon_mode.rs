...
use autter::authorship::working_log::CheckpointKind;
use autter::daemon::{
...
use autter::git::find_repository_in_path;
...
fn repo_storage(repo: &TestRepo) -> autter::git::repository::Repository {
...
fn start_daemon_for_repo(repo: &TestRepo) {
    ...
    command
        ...
        .env("AUTTER_TEST_DB_PATH", repo.test_db_path())
        .env("AUTTER_TEST_DB_PATH", repo.test_db_path())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
...
fn configure_test_home_env(command: &mut Command, test_home: &Path) {
    ...
    // Sanitize PATH to remove directories containing the Nix autter
    ...
    if let Ok(path) = std::env::var("PATH") {
        let sanitized: Vec<&str> = path
            .split(':')
            .filter(|dir| {
                // Keep only dirs that do NOT contain a autter wrapper
                // (heuristic: skip dirs where the `git` binary is a
                //  shell-script wrapper for autter, or a symlink to autter).
...
fn configure_test_daemon_env(
    ...
    command.env("AUTTER_DAEMON_HOME", daemon_home);
    command.env("AUTTER_DAEMON_CONTROL_SOCKET", control_socket_path);
    command.env("AUTTER_DAEMON_TRACE_SOCKET", trace_socket_path);
}
...
impl DaemonGuard {
    ...
    fn start_with_env(repo: &TestRepo, extra_env: &[(&str, &str)]) -> Self {
        ...
        let child = command.spawn().expect("failed to spawn autter subprocess");
        let mut daemon = Self {
            child,
            control_socket_path,
            trace_socket_path,
...
impl WorkdirRaceHarness {
    ...
    fn run_traced_git(&self, workdir: &Path, args: &[&str]) {
        ...
        let output = command
            .env("AUTTER_TEST_DB_PATH", &self.test_db_path)
            .env("AUTTER_TEST_DB_PATH", &self.test_db_path)
            .env(
                "GIT_TRACE2_EVENT",
    ...
    fn run_delegated_checkpoint(&self, workdir: &Path, file_rel: &str) {
        ...
        let output = command
            .env("AUTTER_TEST_DB_PATH", &self.test_db_path)
            .env("AUTTER_TEST_DB_PATH", &self.test_db_path)
            .env("AUTTER_DAEMON_CHECKPOINT_DELEGATE", "true")
            .output()
...
fn assert_blame_lines_for_workdir(
    ...
    let blame_output = repo
        .autter_from_working_dir(workdir, &["blame", file_rel])
        .unwrap_or_else(|e| {
            panic!(
                "autter blame failed in {} for {}: {}",
                workdir.display(),
                file_rel,
                e
            )
...
fn assert_post_commit_uploads_prompt_cas(mode: GitTestMode) {
    ...
    let _api_base_url = ScopedEnvVar::set("AUTTER_API_BASE_URL", mock_api.base_url());
    let _api_key = ScopedEnvVar::set("AUTTER_API_KEY", "test-api-key");
    ...
    repo.autter(&["checkpoint", "claude", "--hook-input", &hook_input])
    ...
    let log =
        autter::authorship::authorship_log_serialization::AuthorshipLog::deserialize_from_string(
            &note,
        )
...
fn checkpoint_delegate_autostarts_daemon_when_unavailable() {
    ...
    repo.autter_with_env(
        &["checkpoint", "mock_ai", "delegate-fallback.txt"],
        &[("AUTTER_DAEMON_CHECKPOINT_DELEGATE", "true")],
    )
...
fn checkpoint_fails_hard_when_daemon_startup_is_blocked() {
    ...
    let result = repo.autter(&["checkpoint", "mock_ai", "delegate-fallback-blocked.txt"]);
    assert!(
        result.is_ok(),
...
fn daemon_windows_stalled_checkpoint_clients_do_not_block_later_control_requests() {
    ...
    let mut daemon = DaemonGuard::start_with_env(
        &repo,
        &[
            ("AUTTER_TEST_WINDOWS_CONTROL_PIPE_WORKERS", "2"),
            ("AUTTER_DAEMON_UPDATE_CHECK_INTERVAL", "86400"),
            ("AUTTER_DAEMON_MAX_UPTIME_SECS", "86400"),
        ],
    );
...
#[test]
#[serial]
fn daemon_test_mode_autter_checkpoint_runs_via_daemon() {
    let repo =
        TestRepo::new_with_mode_and_daemon_scope(GitTestMode::Daemon, DaemonTestScope::Dedicated);
    ...
    repo.autter(&["checkpoint", "mock_ai", "daemon-mode-checkpoint.txt"])
...
fn daemon_test_mode_human_checkpoint_with_explicit_preset_queues_via_daemon() {
    ...
    repo.autter(&["checkpoint", "human", "human-direct-path.txt"])
        .expect("human checkpoint with preset should succeed");

    repo.wait_for_next_daemon_checkpoint_completion(completion_baseline);

    let autter_repo = autter::git::repository::find_repository_in_path(
        repo.path()
            .to_str()
            .expect("repo path should be valid UTF-8"),
    )
    .expect("repository should still be discoverable");
    let base_commit = autter_repo
        .head()
        .ok()
        .and_then(|head| head.target().ok())
        .unwrap_or_else(|| "initial".to_string());
    let checkpoints = autter_repo
        .storage
...
fn daemon_commit_replay_recovers_stash_restore_when_working_log_is_missing() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "stash-recover.txt"])
    ...
    let autter_repo = repo_storage(&repo);
    autter_repo
        .storage
...
fn daemon_commit_replay_recovers_backward_reset_when_working_log_is_missing() {
    ...
    let autter_repo = repo_storage(&repo);
    assert!(
        autter_repo.storage.has_working_log(&head),
...
fn daemon_commit_replay_recovers_same_head_pathspec_reset_when_working_log_is_missing() {
    ...
    let head = current_head_sha(&repo);
    let autter_repo = repo_storage(&repo);
    let working_log_dir = autter_repo
        .storage
        .working_log_for_base_commit(&head)
        .unwrap()
        .dir;
    let backup_dir = repo.path().join(".autter-test-pathspec-reset-backup");
    ...
    let new_working_log = autter_repo
        .storage
...
fn daemon_commit_replay_recovers_switch_migration_when_working_log_is_missing() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "switch-recover.txt"])
...
fn daemon_recovers_from_panic_in_side_effect_pipeline() {
    ...
    let mut daemon = DaemonGuard::start_with_env(
        &repo,
        &[(
            "AUTTER_TEST_PANIC_IN_SIDE_EFFECT_FLAG",
            panic_flag_path
                .to_str()
...
fn daemon_shuts_down_when_socket_files_are_deleted() {
    ...
    let mut daemon = DaemonGuard::start_with_env(
        &repo,
        &[
            ("AUTTER_DAEMON_SOCKET_HEALTH_CHECK_SECS", "1"),
            ("AUTTER_DAEMON_UPDATE_CHECK_INTERVAL", "86400"),
            ("AUTTER_DAEMON_MAX_UPTIME_SECS", "86400"),
        ],
    );
...
/// spawn a detached `autter bg restart --hard` process that reaps the
...
fn daemon_self_heals_after_socket_deletion() {
    ...
    let mut daemon = DaemonGuard::start_with_env(
        ...
        &[
            ...
            ("AUTTER_DAEMON_UPDATE_CHECK_INTERVAL", "86400"),
            ("AUTTER_DAEMON_MAX_UPTIME_SECS", "86400"),
            ("AUTTER_DAEMON_MIN_UPTIME_FOR_RESTART_SECS", "0"),
        ],
    );
