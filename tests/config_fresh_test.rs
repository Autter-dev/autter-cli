use autter::api::client::ApiContext;
use autter::auth::client::OAuthClient;
...
use autter::config::{Config, load_file_config_public, save_file_config};
...
fn test_config_fresh_picks_up_file_changes() {
    ...
    let config_dir = temp_dir.path().join(".autter");
    std::fs::create_dir_all(&config_dir).expect("Failed to create config dir");

    // Write initial config with api_base_url = "https://old.example.com"
    let mut file_config = autter::config::FileConfig {
...
fn test_config_get_uses_cache() {
    ...
    let file_config = autter::config::FileConfig {
...
fn test_config_fresh_picks_up_api_key_changes() {
    ...
    let file_config = autter::config::FileConfig::default();
    save_file_config(&file_config).expect("Failed to save config");
...
fn test_config_fresh_respects_env_vars() {
    ...
    let original_api_url = env::var("AUTTER_API_BASE_URL").ok();
    ...
    unsafe {
        env::set_var("AUTTER_API_BASE_URL", "https://env-var.example.com");
    }
    let config2 = Config::fresh();
    ...
    unsafe {
        if let Some(api_url) = original_api_url {
            env::set_var("AUTTER_API_BASE_URL", api_url);
        } else {
            env::remove_var("AUTTER_API_BASE_URL");
        }
    }
