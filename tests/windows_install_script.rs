...
fn installed_autter_path(repo: &TestRepo) -> PathBuf {
    repo.test_home_path()
        .join(".autter")
        .join("bin")
        .join("autter.exe")
...
fn configure_install_env(command: &mut Command, repo: &TestRepo) {
    ...
    command.env("AUTTER_LOCAL_BINARY", get_binary_path());
    command.env("AUTTER_SKIP_PATH_UPDATE", "1");
    ...
    command.env("AUTTER_TEST_DB_PATH", repo.test_db_path());
    command.env("AUTTER_TEST_DB_PATH", repo.test_db_path());
...
fn run_installed_autter(repo: &TestRepo, args: &[&str], timeout: Duration) -> CommandResult {
    let mut command = Command::new(installed_autter_path(repo));
...
fn kill_installed_processes(repo: &TestRepo) {
    let script = format!(
        ...
        installed_autter_path(repo).display(),
        repo.test_home_path()
            .join(".autter")
            .join("bin")
            .join("git.exe")
...
fn windows_install_script_reinstall_stops_running_daemon() {
    ...
    let installed_autter = installed_autter_path(&repo);
    assert!(
        installed_autter.exists(),
        "autter.exe should be installed at {}",
        installed_autter.display()
    );
    ...
    let version = run_installed_autter(&repo, &["--version"], Duration::from_secs(15));
    assert!(
        version.status.success(),
        "installed autter should remain usable after reinstall\nstdout:\n{}\nstderr:\n{}",
        version.stdout,
        version.stderr
    );
...
fn seed_existing_wrapper(repo: &TestRepo) {
    let bin_dir = repo.test_home_path().join(".autter").join("bin");
    fs::create_dir_all(&bin_dir).expect("failed to create install dir");
    fs::write(bin_dir.join("autter.exe"), b"").expect("failed to create autter.exe stub");
    fs::write(bin_dir.join("git.exe"), b"").expect("failed to create git.exe stub");
}

#[test]
#[serial]
fn windows_git_extension_upgrade_requires_direct_autter_binary() {
    let repo =
        TestRepo::new_with_mode_and_daemon_scope(GitTestMode::Daemon, DaemonTestScope::NoDaemon);
    ...
    assert!(
        ...
        "`autter upgrade` should fail fast on Windows\nstdout:\n{}\nstderr:\n{}",
        result.stdout,
        result.stderr
    );
    assert!(
        combined.contains("`autter upgrade` is not supported on Windows"),
        "expected Windows upgrade guard message, got:\n{}",
        combined
    );
    assert!(
        combined.contains("autter upgrade"),
...
fn windows_install_script_skips_wrapper_for_new_users() {
    ...
    assert!(
        installed_autter_path(&repo).exists(),
        "autter.exe should be installed at {}",
        installed_autter_path(&repo).display()
    );
...
fn windows_install_script_gates_daemon_restart_to_self_update() {
    let script = fs::read_to_string(install_script_path()).expect("failed to read install.ps1");
    assert!(
        script.contains("AUTTER_RESTART_DAEMON_AFTER_INSTALL"),
