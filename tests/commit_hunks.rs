...
use autter::commands::diff::{
    DiffCommandOptions, DiffJsonHunk, build_diff_artifacts_from_hunks,
    build_diff_artifacts_with_note, get_diff_with_line_numbers,
};
use autter::git::repository::Repository as AutterRepository;
use repos::test_file::ExpectedLineExt;
use repos::test_repo::TestRepo;
use sha2::{Digest, Sha256};
use std::fs;

fn get_repo(test_repo: &TestRepo) -> AutterRepository {
    autter::git::find_repository_in_path(test_repo.path().to_str().unwrap())
...
fn test_commit_hunks_multiple_files() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "a.txt"]).unwrap();

    fs::write(&file_b, "B line 1\n").unwrap();
    repo.autter(&["checkpoint", "mock_known_human", "b.txt"])
        .unwrap();
...
fn test_commit_hunks_deletions_unattributed() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "deleteme.txt"])
        .unwrap();
...
fn test_commit_hunks_consecutive_same_attribution_merges() {
    ...
    let content = (1..=10)
        ...
        + "\n";
    fs::write(&file_path, &content).unwrap();
    repo.autter(&["checkpoint", "mock_ai", "merge.txt"])
        .unwrap();
...
fn test_commit_hunks_attribution_boundaries_split() {
    ...
    repo.autter(&["checkpoint", "human", "split.txt"]).unwrap();

    // Then AI adds
    fs::write(&file_path, "Untracked\nAI added\n").unwrap();
    repo.autter(&["checkpoint", "mock_ai", "split.txt"])
        .unwrap();
...
fn test_commit_hunks_session_id_extraction() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "session.txt"])
        .unwrap();
...
fn test_commit_hunks_empty_diff() {
    ...
    repo.autter(&["checkpoint", "mock_ai", "empty.txt"])
        .unwrap();
...
fn test_commit_hunks_content_hash_correctness() {
    ...
    fs::write(&file_path, &content).unwrap();
    repo.autter(&["checkpoint", "mock_ai", "hash_test.txt"])
        .unwrap();
...
fn test_commit_hunks_interleaved_attributions() {
    ...
    repo.autter(&["checkpoint", "mock_known_human", "interleaved.txt"])
        .unwrap();

    // AI adds lines 2-3
    fs::write(&file_path, "Human 1\nAI 1\nAI 2\n").unwrap();
    repo.autter(&["checkpoint", "mock_ai", "interleaved.txt"])
        .unwrap();
