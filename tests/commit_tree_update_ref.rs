...
use autter::git::refs::show_authorship_note;
...
fn open_repo(repo: &TestRepo) -> AutterRepository {
    find_repository_in_path(repo.path().to_str().unwrap())
        .expect("failed to open autter repository")
}
...
fn test_commit_tree_update_ref_preserves_authorship_notes_on_reparent() {
    ...
    let autter_repo = open_repo(&repo);
    assert!(
        show_authorship_note(&autter_repo, &feature_commit.commit_sha).is_some(),
    ...
    let autter_repo = open_repo(&repo);
    assert!(
        show_authorship_note(&autter_repo, &new_head).is_some(),
...
fn test_commit_tree_update_ref_moves_working_log_to_rewritten_head() {
    ...
    let autter_repo = open_repo(&repo);
    assert!(
        autter_repo.storage.has_working_log(&old_head),
    ...
    let autter_repo = open_repo(&repo);
    assert!(
        autter_repo.storage.has_working_log(&new_head),
...
fn test_update_ref_restack_after_parent_amend_preserves_child_attribution() {
    ...
    let autter_repo = open_repo(&repo);
    assert!(
        show_authorship_note(&autter_repo, &child_commit.commit_sha).is_some(),
    ...
    let autter_repo = open_repo(&repo);
    assert!(
        show_authorship_note(&autter_repo, &new_child_head).is_some(),
...
/// autter must detect the N-commit rewrite and remap all N authorship notes.
#[test]
fn test_graphite_style_multi_commit_single_update_ref() {
    ...
    for &sha in &feature_commits {
        assert!(
            show_authorship_note(&autter_repo, sha).is_some(),
