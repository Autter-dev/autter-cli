# Contributing to Autter

Thank you for your interest in contributing to `autter`. This is a cool moment for the industry and we're all here to build ~~a~~ the standard for tracking AI code. 
...
2. **Clone your fork**:
   ```bash
   git clone https://github.com/YOUR_USERNAME/autter.git
   cd autter
   ```
...
It's often helpful to point your `autter` to a development build. The dev script builds the binary and installs it to `~/.autter/bin/autter`, replacing the production binary so you can test changes with real git repositories.

```bash
task dev
```

If `~/.autter` isn't set up yet, the script will run the installer automatically first.
