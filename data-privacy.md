...
If you install Autter open source and don't login **no code, prompts, or agent usage data is ever sent to Autter**. Autter runs entirely on your machine and writes attribution data into your local git repository and prompts to a local SQLite.

The only data Autter sends externally in open source mode is error and exception telemetry, which is enabled by default to help us improve the tool. You can disable or redirect it at any time by turning `telemetry_oss` to `off`. See [configuration options](https://autter.dev/docs/cli/configuration#configuration-options) for details.

## Data

### Local-only data

- **Prompts** are stored locally on the developer's laptop. They are never shared with teammates or Autter.
...
- **Error & exception telemetry** — shared by default with Autter. You can disable it or redirect it to your own endpoint. See [configuration options](https://autter.dev/docs/cli/configuration#configuration-options).

---

## Autter Cloud (Personal Dashboards)

If you opt in to a personal dashboard, the following is uploaded to Autter Cloud:
...
See our [Cloud Privacy Policy](https://autter.dev/privacy-policy) for details.

---

## Autter for Teams and Enterprise
...
- **Full prompts** uploaded to the Autter prompt store
...
- **Error & exception telemetry** — shared by default with Autter, unless disabled or redirected to your own endpoint. See [configuration options](https://autter.dev/docs/cli/configuration#configuration-options).

For more information, see our [Trust Center](https://trust.autter.dev).

In [self-hosted deployments](https://github.com/autter-dev/self-hosted), all data is sent only to your team's Autter instance.
