...
fn select_target(argv: &[String]) -> Result<(String, bool), String> {
    let tracked_target = env::var("AUTTER_TEST_GIT_SHIM_TARGET")
        .map_err(|_| "AUTTER_TEST_GIT_SHIM_TARGET is required".to_string())?;
    let fallback_target =
        env::var("AUTTER_TEST_GIT_SHIM_FALLBACK_TARGET").unwrap_or_else(|_| tracked_target.clone());
    let tracked_target_uses_autter =
        env::var("AUTTER_TEST_GIT_SHIM_TARGET_USE_AUTTER").as_deref() == Ok("1");
    ...
    if tracks_parsed_git_invocation_for_test_sync(&parsed) {
        Ok((tracked_target, tracked_target_uses_autter))
    } else {
...
fn new_test_sync_session() -> String {
    format!("gt-shim-{}", autter::uuid::generate_v4())
}
...
#[cfg(unix)]
fn exec_target(target: &str, argv: &[String], use_autter_wrapper_mode: bool) -> ! {
    let mut command = Command::new(target);
    command.args(argv);
    if use_autter_wrapper_mode {
        command.env("AUTTER", "git");
    }
    let error = command.exec();
    eprintln!("autter-test-git-shim failed to exec {target}: {error}");
...
fn exec_target(target: &str, argv: &[String], use_autter_wrapper_mode: bool) -> ! {
    ...
    match command.status() {
        ...
        Err(error) => {
            eprintln!("autter-test-git-shim failed to spawn {target}: {error}");
            std::process::exit(127);
        }
    }
}
...
fn main() {
    ...
    if let Ok(log_path) = env::var("AUTTER_TEST_SYNC_START_LOG") {
        ...
        if let Err(error) = append_started_log(&log_path, &argv, test_sync_session.as_deref()) {
            panic!("autter-test-git-shim failed: {error}");
        }
    }
    exec_target(&target, &effective_argv, use_autter_wrapper_mode);
}
