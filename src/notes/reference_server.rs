...
//!
//! This module exists to make the wire contract between `autter` and a
...
impl ReferenceServer {
    ...
    /// shared [`NotesStore`].
    pub fn start(bind_addr: &str) -> Result<Self, AutterError> {
        let listener = TcpListener::bind(bind_addr)
            .map_err(|e| AutterError::Generic(format!("bind {}: {}", bind_addr, e)))?;
        ...
        listener
            ...
            .map_err(AutterError::IoError)?;

        let addr = listener.local_addr().map_err(AutterError::IoError)?;
...
/// `autter notes serve` CLI entry point.
pub fn run_blocking(bind_addr: &str) -> Result<(), AutterError> {
...
fn handle_connection(mut stream: TcpStream, store: &NotesStore) -> Result<(), AutterError> {
    let request = read_request(&mut stream)?;
    let response = dispatch(&request, store);
    write_response(&mut stream, &response)
}

fn read_request(stream: &mut TcpStream) -> Result<Request, AutterError> {
    let mut reader = BufReader::new(stream.try_clone().map_err(AutterError::IoError)?);
    ...
    loop {
        ...
        let n = reader.read_line(&mut line).map_err(AutterError::IoError)?;
        if n == 0 || line == "\r\n" || line == "\n" {
            break;
        }
    ...
    if content_length > 0 {
        reader.read_exact(&mut body).map_err(AutterError::IoError)?;
    }
...
fn write_response(stream: &mut TcpStream, response: &Response) -> Result<(), AutterError> {
    ...
    stream
        ...
        .map_err(AutterError::IoError)?;
    stream.flush().map_err(AutterError::IoError)?;
    Ok(())
}
