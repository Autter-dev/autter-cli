//! Dedicated notes-backend storage at `~/.autter/internal/notes-db`.
//!
...
impl NotesDatabase {
    ...
    ///
    /// In test builds with `AUTTER_TEST_NOTES_DB_PATH` set, the OnceLock singleton
    ...
    pub fn global() -> Result<&'static Mutex<NotesDatabase>, AutterError> {
        let db_mutex = NOTES_DB.get_or_init(|| match Self::new() {
            ...
            Err(e) => {
                ...
                let temp_path = std::env::temp_dir().join("autter-notes-db-failed");
    ...
    ///
    /// In tests, the `AUTTER_TEST_NOTES_DB_PATH` environment variable overrides
    /// the default location so that test runs are isolated.
    fn database_path() -> Result<PathBuf, AutterError> {
        #[cfg(any(test, feature = "test-support"))]
        if let Ok(test_path) = std::env::var("AUTTER_TEST_NOTES_DB_PATH") {
        ...
        Ok(home.join(".autter").join("internal").join("notes-db"))
    }
    ...
    /// enqueued for upload.
    pub fn cache_synced_notes(&mut self, entries: &[(String, String)]) -> Result<(), AutterError> {
        if entries.is_empty() {
            return Ok(());
        }
        let now = unix_now();
    ...
    pub fn dequeue_pending(&mut self, batch_size: usize) -> Result<Vec<PendingNote>, AutterError> {
        let now = unix_now();
    ...
    /// of rows updated.
    pub fn mark_synced(&mut self, commit_shas: &[String]) -> Result<usize, AutterError> {
    ...
    pub fn mark_failed(&mut self, commit_shas: &[String], error: &str) -> Result<(), AutterError> {
        if commit_shas.is_empty() {
            return Ok(());
        }
        let now = unix_now();
    ...
    pub fn get_note(&self, commit_sha: &str) -> Result<Option<String>, AutterError> {
    ...
    pub fn get_synced_shas(&self, commit_shas: &[&str]) -> Result<HashSet<String>, AutterError> {
        if commit_shas.is_empty() {
    ...
    /// from the result map.
    pub fn get_notes(&self, commit_shas: &[&str]) -> Result<HashMap<String, String>, AutterError> {
        if commit_shas.is_empty() {
...
mod tests {
    ...
    fn test_database_path_contains_expected_segments() {
        // Without the test env var set we expect the path to include .autter/internal/notes-db
        ...
        assert!(
            path_str.contains(".autter"),
            "path should contain .autter: {}",
            path_str
        );
        assert!(
