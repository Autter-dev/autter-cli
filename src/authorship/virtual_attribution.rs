...
impl VirtualAttributions {
    ...
    async fn discover_and_load_foreign_prompts(&mut self) -> Result<(), AutterError> {
    ...
    async fn load_prompts_concurrent(
        &self,
        missing_ids: &[String],
    ) -> Result<Vec<(String, String, PromptRecord)>, AutterError> {
    ...
    fn find_prompt_in_history_static(
        repo: &Repository,
        prompt_id: &str,
    ) -> Result<(String, crate::authorship::authorship_log::PromptRecord), AutterError> {
    ...
    async fn load_sessions_concurrent(
        &self,
        missing_ids: &[String],
    ) -> Result<Vec<(String, SessionRecord)>, AutterError> {
        const MAX_CONCURRENT: usize = 30;
    ...
    fn find_session_in_history_static(
        repo: &Repository,
        session_id: &str,
    ) -> Result<SessionRecord, AutterError> {
    ...
    #[allow(dead_code)]
    pub async fn add_pathspec(&mut self, pathspec: &str) -> Result<(), AutterError> {
        self.add_pathspecs_concurrent(&[pathspec.to_string()]).await
    }

    /// Add multiple pathspecs concurrently
    async fn add_pathspecs_concurrent(&mut self, pathspecs: &[String]) -> Result<(), AutterError> {
        const MAX_CONCURRENT: usize = 30;
    ...
    pub fn to_authorship_log(
        &self,
    ) -> Result<crate::authorship::authorship_log_serialization::AuthorshipLog, AutterError> {
...
) -> Result<HashMap<String, Vec<LineRange>>, AutterError> {
...
impl VirtualAttributions {
    ...
    > {
        ...
        for (file_path, (_, line_attrs)) in &self.attributions {
            ...
            // leaves gaps in committed_hunks that show as [no-data] in `autter diff`.
            //
...
) -> Result<VirtualAttributions, AutterError> {
...
fn transform_attributions_to_final(
    ...
    new_content: &str,
    ts: u128,
) -> Result<Vec<Attribution>, AutterError> {
...
fn compute_attributions_for_file(
    ...
    ts: u128,
    blame_start_commit: Option<String>,
) -> Result<Option<(String, String, Vec<Attribution>, Vec<LineAttribution>)>, AutterError> {
    // Set up blame options
    let mut ai_blame_opts = AutterBlameOptions::default();
