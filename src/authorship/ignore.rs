...
/// Load ignore patterns from a `.autter-ignore` file at the repository root.
/// The file follows `.gitignore` syntax: one glob pattern per line, blank lines
/// and lines starting with `#` are skipped.
pub fn load_autter_ignore_patterns(repo: &Repository) -> Vec<String> {
    let Some(contents) = load_root_autter_ignore_contents(repo) else {
        return Vec::new();
    };
...
fn load_root_autter_ignore_contents(repo: &Repository) -> Option<String> {
    if repo.is_bare_repository().unwrap_or(false) {
        return repo
            .get_file_content(".autter-ignore", "HEAD")
            .ok()
            .and_then(|bytes| String::from_utf8(bytes).ok());
    }

    let workdir = repo.workdir().ok()?;
    let ignore_path = workdir.join(".autter-ignore");
    fs::read_to_string(ignore_path).ok()
}

/// Load `.autter-ignore` patterns from a repo root path directly (no Repository object needed).
/// Use this when you have a `&Path` but not a `Repository` (e.g. in snapshot capture code).
pub fn load_autter_ignore_patterns_from_path(repo_root: &Path) -> Vec<String> {
    let contents = match fs::read_to_string(repo_root.join(".autter-ignore")) {
        Ok(s) => s,
...
pub fn effective_ignore_patterns(
    ...
    patterns.extend(load_autter_ignore_patterns(repo));
