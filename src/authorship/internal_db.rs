...
/// Database wrapper for internal autter storage
...
impl InternalDatabase {
    ...
    pub fn global() -> Result<&'static Mutex<InternalDatabase>, AutterError> {
        ...
        let db_mutex = INTERNAL_DB.get_or_init(|| {
            match Self::new() {
                ...
                Err(e) => {
                    ...
                    let temp_path = std::env::temp_dir().join("autter-db-failed");
    ...
    /// Get database path: ~/.autter/internal/db
    /// In test mode, can be overridden via AUTTER_TEST_DB_PATH environment variable.
    /// We also support AUTTER_TEST_DB_PATH because some git hook execution paths
    ...
    fn database_path() -> Result<PathBuf, AutterError> {
        ...
        if let Ok(test_path) =
            std::env::var("AUTTER_TEST_DB_PATH").or_else(|_| std::env::var("AUTTER_TEST_DB_PATH"))
        {
        ...
        Ok(home.join(".autter").join("internal").join("db"))
    }
    ...
    pub fn dequeue_cas_batch(
        &mut self,
        batch_size: usize,
    ) -> Result<Vec<CasSyncRecord>, AutterError> {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs() as i64;
    ...
    pub fn delete_cas_by_hashes(&mut self, hashes: &[String]) -> Result<usize, AutterError> {
        if hashes.is_empty() {
            return Ok(0);
        }
    ...
    pub fn update_cas_sync_failure(&mut self, id: i64, error: &str) -> Result<(), AutterError> {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs() as i64;
...
mod tests {
    ...
    fn test_database_path() {
        let override_path = std::env::var("AUTTER_TEST_DB_PATH").ok();
        let path = InternalDatabase::database_path().unwrap();
        if let Some(override_path) = override_path {
            assert_eq!(path, PathBuf::from(override_path));
        } else {
            assert!(path.to_string_lossy().contains(".autter"));
