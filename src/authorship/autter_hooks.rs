...
/// Dispatch configured `autter_hooks.post_notes_updated` shell commands.
///
...
/// detaching and continuing so autter does not block.
pub fn post_notes_updated(repo: &Repository, notes: &[(String, String)]) {
    ...
    let hook_commands = Config::get()
        .autter_hook_commands(POST_NOTES_UPDATED_HOOK)
        .cloned()
        .unwrap_or_default();
    ...
    let payload_json = match serde_json::to_string(&payload) {
        ...
        Err(e) => {
            tracing::debug!(
                "[autter_hooks] Failed to serialize post_notes_updated payload: {}",
                e
            );
            return;
        }
    ...
    for hook_command in hook_commands {
        let mut child = match spawn_shell_command(&hook_command) {
            ...
            Err(e) => {
                tracing::debug!(
                    "[autter_hooks] Failed to spawn post_notes_updated hook '{}': {}",
                    hook_command,
                    e
                );
                continue;
        ...
        if let Some(mut stdin) = child.stdin.take() {
            ...
            std::thread::spawn(move || {
                ...
                if let Err(e) = stdin.write_all(payload_for_stdin.as_bytes()) {
                    tracing::debug!(
                        "[autter_hooks] Failed to write post_notes_updated stdin for '{}': {}",
                        command_for_log,
                        e
                    );
                }
            ...
            tracing::debug!(
                "[autter_hooks] Hook '{}' was spawned without a stdin pipe",
                hook_command
            );
        }
...
fn wait_for_hooks_or_detach(mut children: Vec<(String, Child)>) {
    ...
    loop {
        ...
        for (command, mut child) in children {
            match child.try_wait() {
                Ok(Some(status)) => {
                    if !status.success() {
                        tracing::debug!(
                            "[autter_hooks] Hook '{}' exited with status {}",
                            command,
                            status
                        );
                    }
                ...
                Err(e) => {
                    tracing::debug!("[autter_hooks] Failed to poll hook '{}': {}", command, e);
                }
            }
        }
        ...
        if Instant::now() >= deadline {
            ...
            tracing::debug!(
                "[autter_hooks] Detaching {} unfinished hook command(s) after {}ms",
                detached_count,
                ...
                for (command, mut child) in still_running {
                    match child.wait() {
                        Ok(status) => {
                            if !status.success() {
                                tracing::debug!(
                                    "[autter_hooks] Detached hook '{}' exited with status {}",
                                    command,
                                    status
                                );
                            }
                        ...
                        Err(e) => {
                            tracing::debug!(
                                "[autter_hooks] Failed waiting detached hook '{}': {}",
                                command,
                                e
                            );
                        }
