...
pub fn load_rebase_note_cache(
    ...
    new_commits: &[String],
) -> Result<RebaseNoteCache, AutterError> {
...
pub fn rewrite_authorship_after_rebase_v2(
    ...
    for (idx, new_commit) in commits_to_process.iter().enumerate() {
        ...
        let authorship_json = if commit_has_attestations {
            ...
            // checkpoint for any of the changed files (written by `autter checkpoint`
    ...
    if let Ok(timing_path) = std::env::var("AUTTER_REBASE_TIMING_FILE") {
...
pub fn collect_changed_file_contents_from_diff(
...
) -> Result<(HashSet<String>, HashMap<String, String>), AutterError> {
    ...
    for delta in diff.deltas() {
        let file_path = delta
            ...
            .ok_or_else(|| AutterError::Generic("File path not available".to_string()))?;
...
fn load_commit_metadata_batch(
    repo: &Repository,
    commit_shas: &[String],
) -> Result<HashMap<String, CommitObjectMetadata>, AutterError> {
    ...
    while pos < data.len() {
        ...
        let size: usize = parts
            .next()
            .ok_or_else(|| {
                AutterError::Generic("Malformed cat-file --batch header: missing size".to_string())
            })?
            .parse()
            .map_err(|e| {
                AutterError::Generic(format!("Invalid cat-file --batch object size: {}", e))
            })?;
...
fn run_diff_tree_with_hunks(
    ...
    pathspecs_lookup: &HashSet<&str>,
    pathspecs: &[String],
) -> Result<(DiffTreeResult, HunksByCommitAndFile), AutterError> {
    if commit_shas.is_empty() {
        return Ok((
            DiffTreeResult {
...
pub fn walk_commits_to_base(
    ...
    head: &str,
    base: &str,
) -> Result<Vec<String>, crate::error::AutterError> {
    if head == base {
        return Ok(Vec::new());
    }
...
pub fn reconstruct_working_log_after_reset(
    ...
    // reset range and are tracked by autter are already carried into `old_head_va` through the
...
) -> Result<Option<AuthorshipLog>, AutterError> {
    use crate::authorship::authorship_log::{HumanRecord, SessionRecord};
...
/// `autter checkpoint` recorded in the working log during `rebase --continue`.
///
...
) -> Result<crate::authorship::virtual_attribution::VirtualAttributions, AutterError> {
