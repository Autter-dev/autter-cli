...
impl<B: GitBackend> TraceNormalizer<B> {
    ...
    fn refresh_pending_mutation_capture(&mut self, root_sid: &str) -> Result<(), AutterError> {
        let primary_hint = {
    ...
    pub fn ingest_payload(
        &mut self,
        payload: &Value,
    ) -> Result<Option<NormalizedCommand>, AutterError> {
        let event = payload
            .get("event")
            .and_then(Value::as_str)
            .ok_or_else(|| AutterError::Generic("trace payload missing event".to_string()))?;
        let sid = payload
            .get("sid")
            .and_then(Value::as_str)
            .ok_or_else(|| AutterError::Generic("trace payload missing sid".to_string()))?;
        ...
        self.merge_pending_stash_metadata(
            &root_sid,
            payload_string_field(payload, "autter_stash_target_oid"),
            payload_string_field(payload, "autter_stash_target_oid_error"),
        );
        self.merge_pending_merge_squash_source_head(
            &root_sid,
            payload_string_field(payload, "autter_merge_squash_source_head"),
        );
        self.merge_pending_carryover_snapshot_id(
            &root_sid,
            payload_string_field(payload, "autter_carryover_snapshot_id"),
        );
    ...
    fn handle_start(
        ...
        let reflog_start_cut = if should_capture_mutation_state {
            payload_reflog_cut(payload, "autter_family_reflog_start")
        } else {
            None
        };
        let worktree_head_start_offset = if should_capture_mutation_state {
            payload
                .get("autter_worktree_head_reflog_start")
                .and_then(Value::as_u64)
        } else {
            None
        };
        let pre_repo = payload_repo_context(payload, "autter_pre_repo");
        let stash_target_oid = payload_string_field(payload, "autter_stash_target_oid");
        let stash_target_error = payload_string_field(payload, "autter_stash_target_oid_error");
        let merge_squash_source_head =
            payload_string_field(payload, "autter_merge_squash_source_head");
        let carryover_snapshot_id = payload_string_field(payload, "autter_carryover_snapshot_id");
    ...
    fn handle_def_param(
        ...
        if let Some(param) = payload.get("param").and_then(Value::as_str)
            && param == "AUTTER_WRAPPER_INVOCATION_ID"
    ...
    fn handle_def_repo(
        ...
        let payload_pre_repo = payload_repo_context(payload, "autter_pre_repo");
        ...
        let repo = if prefer_def_repo_target {
            payload_worktree
                .or(payload_repo)
                .or(pending_worktree)
                .ok_or_else(|| AutterError::Generic("def_repo missing repo path".to_string()))?
        } else {
            payload_worktree
                .or(pending_worktree)
                .or(payload_repo)
    ...
    fn handle_cmd_name(
        ...
        let cmd = payload
            .get("name")
            .and_then(Value::as_str)
            .ok_or_else(|| AutterError::Generic("cmd_name missing name".to_string()))?
            .to_string();
    ...
    fn handle_exit(
        ...
        let exit_code = payload
            ...
            .unwrap_or(0) as i32;
        let payload_pre_repo = payload_repo_context(payload, "autter_pre_repo");
        let payload_post_repo = payload_repo_context(payload, "autter_post_repo");
        let (payload_head_start, payload_head_end) = payload_worktree_head_offsets(payload);
    ...
    fn finalize_root_exit(
        ...
        let mut pending = self.state.pending.remove(root_sid).ok_or_else(|| {
            AutterError::Generic("missing pending command at finalize".to_string())
        })?;
...
fn trace_debug_lifecycle(message: &str) {
    if std::env::var("AUTTER_DEBUG_DAEMON_TRACE").is_ok() {
        eprintln!("\u{1b}[1;33m[autter]\u{1b}[0m {}", message);
    }
}
...
fn worktree_head_reflog_delta(
    ...
    start_offset: u64,
    end_offset: u64,
) -> Result<Vec<RefChange>, AutterError> {
...
fn payload_worktree_head_offsets(payload: &Value) -> (Option<u64>, Option<u64>) {
    let start = payload
        ...
        .and_then(Value::as_u64);
    let end = payload
        .get("autter_worktree_head_reflog_end")
        .and_then(Value::as_u64);
    (start, end)
}
...
fn payload_family_reflog_cuts(payload: &Value) -> (Option<ReflogCut>, Option<ReflogCut>) {
    (
        payload_reflog_cut(payload, "autter_family_reflog_start"),
        payload_reflog_cut(payload, "autter_family_reflog_end"),
    )
}

fn payload_reflog_changes(payload: &Value) -> Vec<RefChange> {
    payload
        .get("autter_family_reflog_changes")
...
fn payload_timestamp_ns(payload: &Value) -> Result<u128, AutterError> {
...
mod tests {
    ...
    impl GitBackend for MockBackend {
        fn resolve_family(&self, worktree: &Path) -> Result<FamilyKey, AutterError> {
            self.family_by_worktree
                .lock()
                .unwrap()
                .get(&normalize_path_key(worktree))
                .cloned()
                .ok_or_else(|| AutterError::Generic("family not found".to_string()))
        }

        fn repo_context(&self, worktree: &Path) -> Result<RepoContext, AutterError> {
            self.context_by_worktree
                .lock()
                .unwrap()
                .get(&normalize_path_key(worktree))
                .cloned()
                .ok_or_else(|| AutterError::Generic("context not found".to_string()))
        }

        fn reflog_cut(&self, _family: &FamilyKey) -> Result<ReflogCut, AutterError> {
            Ok(ReflogCut {
                offsets: HashMap::new(),
            })
        }
    ...
    fn alias_commit_captures_mutation_state_at_start() {
        ...
        let start = serde_json::json!({
            ...
            "argv":["git","ci","-m","msg"],
            "worktree":worktree,
            "autter_family_reflog_start": {"HEAD": 10}
        });
        let exit = serde_json::json!({
            "event":"exit",
            "sid":"alias-commit",
            "ts":2,
            "code":0,
            "autter_family_reflog_end": {"HEAD": 11}
        });
    ...
    fn interleaved_roots_with_out_of_order_exits_finalize_independently() {
        ...
        let start_a = serde_json::json!({
            ...
            "argv":["git","commit","-m","a"],
            "worktree":repo_a,
            "autter_family_reflog_start": {"HEAD": 100}
        });
        let start_b = serde_json::json!({
            ...
            "worktree":repo_b,
            "autter_family_reflog_start": {"HEAD": 200}
        });
        let exit_b = serde_json::json!({
            ...
            "ts":3,
            "code":0,
            "autter_family_reflog_end": {"HEAD": 201}
        });
        let exit_a = serde_json::json!({
            ...
            "ts":4,
            "code":0,
            "autter_family_reflog_end": {"HEAD": 101}
        });
    ...
    fn start_ignores_repo_gitdir_hint_and_uses_cwd_for_worktree_resolution() {
        ...
        let start = serde_json::json!({
            ...
            "autter_family_reflog_start": {"HEAD": 300}
        });
        let def_repo = serde_json::json!({
            "event":"def_repo",
            "sid":"s-repo-field",
        ...
        let exit = serde_json::json!({
            ...
            "ts":4,
            "code":0,
            "autter_family_reflog_end": {"HEAD": 301}
        });
    ...
    fn stash_target_oid_can_arrive_after_start_on_def_repo() {
        ...
        let start = serde_json::json!({
            ...
            "ts":1,
            "argv":["git","stash","pop"],
            "autter_family_reflog_start": {"refs/stash": 9}
        });
        let def_repo = serde_json::json!({
            "event":"def_repo",
            "sid":"stash-late-meta",
            "ts":2,
            "worktree":repo,
            "autter_stash_target_oid":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        });
        let exit = serde_json::json!({
            "event":"exit",
            "sid":"stash-late-meta",
            ...
            "code":0,
            "autter_family_reflog_start": {"refs/stash": 9},
            "autter_family_reflog_end": {"refs/stash": 9}
        });
    ...
    fn destructive_stash_can_normalize_without_pre_command_target_oid() {
        ...
        let start = serde_json::json!({
            ...
            "argv":["git","stash","pop"],
            "worktree":repo,
            "autter_family_reflog_start": {"refs/stash": 11}
        });
        let exit = serde_json::json!({
            "event":"exit",
            "sid":"stash-missing-meta",
            "ts":2,
            "code":0,
            "autter_family_reflog_end": {"refs/stash": 11}
        });
