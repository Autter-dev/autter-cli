...
impl<B: GitBackend> Coordinator<B> {
    ...
    ) -> Result<AppliedCommand, AutterError> {
        match &cmd.scope {
    ...
    pub async fn apply_checkpoint(&self, repo_working_dir: &Path) -> Result<ApplyAck, AutterError> {
    ...
    ) -> Result<WatermarkState, AutterError> {
        let family = self.backend.resolve_family(repo_working_dir)?;
        let actor = self.get_or_create_family_actor(family).await;
...
mod tests {
    ...
    impl GitBackend for MockBackend {
        ...
        fn repo_context(&self, _worktree: &Path) -> Result<RepoContext, AutterError> {
            Err(AutterError::Generic("unused".to_string()))
        }
