    ...
    let file_content_hashes = smol::block_on(async {
        ...
        let futures = files.iter().map(|file_path| {
            ...
            async move {
                ...
                Ok::<(String, String), AutterError>((file_path, sha))
            }
        });

        // Collect results from all concurrent operations
        let results: Vec<Result<(String, String), AutterError>> =
        ...
        Ok::<HashMap<String, String>, AutterError>(file_content_hashes)
    })?;
...
fn get_checkpoint_entry_for_file(
    ...
    ts: u128,
) -> Result<Option<(WorkingLogEntry, FileLineStats)>, AutterError> {
    ...
    // Preserve existing `autter checkpoint` behavior for human-only files by writing an
...
async fn get_checkpoint_entries(
    ...
    trace_id: String,
) -> Result<(Vec<WorkingLogEntry>, Vec<FileLineStats>), AutterError> {
...
) -> Result<(WorkingLogEntry, FileLineStats), AutterError> {
    let FileEntryInput {
        file_path,
        blob_sha,
        author_id,
...
) -> Result<crate::authorship::working_log::CheckpointLineStats, AutterError> {
