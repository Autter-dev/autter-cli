...
impl GlobalActorHandle {
    pub async fn apply(&self, cmd: NormalizedCommand) -> Result<AppliedCommand, AutterError> {
        ...
        self.tx
            ...
            .await
            .map_err(|_| AutterError::Generic("global actor apply send failed".to_string()))?;
        rx.await
            .map_err(|_| AutterError::Generic("global actor apply receive failed".to_string()))?
    }
    ...
    pub async fn shutdown(&self) -> Result<(), AutterError> {
        self.tx
            ...
            .await
            .map_err(|_| AutterError::Generic("global actor shutdown send failed".to_string()))
    }
}
