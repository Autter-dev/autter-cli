...
pub enum FamilyMsg {
    Apply(
        ...
        oneshot::Sender<Result<AppliedCommand, AutterError>>,
    ),
    ApplyCheckpoint(oneshot::Sender<Result<ApplyAck, AutterError>>),
    Status(oneshot::Sender<Result<FamilyStatus, AutterError>>),
    GetWatermarks(oneshot::Sender<Result<WatermarkState, AutterError>>),
...
impl FamilyActorHandle {
    pub async fn apply(&self, cmd: NormalizedCommand) -> Result<AppliedCommand, AutterError> {
        let (tx, rx) = oneshot::channel();
        self.tx
            .send(FamilyMsg::Apply(Box::new(cmd), tx))
            .await
            .map_err(|_| AutterError::Generic("family actor apply send failed".to_string()))?;
        rx.await
            .map_err(|_| AutterError::Generic("family actor apply receive failed".to_string()))?
    }

    pub async fn apply_checkpoint(&self) -> Result<ApplyAck, AutterError> {
        let (tx, rx) = oneshot::channel();
        self.tx
            .send(FamilyMsg::ApplyCheckpoint(tx))
            .await
            .map_err(|_| AutterError::Generic("family actor checkpoint send failed".to_string()))?;
        rx.await.map_err(|_| {
            AutterError::Generic("family actor checkpoint receive failed".to_string())
        })?
    }

    pub async fn status(&self) -> Result<FamilyStatus, AutterError> {
        let (tx, rx) = oneshot::channel();
        self.tx
            .send(FamilyMsg::Status(tx))
            .await
            .map_err(|_| AutterError::Generic("family actor status send failed".to_string()))?;
        rx.await
            .map_err(|_| AutterError::Generic("family actor status receive failed".to_string()))?
    }

    pub async fn watermarks(&self) -> Result<WatermarkState, AutterError> {
        let (tx, rx) = oneshot::channel();
        self.tx
            .send(FamilyMsg::GetWatermarks(tx))
            .await
            .map_err(|_| AutterError::Generic("family actor watermarks send failed".to_string()))?;
        rx.await.map_err(|_| {
            AutterError::Generic("family actor watermarks receive failed".to_string())
        })?
    }

    pub async fn update_watermarks(&self, update: WatermarkState) -> Result<(), AutterError> {
        self.tx
            .send(FamilyMsg::UpdateWatermarks(update))
            .await
            .map_err(|_| {
                AutterError::Generic("family actor update_watermarks send failed".to_string())
            })
    }

    pub async fn shutdown(&self) -> Result<(), AutterError> {
        self.tx
            .send(FamilyMsg::Shutdown)
            .await
            .map_err(|_| AutterError::Generic("family actor shutdown send failed".to_string()))
    }
}
