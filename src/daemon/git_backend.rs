...
pub trait GitBackend: Send + Sync + 'static {
    ...
    fn reflog_cut(&self, family: &FamilyKey) -> Result<ReflogCut, AutterError>;
...
impl SystemGitBackend {
    ...
    fn resolve_alias_cached(
        ...
        let cache = self
            .alias_cache
            .lock()
            .map_err(|_| AutterError::Generic("alias cache lock poisoned".to_string()))?;
...
fn rev_parse_head(worktree: &Path) -> Result<String, AutterError> {
...
fn run_git_allow_nonzero(args: &[&str]) -> Result<std::process::Output, AutterError> {
    let args_owned = args
        .iter()
...
fn run_git_str_allow_nonzero(args: &[&str]) -> Result<String, AutterError> {
...
fn git_error_for(args: &[&str], output: &std::process::Output) -> AutterError {
...
fn reflog_offsets(common_dir: &Path) -> Result<HashMap<String, u64>, AutterError> {
...
mod tests {
    ...
    fn init_target_resolves_dot_when_cwd_hint_is_provided() {
        ...
        let cwd = std::env::temp_dir().join("autter-test-my-repo");
        assert!(
            cwd.is_absolute(),
    ...
    fn builtin_primary_command_skips_repository_lookup() {
        let backend = SystemGitBackend::new();
        let missing_worktree = PathBuf::from("/definitely/missing/autter-backend-test");
