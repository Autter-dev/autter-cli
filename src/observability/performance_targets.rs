...
pub fn log_performance_for_checkpoint(
    ...
    // Output structured JSON for benchmarking (when AUTTER_DEBUG_PERFORMANCE >= 2)
    // For autter commands like checkpoint, there's no pre/post/git breakdown - just total time
    let perf_json = json!({
        "command": "checkpoint",
