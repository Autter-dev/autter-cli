    ...
    fn s3_migration_old_matcher_no_user_hooks() {
        ...
        for hook_type in &["BeforeTool", "AfterTool"] {
            ...
            assert_eq!(
                catch_all.len(),
                1,
                "{hook_type}: autter should be in catch-all"
            );
    ...
    fn s10_stale_command_upgraded() {
        ...
        fs::write(
            ...
            serde_json::to_string_pretty(&json!({
                "tools": {"enableHooks": true},
                "hooks": {
                    "BeforeTool": [{"matcher": "*", "hooks": [{"type":"command","command": "/old/autter checkpoint gemini"}]}],
                    "AfterTool": [{"matcher": "*", "hooks": [{"type":"command","command": "/old/autter checkpoint gemini"}]}]
                }
            }))
            .unwrap(),
        )
