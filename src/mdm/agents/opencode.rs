...
const OPENCODE_PLUGIN_CONTENT: &str = include_str!(concat!(
    ...
    "/agent-support/opencode/autter.ts"
));
...
impl OpenCodeInstaller {
    ...
    fn generate_plugin_content(binary_path: &Path) -> String {
        ...
        OPENCODE_PLUGIN_CONTENT.replace("__AUTTER_BINARY_PATH__", &path_str)
    }
}
...
mod tests {
    ...
    fn test_opencode_install_plugin_creates_file_from_scratch() {
        ...
        assert!(content.contains("AutterPlugin"));
        ...
        assert!(!content.contains("__AUTTER_BINARY_PATH__"));
        assert!(content.contains(r#"const AUTTER_BIN = "/usr/local/bin/autter""#));
    }
    ...
    fn test_opencode_plugin_content_is_valid_typescript() {
        ...
        assert!(content.contains("export const AutterPlugin: Plugin"));
        assert!(content.contains("export default AutterPlugin"));
    ...
    fn test_opencode_plugin_placeholder_substitution() {
        ...
        assert!(content.contains("spawn(AUTTER_BIN"));
    ...
    fn test_opencode_plugin_updates_outdated_content() {
        ...
        assert!(content_after.contains("AutterPlugin"));
        assert!(!content_after.contains("OldPlugin"));
    }

    #[test]
    fn test_opencode_plugin_windows_path_escaping() {
        let binary_path = PathBuf::from(r"C:\Users\foo\.autter\bin\autter.exe");
        let content = OpenCodeInstaller::generate_plugin_content(&binary_path);

        assert!(!content.contains("__AUTTER_BINARY_PATH__"));
        // Backslashes should be doubled for the TS string literal
        assert!(
            content.contains(r#"const AUTTER_BIN = "C:\\Users\\foo\\.autter\\bin\\autter.exe""#)
        );
    }
