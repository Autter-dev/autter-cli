...
use crate::mdm::utils::{
    ...
    is_autter_checkpoint_command, normalize_windows_path_for_shell, parse_version,
...
impl ClaudeCodeInstaller {
    ...
    fn install_hooks_at(
        ...
        for (hook_type, desired_cmd) in &[
            ...
            // Step 1: Strip autter from every non-catch-all matcher block (migration).
            ...
            for (i, block) in hook_type_array.iter_mut().enumerate() {
                    ...
                    && let Some(hooks) = block.get_mut("hooks").and_then(|h| h.as_array_mut())
                {
                    ...
                    hooks.retain(|hook| {
                        hook.get("command")
                            .and_then(|c| c.as_str())
                            .map(|cmd| !is_autter_checkpoint_command(cmd))
                            .unwrap_or(true)
                    });
            ...
            // Step 3: Ensure exactly one autter command in the catch-all block.
            ...
            for (idx, hook) in hooks_array.iter().enumerate() {
                if let Some(cmd) = hook.get("command").and_then(|c| c.as_str())
                    && is_autter_checkpoint_command(cmd)
                    && found_idx.is_none()
                {
                    found_idx = Some(idx);
                    if cmd != *desired_cmd {
            ...
            match found_idx {
                Some(idx) => {
                    ...
                    // Remove duplicates: keep the first, drop any subsequent autter entries.
                    let keep_idx = idx;
                    ...
                    hooks_array.retain(|hook| {
                        if current_idx == keep_idx {
                            ...
                            let is_dup = is_autter_checkpoint_command(cmd);
                            current_idx += 1;
                            !is_dup
                        } else {
                            current_idx += 1;
...
mod tests {
    ...
    fn autter_blocks_in(hook_type_array: &[Value]) -> Vec<&Value> {
        hook_type_array
            .iter()
            .filter(|block| {
                block
    ...
    fn s3_migration_old_matcher_no_user_hooks() {
        ...
        let settings = read_settings(&path);
        for hook_type in &["PreToolUse", "PostToolUse"] {
            // autter must be in the catch-all block
            let hooks = hooks_in_catch_all(&settings, hook_type);
            assert_eq!(hooks.len(), 1, "{hook_type}: expected autter in catch-all");
    ...
    fn s4_migration_old_matcher_user_hook_preserved() {
        ...
        for (hook_type, user_cmd) in &[
            ...
            assert!(
                ...
                "{hook_type}: autter should not be in old matcher block after migration"
            );
        }
    }
    ...
    fn s6_fresh_install_user_has_catch_all_hook() {
        ...
        for hook_type in &["PreToolUse", "PostToolUse"] {
            ...
            assert_eq!(
                catch_all.len(),
                2,
                "{hook_type}: should have user hook + autter"
            );
            assert_eq!(
                catch_all[0]
                    .get("command")
            ...
            assert!(is_autter_checkpoint_command(
                catch_all[1]
                    .get("command")
                    .and_then(|c| c.as_str())
                    .unwrap()
    ...
    #[test]
    fn s7_idempotent_user_catch_all_plus_autter() {
        let (_td, path) = setup_test_env();
        let cmd = expected_cmd();
        let before = json!({
            "hooks": {
    ...
    #[test]
    fn s8_deduplication_autter_in_both_catch_all_and_old_matcher() {
        let (_td, path) = setup_test_env();
        let cmd = expected_cmd();
        ...
        let settings = read_settings(&path);
        for hook_type in &["PreToolUse", "PostToolUse"] {
            // exactly one autter in catch-all
            let catch_all = hooks_in_catch_all(&settings, hook_type);
            assert_eq!(catch_all.len(), 1);

            // old matcher block has user hook but NOT autter
            let blocks = settings
                .get("hooks")
                .and_then(|h| h.get(*hook_type))
                .and_then(|v| v.as_array())
    ...
    #[test]
    fn s9_deduplication_two_autter_in_catch_all_block() {
        let (_td, path) = setup_test_env();
        let cmd = expected_cmd();
        fs::write(
            &path,
    ...
    fn s10_stale_command_upgraded_in_catch_all() {
        ...
        fs::write(
            ...
            serde_json::to_string_pretty(&json!({
                "hooks": {
                    "PreToolUse": [{"matcher": "*", "hooks": [{"type":"command","command": "/old/path/autter checkpoint claude --hook-input stdin"}]}],
                    "PostToolUse": [{"matcher": "*", "hooks": [{"type":"command","command": "/old/path/autter checkpoint claude --hook-input stdin"}]}]
                }
            }))
            .unwrap(),
        )
    ...
    #[test]
    fn s11_autter_in_arbitrary_old_matcher_migrated() {
        let (_td, path) = setup_test_env();
        let cmd = expected_cmd();
        fs::write(
            &path,
        ...
        let settings = read_settings(&path);
        for hook_type in &["PreToolUse", "PostToolUse"] {
            // autter now in catch-all
            let catch_all = hooks_in_catch_all(&settings, hook_type);
            assert_eq!(catch_all.len(), 1);

            // user-bash-hook preserved in Bash block, autter removed
            let blocks = settings
                .get("hooks")
                .and_then(|h| h.get(*hook_type))
                .and_then(|v| v.as_array())
    ...
    #[test]
    fn s12_autter_spread_across_multiple_old_blocks() {
        let (_td, path) = setup_test_env();
        let cmd = expected_cmd();
        fs::write(
            &path,
        ...
        let settings = read_settings(&path);
        for hook_type in &["PreToolUse", "PostToolUse"] {
            // exactly one autter total, in catch-all
            let all_blocks = settings
                .get("hooks")
                .and_then(|h| h.get(*hook_type))
                .and_then(|v| v.as_array())
                .unwrap();
            let autter_blocks = autter_blocks_in(all_blocks);
            assert_eq!(autter_blocks.len(), 1);
            assert_eq!(
                autter_blocks[0]
                    .get("matcher")
                    .and_then(|m| m.as_str())
    ...
    fn s13_hook_types_handled_independently() {
        let (_td, path) = setup_test_env();
        let cmd = expected_cmd();
        // PreToolUse: autter on old matcher; PostToolUse: autter already on catch-all
        fs::write(
            &path,
            serde_json::to_string_pretty(&json!({
                "hooks": {
    ...
    fn u1_uninstall_from_catch_all() {
        ...
        for hook_type in &["PreToolUse", "PostToolUse"] {
            ...
            assert!(
                ...
                "{hook_type}: autter should be removed"
            );
        }
    }
    ...
    #[test]
    fn u4_noop_uninstall_when_no_autter() {
        let (_td, path) = setup_test_env();
        fs::write(
            &path,
            serde_json::to_string_pretty(&json!({
    ...
    #[test]
    fn c3_autter_only_in_old_matcher_returns_installed_but_not_up_to_date() {
        let cmd = expected_cmd();
        let settings = json!({
            "hooks": {
                "PreToolUse": [{"matcher": "Write|Edit|MultiEdit", "hooks": [{"type":"command","command": cmd}]}]
    ...
    fn test_claude_hook_commands_no_windows_extended_path_prefix() {
        let raw_path = PathBuf::from(r"\\?\C:\Users\USERNAME\.autter\bin\autter.exe");
    ...
    fn test_claude_hook_commands_use_forward_slash_path_on_windows() {
        let binary_path = PathBuf::from(r"C:\Users\Administrator\.autter\bin\autter.exe");
        let binary_path_str = normalize_windows_path_for_shell(&binary_path);
        let pre_tool_cmd = format!("{} {}", binary_path_str, CLAUDE_PRE_TOOL_CMD);
        let post_tool_cmd = format!("{} {}", binary_path_str, CLAUDE_POST_TOOL_CMD);
        ...
        assert_eq!(
            pre_tool_cmd,
            "C:/Users/Administrator/.autter/bin/autter.exe checkpoint claude --hook-input stdin",
    ...
    fn test_claude_hook_commands_preserve_unix_path() {
        let binary_path = PathBuf::from("/usr/local/bin/autter");
        let binary_path_str = normalize_windows_path_for_shell(&binary_path);
        let pre_tool_cmd = format!("{} {}", binary_path_str, CLAUDE_PRE_TOOL_CMD);

        assert_eq!(
            pre_tool_cmd, "/usr/local/bin/autter checkpoint claude --hook-input stdin",
