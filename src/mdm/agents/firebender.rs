...
impl FirebenderInstaller {
    ...
    fn is_firebender_checkpoint_command(cmd: &str) -> bool {
        cmd.contains("checkpoint firebender")
            && (cmd.contains("autter") || cmd.ends_with(FIREBENDER_CHECKPOINT_CMD))
    }
}
...
mod tests {
    ...
    fn test_firebender_command_detection() {
        assert!(FirebenderInstaller::is_firebender_checkpoint_command(
            "/usr/local/bin/autter checkpoint firebender --hook-input stdin"
        ));
        assert!(FirebenderInstaller::is_firebender_checkpoint_command(
            "autter something checkpoint firebender"
        ));
        assert!(!FirebenderInstaller::is_firebender_checkpoint_command(
            "autter checkpoint cursor"
        ));
    }
    ...
    fn test_install_hooks_updates_existing_firebender_command() {
        with_temp_home(|home| {
            ...
            let existing = json!({
                "version": 1,
                "hooks": {
                    "preToolUse": [{ "command": "/old/path/autter checkpoint firebender --hook-input stdin" }],
                    "postToolUse": [{ "command": "/old/path/autter checkpoint firebender --hook-input stdin" }]
                }
            });
            fs::write(
                &hooks_path,
    ...
    fn test_uninstall_hooks_removes_only_firebender_entries() {
        with_temp_home(|home| {
            ...
            let existing = json!({
                ...
                "hooks": {
                    "preToolUse": [
                        { "command": "/usr/local/bin/autter checkpoint firebender --hook-input stdin" },
    ...
    fn test_check_hooks_not_up_to_date_when_matcher_present() {
        with_temp_home(|home| {
            ...
            let existing = json!({
                "version": 1,
                "hooks": {
                    "preToolUse": [{ "matcher": "Write|Edit|Delete", "command": "/usr/local/bin/autter checkpoint firebender --hook-input stdin" }],
                    "postToolUse": [{ "matcher": "Write|Edit|Delete", "command": "/usr/local/bin/autter checkpoint firebender --hook-input stdin" }]
                }
            });
            fs::write(
                &hooks_path,
    ...
    fn test_install_hooks_removes_matcher_from_existing_entry() {
        with_temp_home(|home| {
            ...
            let existing = json!({
                "version": 1,
                "hooks": {
                    "preToolUse": [{ "matcher": "Write|Edit|Delete", "command": "/old/path/autter checkpoint firebender --hook-input stdin" }],
                    "postToolUse": [{ "matcher": "Write|Edit|Delete", "command": "/old/path/autter checkpoint firebender --hook-input stdin" }]
                }
            });
            fs::write(
                &hooks_path,
