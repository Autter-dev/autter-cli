...
use crate::mdm::utils::{
    MIN_CODE_VERSION, get_editor_version, home_dir,
    install_vsc_editor_extension_with_vsix_fallback, is_github_codespaces,
...
impl HookInstaller for VSCodeInstaller {
    ...
    fn install_extras(
        ...
        if is_github_codespaces() {
            results.push(InstallResult {
                changed: false,
                diff: None,
                message: "VS Code: Unable to install extension in GitHub Codespaces. Add to your devcontainer.json: \"customizations\": { \"vscode\": { \"extensions\": [\"autter.autter-vscode\"] } }".to_string(),
            });
            return Ok(results);
        }
                        ...
                        match install_vsc_editor_extension_with_vsix_fallback(
                            ...
                            Err(e) => {
                                ...
                                results.push(InstallResult {
                                    changed: false,
                                    diff: None,
                                    message: "VS Code: Unable to automatically install extension. Please cmd+click on the following link to install: vscode:extension/autter.autter-vscode (or navigate to https://marketplace.visualstudio.com/items?itemName=autter.autter-vscode in your browser)".to_string(),
                                });
                            }
                        }
                    }
    ...
    fn uninstall_extras(
        ...
        _params: &HookInstallerParams,
        _dry_run: bool,
    ) -> Result<Vec<UninstallResult>, AutterError> {
...
mod tests {
    ...
    fn test_vscode_uninstall_extras_returns_manual_message() {
        ...
        let params = HookInstallerParams {
            binary_path: std::path::PathBuf::from("/usr/local/bin/autter"),
        };
