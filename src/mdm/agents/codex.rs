...
use crate::mdm::utils::{
    binary_exists, generate_diff, home_dir, is_autter_checkpoint_command, write_atomic,
};
...
impl CodexInstaller {
    ...
    fn parse_config_toml(content: &str) -> Result<TomlValue, AutterError> {
        if content.trim().is_empty() {
            return Ok(TomlValue::Table(Map::new()));
        }

        let parsed: TomlValue = toml::from_str(content)
            .map_err(|e| AutterError::Generic(format!("Failed to parse Codex config.toml: {e}")))?;
    ...
    fn parse_hooks_json(content: &str) -> Result<JsonValue, AutterError> {
        if content.trim().is_empty() {
            return Ok(json!({}));
        }
    ...
    fn is_autter_codex_command(cmd: &str) -> bool {
        is_autter_checkpoint_command(cmd) && cmd.contains("checkpoint codex")
    }

    fn is_autter_codex_notify_args(args: &[String]) -> bool {
        if args.len() < 4 {
            return false;
        }

        let has_autter_bin = args
            .first()
            .map(|bin| {
                bin == "autter"
                    || bin.ends_with("/autter")
                    || bin.ends_with("\\autter")
                    || bin.ends_with("/autter.exe")
                    || bin.ends_with("\\autter.exe")
            })
            .unwrap_or(false);
        ...
        has_autter_bin && has_checkpoint_codex && has_hook_input
    }
    ...
    fn compute_trust_hash(event_name_snake: &str, command: &str) -> Result<String, AutterError> {
        ...
        let bytes = serde_json::to_vec(&canonical).map_err(|e| {
            AutterError::Generic(format!("Failed to serialize JSON for trust hash: {e}"))
        })?;
                                ...
                                .map(|hooks| {
                                    hooks.iter().any(|hook| {
                                        hook.get("command")
                                            ...
                                            .map(Self::is_autter_codex_command)
                                            .unwrap_or(false)
                                    })
                                })
                                .unwrap_or(false)
    ...
    fn config_with_hooks_feature_enabled(config: &TomlValue) -> Result<TomlValue, AutterError> {
        let mut merged = Self::remove_notify_if_autter(config)?.unwrap_or(config.clone());
        let root = merged
            .as_table_mut()
            .ok_or_else(|| AutterError::Generic("Codex config root must be a table".to_string()))?;
    ...
    fn config_with_installed_hooks(
        ...
        let hooks_obj = hooks_table.as_table_mut().ok_or_else(|| {
            AutterError::Generic("Codex config hooks field must be a table".to_string())
        })?;
            ...
            for block in blocks {
                ...
                let cleaned_hooks = cleaned_block
                    ...
                    .filter(|hook| {
                        hook.get("command")
                            .and_then(|value| value.as_str())
                            .map(|cmd| !Self::is_autter_codex_command(cmd))
                            .unwrap_or(true)
                    })
                    .collect::<Vec<_>>();
        ...
        let state_obj = state_table.as_table_mut().ok_or_else(|| {
            AutterError::Generic("Codex config hooks.state must be a table".to_string())
        })?;
    ...
    fn remove_notify_if_autter(config: &TomlValue) -> Result<Option<TomlValue>, AutterError> {
        let Some(notify_args) = Self::notify_args_from_config(config) else {
            return Ok(None);
        };

        if !Self::is_autter_codex_notify_args(&notify_args) {
            return Ok(None);
        }
    ...
    fn remove_inline_hooks_from_config(
        config: &TomlValue,
    ) -> Result<(TomlValue, bool), AutterError> {
        let mut merged = config.clone();
        let root = merged
            .as_table_mut()
            .ok_or_else(|| AutterError::Generic("Codex config root must be a table".to_string()))?;
        ...
        // Remove trust state entries for autter hooks
        let config_path_str = Self::config_path().to_string_lossy().to_string();
    ...
    fn remove_feature_flags(config: &TomlValue) -> Result<TomlValue, AutterError> {
        let mut merged = config.clone();
        let root = merged
            .as_table_mut()
            .ok_or_else(|| AutterError::Generic("Codex config root must be a table".to_string()))?;
    ...
    fn remove_codex_hooks_from_json(
    ...
    ) -> Result<(JsonValue, bool), AutterError> {
        let mut merged = hooks_json.clone();
        let root = merged.as_object_mut().ok_or_else(|| {
            AutterError::Generic("Codex hooks.json root must be a JSON object".to_string())
        })?;
        ...
        let hooks_obj = hooks_entry.as_object_mut().ok_or_else(|| {
            AutterError::Generic("Codex hooks field must be a JSON object".to_string())
        })?;
    ...
    fn hooks_json_with_installed_hooks(
        ...
        for event_name in CODEX_HOOK_EVENTS {
            ...
            let block_obj = blocks[target_idx].as_object_mut().ok_or_else(|| {
                AutterError::Generic("Codex hooks.json hook block must be an object".to_string())
            })?;
            ...
            let hooks_array = hooks.as_array_mut().ok_or_else(|| {
                AutterError::Generic("Codex hooks.json hooks entry must be an array".to_string())
            })?;
    ...
    fn hooks_json_has_autter_entries(hooks_json: &JsonValue) -> bool {
...
impl HookInstaller for CodexInstaller {
    ...
    fn check_hooks(&self, params: &HookInstallerParams) -> Result<HookCheckResult, AutterError> {
        ...
        if Config::fresh().codex_hooks_format() == CodexHooksFormat::HooksJson {
            let config_without_notify =
                Self::remove_notify_if_autter(&config)?.unwrap_or(config.clone());
            ...
            let has_json_hooks = Self::hooks_json_has_autter_entries(&hooks_json);
        ...
        let has_legacy_hooks_json = Self::hooks_json_has_autter_entries(&hooks_json);
    ...
    fn install_hooks(
        ...
        if Config::fresh().codex_hooks_format() == CodexHooksFormat::HooksJson {
            let config_without_notify =
                Self::remove_notify_if_autter(&existing_config)?.unwrap_or(existing_config.clone());
            let (config_without_inline_hooks, _) =
                Self::remove_inline_hooks_from_config(&config_without_notify)?;
            let merged_config =
                Self::config_with_hooks_feature_enabled(&config_without_inline_hooks)?;
            ...
            if config_changed {
                let new_config_content = toml::to_string_pretty(&merged_config).map_err(|e| {
                    AutterError::Generic(format!("Failed to serialize Codex config.toml: {e}"))
                })?;
...
mod tests {
    ...
    fn test_binary_path() -> PathBuf {
        PathBuf::from("/usr/local/bin/autter")
    }
    ...
    fn write_autter_config(home: &Path, codex_hooks_format: &str) {
        let autter_dir = home.join(".autter");
        fs::create_dir_all(&autter_dir).unwrap();
        fs::write(
    ...
    #[test]
    fn test_is_autter_codex_notify_args_true_for_absolute_binary() {
        let args = vec![
            "/usr/local/bin/autter".to_string(),
            "checkpoint".to_string(),
            "codex".to_string(),
            "--hook-input".to_string(),
        ];

        assert!(CodexInstaller::is_autter_codex_notify_args(&args));
    }

    #[test]
    fn test_is_autter_codex_notify_args_true_for_legacy_via_codex_notify_args() {
        let args = vec![
            "/Users/svarlamov/.autter/bin/autter".to_string(),
            "checkpoint".to_string(),
            "codex".to_string(),
    ...
    #[test]
    fn test_is_autter_codex_notify_args_false_for_non_autter_command() {
        let args = vec![
    ...
    #[test]
    fn test_remove_notify_if_autter_removes_only_autter_notify() {
        let config = CodexInstaller::parse_config_toml(
            r#"
model = "gpt-5"
notify = ["/usr/local/bin/autter", "checkpoint", "codex", "--hook-input"]
"#,
        )
        .unwrap();

        let merged = CodexInstaller::remove_notify_if_autter(&config)
            .unwrap()
    ...
    #[test]
    fn test_remove_notify_if_autter_removes_legacy_via_codex_notify_args() {
        let config = CodexInstaller::parse_config_toml(
            r#"
model = "gpt-5"
notify = ["/Users/svarlamov/.autter/bin/autter", "checkpoint", "codex", "--via-codex-notify", "--hook-input", "stdin"]
"#,
        )
        .unwrap();
        ...
        let merged = CodexInstaller::remove_notify_if_autter(&config)
            .unwrap()
            .expect("legacy autter notify should be removed");
        assert!(merged.get("notify").is_none());
        assert_eq!(merged.get("model").and_then(|v| v.as_str()), Some("gpt-5"));
    }

    #[test]
    fn test_remove_notify_if_autter_preserves_custom_notify() {
        let config = CodexInstaller::parse_config_toml(
            r#"
model = "gpt-5"
...
"#,
        ...
        let merged = CodexInstaller::remove_notify_if_autter(&config).unwrap();
        assert!(
            merged.is_none(),
    ...
    fn test_config_with_installed_hooks_adds_inline_hooks_for_all_events() {
        ...
        for event_name in CODEX_HOOK_EVENTS {
            ...
            assert!(
                ...
                "expected unscoped autter block for {event_name}"
            );
        }
    }
...
[[hooks.PreToolUse.hooks]]
type = "command"
command = "/usr/local/bin/autter checkpoint codex --hook-input stdin"
...
"#,
    ...
    #[test]
    fn test_remove_inline_hooks_preserves_non_autter_hooks() {
        let config = CodexInstaller::parse_config_toml(
            r#"
model = "gpt-5"
...
"#,
    ...
    #[test]
    fn test_remove_codex_hooks_from_json_removes_only_autter_entries() {
        let existing = json!({
            "hooks": {
                "PreToolUse": [
                    {
                        "hooks": [
                            { "type": "command", "command": "/usr/local/bin/autter checkpoint codex --hook-input stdin" },
...
"#,
            ...
            assert!(
                ...
                "autter notify should be removed during migration"
            );
            assert_eq!(
                parsed
                    .get("features")
...
"#,
            ...
            assert!(
                CodexInstaller::notify_args_from_config(&parsed).is_none(),
                "legacy autter notify should be removed during migration"
            );
        });
    }
...
"#,
            ...
            assert_eq!(
                ...
                "non-autter notify must be preserved"
            );
            assert_eq!(
                parsed
                    .get("features")
...
"#,
                ...
                serde_json::to_string_pretty(&json!({
                    "hooks": {
                        "PreToolUse": [
                            {
                                "hooks": [
                                    { "type": "command", "command": "/old/autter checkpoint codex --hook-input stdin" },
                                ]
                            }
                        ],
                        "PostToolUse": [
    ...
    #[test]
    #[serial]
    fn test_install_hooks_migrates_hooks_json_preserves_non_autter_entries() {
        with_temp_home(|home| {
            let codex_dir = home.join(".codex");
            fs::create_dir_all(&codex_dir).unwrap();
            let config_path = codex_dir.join("config.toml");
            ...
            assert!(
                hooks_json_path.exists(),
                "hooks.json should be preserved when non-autter entries remain"
            );
            ...
            assert!(
                ...
                "non-autter hooks should be preserved in hooks.json"
            );
            assert!(
                !pre_blocks.iter().any(|block| {
                    block["hooks"].as_array().is_some_and(|hooks| {
                        hooks.iter().any(|hook| {
                            hook["command"]
                                .as_str()
                                .map(CodexInstaller::is_autter_codex_command)
                                .unwrap_or(false)
                        })
                    })
                }),
                "autter hooks should be removed from hooks.json"
            );
        });
    }
    ...
    fn test_install_hooks_prefers_hooks_json_when_configured() {
        with_temp_home(|home| {
            let codex_dir = home.join(".codex");
            write_autter_config(home, "hooks_json");
            fs::create_dir_all(&codex_dir).unwrap();
            let config_path = codex_dir.join("config.toml");
            let hooks_json_path = codex_dir.join("hooks.json");
            fs::write(&config_path, "model = \"gpt-5\"\n").unwrap();
            ...
            assert!(
                !CodexInstaller::config_has_inline_hooks(&parsed),
                "configured hooks_json format should not install autter inline hooks"
            );

            let hooks_content = fs::read_to_string(&hooks_json_path).unwrap();
            let hooks_json: serde_json::Value = serde_json::from_str(&hooks_content).unwrap();
            assert!(
                CodexInstaller::hooks_json_has_autter_entries(&hooks_json),
                "autter hooks should be installed into hooks.json"
            );
            let pre_blocks = hooks_json["hooks"]["PreToolUse"].as_array().unwrap();
            assert!(
                pre_blocks.iter().any(|block| {
    ...
    #[test]
    #[serial]
    fn test_install_hooks_prefers_hooks_json_removes_existing_inline_autter_hooks() {
        with_temp_home(|home| {
            let codex_dir = home.join(".codex");
            write_autter_config(home, "hooks_json");
            fs::create_dir_all(&codex_dir).unwrap();
...
"#,
            ...
            assert!(
                !CodexInstaller::config_has_inline_hooks(&parsed),
                "old autter inline hooks should be removed in hooks_json mode"
            );
            ...
            assert!(
                ...
                "non-autter inline hooks should be preserved"
            );
...
"#,
            ...
            fs::write(
                ...
                serde_json::to_string_pretty(&json!({
                    "hooks": {
                        "PreToolUse": [{ "hooks": [{ "type": "command", "command": "/usr/local/bin/autter checkpoint codex --hook-input stdin" }] }],
                        "PostToolUse": [{ "hooks": [{ "type": "command", "command": "/usr/local/bin/autter checkpoint codex --hook-input stdin" }] }],
                        "Stop": [{ "hooks": [{ "type": "command", "command": "/usr/local/bin/autter checkpoint codex --hook-input stdin" }] }],
                    }
                }))
                .unwrap(),
            )
            ...
            assert!(
                !hooks_json_path.exists(),
                "hooks.json should be removed when only autter entries existed"
            );
        });
    }
...
"#,
    ...
    fn test_compute_trust_hash_deterministic() {
        let hash1 = CodexInstaller::compute_trust_hash(
            "pre_tool_use",
            "/usr/local/bin/autter checkpoint codex --hook-input stdin",
        )
        .unwrap();
    ...
    fn test_compute_trust_hash_differs_by_command() {
        ...
        let hash2 = CodexInstaller::compute_trust_hash(
            "pre_tool_use",
            "/opt/bin/autter checkpoint codex --hook-input stdin",
        )
        .unwrap();
...
"#,
            ...
            assert!(
                ...
                "non-autter state entry should be preserved"
            );
            ...
            assert!(
                ...
                "autter state entry should be added"
            );
        });
    }
