...
impl CursorInstaller {
    ...
    fn is_cursor_checkpoint_command(cmd: &str) -> bool {
        cmd.contains("autter checkpoint cursor")
            || (cmd.contains("autter") && cmd.contains("checkpoint") && cmd.contains("cursor"))
    }
}
...
impl HookInstaller for CursorInstaller {
    ...
    fn install_hooks(
        ...
        for hook_name in &["preToolUse", "postToolUse"] {
            ...
            // Update outdated autter checkpoint commands (or add if missing)
            for desired_hook in desired_hooks {
                ...
                // Look for existing autter checkpoint cursor commands
    ...
    fn uninstall_hooks(
        ...
        // Remove autter checkpoint cursor commands from both hook types
        for hook_name in &["preToolUse", "postToolUse"] {
    ...
    fn install_extras(
        ...
        _params: &HookInstallerParams,
        dry_run: bool,
    ) -> Result<Vec<InstallResult>, AutterError> {
        let mut results = Vec::new();

        // Install VS Code extension
        if let Some(cli) = resolve_editor_cli("cursor") {
            match is_vsc_editor_extension_installed(&cli, "autter.autter-vscode") {
                Ok(true) => {
...
mod tests {
    ...
    fn test_install_hooks_creates_file_from_scratch() {
        ...
        let autter_cmd = format!("{} {}", binary_path.display(), CURSOR_PRE_TOOL_USE_CMD);
        ...
        assert!(
            pre_tool_use[0]
                ...
                .as_str()
                .unwrap()
                .contains("autter checkpoint cursor")
        );
    }
    ...
    fn test_install_hooks_preserves_existing_hooks() {
        ...
        for hook_name in &["preToolUse", "postToolUse"] {
            ...
            let mut hooks_array = hooks_obj
                ...
                .unwrap()
                .clone();
            hooks_array.push(json!({"command": autter_cmd.clone()}));
            hooks_obj
                .as_object_mut()
                .unwrap()
    ...
    fn test_install_hooks_updates_outdated_command() {
        ...
        let existing = json!({
            ...
            "hooks": {
                "preToolUse": [
                    {
                        "command": "autter checkpoint cursor 2>/dev/null || true"
                    }
                ],
                "postToolUse": [
                    {
                        "command": "/old/path/autter checkpoint cursor"
                    }
                ]
            }
        });
        ...
        for hook_name in &["preToolUse", "postToolUse"] {
            ...
            for hook in hooks_array.iter_mut() {
                ...
                {
                    *hook = json!({"command": autter_cmd.clone()});
                }
            }
