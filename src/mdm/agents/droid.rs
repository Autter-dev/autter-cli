...
use crate::mdm::utils::{generate_diff, home_dir, is_autter_checkpoint_command, write_atomic};
...
mod tests {
    ...
    fn s3_migration_old_matcher_no_user_hooks() {
        ...
        for hook_type in &["PreToolUse", "PostToolUse"] {
            let hooks = hooks_in_catch_all(&settings, hook_type);
            assert_eq!(hooks.len(), 1, "{hook_type}: autter should be in catch-all");
    ...
    #[test]
    fn s8_deduplication_autter_in_both_blocks() {
        let (_td, path) = setup_test_env();
        let cmd = expected_cmd();
        fs::write(
            &path,
    ...
    #[test]
    fn s9_deduplication_two_autter_in_catch_all() {
        let (_td, path) = setup_test_env();
        let cmd = expected_cmd();
        fs::write(
            &path,
    ...
    fn s10_stale_command_upgraded() {
        ...
        fs::write(
            ...
            serde_json::to_string_pretty(&json!({
                "hooks": {
                    "PreToolUse": [{"matcher": "*", "hooks": [{"type":"command","command": "/old/autter checkpoint droid"}]}],
                    "PostToolUse": [{"matcher": "*", "hooks": [{"type":"command","command": "/old/autter checkpoint droid"}]}],
                    "claudeHooksImported": true
                }
            }))
            .unwrap(),
    ...
    #[test]
    fn c3_autter_only_in_old_matcher_not_up_to_date() {
        let cmd = expected_cmd();
