...
/// Check if the Autter plugin is installed for a detected IDE
...
mod tests {
    ...
    fn test_is_plugin_installed_detects_legacy_extracted_directory() {
        ...
        std::fs::create_dir_all(temp.path().join("autter-intellij")).unwrap();
    ...
    fn test_is_plugin_installed_detects_marketplace_directory_name() {
        let temp = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(temp.path().join("Autter")).unwrap();
