...
pub fn download_plugin_from_marketplace(
    ...
    let response = crate::http::send(request)
        .map_err(|e| AutterError::Generic(format!("Failed to download plugin: {}", e)))?;
...
pub fn install_plugin_to_directory(zip_data: &[u8], plugin_dir: &Path) -> Result<(), AutterError> {
    use zip::ZipArchive;
    ...
    let mut archive = ZipArchive::new(cursor)
        .map_err(|e| AutterError::Generic(format!("Failed to read plugin ZIP: {}", e)))?;

    for i in 0..archive.len() {
        let mut file = archive
            .by_index(i)
            .map_err(|e| AutterError::Generic(format!("Failed to read ZIP entry: {}", e)))?;
        ...
        if file.name().ends_with('/') {
            ...
            file.read_to_end(&mut buffer).map_err(|e| {
                AutterError::Generic(format!("Failed to read ZIP file contents: {}", e))
            })?;

            std::io::Write::write_all(&mut outfile, &buffer).map_err(|e| {
                AutterError::Generic(format!("Failed to write file {}: {}", outpath.display(), e))
            })?;
...
pub fn install_plugin_via_cli(binary_path: &Path, plugin_id: &str) -> Result<bool, AutterError> {
