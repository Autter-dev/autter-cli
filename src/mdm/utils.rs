...
pub fn get_binary_version(binary: &str) -> Result<String, AutterError> {
    let output = Command::new(binary)
        .arg("--version")
        .output()
        .map_err(|e| AutterError::Generic(format!("Failed to run {} --version: {}", binary, e)))?;
...
pub fn get_editor_version(cli: &EditorCliCommand) -> Result<String, AutterError> {
    let output = cli.command(&["--version"]).output().map_err(|e| {
        AutterError::Generic(format!("Failed to run {} --version: {}", cli.program, e))
    })?;
...
pub fn write_atomic(path: &Path, data: &[u8]) -> Result<(), AutterError> {
...
pub fn ensure_parent_dir(path: &Path) -> Result<(), AutterError> {
...
/// Check if a command is a autter checkpoint command
pub fn is_autter_checkpoint_command(cmd: &str) -> bool {
    // Must contain "autter" and "checkpoint"
    cmd.contains("autter") && cmd.contains("checkpoint")
}
...
pub fn is_vsc_editor_extension_installed(
    ...
    Err(AutterError::Generic(last_error_message.unwrap_or_else(
...
/// e.g. `C:\Users\Administrator\.autter\bin\autter.exe` → `C:/Users/Administrator/.autter/bin/autter.exe`
...
pub fn get_current_binary_path() -> Result<PathBuf, AutterError> {
...
mod tests {
    ...
    #[test]
    fn test_is_autter_checkpoint_command() {
        assert!(is_autter_checkpoint_command("autter checkpoint"));
        assert!(is_autter_checkpoint_command(
            "autter checkpoint claude --hook-input stdin"
        ));
        assert!(is_autter_checkpoint_command("autter checkpoint claude"));
        assert!(is_autter_checkpoint_command(
            "autter checkpoint --hook-input"
        ));
        assert!(is_autter_checkpoint_command(
            "autter checkpoint claude --hook-input \"$(cat)\""
        ));
        assert!(is_autter_checkpoint_command("autter checkpoint gemini"));
        assert!(is_autter_checkpoint_command(
            "autter checkpoint gemini --hook-input stdin"
        ));

        // Non-matching commands
        assert!(!is_autter_checkpoint_command("echo hello"));
        assert!(!is_autter_checkpoint_command("git status"));
        assert!(!is_autter_checkpoint_command("checkpoint"));
        assert!(!is_autter_checkpoint_command("autter"));
    }
...
"#;
    ...
    fn test_normalize_windows_path_for_shell_converts_windows_path() {
        ...
        let path = PathBuf::from(r"C:\Users\Administrator\.autter\bin\autter.exe");
        let result = normalize_windows_path_for_shell(&path);
        assert_eq!(
            result, "C:/Users/Administrator/.autter/bin/autter.exe",
    ...
    fn test_normalize_windows_path_for_shell_preserves_unix_path() {
        let path = PathBuf::from("/usr/local/bin/autter");
        let result = normalize_windows_path_for_shell(&path);
        assert_eq!(
            result, "/usr/local/bin/autter",
    ...
    fn test_normalize_windows_path_for_shell_handles_extended_prefix_after_clean() {
        ...
        let raw = PathBuf::from(r"\\?\C:\Users\USERNAME\.autter\bin\autter.exe");
        let cleaned = clean_path(raw);
        let result = normalize_windows_path_for_shell(&cleaned);
        assert_eq!(
            result, "C:/Users/USERNAME/.autter/bin/autter.exe",
    ...
    fn test_clean_path_strips_windows_prefix() {
        let path = PathBuf::from(r"\\?\C:\Users\test\.autter\bin\autter.exe");
