...
const EMBEDDED_SKILLS: &[EmbeddedSkill] = &[
    ...
    EmbeddedSkill {
        name: "autter-search",
        skill_md: include_str!("../../skills/autter-search/SKILL.md"),
    },
    EmbeddedSkill {
        name: "ask",
...
fn link_skill_dir(target: &PathBuf, link_path: &PathBuf) -> Result<(), AutterError> {
...
#[cfg(windows)]
fn copy_dir_recursive(src: &PathBuf, dst: &PathBuf) -> Result<(), AutterError> {
...
fn remove_skill_link(link_path: &PathBuf) -> Result<(), AutterError> {
...
/// Install all embedded skills to ~/.autter/skills/
...
pub fn install_skills(
...
) -> Result<SkillsInstallResult, AutterError> {
    let skills_base = skills_dir_path().ok_or_else(|| {
        AutterError::Generic("Could not determine skills directory path".to_string())
    })?;
    ...
    for skill in EMBEDDED_SKILLS {
        // Create skill directory: ~/.autter/skills/{skill-name}/
        ...
        // ~/.agents/skills/{skill-name} -> ~/.autter/skills/{skill-name}
        ...
        // ~/.claude/skills/{skill-name} -> ~/.autter/skills/{skill-name}
        ...
        // ~/.cursor/skills/{skill-name} -> ~/.autter/skills/{skill-name}
...
/// Uninstall all skills by removing ~/.autter/skills/ and linked skill directories
pub fn uninstall_skills(dry_run: bool, _verbose: bool) -> Result<SkillsInstallResult, AutterError> {
    let skills_base = skills_dir_path().ok_or_else(|| {
        AutterError::Generic("Could not determine skills directory path".to_string())
    })?;
...
mod tests {
    ...
    #[test]
    fn test_skills_dir_path_is_under_autter() {
        if let Some(path) = skills_dir_path() {
            assert!(path.ends_with("skills"));
            let parent = path.parent().unwrap();
            assert!(parent.ends_with(".autter"));
        }
    }
