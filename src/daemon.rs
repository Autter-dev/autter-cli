...
const TRACE_INGEST_SEQ_FIELD: &str = "autter_ingest_seq";
...
impl DaemonConfig {
    fn from_internal_dir(internal_dir: PathBuf) -> Self {
        ...
        let (lock_path, trace_socket_path, control_socket_path) = {
            ...
            if too_long(&trace_socket_path) || too_long(&control_socket_path) {
                ...
                let short = &digest[..16];
                let short_dir = std::env::temp_dir().join(format!("autter-d-{}", short));
        ...
        let (lock_path, trace_socket_path, control_socket_path) = {
            ...
            (
                ...
                PathBuf::from(format!(r"\\.\pipe\autter-{}-trace2", short)),
                PathBuf::from(format!(r"\\.\pipe\autter-{}-control", short)),
            )
        };
    ...
    pub fn from_home(home: &Path) -> Self {
        let internal_dir = home.join(".autter").join("internal");
        Self::from_internal_dir(internal_dir)
    }

    pub fn from_default_paths() -> Result<Self, AutterError> {
        let internal_dir = config::internal_dir_path().ok_or_else(|| {
            AutterError::Generic("Unable to determine ~/.autter/internal path".to_string())
        })?;
        Ok(Self::from_internal_dir(internal_dir))
    }

    pub fn from_env_or_default_paths() -> Result<Self, AutterError> {
        let mut config = if let Ok(home) = std::env::var("AUTTER_DAEMON_HOME")
            && !home.trim().is_empty()
        {
        ...
        if let Ok(path) = std::env::var("AUTTER_DAEMON_CONTROL_SOCKET")
            && !path.trim().is_empty()
        {
            config.control_socket_path = PathBuf::from(path);
        }

        if let Ok(path) = std::env::var("AUTTER_DAEMON_TRACE_SOCKET")
            && !path.trim().is_empty()
        {
    ...
    pub fn ensure_parent_dirs(&self) -> Result<(), AutterError> {
        let daemon_dir = self
            .lock_path
            .parent()
            .ok_or_else(|| AutterError::Generic("daemon lock path has no parent".to_string()))?;
...
impl DaemonLock {
    pub fn acquire(path: &Path) -> Result<Self, AutterError> {
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent)?;
        }
        let lock = LockFile::try_acquire(path).ok_or_else(|| {
            AutterError::Generic(
                "autter background service is already running (lock held)".to_string(),
            )
        })?;
        Ok(Self { _lock: lock })
    }
...
) -> Result<Option<HashMap<String, String>>, AutterError> {
...
) -> Result<Option<(String, String)>, AutterError> {
...
) -> Result<Vec<crate::daemon::domain::RefChange>, AutterError> {
...
fn apply_checkpoint_side_effect(request: CheckpointRequest) -> Result<(), AutterError> {
...
) -> Result<Option<crate::daemon::checkpoint::ResolvedCheckpointExecution>, AutterError> {
...
fn apply_clone_notes_sync_side_effect(worktree: &str) -> Result<(), AutterError> {
    let repo = find_repository_in_path(worktree)?;
...
) -> Result<(Vec<String>, HashMap<String, String>), AutterError> {
...
) -> Result<Option<Box<RecentWorkingLogSnapshot>>, AutterError> {
...
fn preceding_merge_squash_for_pending_commit(
    repo: &Repository,
    base_commit: &str,
) -> Result<Option<MergeSquashEvent>, AutterError> {
    let events = repo.storage.read_rewrite_events()?;
    for event in events {
        match event {
...
fn latest_reset_for_base_commit(
    repo: &Repository,
    base_commit: &str,
) -> Result<Option<ResetEvent>, AutterError> {
...
fn rewrite_log_mentions_commit(repo: &Repository, commit_sha: &str) -> Result<bool, AutterError> {
    if commit_sha.trim().is_empty()
        || commit_sha == "initial"
        || !is_valid_oid(commit_sha)
        || is_zero_oid(commit_sha)
...
fn deferred_commit_carryover_context(
    ...
    author: &str,
    carryover_snapshot: Option<&HashMap<String, String>>,
) -> Result<Option<DeferredCommitCarryover>, AutterError> {
...
fn processed_rebase_new_heads(repository: &Repository) -> Result<HashSet<String>, AutterError> {
...
fn maybe_rebase_mappings_from_repository(
    ...
    context: &str,
) -> Result<Option<RebaseCommitMappings>, AutterError> {
...
fn strict_cherry_pick_mappings_from_command(
    ...
    context: &str,
) -> Result<(String, Vec<String>, Vec<String>), AutterError> {
...
#[cfg(not(windows))]
fn set_socket_owner_only(path: &Path) -> Result<(), AutterError> {
    #[cfg(unix)]
    {
...
pub fn daemon_log_file_path(config: &DaemonConfig) -> Result<PathBuf, AutterError> {
...
fn write_pid_metadata(config: &DaemonConfig) -> Result<(), AutterError> {
...
pub fn read_daemon_pid(config: &DaemonConfig) -> Result<u32, AutterError> {
    let meta_path = pid_metadata_path(config);
    let contents = fs::read_to_string(&meta_path).map_err(|e| {
        AutterError::Generic(format!(
            "failed to read daemon pid metadata at {}: {}",
...
fn remove_pid_metadata(config: &DaemonConfig) -> Result<(), AutterError> {
    let path = pid_metadata_path(config);
    if path.exists() {
        fs::remove_file(path)?;
    }
...
fn daemon_is_test_mode() -> bool {
    std::env::var_os("AUTTER_TEST_DB_PATH").is_some()
        || std::env::var_os("AUTTER_TEST_DB_PATH").is_some()
}
...
#[cfg(unix)]
fn setup_daemon_log_file(config: &DaemonConfig) -> Result<DaemonLogGuard, AutterError> {
    ...
    unsafe {
        if libc::dup2(fd, libc::STDOUT_FILENO) == -1 {
            return Err(AutterError::Generic("dup2 stdout failed".to_string()));
        }
        if libc::dup2(fd, libc::STDERR_FILENO) == -1 {
            return Err(AutterError::Generic("dup2 stderr failed".to_string()));
        }
    }
...
fn setup_daemon_log_file(config: &DaemonConfig) -> Result<DaemonLogGuard, AutterError> {
    ...
    eprintln!("[autter] daemon log initialized at {}", log_path.display());

    Ok(DaemonLogGuard { _file: file })
}

#[cfg(windows)]
fn redirect_windows_stdio_to_log_file(file: &File) -> Result<(), AutterError> {
...
fn read_json_line<R: BufRead>(reader: &mut R) -> Result<Option<String>, AutterError> {
...
enum FamilySequencerEntry {
    ...
    Checkpoint {
        ...
        respond_to: Option<oneshot::Sender<Result<u64, AutterError>>>,
    },
    Canceled,
}
...
impl ActorDaemonCoordinator {
    fn new() -> Self {
        ...
        Self {
            ...
            test_completion_log_dir: std::env::var("AUTTER_TEST_DB_PATH")
                .ok()
                .or_else(|| std::env::var("AUTTER_TEST_DB_PATH").ok())
                .map(|_| {
                    DaemonConfig::from_env_or_default_paths()
                        .map(|config| config.test_completion_log_dir())
                        .unwrap_or_else(|_| {
                            std::env::temp_dir().join("autter-daemon-test-completions-fallback")
                        })
                }),
    ...
    fn begin_family_effect(&self, family: &str) -> Result<(), AutterError> {
        let mut map = self
            .inflight_effects_by_family
            .lock()
            .map_err(|_| AutterError::Generic("inflight effects map lock poisoned".to_string()))?;
        let entry = map.entry(family.to_string()).or_insert(0);
        *entry = entry.saturating_add(1);
        Ok(())
    }

    fn end_family_effect(&self, family: &str) -> Result<(), AutterError> {
        let mut map = self
            .inflight_effects_by_family
            .lock()
            .map_err(|_| AutterError::Generic("inflight effects map lock poisoned".to_string()))?;
    ...
    fn append_pending_root_entry(
    ...
    ) -> Result<(), AutterError> {
        {
            let pending_slots = self.pending_root_slots_by_root.lock().map_err(|_| {
                AutterError::Generic("pending root slots map lock poisoned".to_string())
            })?;
        ...
        let order = {
            let mut sequencers = self.family_sequencers_by_family.lock().map_err(|_| {
                AutterError::Generic("family sequencer map lock poisoned".to_string())
            })?;
            let state =
                sequencers
        ...
        self.pending_root_slots_by_root
            .lock()
            .map_err(|_| AutterError::Generic("pending root slots map lock poisoned".to_string()))?
            .insert(
                root_sid.to_string(),
                PendingRootSlot {
    ...
    fn take_pending_root_slot(
        &self,
        root_sid: &str,
    ) -> Result<Option<PendingRootSlot>, AutterError> {
        self.pending_root_slots_by_root
            .lock()
            .map_err(|_| AutterError::Generic("pending root slots map lock poisoned".to_string()))
    ...
    fn record_recent_replay_prerequisite(
        ...
        let mut map = self
            ...
            .lock()
            .map_err(|_| {
                AutterError::Generic("recent replay prerequisites map lock poisoned".to_string())
            })?;
    ...
    fn recent_replay_prerequisites_for_base(
        ...
        family: &str,
        base_commit: &str,
    ) -> Result<Vec<RecentReplayPrerequisite>, AutterError> {
        let map = self
            .recent_replay_prerequisites_by_family
            .lock()
            .map_err(|_| {
    ...
    fn maybe_append_test_completion_log(
        ...
        let _guard = self
            ...
            .lock()
            .map_err(|_| AutterError::Generic("test completion log lock poisoned".to_string()))?;
        ...
        let line = serde_json::to_string(entry).map_err(AutterError::from)?;
    ...
    fn mark_trace_root_activity(&self, root_sid: &str) -> Result<(), AutterError> {
        let mut ingress = self
            .trace_ingress_state
            .lock()
            .map_err(|_| AutterError::Generic("trace ingress state lock poisoned".to_string()))?;
        ingress
    ...
    fn trace_root_connection_opened(&self, root_sid: &str) -> Result<(), AutterError> {
        let mut ingress = self
            .trace_ingress_state
            .lock()
            .map_err(|_| AutterError::Generic("trace ingress state lock poisoned".to_string()))?;
    ...
    fn record_trace_connection_close(&self, roots: &[String]) -> Result<Vec<String>, AutterError> {
        let mut ingress = self
            .trace_ingress_state
            .lock()
            .map_err(|_| AutterError::Generic("trace ingress state lock poisoned".to_string()))?;
    ...
    fn record_trace_payload_enqueued(&self, payload: &Value) -> Result<(), AutterError> {
    ...
    fn record_trace_payload_enqueued_root(&self, root_sid: Option<&str>) -> Result<(), AutterError> {
        ...
        let mut queued = self.queued_trace_payloads_by_root.lock().map_err(|_| {
            AutterError::Generic("queued trace payloads by root lock poisoned".to_string())
        })?;
    ...
    fn enqueue_stale_connection_close_fallbacks(&self, roots: &[String]) -> Result<(), AutterError> {
        ...
        for root_sid in stale_roots {
            let mut payload = json!({
                ...
                "autter_connection_close_fallback": true,
            });
    ...
    fn clear_trace_root_tracking(&self, root_sid: &str) -> Result<(), AutterError> {
        let mut ingress = self
            .trace_ingress_state
            .lock()
            .map_err(|_| AutterError::Generic("trace ingress state lock poisoned".to_string()))?;
    ...
    fn discard_carryover_snapshots_for_root(&self, root_sid: &str) -> Result<(), AutterError> {
        let snapshot_ids = self
            .carryover_snapshot_ids_by_root
            .lock()
            .map_err(|_| {
                AutterError::Generic("carryover snapshot root map lock poisoned".to_string())
            })?
            .remove(root_sid)
            .unwrap_or_default();
    ...
    fn start_trace_ingest_worker(self: &Arc<Self>) -> Result<(), AutterError> {
    ...
    fn enqueue_trace_payload(&self, payload: Value) -> Result<(), AutterError> {
        let tx =
            self.trace_ingest_tx.get().cloned().ok_or_else(|| {
                AutterError::Generic("trace ingest worker not started".to_string())
            })?;
    ...
    fn augment_trace_payload_with_reflog_metadata(&self, payload: &mut Value) -> bool {
        ...
        if let Some(object) = payload.as_object_mut() {
            if let Some(repo) = pre_repo.as_ref() {
                object.insert("autter_pre_repo".to_string(), json!(repo));
            }
            if object.get("autter_merge_squash_source_head").is_none() {
                ...
                match inflight_merge_squash {
                    Ok(Some(source_head)) => {
                        ...
                        object.insert(
                            "autter_merge_squash_source_head".to_string(),
                            json!(source_head),
                        );
                    }
                    Ok(None) => {}
            ...
            if object.get("autter_stash_target_oid").is_none()
                && object.get("autter_stash_target_oid_error").is_none()
            {
                match resolve_stash_target_oid_for_command(&worktree, &effective_argv) {
                    Ok(Some(stash_target_oid)) => {
                        object.insert(
                            "autter_stash_target_oid".to_string(),
                    ...
                    Err(error) => {
                        ...
                        object.insert(
                            "autter_stash_target_oid_error".to_string(),
            ...
            && let Some(object) = payload.as_object_mut()
        {
            if let Some(state) = post_repo.as_ref() {
                object.insert("autter_post_repo".to_string(), json!(state));
            }
            ...
            && let Some(object) = payload.as_object_mut()
        {
                ...
                if let Some(start_offsets) = ingress.root_family_reflog_start_offsets.get(&root) {
                            ...
                            daemon_reflog_offsets_for_refs(&worktree, refs)
                    {
                        ...
                        match daemon_reflog_delta_from_offsets(
                            ...
                            Ok(ref_changes) => {
                                object.insert(
                                    "autter_family_reflog_changes".to_string(),
                                    json!(&ref_changes),
                                );
                        ...
                        object.insert("autter_family_reflog_end".to_string(), json!(end_offsets));
                    }
                }
            }
            if object.get("autter_stash_target_oid").is_none() {
                match resolve_stash_target_oid_for_terminal_payload(
                    ...
                    Ok(Some(stash_target_oid)) => {
                        object.remove("autter_stash_target_oid_error");
                        object.insert(
                            "autter_stash_target_oid".to_string(),
                            json!(stash_target_oid),
                        );
            ...
            if object.get("autter_carryover_snapshot_id").is_none() {
                ...
                match self.capture_carryover_snapshot_for_command(CarryoverCaptureInput {
                    ...
                    Ok(Some(snapshot_id)) => {
                        object.insert(
                            "autter_carryover_snapshot_id".to_string(),
                            json!(snapshot_id),
                        );
                    }
                    Ok(None) => {}
    ...
    fn side_effect_exec_lock(&self, family: &str) -> Result<Arc<AsyncMutex<()>>, AutterError> {
        let mut map = self
            .side_effect_exec_locks
            .lock()
            .map_err(|_| AutterError::Generic("side effect lock map lock poisoned".to_string()))?;
        Ok(map
            .entry(family.to_string())
                ...
                FamilySequencerEntry::ReadyCommand(command) => {
                    ...
                    let side_effect_result = {
                        let future = async {
                            ...
                            let side_effect = self
                                ...
                                .await;
                            Ok::<_, AutterError>((applied, side_effect))
                        };
                        let caught = std::panic::AssertUnwindSafe(future);
                    ...
                    match side_effect_result {
                        ...
                        Err(panic_payload) => {
                            ...
                            let error = AutterError::Generic(format!(
    ...
    fn set_pending_rebase_original_head_for_worktree(
        ...
        let mut map = self
            ...
            .lock()
            .map_err(|_| {
                AutterError::Generic("pending rebase original-head map lock poisoned".to_string())
            })?;
    ...
    fn set_pending_cherry_pick_sources_for_worktree(
        ...
        let mut map = self
            ...
            .lock()
            .map_err(|_| {
                AutterError::Generic("pending cherry-pick sources map lock poisoned".to_string())
            })?;
    ...
    fn rewrite_events_from_semantic_events(
        ...
        cmd: &crate::daemon::domain::NormalizedCommand,
        events: &[crate::daemon::domain::SemanticEvent],
    ) -> Result<Vec<RewriteLogEvent>, AutterError> {
        let mut out = Vec::new();
        ...
        for event in events {
            match event {
                ...
                crate::daemon::domain::SemanticEvent::RebaseComplete {
                    ...
                    let worktree = cmd.worktree.as_ref().ok_or_else(|| {
                        AutterError::Generic("rebase complete missing worktree".to_string())
                    })?;
                ...
                crate::daemon::domain::SemanticEvent::PullCompleted { strategy, .. } => {
                    if matches!(
                        ...
                        let worktree = cmd.worktree.as_ref().ok_or_else(|| {
                            AutterError::Generic("pull --rebase missing worktree".to_string())
                        })?;
                        let repository =
    ...
    async fn maybe_apply_side_effects_for_applied_command(
        ...
        if let Ok(path) = std::env::var("AUTTER_TEST_PANIC_IN_SIDE_EFFECT_FLAG")
        ...
        if std::env::var("AUTTER_DEBUG_DAEMON_TRACE")
            .ok()
            .as_deref()
            .is_some_and(|v| v == "1")
        {
                ...
                || (saw_pull_event && pull_uses_rebase))
        {
            ...
            if !old_head.is_empty() && !new_head.is_empty() && old_head != new_head {
                ...
                if !tracked_files.is_empty() {
                    ...
                    // checkpoints written by `autter checkpoint` during `rebase --continue`, rather
    ...
    async fn apply_trace_payload_to_state(
        &self,
        payload: Value,
    ) -> Result<TracePayloadApplyOutcome, AutterError> {
    ...
    async fn ingest_trace_payload_fast(self: Arc<Self>, payload: Value) -> Result<(), AutterError> {
    ...
    async fn ingest_checkpoint_payload(
        &self,
        request: CheckpointRequest,
    ) -> Result<ControlResponse, AutterError> {
        if request.files.is_empty() {
    ...
    ) -> Result<crate::daemon::domain::WatermarkState, AutterError> {
        self.coordinator
    ...
    async fn apply_wrapper_state_overlay(
        ...
        loop {
            ...
            if tokio::time::Instant::now() >= deadline {
                eprintln!(
                    "autter: wrapper state timeout for invocation {} (pre={}, post={}), using internal state",
    ...
    fn wrapper_state_wait_timeout(&self) -> Duration {
        let is_test = std::env::var_os("AUTTER_TEST_DB_PATH").is_some()
            || std::env::var_os("AUTTER_TEST_DB_PATH").is_some();
        if is_test {
...
fn control_listener_loop_actor(
    ...
    #[cfg(not(windows))]
    {
        ...
        let listener = ListenerOptions::new()
            ...
            .create_sync()
            .map_err(|e| AutterError::Generic(format!("failed binding control socket: {}", e)))?;
    ...
    #[cfg(windows)]
    {
        ...
        for worker in workers {
            let result = worker
                .join()
                .map_err(|_| AutterError::Generic("daemon control worker panicked".to_string()))?;
            result?;
        }
...
fn windows_pipe_connecting_server(
    pipe_path: &Path,
    first_instance: bool,
) -> Result<WindowsConnectingServer, AutterError> {
...
fn windows_control_pipe_worker_count() -> usize {
    #[cfg(feature = "test-support")]
    if let Ok(raw) = std::env::var("AUTTER_TEST_WINDOWS_CONTROL_PIPE_WORKERS")
...
fn trace_listener_loop_actor(
    ...
    #[cfg(not(windows))]
    {
        ...
        let listener = ListenerOptions::new()
            ...
            .create_sync()
            .map_err(|e| AutterError::Generic(format!("failed binding trace socket: {}", e)))?;
    ...
    #[cfg(windows)]
    {
        ...
        for worker in workers {
            let result = worker
                .join()
                .map_err(|_| AutterError::Generic("daemon trace worker panicked".to_string()))?;
            result?;
        }
...
fn daemon_update_check_interval() -> u64 {
    std::env::var("AUTTER_DAEMON_UPDATE_CHECK_INTERVAL")
        .ok()
...
fn daemon_max_uptime_ns() -> u128 {
    let secs = std::env::var("AUTTER_DAEMON_MAX_UPTIME_SECS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
...
fn daemon_socket_health_check_interval() -> u64 {
    std::env::var("AUTTER_DAEMON_SOCKET_HEALTH_CHECK_SECS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(DAEMON_SOCKET_HEALTH_CHECK_SECS)
}

/// Spawn a detached `autter bg restart --hard` process that will reap the
/// current (zombie) daemon and start a fresh one.  The child inherits the
/// daemon env vars (AUTTER_DAEMON_HOME, etc.) so it targets the same
...
fn spawn_self_restart() -> Result<(), String> {
    let exe = crate::utils::current_autter_exe().map_err(|e| e.to_string())?;
...
fn daemon_min_uptime_for_self_restart() -> u64 {
    std::env::var("AUTTER_DAEMON_MIN_UPTIME_FOR_RESTART_SECS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
...
pub(crate) async fn run_daemon(config: DaemonConfig) -> Result<DaemonExitAction, AutterError> {
    ...
    // captures stderr (fd 2). After dup2, writes go to the daemon log file.
    {
        ...
        let env_filter = if std::env::var("AUTTER_DEBUG").as_deref() == Ok("1") {
            EnvFilter::new("debug")
        } else {
...
#[cfg(not(windows))]
fn local_socket_name<'a>(socket_path: &'a Path) -> Result<Name<'a>, AutterError> {
    socket_path
        .to_fs_name::<GenericFilePath>()
        .map_err(|e| AutterError::Generic(format!("invalid daemon socket path: {}", e)))
}

pub fn open_local_socket_stream_with_timeout(
    socket_path: &Path,
    timeout: Duration,
) -> Result<DaemonClientStream, AutterError> {
    #[cfg(windows)]
    {
...
fn open_windows_named_pipe_client_with_timeout(
    socket_path: &Path,
    timeout: Duration,
) -> Result<WindowsPipeClient, AutterError> {
...
fn send_control_request_with_timeouts_windows(
    ...
    serde_json::from_str(line.trim()).map_err(AutterError::from)
}
...
mod tests {
    ...
    fn checkpoint_control_timeout_uses_ci_env_var() {
        let _unset_test = EnvVarGuard::unset("AUTTER_TEST_DB_PATH");
        let _unset_legacy_test = EnvVarGuard::unset("AUTTER_TEST_DB_PATH");
    ...
    fn checkpoint_control_timeout_uses_test_db_env_var() {
        ...
        let _unset_legacy_test = EnvVarGuard::unset("AUTTER_TEST_DB_PATH");
        let _set_test = EnvVarGuard::set("AUTTER_TEST_DB_PATH", "/tmp/autter-test.db");
