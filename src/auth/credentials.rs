...
impl CredentialStore {
    ...
    fn default_test_path() -> PathBuf {
        ...
        std::env::temp_dir().join("autter-test").join(format!(
            "credentials-{}-{}",
            std::process::id(),
            thread_num
        ))
...
mod tests {
    ...
    fn test_file_backend_creates_directory() {
        let temp_dir = env::temp_dir().join("autter-test-dir-create");
    ...
    #[test]
    fn test_fallback_path_contains_autter_test_dir() {
        let path = test_fallback_path();
        let path_str = path.to_string_lossy();
        assert!(path_str.contains("autter-test"));
    }
