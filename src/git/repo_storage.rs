...
impl RepoStorage {
    ...
    fn for_ai_dir(ai_dir: &Path, repo_workdir: &Path) -> Result<RepoStorage, AutterError> {
    ...
    #[doc(hidden)]
    pub fn ensure_config_directory(&self) -> Result<(), AutterError> {
    ...
    pub fn working_log_for_base_commit(
        &self,
        sha: &str,
    ) -> Result<PersistedWorkingLog, AutterError> {
    ...
    pub fn delete_working_log_for_base_commit(&self, sha: &str) -> Result<(), AutterError> {
        let working_log_dir = self.working_logs.join(sha);
    ...
    pub fn rename_working_log(&self, old_sha: &str, new_sha: &str) -> Result<(), AutterError> {
    ...
    pub fn read_rewrite_events(&self) -> Result<Vec<RewriteLogEvent>, AutterError> {
...
impl PersistedWorkingLog {
    ...
    pub fn reset_working_log(&self) -> Result<(), AutterError> {
    ...
    /* blob storage */
    pub fn get_file_version(&self, sha: &str) -> Result<String, AutterError> {
        let blob_path = self.dir.join("blobs").join(sha);
        Ok(fs::read_to_string(blob_path)?)
    }

    #[allow(dead_code)]
    pub fn persist_file_version(&self, content: &str) -> Result<String, AutterError> {
    ...
    pub fn read_current_file_content(&self, file_path: &str) -> Result<String, AutterError> {
    ...
    /* append checkpoint */
    pub fn append_checkpoint(&self, checkpoint: &Checkpoint) -> Result<(), AutterError> {
    ...
    pub fn read_all_checkpoints(&self) -> Result<Vec<Checkpoint>, AutterError> {
        let checkpoints_file = self.dir.join("checkpoints.jsonl");
    ...
    pub fn mutate_all_checkpoints<F>(&self, mutator: F) -> Result<Vec<Checkpoint>, AutterError>
    where
        F: FnOnce(&mut Vec<Checkpoint>) -> Result<(), AutterError>,
    {
    ...
    pub fn all_touched_files(&self) -> Result<HashSet<String>, AutterError> {
    ...
    #[allow(dead_code)]
    pub fn all_ai_touched_files(&self) -> Result<HashSet<String>, AutterError> {
        let checkpoints = self.read_all_checkpoints()?;
        let mut touched_files = HashSet::new();
        for checkpoint in checkpoints {
    ...
    pub fn write_initial(&self, initial: InitialAttributions) -> Result<(), AutterError> {
