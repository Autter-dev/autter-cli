...
impl<'a> Object<'a> {
    ...
    pub fn peel_to_commit(&self) -> Result<Commit<'a>, AutterError> {
...
impl<'a> CommitRange<'a> {
    ...
    pub fn is_valid(&self) -> Result<(), AutterError> {
...
impl<'a> Commit<'a> {
    ...
    pub fn tree(&self) -> Result<Tree<'a>, AutterError> {
    ...
    pub fn parent(&self, i: usize) -> Result<Commit<'a>, AutterError> {
        let mut args = self.repo.global_args_for_exec();
        args.push("rev-parse".to_string());
        // args.push("-q".to_string());
        args.push("--verify".to_string());
    ...
    #[allow(dead_code)]
    pub fn parent_count(&self) -> Result<usize, AutterError> {
    ...
    pub fn summary(&self) -> Result<String, AutterError> {
        let mut args = self.repo.global_args_for_exec();
        args.push("show".to_string());
        args.push("-s".to_string());
    ...
    pub fn body(&self) -> Result<String, AutterError> {
        let mut args = self.repo.global_args_for_exec();
        args.push("show".to_string());
        args.push("-s".to_string());
        args.push("--no-notes".to_string());
    ...
    // Get the author email of the git commit.
    pub fn author_email(&self) -> Result<String, AutterError> {
        let mut args = self.repo.global_args_for_exec();
        args.push("show".to_string());
        args.push("-s".to_string());
        args.push("--no-notes".to_string());
    ...
    pub fn parent_on_refname(&self, refname: &str) -> Result<Commit<'a>, AutterError> {
...
impl<'a> Tree<'a> {
    ...
    pub fn get_path(&self, path: &Path) -> Result<TreeEntry<'a>, AutterError> {
        let reader = crate::git::fast_reader::FastObjectReader::new(&self.repo.git_common_dir);
        ...
        match found_entry {
            Some(entry) => Ok(entry),
            None => Err(AutterError::Generic(format!(
...
impl<'a> Blob<'a> {
    ...
    pub fn content(&self) -> Result<Vec<u8>, AutterError> {
        let reader = crate::git::fast_reader::FastObjectReader::new(&self.repo.git_common_dir);
...
impl<'a> Reference<'a> {
    ...
    pub fn shorthand(&self) -> Result<String, AutterError> {
        let mut args = self.repo.global_args_for_exec();
        args.push("rev-parse".to_string());
    ...
    pub fn target(&self) -> Result<String, AutterError> {
...
impl<'a> Iterator for References<'a> {
    type Item = Result<Reference<'a>, AutterError>;
...
impl GitAuthorIdentity {
    /// Apply autter's optional author config as a partial override.
...
pub fn global_git_config_committer_identity() -> Result<GitAuthorIdentity, AutterError> {
    Ok(global_git_config_identity_resolution()?.identity)
}

pub fn global_git_config_identity_resolution() -> Result<GitConfigIdentityResolution, AutterError> {
    let config =
        gix_config::File::from_globals().map_err(|e| AutterError::GixError(e.to_string()))?;
...
impl Repository {
    ...
    fn object_type(&self, oid: &str) -> Result<String, AutterError> {
    ...
    pub fn head<'a>(&'a self) -> Result<Reference<'a>, AutterError> {
        use crate::git::fast_reader::{FastRefReader, HeadKind};
    ...
    pub fn workdir(&self) -> Result<PathBuf, AutterError> {
    ...
    pub fn is_bare_repository(&self) -> Result<bool, AutterError> {
        let mut args = self.global_args_for_exec();
        args.push("rev-parse".to_string());
    ...
    pub fn remotes(&self) -> Result<Vec<String>, AutterError> {
        Ok(self
            .remotes_with_urls()?
            .into_iter()
            .map(|(name, _)| name)
    ...
    pub fn remotes_with_urls(&self) -> Result<Vec<(String, String)>, AutterError> {
    ...
    ) -> Result<Option<gix_config::File<'static>>, AutterError> {
        if !path.exists() {
            return Ok(None);
        }
        gix_config::File::from_path_no_includes(path.to_path_buf(), source)
            .map(Some)
            .map_err(|e| AutterError::GixError(e.to_string()))
    }

    pub(crate) fn get_git_config_file(&self) -> Result<gix_config::File<'static>, AutterError> {
        git_config_file_for_repo_paths(self.path(), self.common_dir())
    }

    /// Get config value for a given key as a String.
    pub fn config_get_str(&self, key: &str) -> Result<Option<String>, AutterError> {
    ...
    ///
    /// For autter authorship metadata, use [`Self::effective_author_identity`] so the
    /// autter author config can override this raw Git identity.
    ...
    /// Get the autter effective author identity for metadata and display.
    ///
    /// This starts from Git's effective committer identity, then overlays any
    /// configured `author.name` and/or `author.email` from autter config.
    ...
    pub fn config_get_regexp(
        ...
        let re = Regex::new(pattern)
            .map_err(|e| AutterError::Generic(format!("Invalid regex pattern: {}", e)))?;
    ...
    #[allow(dead_code)]
    pub fn remote_head(&self, remote_name: &str) -> Result<String, AutterError> {
        let mut args = self.global_args_for_exec();
        args.push("symbolic-ref".to_string());
    ...
    pub fn revparse_single(&self, spec: &str) -> Result<Object<'_>, AutterError> {
        let mut args = self.global_args_for_exec();
        args.push("rev-parse".to_string());
        // args.push("-q".to_string());
        args.push("--verify".to_string());
    ...
    pub fn get_default_remote(&self) -> Result<Option<String>, AutterError> {
    ...
    #[allow(dead_code)]
    pub fn push_authorship(&self, remote_name: &str) -> Result<(), AutterError> {
        push_authorship_notes(self, remote_name)
    }

    pub fn upstream_remote(&self) -> Result<Option<String>, AutterError> {
    ...
    pub fn resolve_author_spec(&self, author_spec: &str) -> Result<Option<String>, AutterError> {
    ...
    pub fn find_commit(&self, oid: String) -> Result<Commit<'_>, AutterError> {
    ...
    pub fn find_blob(&self, oid: String) -> Result<Blob<'_>, AutterError> {
        let typ = self.object_type(&oid)?;
        if typ != "blob" {
            return Err(AutterError::Generic(format!(
    ...
    // Lookup a reference to one of the objects in a repository.
    pub fn find_tree(&self, oid: String) -> Result<Tree<'_>, AutterError> {
        let typ = self.object_type(&oid)?;
        if typ != "tree" {
            return Err(AutterError::Generic(format!(
                ...
                async move {
                    ...
                    let result = exec_git(&args).and_then(|output| {
                        String::from_utf8(output.stdout)
                            .map_err(|e| AutterError::Utf8Error(e.utf8_error()))
                    });
                    (file_path, result)
                }
            })
    ...
    pub fn get_all_staged_file_blob_oids(&self) -> Result<HashMap<String, String>, AutterError> {
        ...
        let index = gix_index::File::at(index_path, object_hash, true, Default::default())
            .map_err(|err| AutterError::GixError(err.to_string()))?;
    ...
    pub fn diff_added_lines(
        ...
        to_ref: &str,
        pathspecs: Option<&HashSet<String>>,
    ) -> Result<HashMap<String, Vec<u32>>, AutterError> {
        let mut args = self.global_args_for_exec();
    ...
    pub fn diff_added_lines_with_deleted_count(
        ...
        from_ref: &str,
        to_ref: &str,
    ) -> Result<(HashMap<String, Vec<u32>>, usize), AutterError> {
        let mut args = self.global_args_for_exec();
        args.push("diff".to_string());
    ...
    pub fn diff_workdir_added_lines_with_insertions(
        ...
        from_ref: &str,
        pathspecs: Option<&HashSet<String>>,
    ) -> Result<(HashMap<String, Vec<u32>>, HashMap<String, Vec<u32>>), AutterError> {
        let mut args = self.global_args_for_exec();
        args.push("diff".to_string());
        args.push("-U0".to_string()); // Zero context lines
        args.push("--no-color".to_string());
    ...
    pub fn fetch_branch(&self, branch_name: &str, remote_name: &str) -> Result<(), AutterError> {
        let mut args = self.global_args_for_exec();
...
pub fn find_repository(global_args: &[String]) -> Result<Repository, AutterError> {
    ...
    let git_dir_str = lines.next().ok_or_else(|| {
        AutterError::Generic("Missing --git-dir output from git rev-parse".to_string())
    })?;
    let git_common_dir_str = lines.next().ok_or_else(|| {
        AutterError::Generic("Missing --git-common-dir output from git rev-parse".to_string())
    })?;
...
#[doc(hidden)]
pub fn resolve_command_base_dir(global_args: &[String]) -> Result<PathBuf, AutterError> {
    ...
    while idx < global_args.len() {
        if global_args[idx] == "-C" {
            let path_arg = global_args.get(idx + 1).ok_or_else(|| {
                AutterError::Generic("Missing path after -C in global git args".to_string())
            })?;
            ...
            base = Some(if next_base.is_absolute() {
                ...
                let current = match &base {
                    ...
                    None => std::env::current_dir().map_err(AutterError::IoError)?,
                };
...
fn discover_repository_paths_no_git_exec(
    path: &Path,
) -> Result<DiscoveredRepositoryPaths, AutterError> {
    ...
    while let Some(dir) = current {
        if dir.join("HEAD").is_file() && dir.join("objects").is_dir() {
            let workdir = dir.parent().ok_or_else(|| {
                AutterError::Generic(format!("Git directory has no parent: {}", dir.display()))
            })?;
            return Ok(DiscoveredRepositoryPaths {
...
fn git_config_file_for_repo_paths(
    git_dir: &Path,
    git_common_dir: &Path,
) -> Result<gix_config::File<'static>, AutterError> {
    let mut config =
        gix_config::File::from_globals().map_err(|e| AutterError::GixError(e.to_string()))?;
...
#[allow(dead_code)]
pub fn from_bare_repository(git_dir: &Path) -> Result<Repository, AutterError> {
    let workdir = git_dir
        .parent()
        .ok_or_else(|| AutterError::Generic("Git directory has no parent".to_string()))?
        .to_path_buf();
...
pub fn discover_repository_in_path_no_git_exec(path: &Path) -> Result<Repository, AutterError> {
    let paths = discover_repository_paths_no_git_exec(path)?;
...
pub fn find_repository_in_path(path: &str) -> Result<Repository, AutterError> {
...
/// * `Err(AutterError)` - If no repository is found or other errors occur
...
pub fn exec_git(args: &[String]) -> Result<Output, AutterError> {
...
pub fn exec_git_allow_nonzero(args: &[String]) -> Result<Output, AutterError> {
...
pub fn exec_git_allow_nonzero_with_profile(
    ...
    cmd.output().map_err(AutterError::IoError)
}
...
pub fn spawn_git_stdout(args: &[String]) -> Result<Child, AutterError> {
    ...
    cmd.spawn().map_err(AutterError::IoError)
}

/// Spawn a git command with stdin/stdout/stderr inherited from autter.
///
...
pub fn spawn_git_passthrough(args: &[String]) -> Result<Child, AutterError> {
    let effective_args = args_with_internal_git_profile(
        &args_with_disabled_hooks_if_needed(args),
        InternalGitProfile::General,
    );
...
pub fn exec_git_with_profile(
    ...
    if !output.status.success() {
        ...
        return Err(AutterError::GitCliError {
            code,
            stderr,
            args: effective_args,
        });
...
pub fn exec_git_stdin(args: &[String], stdin_data: &[u8]) -> Result<Output, AutterError> {
...
pub fn exec_git_stdin_with_profile(
    ...
    let mut child = cmd.spawn().map_err(AutterError::IoError)?;
    ...
    let output = child.wait_with_output().map_err(AutterError::IoError)?;

    if let Some(handle) = stdin_handle
        && let Err(e) = handle.join().expect("stdin writer thread panicked")
        && e.kind() != std::io::ErrorKind::BrokenPipe
    {
        return Err(AutterError::IoError(e));
    }
