...
//!   - integration tests that depend on `autter` with `features = ["test-support"]`
//!   - benchmarks (same — `[dev-dependencies] autter = { features = ["test-support"] }`)
//!
...
pub fn init_test_git_config() {
    ...
    ONCE.get_or_init(|| {
        ...
        unsafe {
            std::env::set_var("GIT_AUTHOR_NAME", "autter test");
            std::env::set_var("GIT_AUTHOR_EMAIL", "test@autter.local");
            std::env::set_var("GIT_COMMITTER_NAME", "autter test");
            std::env::set_var("GIT_COMMITTER_EMAIL", "test@autter.local");
        }
    });
}
...
impl TmpRepo {
    ...
    pub fn new() -> Result<Self, AutterError> {
        ...
        for (k, v) in [
            ("user.email", "test@autter.local"),
            ("user.name", "autter test"),
        ...
        let path_str = path
            .to_str()
            .ok_or_else(|| AutterError::Generic("TmpRepo path is not utf-8".to_string()))?;
    ...
    /// Borrow the wrapped `autter` `Repository`.
    pub fn autter_repo(&self) -> &Repository {
        &self.repo
    }
    ...
    pub fn git_command(&self, args: &[&str]) -> Result<String, AutterError> {
        run_git_in(&self.path, args)
    }

    /// Stage all changes and create a commit. Returns the resulting commit SHA.
    pub fn commit_all(&self, message: &str) -> Result<String, AutterError> {
    ...
    pub fn commit_with_message(&self, message: &str) -> Result<String, AutterError> {
        self.commit_all(message)
    }

    /// Create a new branch (does not switch to it).
    pub fn create_branch(&self, name: &str) -> Result<(), AutterError> {
        self.git_command(&["branch", name])?;
        Ok(())
    }

    /// Switch to an existing branch.
    pub fn switch_branch(&self, name: &str) -> Result<(), AutterError> {
    ...
    pub fn rebase_onto(&self, _feature_branch: &str, onto: &str) -> Result<(), AutterError> {
        self.git_command(&["rebase", onto])?;
        Ok(())
    }

    /// Bench-only no-op stub; preserved for API compatibility with callers that
    /// previously triggered a `autter checkpoint` here. Benchmarks that need
    /// real attribution data should pre-write notes via `notes_add`.
    pub fn trigger_checkpoint_with_author(&self, _author: &str) -> Result<(), AutterError> {
        Ok(())
    }
}

fn run_git_in(cwd: &Path, args: &[&str]) -> Result<String, AutterError> {
