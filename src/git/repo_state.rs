...
fn read_head_reflog_transitions_for_worktree_internal(
    worktree: &Path,
    include_noop: bool,
) -> Result<Vec<HeadReflogTransition>, AutterError> {
...
fn read_complete_rebase_segments_for_worktree(
    worktree: &Path,
) -> Result<Vec<RebaseReflogSegment>, AutterError> {
...
) -> Result<Option<RebaseReflogSegment>, AutterError> {
...
pub fn resolve_linear_head_commit_chain_for_worktree(
    ...
    expected_count: usize,
    message_fragment: Option<&str>,
) -> Result<(String, Vec<String>), AutterError> {
    if expected_count == 0 {
