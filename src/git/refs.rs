...
pub fn notes_add_batch(repo: &Repository, entries: &[(String, String)]) -> Result<(), AutterError> {
    if entries.is_empty() {
        return Ok(());
    }
    ...
    let existing_notes_tip = match exec_git(&args) {
        ...
        Err(AutterError::GitCliError {
            code: Some(128), ..
        })
        | Err(AutterError::GitCliError { code: Some(1), .. }) => None,
        Err(e) => return Err(e),
    };
    ...
    let now = std::time::SystemTime::now()
        ...
        .map_err(|e| AutterError::Generic(format!("System clock before epoch: {}", e)))?
        .as_secs();
    ...
    script.extend_from_slice(format!("committer autter <autter@local> {} +0000\n", now).as_bytes());
    ...
    crate::authorship::autter_hooks::post_notes_updated(repo, &deduped_entries);
...
pub fn notes_add_blob_batch(
    ...
    let has_post_notes_updated_hooks = crate::config::Config::get()
        .autter_hook_commands("post_notes_updated")
        .is_some_and(|commands| !commands.is_empty());
    if has_post_notes_updated_hooks {
        let hook_entries = (|| -> Result<Vec<(String, String)>, AutterError> {
        ...
        match hook_entries {
            Ok(entries) if !entries.is_empty() => {
                crate::authorship::autter_hooks::post_notes_updated(repo, &entries)
            }
            Ok(_) => {}
            Err(e) => tracing::debug!(
...
pub fn get_commits_with_notes_from_list(
    ...
    let stdout = String::from_utf8(output.stdout)
        .map_err(|_| AutterError::Generic("Failed to parse git rev-list output".to_string()))?;
...
pub fn get_reference_as_working_log(
    repo: &Repository,
    commit_sha: &str,
) -> Result<Vec<Checkpoint>, AutterError> {
    let content = show_authorship_note(repo, commit_sha)
        .ok_or_else(|| AutterError::Generic("No authorship note found".to_string()))?;
...
pub fn merge_notes_from_ref(repo: &Repository, source_ref: &str) -> Result<(), AutterError> {
    let mut args = repo.global_args_for_exec();
    args.push("notes".to_string());
...
pub fn fallback_merge_notes_ours(repo: &Repository, source_ref: &str) -> Result<(), AutterError> {
    ...
    stream.push_str("committer autter <autter@noreply> 0 +0000\n");
...
fn list_all_notes(
    ...
    let output = exec_git(&args)?;
    let stdout = String::from_utf8(output.stdout)
        .map_err(|_| AutterError::Generic("Failed to parse notes list output".to_string()))?;
...
fn rev_parse(repo: &Repository, rev: &str) -> Result<String, AutterError> {
    let mut args = repo.global_args_for_exec();
    args.extend_from_slice(&["rev-parse".to_string(), rev.to_string()]);
    let output = exec_git(&args)?;
    String::from_utf8(output.stdout)
        .map_err(|_| AutterError::Generic("Failed to parse rev-parse output".to_string()))
        .map(|s| s.trim().to_string())
}

/// Copy a ref to another location (used for initial setup of local notes from tracking ref)
pub fn copy_ref(repo: &Repository, source_ref: &str, dest_ref: &str) -> Result<(), AutterError> {
    let mut args = repo.global_args_for_exec();
...
pub fn grep_ai_notes(repo: &Repository, pattern: &str) -> Result<Vec<String>, AutterError> {
    let mut args = repo.global_args_for_exec();
    ...
    let output = exec_git(&args)?;
    let stdout = String::from_utf8(output.stdout)
        .map_err(|_| AutterError::Generic("Failed to parse git grep output".to_string()))?;
    ...
    if shas.len() > 1 {
        ...
        let output = exec_git(&args)?;
        let stdout = String::from_utf8(output.stdout)
            .map_err(|_| AutterError::Generic("Failed to parse git log output".to_string()))?;
