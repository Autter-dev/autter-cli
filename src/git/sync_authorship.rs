...
pub fn fetch_authorship_notes(
    repository: &Repository,
    remote_name: &str,
) -> Result<NotesExistence, AutterError> {
...
fn is_missing_remote_notes_ref_error(error: &AutterError) -> bool {
    let AutterError::GitCliError { stderr, .. } = error else {
        return false;
    };
...
pub fn push_authorship_notes(
    ...
    Err(last_error
        .unwrap_or_else(|| AutterError::Generic("notes push exhausted retries".to_string())))
}
...
fn is_non_fast_forward_error(error: &AutterError) -> bool {
    let AutterError::GitCliError { stderr, .. } = error else {
        return false;
    };
