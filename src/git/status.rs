...
impl Repository {
    ...
    pub fn get_staged_filenames(&self) -> Result<HashSet<String>, AutterError> {
    ...
    // Get status for tracked files that changed
    pub fn get_staged_and_unstaged_filenames(&self) -> Result<HashSet<String>, AutterError> {
        let mut args = self.global_args_for_exec();
    ...
    pub fn status(
        ...
        skip_untracked: bool,
    ) -> Result<Vec<StatusEntry>, AutterError> {
...
fn parse_porcelain_v2(data: &[u8]) -> Result<Vec<StatusEntry>, AutterError> {
    ...
    while let Some(raw) = parts.next() {
        ...
        let tag = chars
            .next()
            .ok_or_else(|| AutterError::Generic("Unexpected empty porcelain v2 record".into()))?;
        ...
        match tag {
            '1' | 'u' => {
                ...
                let xy = fields
                    .next()
                    .ok_or_else(|| AutterError::Generic("Missing XY field".into()))?;
                if xy.len() != 2 {
                    return Err(AutterError::Generic(format!(
                ...
                let path = nfc_path(
                    fields
                        .next()
                        .ok_or_else(|| AutterError::Generic("Missing path field".into()))?
                        .to_string(),
                );
            ...
            '2' => {
                ...
                let orig_path_bytes = parts.next().ok_or_else(|| {
                    AutterError::Generic("Missing original path for rename/copy".into())
                })?;
