...
pub fn get_github_ci_context() -> Result<Option<CiContext>, AutterError> {
    ...
    if !previous_head_is_local {
        ...
        let previous_head_ref = format!("refs/autter/github/pr/{}/previous-head", pr_number);
        exec_git(&[
            "-C".to_string(),
            clone_dir.clone(),
            "fetch".to_string(),
...
/// Writes the embedded template to .github/workflows/autter.yaml at the repo root
pub fn install_github_ci_workflow() -> Result<PathBuf, AutterError> {
    ...
    fs::create_dir_all(&workflows_dir)
        .map_err(|e| AutterError::Generic(format!("Failed to create workflows dir: {}", e)))?;

    // Write template
    let dest_path = workflows_dir.join("autter.yaml");
    fs::write(&dest_path, GITHUB_CI_TEMPLATE_YAML)
        .map_err(|e| AutterError::Generic(format!("Failed to write workflow file: {}", e)))?;
