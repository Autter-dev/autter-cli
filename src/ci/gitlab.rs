...
/// `User-Agent: autter/<version>`, one of `PRIVATE-TOKEN` / `JOB-TOKEN`.
...
pub fn get_gitlab_ci_context() -> Result<Option<CiContext>, AutterError> {
    // Read required environment variables
    let api_url = std::env::var("CI_API_V4_URL").map_err(|_| {
        AutterError::Generic("CI_API_V4_URL environment variable not set".to_string())
    })?;
    let project_id = std::env::var("CI_PROJECT_ID").map_err(|_| {
        AutterError::Generic("CI_PROJECT_ID environment variable not set".to_string())
    })?;
    let commit_sha = std::env::var("CI_COMMIT_SHA").map_err(|_| {
        AutterError::Generic("CI_COMMIT_SHA environment variable not set".to_string())
    })?;
    let server_url = std::env::var("CI_SERVER_URL").map_err(|_| {
        AutterError::Generic("CI_SERVER_URL environment variable not set".to_string())
    })?;
    let project_path = std::env::var("CI_PROJECT_PATH").map_err(|_| {
        AutterError::Generic("CI_PROJECT_PATH environment variable not set".to_string())
    })?;
    ...
    let lookback_minutes = std::env::var("AUTTER_CI_LOOKBACK_MINUTES")
        .ok()
    ...
    let response = gitlab_api_get(&endpoint, auth_header_name, &auth_token)
        .map_err(|e| AutterError::Generic(format!("GitLab API request failed: {}", e)))?;
    ...
    let merge_requests: Vec<GitLabMergeRequest> =
        serde_json::from_str(response.as_str().unwrap_or("[]")).map_err(|e| {
            AutterError::Generic(format!("Failed to parse GitLab API response: {}", e))
        })?;
    ...
    let clone_dir = "autter-ci-clone".to_string();
...
mod tests {
    ...
    fn test_lookback_minutes_defaults_to_15() {
        unsafe { std::env::remove_var("AUTTER_CI_LOOKBACK_MINUTES") };
        let lookback = std::env::var("AUTTER_CI_LOOKBACK_MINUTES")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(15i64);
    ...
    fn test_lookback_minutes_reads_env_var() {
        unsafe { std::env::set_var("AUTTER_CI_LOOKBACK_MINUTES", "4320") };
        let lookback = std::env::var("AUTTER_CI_LOOKBACK_MINUTES")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(15i64);
    ...
    fn test_lookback_minutes_falls_back_on_invalid_value() {
        unsafe { std::env::set_var("AUTTER_CI_LOOKBACK_MINUTES", "not-a-number") };
        let lookback = std::env::var("AUTTER_CI_LOOKBACK_MINUTES")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(15i64);
