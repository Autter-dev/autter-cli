...
impl CiContext {
    ...
    pub fn run(&self) -> Result<CiRunResult, AutterError> {
        self.run_with_options(CiRunOptions::default())
    }

    pub fn run_with_options(&self, options: CiRunOptions) -> Result<CiRunResult, AutterError> {
        match &self.event {
            CiEvent::Merge {
                merge_commit_sha,
                head_ref,
            ...
            CiEvent::Sync {
                ...
                ensure_commit_available_for_sync(
                    ...
                    "refs/autter/ci-sync/previous-head",
    ...
    pub fn teardown(&self) -> Result<(), AutterError> {
    ...
    fn fetch_fork_notes(repo: &Repository, fork_url: &str) -> Result<bool, AutterError> {
    ...
    pub fn get_rebased_commits(
        ...
        // `autter ci local merge` may pass an abbreviated `merge_commit_sha`; the
...
fn stable_patch_id_for_commit(repo: &Repository, commit_sha: &str) -> Result<String, AutterError> {
...
mod tests {
    ...
    fn commit_is_ancestor_returns_false_for_unrelated_histories() {
        ...
        assert!(
            !commit_is_ancestor(repo.autter_repo(), &main_sha, &unrelated_sha)
    ...
    fn commit_is_ancestor_errors_for_invalid_descendant() {
        ...
        assert!(commit_is_ancestor(repo.autter_repo(), &main_sha, "not-a-sha").is_err());
    }
    ...
    fn sync_fetch_uses_blobless_only_for_named_promisor_remote() {
        ...
        assert!(
            !sync_fetch_remote_supports_lazy_blobs(repo.autter_repo(), "origin")
