...
//! variables and stores credentials in ~/.autter/internal/credentials.
//!
//! On failure, exits with code 1 silently so the install script can fall back
//! to running `autter login`. Errors are recorded server-side for debugging.
...
///
/// Exits with code 1 on failure (silently) so install script can run `autter login`.
