...
fn maybe_show_async_post_commit_stats(parsed: &ParsedGitInvocation, repo: &Repository) {
    ...
    let is_interactive =
        std::io::stdout().is_terminal() || std::env::var_os("AUTTER_TEST_FORCE_TTY").is_some();
    if !is_interactive {
        return;
    }
    ...
    // AUTTER_POST_COMMIT_TIMEOUT_MS allows tests to override the timeout.
    let timeout = if let Some(ms) = std::env::var("AUTTER_POST_COMMIT_TIMEOUT_MS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
    {
        std::time::Duration::from_millis(ms)
    } else if std::env::var_os("AUTTER_TEST_DB_PATH").is_some() {
    ...
    if !note_found {
        eprintln!(
            "[autter] still processing commit {}... run `autter stats` to see stats.",
    ...
    {
        eprintln!(
            "[autter] Skipped autter stats for large commit. Run `autter stats {}` to compute stats on demand.",
            commit_sha
        );
        return;
    }
    ...
    let child = {
        #[cfg(unix)]
        {
            ...
            if let Some(id) = wrapper_invocation_id {
                cmd.env("AUTTER_WRAPPER_INVOCATION_ID", id);
                cmd.env("GIT_TRACE2_ENV_VARS", "AUTTER_WRAPPER_INVOCATION_ID");
            }
            unsafe {
