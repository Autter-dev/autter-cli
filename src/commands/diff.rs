...
use crate::commands::blame::AutterBlameOptions;
...
/// JSON output format for autter diff --json
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            arg if arg.starts_with("--") => {
                return Err(AutterError::Generic(format!("Unknown option: {}", arg)));
            }
            _ => {
...
pub fn execute_diff(repo: &Repository, parsed: ParsedDiffArgs) -> Result<String, AutterError> {
...
fn resolve_commit(repo: &Repository, rev: &str) -> Result<String, AutterError> {
    ...
    let sha = String::from_utf8(output.stdout)
        .map_err(|e| AutterError::Generic(format!("Failed to parse rev-parse output: {}", e)))?
        .trim()
        .to_string();
...
fn resolve_parent(repo: &Repository, commit: &str) -> Result<String, AutterError> {
    ...
    match output {
        Ok(out) => {
            let sha = String::from_utf8(out.stdout)
                .map_err(|e| AutterError::Generic(format!("Failed to parse parent SHA: {}", e)))?
                .trim()
                .to_string();
...
pub fn get_diff_with_line_numbers(
    ...
    from: &str,
    to: &str,
) -> Result<Vec<DiffHunk>, AutterError> {
...
fn parse_diff_hunks(diff_text: &str) -> Result<Vec<DiffHunk>, AutterError> {
...
) -> Result<Option<DiffHunk>, AutterError> {
...
pub fn overlay_diff_attributions(
    ...
    to_commit: &str,
    hunks: &[DiffHunk],
) -> Result<HashMap<DiffLineKey, Attribution>, AutterError> {
...
) -> Result<DiffBuildArtifacts, AutterError> {
...
fn apply_blame_for_side(
    ...
    let mut blame_options = AutterBlameOptions {
        line_ranges,
        no_output: true,
...
fn build_json_hunks(
    ...
    diff_to_commit: &str,
    commits: &mut BTreeMap<String, DiffCommitMetadata>,
) -> Result<Vec<DiffJsonHunk>, AutterError> {
...
fn load_commit_metadata(
    repo: &Repository,
    commit_sha: &str,
) -> Result<DiffCommitMetadata, AutterError> {
    let mut args = repo.global_args_for_exec();
    ...
    let stdout = String::from_utf8(output.stdout)
        .map_err(|e| AutterError::Generic(format!("Failed to parse commit metadata: {}", e)))?;
