...
/// authorship notes through the configured notes backend, and renders Autter
    ...
    for arg in args {
        ...
        match arg.as_str() {
            ...
            _ if is_unsupported_render_arg(arg) => {
                return Err(format!(
                    "unsupported git log rendering option '{}'. `autter log` owns rendering so it can show authorship stats; use plain `git log` for this option.",
                    arg
                ));
            }
...
fn print_log_help() {
    println!("Usage: autter log [--raw|--notes] [--plain] [git log filters] [--] [pathspecs...]");
    println!();
    println!("Shows commit history with Autter authorship stats.");
    println!();
    println!("Options:");
...
fn render_stats(
    ...
    {
        return Err(format!(
            "stats skipped for large commit; run `autter stats {}` to compute on demand",
            commit_sha
        ));
    }
...
fn draw_pager(
    ...
    let status = if pager.is_eof() {
        format!(
            " autter log  lines {}  q quit  ↑/↓ scroll  PgUp/PgDn page ",
            pager.line_count()
        )
    } else {
        format!(
            " autter log  loaded {} lines  q quit  ↑/↓ scroll  PgUp/PgDn page ",
            pager.line_count()
        )
    };
    queue!(
...
fn unique_spool_path() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or_default();
    std::env::temp_dir().join(format!("autter-log-{}-{}.tmp", std::process::id(), nanos))
}
...
impl From<AutterError> for LogError {
    fn from(value: AutterError) -> Self {
        LogError::Git(value)
    }
}
