...
/// Entry point for `autter notes migrate`.
pub fn handle_notes_migrate(args: &[String]) {
        ...
        None => {
            eprintln!(
                "error: notes_backend.backend_url is not configured.\n\
                 ...
                 \n\
                 \x20 autter config set notes_backend.backend_url https://your-backend.example.com"
            );
            std::process::exit(1);
        }
    };
    ...
    if !client.is_logged_in() && !client.has_api_key() {
        eprintln!("error: not authenticated. Log in first with `autter login` or set an API key.");
        std::process::exit(1);
    }
...
fn list_notes(
    ...
    let output = exec_git(&args)
        .map_err(|e| AutterError::Generic(format!("git notes --ref=ai list failed: {}", e)))?;
...
fn cat_file_batch(
    ...
    let mut stdin = child.stdin.take().ok_or_else(|| {
        AutterError::Generic("failed to open git cat-file --batch stdin".to_string())
    })?;
    ...
    let output = child
        .wait_with_output()
        .map_err(|e| AutterError::Generic(format!("git cat-file --batch failed: {}", e)))?;
...
fn print_help() {
    ...
    eprintln!("    autter config set notes_backend.kind http");
    eprintln!();
...
mod tests {
    ...
    fn migration_uploads_notes_and_caches_with_synced_1() {
        ...
        unsafe {
            ...
            std::env::set_var("AUTTER_API_KEY", "migrate-test-key");
        }

        // --- Run the migration core logic ---
        let note_pairs = list_notes(repo.autter_repo()).expect("list_notes");
        ...
        let blob_contents = cat_file_batch(repo.autter_repo(), &blob_shas).expect("cat_file_batch");
    ...
    fn list_notes_returns_empty_for_repo_without_notes() {
        ...
        let pairs = list_notes(repo.autter_repo()).expect("list_notes");
        assert!(
            pairs.is_empty(),
    ...
    fn cat_file_batch_empty_input() {
        let repo = TmpRepo::new().expect("TmpRepo::new");
        let result = cat_file_batch(repo.autter_repo(), &[]).expect("cat_file_batch");
    ...
    fn force_flag_bypasses_synced_cache_filter() {
        ...
        // With force: no filtering applied, all entries remain.
        {
            ...
            unsafe {
                std::env::set_var("AUTTER_NOTES_BACKEND_URL", &server_url);
                std::env::set_var("AUTTER_API_KEY", "force-test-key");
            }
