...
const AUTTER_DAEMON_UPGRADE_ENV: &str = "AUTTER_DAEMON_UPGRADE";
...
fn exit_if_invoked_via_git_extension() {
    if should_block_git_extension_upgrade(
        ...
        eprintln!(
            "error: `autter upgrade` is not supported on Windows. Run `autter upgrade` instead."
        );
        std::process::exit(1);
    }
}
...
fn run_install_script(script_content: &str, tag: &str, silent: bool) -> Result<(), String> {
    #[cfg(windows)]
    {
        ...
        // On Windows, we need to run the installer detached because the current autter
        ...
        let ps_wrapper = format!(
            ...
            log_path_str, script_path_str, AUTTER_RESTART_DAEMON_AFTER_INSTALL_ENV, script_path_str
        );
        ...
        let spawn_powershell = |exe: &str| -> std::io::Result<std::process::Child> {
            ...
            cmd.arg("-NoProfile")
                ...
                .arg("-Command")
                .arg(&ps_wrapper)
                .env(AUTTER_RELEASE_ENV, tag);

            // Hide the spawned console to prevent any host/UI bleed-through
            cmd.creation_flags(CREATE_NO_WINDOW);

            if silent {
                cmd.env(AUTTER_RESTART_DAEMON_AFTER_INSTALL_ENV, "1");
                cmd.env(AUTTER_DAEMON_UPGRADE_ENV, "1");
        ...
        match spawn_result {
            Ok(_) => {
                if !silent {
                    ...
                    println!(
                        "The installer will stop lingering autter background processes if needed, but active git commands can still delay completion."
                    );
                }
                Ok(())
            }
    ...
    #[cfg(not(windows))]
    {
        ...
        // Write script to ~/.autter/tmp/ to avoid /tmp noexec or permission issues.
        // Fall back to the system temp dir if the home-based path is unavailable.
        let temp_dir = crate::config::autter_dir_path()
            .map(|p| p.join("tmp"))
            .unwrap_or_else(std::env::temp_dir);
        fs::create_dir_all(&temp_dir)
            .map_err(|e| format!("Failed to create temp directory: {}", e))?;
        let script_path = temp_dir.join(format!("autter-install-{}.sh", std::process::id()));
        ...
        cmd.arg(&script_path_str).env(AUTTER_RELEASE_ENV, tag);
...
fn spawn_background_upgrade_process() -> bool {
    crate::utils::spawn_internal_autter_subcommand(
        "upgrade",
        &["--background"],
...
mod tests {
    ...
    fn set_test_cache_dir(dir: &tempfile::TempDir) {
        unsafe {
            std::env::set_var("AUTTER_TEST_CACHE_DIR", dir.path());
        }
    }

    fn clear_test_cache_dir() {
        unsafe {
            std::env::remove_var("AUTTER_TEST_CACHE_DIR");
        }
    }
    ...
    fn test_is_git_process_name() {
        ...
        assert!(!is_git_process_name("autter.exe"));
    ...
    fn test_parse_checksums_valid_format() {
        let content = "594de6cf107e8ffb6efd9029bf727b465ab55a9b4c4c3995eb3e628c857dc423  autter-linux-arm64\n\
                       88db3c0c7fc62a815579ec0ca42535c2b83ab18d9e3af8efe345dee96677b1d8  autter-linux-x64\n\
                       75d1692d347c3e08a208dc6373df4cee2b5ffd0e2aee62ccb1bb47aae866b2c8  install.sh";

        let checksums = parse_checksums(content);
        assert_eq!(checksums.len(), 3);
        assert_eq!(
            checksums.get("autter-linux-arm64"),
            Some(&"594de6cf107e8ffb6efd9029bf727b465ab55a9b4c4c3995eb3e628c857dc423".to_string())
        );
        assert_eq!(
            checksums.get("autter-linux-x64"),
    ...
    fn test_parse_checksums_with_extensions() {
        let content = "23c693a25f4f2e99463c911e67d534ae17cbd9b98513aa65f0ae9da861775d54  autter-windows-x64.exe\n\
                       f895af791eb30f6b074b2ab9f0f803e91230b084f5864befcb51ee9ced752adf  install.ps1";

        let checksums = parse_checksums(content);
        assert_eq!(checksums.len(), 2);
        assert!(checksums.contains_key("autter-windows-x64.exe"));
