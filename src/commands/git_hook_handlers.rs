...
/// Remove autter managed hooks from a repository, restoring the previous
/// `core.hooksPath` value if one was saved in the hook state file.
pub fn remove_repo_hooks(
    repo: &Repository,
    dry_run: bool,
) -> Result<RemoveRepoHooksReport, AutterError> {
...
/// Resolve the hooks path that was active before autter managed hooks were
...
fn write_config(path: &Path, cfg: &gix_config::File<'_>) -> Result<(), AutterError> {
...
fn unset_hooks_path_in_local_config(repo: &Repository, dry_run: bool) -> Result<bool, AutterError> {
    let local_config_path = repo_local_config_path(repo);
...
fn read_repo_hook_state(path: &Path) -> Result<Option<RepoHookState>, AutterError> {
    if !path.exists() {
        return Ok(None);
    }
...
fn delete_state_file(path: &Path, dry_run: bool) -> Result<bool, AutterError> {
    if !path.exists() {
        return Ok(false);
    }
    if !dry_run {
...
fn remove_hook_entry(hook_path: &Path) -> Result<(), AutterError> {
...
fn is_path_inside_any_autter_dir(path: &Path) -> bool {
...
fn is_disallowed_forward_hooks_path(
    ...
    if is_path_inside_component(path, ".autter") {
        return true;
    }
    if is_path_inside_any_autter_dir(path) {
        return true;
    }
