...
impl PiHookEvent {
    fn parse(value: &str) -> Result<Self, AutterError> {
        match value {
            ...
            other => Err(AutterError::PresetError(format!(
...
impl PiPreset {
    ...
    fn validate_tool_name(tool_name: &str) -> Result<(), AutterError> {
        match bash_tool::classify_tool(Agent::Pi, tool_name) {
            ToolClass::FileEdit | ToolClass::Bash => Ok(()),
            ToolClass::Skip => Err(AutterError::PresetError(format!(
