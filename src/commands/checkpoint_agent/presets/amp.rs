...
impl AmpPreset {
    ...
    fn amp_threads_dir() -> Result<PathBuf, AutterError> {
        if let Ok(test_path) = std::env::var("AUTTER_AMP_THREADS_PATH") {
...
impl AgentPreset for AmpPreset {
    fn parse(&self, hook_input: &str, trace_id: &str) -> Result<Vec<ParsedHookEvent>, AutterError> {
        ...
        if let Ok(threads_path) = std::env::var("AUTTER_AMP_THREADS_PATH")
