...
impl OpenCodePreset {
    ...
    fn resolve_stream_source(session_id: &str) -> Option<(StreamSource, PathBuf)> {
        let opencode_path = if let Ok(test_path) = std::env::var("AUTTER_OPENCODE_STORAGE_PATH") {
    ...
    fn opencode_data_path() -> Result<PathBuf, AutterError> {
        #[cfg(target_os = "macos")]
        {
...
impl AgentPreset for OpenCodePreset {
    fn parse(&self, hook_input: &str, trace_id: &str) -> Result<Vec<ParsedHookEvent>, AutterError> {
        ...
        if let Ok(test_path) = std::env::var("AUTTER_OPENCODE_STORAGE_PATH") {
