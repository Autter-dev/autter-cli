...
use crate::authorship::ignore::{
    default_ignore_patterns, load_autter_ignore_patterns_from_path,
...
///
/// Only stores entries for files that pass the autter ignore filter AND have
...
fn get_git_dir(repo_root: &Path) -> Result<PathBuf, AutterError> {
    let args = vec![
        "-C".to_string(),
...
/// Build the autter ignore ruleset for use in `filter_entry` on the snapshot walker.
///
/// Only covers the autter-specific patterns:
/// - Default ignore patterns (lock files, node_modules, etc.)
/// - Patterns from `.autter-ignore` at the repo root
...
pub fn build_gitignore(repo_root: &Path) -> Result<Gitignore, AutterError> {
    let mut builder = GitignoreBuilder::new(repo_root);

    // autter-specific patterns: same source of truth as non-bash checkpoints.
    let shared_patterns: Vec<String> = default_ignore_patterns()
        .into_iter()
        .chain(load_autter_ignore_patterns_from_path(repo_root))
    ...
    builder
        .build()
        .map_err(|e| AutterError::Generic(format!("Failed to build gitignore rules: {}", e)))
}
...
///
/// Only stores entries for files that pass the autter ignore filter (gitignore
/// + defaults + .autter-ignore + linguist) AND have `mtime > effective_wm + GRACE`.
///
/// Filtering is applied uniformly to all files — there is no special treatment
/// for git-tracked vs untracked files.
///
...
pub fn snapshot(
...
) -> Result<StatSnapshot, AutterError> {
    ...
    // Build the autter ignore ruleset: gitignore + defaults + .autter-ignore + linguist.
    ...
    // Pass the autter ignore ruleset directly into the walker via filter_entry.
    ...
    // catches the rest (defaults, .autter-ignore, linguist-generated).
    ...
    for result in walker {
        ...
        if elapsed >= walk_timeout {
            ...
            return Err(AutterError::Generic(msg));
        }
...
///
/// Both snapshots apply the same autter ignore filter at snapshot time, so
...
pub fn git_status_fallback(repo_root: &Path) -> Result<Vec<String>, AutterError> {
    let args = vec![
        "-C".to_string(),
        repo_root.to_string_lossy().into_owned(),
        "status".to_string(),
...
pub fn handle_bash_pre_tool_use_with_context(
    ...
    let socket = effective_daemon_socket().ok_or_else(|| {
        AutterError::Generic("no daemon socket available for BashSessionStart".into())
    })?;
...
pub fn handle_bash_post_tool_use(
    ...
    session_id: &str,
    tool_use_id: &str,
) -> Result<BashPostHookResult, AutterError> {
    let invocation_key = format!("{}:{}", session_id, tool_use_id);
...
mod tests {
    ...
    /// Patterns in .autter-ignore are respected, suppressing untracked files
    /// that aren't covered by .gitignore.
    #[test]
    fn test_build_gitignore_reads_autter_ignore() {
        let dir = tempfile::tempdir().unwrap();
        init_git_repo(dir.path());

        fs::write(dir.path().join(".autter-ignore"), "secrets/\n*.pem\n").unwrap();

        let gitignore = build_gitignore(dir.path()).unwrap();

        assert!(
            !should_include_new_file(&gitignore, Path::new("secrets/token.txt"), false),
            "secrets/ should be ignored via .autter-ignore"
        );
        assert!(
            !should_include_new_file(&gitignore, Path::new("server.pem"), false),
            "*.pem should be ignored via .autter-ignore"
        );
        assert!(
