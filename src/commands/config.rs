...
fn print_config_help() {
    println!("autter config - View and manage autter configuration");
    println!();
    println!("Usage:");
    println!("  autter config                Show all config as formatted JSON");
    println!("  autter config <key>          Show specific config value");
    println!("  autter config set <key> <value>          Set a config value");
    println!("  autter config set <key> <value> --add    Add to array (extends existing)");
    println!("  autter config --add <key> <value>        Add to array or upsert into object");
    println!("  autter config unset <key>    Remove config value (reverts to default)");
    println!();
    ...
    println!("  author.name                  autter author display name override");
    println!("  author.email                 autter author email override");
    ...
    println!("  autter_hooks                 Hook name -> shell commands map (object)");
    ...
    println!(
        "                               e.g. \"https://app.example.com/api/autter\" -> requests are"
    );
    ...
    println!();
    println!("Examples:");
    println!("  autter config exclude_repositories");
    println!("  autter config set disable_auto_updates true");
    println!("  autter config set author.name \"Alice Example\"");
    println!("  autter config set author.email alice@example.com");
    println!("  autter config set exclude_repositories \"private/*\"");
    println!("  autter config set exclude_repositories .         # Uses current repo's remotes");
    println!("  autter config --add exclude_repositories \"temp/*\"");
    println!("  autter config --add allow_repositories ~/projects/my-repo");
    println!("  autter config --add feature_flags.my_flag true");
    println!("  autter config --add autter_hooks.post_notes_updated \"./my-hook.sh\"");
    println!("  autter config set codex_hooks_format hooks_json");
    println!("  autter config unset exclude_repositories");
    println!();
    std::process::exit(0);
}
...
pub fn handle_config(args: &[String]) {
    ...
    match filtered_args[0].as_str() {
        "set" => {
            ...
            {
                println!("Run `autter bg restart` for changes to take effect.");
            }
        }
        "unset" => {
...
fn show_all_config() -> Result<(), String> {
    ...
    effective_config.insert(
        "autter_hooks".to_string(),
        serde_json::to_value(runtime_config.autter_hooks())
            .unwrap_or_else(|_| Value::Object(serde_json::Map::new())),
    );
...
fn get_config_value(key: &str) -> Result<(), String> {
    ...
    if key_path.len() == 1 {
        let value = match key_path[0].as_str() {
            ...
            "author" => serde_json::to_value(runtime_config.author())
                .unwrap_or_else(|_| Value::Object(serde_json::Map::new())),
            "autter_hooks" => serde_json::to_value(runtime_config.autter_hooks())
                .unwrap_or_else(|_| Value::Object(serde_json::Map::new())),
            "codex_hooks_format" => {
                Value::String(runtime_config.codex_hooks_format().as_str().to_string())
            }
    ...
    if key_path[0] == "feature_flags" || key_path[0] == "autter_hooks" {
    ...
    Err(
        "Nested keys are only supported for feature_flags, autter_hooks, notes_backend, and author"
            .to_string(),
    )
}
...
fn set_config_value(key: &str, value: &str, add_mode: bool) -> Result<(), String> {
    ...
    if key_path.len() == 1 {
        match key_path[0].as_str() {
            ...
            "autter_hooks" => {
                if add_mode {
                    return Err("Cannot use --add with autter_hooks at top level. Use dot notation: autter_hooks.post_notes_updated".to_string());
                }
                file_config.autter_hooks = Some(parse_autter_hooks_object(value)?);
                crate::config::save_file_config(&file_config)?;
                println!("[autter_hooks]: {}", value);
            }
            "codex_hooks_format" => {
    ...
    if key_path[0] == "autter_hooks" {
        if key_path.len() != 2 {
            return Err(
                "autter_hooks requires a hook name (e.g., autter_hooks.post_notes_updated)"
                    .to_string(),
            );
        }

        let hook_name = key_path[1].clone();
        let mut hooks = file_config.autter_hooks.unwrap_or_default();
        ...
        if add_mode {
            ...
            file_config.autter_hooks = Some(hooks);
            crate::config::save_file_config(&file_config)?;
...
fn unset_config_value(key: &str) -> Result<(), String> {
    ...
    if key_path.len() == 1 {
        match key_path[0].as_str() {
            ...
            "autter_hooks" => {
                let old_value = file_config.autter_hooks.take();
                crate::config::save_file_config(&file_config)?;
                if let Some(v) = old_value {
                    println!("- [autter_hooks]: {:?}", v);
                }
            }
            "codex_hooks_format" => {
    ...
    if key_path[0] == "autter_hooks" {
        ...
        file_config.autter_hooks = if hooks.is_empty() { None } else { Some(hooks) };
        crate::config::save_file_config(&file_config)?;
...
fn parse_autter_hooks_object(value: &str) -> Result<HashMap<String, Vec<String>>, String> {
    let parsed: Value =
        serde_json::from_str(value).map_err(|e| format!("Invalid JSON for autter_hooks: {}", e))?;
    let obj = parsed
        .as_object()
        .ok_or_else(|| "autter_hooks must be a JSON object".to_string())?;

    let mut hooks = HashMap::new();
    for (hook_name, commands_value) in obj {
        let name = hook_name.trim();
        if name.is_empty() {
            return Err("autter_hooks contains an empty hook name".to_string());
        }
...
fn parse_hook_commands_value(value: &Value) -> Result<Vec<String>, String> {
    match value {
        ...
        Value::Array(items) => {
            ...
            for item in items {
                let command = item.as_str().ok_or_else(|| {
                    "autter_hooks hook values must be a string or an array of strings".to_string()
                })?;
                let trimmed = command.trim();
                if trimmed.is_empty() {
                    return Err("Hook command cannot be empty".to_string());
        ...
        _ => Err("autter_hooks hook values must be a string or an array of strings".to_string()),
    }
}
...
mod tests {
    ...
    #[test]
    fn test_parse_autter_hooks_object() {
        let hooks =
            parse_autter_hooks_object(r#"{"post_notes_updated":["./hook-a.sh","./hook-b.sh"]}"#)
                .unwrap();
        assert_eq!(
