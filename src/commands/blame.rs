...
//🐰🥚 @todo use actual date Autter was installed in each repo
...
impl Repository {
    ...
    fn normalize_blame_file_path(&self, file_path: &str) -> Result<String, AutterError> {
        let repo_root = self.workdir().map_err(|e| {
            AutterError::Generic(format!("Repository has no working directory: {}", e))
        })?;
    ...
    fn effective_blame_options(options: &AutterBlameOptions) -> AutterBlameOptions {
    ...
    fn read_blame_file_content(
        ...
        if let Some(ref data) = options.contents_data {
            ...
            match tree.get_path(std::path::Path::new(relative_file_path)) {
                ...
                Err(_) => Err(AutterError::Generic(format!(
    ...
    fn prepare_blame_request(
        ...
        file_path: &str,
        options: &AutterBlameOptions,
    ) -> Result<PreparedBlameRequest, AutterError> {
    ...
    fn blame_requested_abbrev_len(options: &AutterBlameOptions, is_boundary: bool) -> usize {
    ...
    fn populate_hunk_abbrev_shas(&self, hunks: &mut [BlameHunk], options: &AutterBlameOptions) {
        if options.long_rev {
            for hunk in hunks {
    ...
    pub fn blame(
        ...
        file_path: &str,
        options: &AutterBlameOptions,
    ) -> Result<(HashMap<u32, String>, HashMap<String, PromptRecord>), AutterError> {
    ...
    pub fn blame_analysis(
        ...
        file_path: &str,
        options: &AutterBlameOptions,
    ) -> Result<BlameAnalysisResult, AutterError> {
        let request = self.prepare_blame_request(file_path, options)?;
    ...
    pub fn blame_hunks(
        ...
        end_line: u32,
        options: &AutterBlameOptions,
    ) -> Result<Vec<BlameHunk>, AutterError> {
...
fn output_json_format(
    ...
    let json_str = serde_json::to_string_pretty(&output)
        .map_err(|e| AutterError::Generic(format!("Failed to serialize JSON output: {}", e)))?;
...
pub fn parse_blame_args(args: &[String]) -> Result<(String, AutterBlameOptions), AutterError> {
    let mut options = AutterBlameOptions::default();
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-L" => {
                if i + 1 >= args.len() {
                    return Err(AutterError::Generic("Missing argument for -L".to_string()));
                }
            ...
            "--contents" => {
                ...
                let data = if contents_arg == "-" {
                    ...
                    io::stdin().read_to_end(&mut buffer).map_err(|e| {
                        AutterError::Generic(format!("Failed to read from stdin: {}", e))
                    })?;
                    buffer
                } else {
                    // Read from file
            ...
            "--since" => {
                ...
                options.oldest_date =
                    Some(DateTime::parse_from_rfc3339(&args[i + 1]).map_err(|e| {
                        AutterError::Generic(format!("Invalid date format for --since: {}", e))
                    })?);
                i += 2;
            }
            // JSON output format
            ...
            // Unknown option
            _ => {
                return Err(AutterError::Generic(format!("Unknown option: {}", args[i])));
            }
        }
    }

    let file_path =
        file_path.ok_or_else(|| AutterError::Generic("No file path specified".to_string()))?;
