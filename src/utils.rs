...
fn resolve_autter_exe_from_invocation_path(path: PathBuf) -> PathBuf {
    ...
    if file_name == git_name {
        let sibling = path.with_file_name(autter_name);
        if sibling.exists() {
            return sibling;
        }

        let canonical_sibling = canonical_path.with_file_name(autter_name);
        if canonical_sibling.exists() {
            return canonical_sibling;
        }

        return PathBuf::from(autter_name);
    }
    ...
    if crate::commands::git_hook_handlers::is_git_hook_binary_name(hook_candidate) {
        if canonical_path
            .file_name()
            .and_then(|n| n.to_str())
            .is_some_and(|name| name == autter_name)
        {
            return canonical_path;
        }
...
pub(crate) fn current_autter_exe() -> Result<PathBuf, AutterError> {
    let path = std::env::current_exe()?;
    Ok(resolve_autter_exe_from_invocation_path(path))
}

fn internal_autter_command_with_exe(exe: PathBuf, subcommand: &str) -> Command {
...
pub fn spawn_internal_autter_subcommand(
    subcommand: &str,
    extra_args: &[&str],
    guard_env: &str,
    extra_env: &[(&str, &str)],
    ...
    let Ok(exe) = current_autter_exe() else {
        return false;
    };
    let mut cmd = internal_autter_command_with_exe(exe, subcommand);
    cmd.args(extra_args);
...
pub fn is_superuser_expected_environment() -> bool {
    ...
    if std::env::var_os("AUTTER_DAEMON_UPGRADE").is_some() {
        return true;
    }
    false
}
...
/// via the `AUTTER_ALLOW_SUPERUSER` env var or `allow_superuser` config flag.
pub fn superuser_is_allowed() -> bool {
    std::env::var("AUTTER_ALLOW_SUPERUSER")
...
pub fn print_superuser_warning() {
    eprintln!(
        "[autter] warning: running as superuser (root/Administrator) is not recommended.\n\
         \n\
         ...
         - Run autter as your normal user (recommended), or\n\
         - Set AUTTER_ALLOW_SUPERUSER=1 or add \"allow_superuser\": true to ~/.autter/config.json\n\
         \n\
...
///
/// ```
/// use autter::utils::unescape_git_path;
///
...
mod tests {
    ...
    #[test]
    fn test_resolve_autter_exe_from_git_sibling_prefers_autter() {
        let dir = tempfile::tempdir().unwrap();
        let git = dir
            .path()
            .join(if cfg!(windows) { "git.exe" } else { "git" });
        let autter = dir.path().join(if cfg!(windows) {
            "autter.exe"
        } else {
            "autter"
        });
        std::fs::write(&git, "").unwrap();
        std::fs::write(&autter, "").unwrap();

        let resolved = resolve_autter_exe_from_invocation_path(git);
        assert_eq!(resolved, autter);
    }

    #[cfg(unix)]
    #[test]
    fn test_resolve_autter_exe_from_hook_symlink_uses_canonical_autter() {
        ...
        std::fs::write(&autter, "").unwrap();
        symlink(&autter, &hook).unwrap();

        let resolved = resolve_autter_exe_from_invocation_path(hook);
        assert_eq!(
            std::fs::canonicalize(resolved).unwrap(),
            std::fs::canonicalize(autter).unwrap()
        );
    }

    #[test]
    fn test_resolve_autter_exe_from_hook_without_target_falls_back_to_autter_name() {
        let dir = tempfile::tempdir().unwrap();
    ...
    #[cfg(windows)]
    #[test]
    fn test_resolve_autter_exe_from_hook_exe_name_falls_back_to_autter_exe() {
        let dir = tempfile::tempdir().unwrap();
        let hook = dir.path().join("pre-push.exe");
        std::fs::write(&hook, "").unwrap();

        let resolved = resolve_autter_exe_from_invocation_path(hook);
        assert_eq!(resolved, PathBuf::from("autter.exe"));
    }

    #[test]
    fn test_internal_autter_command_sets_skip_all_hooks_env() {
        let exe = PathBuf::from("/tmp/autter-test");
        let cmd = internal_autter_command_with_exe(exe.clone(), "status");
        ...
        assert!(
            ...
            "internal command must always set AUTTER_SKIP_ALL_HOOKS=1"
        );
    }

    #[test]
    fn test_spawn_internal_autter_subcommand_respects_guard_env() {
        let key = "AUTTER_TEST_WORKER_GUARD";
        unsafe {
            std::env::set_var(key, "1");
        }
        let spawned = spawn_internal_autter_subcommand("status", &[], key, &[]);
        unsafe {
    ...
    #[test]
    fn test_spawn_internal_autter_subcommand_requires_non_empty_guard_env() {
        let spawned = spawn_internal_autter_subcommand("status", &[], "", &[]);
    ...
    // =========================================================================
    // current_autter_exe Tests
    // =========================================================================

    #[test]
    fn test_current_autter_exe_returns_path() {
        // Should return a path (either current exe or autter)
        let result = current_autter_exe();
        assert!(result.is_ok(), "current_autter_exe should not fail");
    ...
    fn test_superuser_is_allowed_env_var() {
        let had_var = std::env::var_os("AUTTER_ALLOW_SUPERUSER");
        unsafe { std::env::set_var("AUTTER_ALLOW_SUPERUSER", "1") };
        assert!(superuser_is_allowed());
        unsafe { std::env::set_var("AUTTER_ALLOW_SUPERUSER", "true") };
        assert!(superuser_is_allowed());
        unsafe { std::env::set_var("AUTTER_ALLOW_SUPERUSER", "TRUE") };
        assert!(superuser_is_allowed());
        unsafe { std::env::set_var("AUTTER_ALLOW_SUPERUSER", "0") };
        assert!(!superuser_is_allowed());
        unsafe { std::env::remove_var("AUTTER_ALLOW_SUPERUSER") };
        assert!(!superuser_is_allowed());
        match had_var {
            Some(v) => unsafe { std::env::set_var("AUTTER_ALLOW_SUPERUSER", v) },
            None => unsafe { std::env::remove_var("AUTTER_ALLOW_SUPERUSER") },
        }
    }
