...
impl FeatureFlags {
    ...
    /// Reads from AUTTER_* prefixed environment variables
    /// Example: AUTTER_REWRITE_STASH=true, AUTTER_AUTH_KEYRING=false
    /// Falls back to defaults for any invalid or missing values
    #[allow(dead_code)]
    pub fn from_env() -> Self {
        let env_flags = DeserializableFeatureFlags::from_env("AUTTER_");
...
mod tests {
    ...
    fn test_from_env_and_file_defaults_only() {
        ...
        unsafe {
            std::env::remove_var("AUTTER_REWRITE_STASH");
            std::env::remove_var("AUTTER_AUTH_KEYRING");
        }
