...
impl MetricsDatabase {
    ...
    pub fn global() -> Result<&'static Mutex<MetricsDatabase>, AutterError> {
        let db_mutex = METRICS_DB.get_or_init(|| {
            match Self::new() {
                ...
                Err(e) => {
                    ...
                    let temp_path = std::env::temp_dir().join("autter-metrics-db-failed");
    ...
    fn database_path() -> Result<PathBuf, AutterError> {
        ...
        if let Ok(test_path) = std::env::var("AUTTER_TEST_METRICS_DB_PATH") {
    ...
    pub fn insert_events(&mut self, events: &[String]) -> Result<(), AutterError> {
        if events.is_empty() {
            return Ok(());
        }
    ...
    pub fn get_batch(&self, limit: usize) -> Result<Vec<MetricRecord>, AutterError> {
        let mut stmt = self
            .conn
    ...
    pub fn count(&self) -> Result<usize, AutterError> {
        let count: i64 = self
            .conn
