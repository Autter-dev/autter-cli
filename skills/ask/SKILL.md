...
**If you want to read an entire file or range of lines AND the corresponding prompts behind them, use `autter blame --show-prompt`.** This is better than `search` for this use case — it gives you every line's authorship plus the full prompt JSON in one call.

```
# Get blame + prompts for a line range (pipe to get prompt dump appended):
autter blame src/commands/blame.rs -L 23,54 --show-prompt | cat
...
Use `autter search` when you need to find prompts by **commit**, **keyword**, or when you don't have a specific file/line range in mind.
...
STEP 1 — Search (one command):
  Run: autter search --file {file_path} --lines {start}-{end} --verbose
  If no results, try ONE fallback: autter search --file {file_path} --verbose
  That's it. Do not run more than 2 autter commands total.
...
- The code might be human-written or predate autter
