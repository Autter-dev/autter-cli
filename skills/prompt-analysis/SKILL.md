...
allowed-tools: ["Bash(autter:*)", "Read", "Glob", "Grep", "Task"]
---
...
Autter is a tool that tracks AI-generated code and prompts in git. It stores:
...
Discovery is notes-only — `autter prompts` always operates on the current repository (the working directory must be inside a git repo). To analyze multiple repos, run the command separately in each.

Run initialization:
```bash
autter prompts [flags]
```
...
```bash
autter prompts exec "SELECT model, AVG(accepted_rate), COUNT(*) FROM prompts GROUP BY model"
```
...
```bash
autter prompts exec "ALTER TABLE prompts ADD COLUMN work_type TEXT"
autter prompts exec "ALTER TABLE prompts ADD COLUMN analysis_notes TEXT"
```

2. **Reset the iteration pointer**:
```bash
autter prompts reset
```

3. **Iterate with subagents** - Launch parallel subagents using Task tool with `subagent_type: "general-purpose"`. Each subagent:
   - Runs `autter prompts next` to get one prompt as JSON
   - Analyzes the `messages` content
   - Updates the database: `autter prompts exec "UPDATE prompts SET work_type='...' WHERE id='...'"`

4. **Final synthesis** - Query the enriched data:
```bash
autter prompts exec "SELECT work_type, COUNT(*), AVG(accepted_rate) FROM prompts GROUP BY work_type"
```
...
```
Run `autter prompts next` to get the next prompt.

Analyze the messages JSON to determine: [specific analysis task]

Then update the database:
autter prompts exec "UPDATE prompts SET [column]='[value]' WHERE id='[prompt_id]'"

Return your analysis result.
```

Spawn 3-5 subagents at a time, check results, spawn more until `autter prompts next` returns "No more prompts."

**IMPORTANT:** The `autter prompts next` command returns ALL the data needed for analysis as JSON, including:
...
Subagents should NOT run additional commands like `git show` or `git log` - everything needed is in the JSON output from `autter prompts next`. Instruct subagents explicitly:

```
IMPORTANT: All data you need is in the JSON output from `autter prompts next`.
...
**Subagent prompt:**
```
Run `autter prompts next` to get the next prompt as JSON.
...
Update the database:
autter prompts exec "UPDATE prompts SET work_type='<category>' WHERE id='<prompt_id>'"
...
**Setup:**
```bash
autter prompts exec "ALTER TABLE prompts ADD COLUMN low_acceptance_reason TEXT"
autter prompts exec "UPDATE pointers SET current_seq_id = (SELECT MIN(seq_id) - 1 FROM prompts WHERE accepted_rate < 0.5 AND accepted_rate IS NOT NULL)"
```
...
Update the database:
autter prompts exec "UPDATE prompts SET low_acceptance_reason='<reason>' WHERE id='<prompt_id>'"
...
**Setup:**
```bash
autter prompts exec "ALTER TABLE prompts ADD COLUMN reusable_pattern TEXT"
autter prompts exec "ALTER TABLE prompts ADD COLUMN pattern_description TEXT"
autter prompts reset
```
...
autter prompts exec "UPDATE prompts SET reusable_pattern='<name>', pattern_description='<desc>' WHERE id='<prompt_id>'"
...
**Setup:**
```bash
autter prompts exec "ALTER TABLE prompts ADD COLUMN clarity_score INTEGER"
autter prompts exec "ALTER TABLE prompts ADD COLUMN clarity_feedback TEXT"
autter prompts reset
```
...
autter prompts exec "UPDATE prompts SET clarity_score=<1-5>, clarity_feedback='<feedback>' WHERE id='<prompt_id>'"
...
**Setup:**
```bash
autter prompts exec "ALTER TABLE prompts ADD COLUMN technique TEXT"
autter prompts exec "ALTER TABLE prompts ADD COLUMN technique_notes TEXT"
autter prompts reset
```
...
autter prompts exec "UPDATE prompts SET technique='<technique>', technique_notes='<notes>' WHERE id='<prompt_id>'"
...
**Setup:**
```bash
autter prompts exec "ALTER TABLE prompts ADD COLUMN failure_analysis TEXT"
autter prompts exec "ALTER TABLE prompts ADD COLUMN failure_category TEXT"
autter prompts exec "ALTER TABLE prompts ADD COLUMN improvement_suggestion TEXT"
-- Set pointer to iterate only over failed prompts
autter prompts exec "DELETE FROM pointers WHERE name='default'"
autter prompts exec "INSERT INTO pointers (name, current_seq_id) VALUES ('default', (SELECT MIN(seq_id) - 1 FROM prompts WHERE accepted_lines IS NULL OR accepted_lines = 0))"
```
...
autter prompts exec "UPDATE prompts SET failure_category='<category>', failure_analysis='<analysis>', improvement_suggestion='<suggestion>' WHERE id='<prompt_id>'"
...
**Setup:**
```bash
autter prompts exec "ALTER TABLE prompts ADD COLUMN grade TEXT"
autter prompts exec "ALTER TABLE prompts ADD COLUMN grade_breakdown TEXT"
autter prompts exec "ALTER TABLE prompts ADD COLUMN grade_feedback TEXT"
autter prompts reset
```
...
autter prompts exec "UPDATE prompts SET grade='<A-F>', grade_breakdown='<json>', grade_feedback='<specific feedback>' WHERE id='<prompt_id>'"
...
Autter may not be set up correctly in this repository. Direct the user to:
- Check https://autter.dev/docs/cli for setup instructions
- Verify `autter` is installed and configured
- Ensure they have made commits with AI assistance after setup

**Important: Autter only tracks data going forward.**
Autter cannot retroactively categorize or track prompts from before it was installed. Only commits made after Autter setup will have prompt data. If the user recently installed Autter, they may have limited data until they accumulate more AI-assisted commits.
...
To avoid being prompted for every `autter` command, add to project or user settings:
