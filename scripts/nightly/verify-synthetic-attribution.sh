...
#   6. autter stats HEAD --json reports ai_additions > 0
#   7. Test file (agent-test-<agent>.txt) appears in raw note text (WARN)
#   8. autter blame shows AI attribution markers (WARN)
#
...
git notes --ref=ai show HEAD > "$NOTE_RAW" 2>/dev/null \
  || fail "No authorship note on HEAD — post-commit hook did not fire (autter hooks may not be wired correctly in repo)"
...
" "$META_JSON")
[ "$SESSION_COUNT" -gt 0 ] \
  || fail "No prompt/session entries in authorship note — synthetic checkpoint data was not stored (check autter checkpoint pipeline)"
...
# ── 6. autter stats reports AI additions ──────────────────────────────────────
# Capture output separately so pipefail doesn't trip on grep finding no DEBUG lines
STATS_RAW=$(autter stats HEAD --json 2>/dev/null) \
  || fail "autter stats HEAD --json command failed"
...
[ "$AI_ADDS" -gt 0 ] \
  || fail "autter stats HEAD reports ai_additions=0 — AI work not tracked in stats (checkpoint data may not have been linked to this commit)"

pass "autter stats HEAD: ai_additions=$AI_ADDS"
...
# ── 8. autter blame shows AI attribution ──────────────────────────────────────
if autter blame "$TEST_FILE" > "$BLAME_OUT" 2>/dev/null; then
  if grep -qiE "ai-generated|${AGENT}|generated|ai_human_author" "$BLAME_OUT" 2>/dev/null; then
    pass "AI attribution visible in autter blame output for $TEST_FILE"
  else
    warn "autter blame does not show explicit AI attribution for $TEST_FILE — agent_id may not be resolved in blame display"
  fi
else
  warn "autter blame command failed for $TEST_FILE — blame verification skipped"
fi
