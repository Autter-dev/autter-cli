#!/usr/bin/env bash
# Verify that autter installed hooks correctly for the given agent.
...
case "$AGENT" in
  ...
  opencode)
    PLUGIN="$HOME/.config/opencode/plugins/autter.ts"
    [ -f "$PLUGIN" ] || fail "OpenCode plugin not found at $PLUGIN"
    pass "OpenCode autter plugin installed at $PLUGIN"
    ;;
