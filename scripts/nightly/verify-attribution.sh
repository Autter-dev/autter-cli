#!/usr/bin/env bash
# Deep end-to-end verification of autter attribution after a live agent run.
#
# Checks (in order):
...
#   9. autter blame shows AI attribution on fibonacci lines
#
...
git notes --ref=ai show HEAD > "$NOTE_RAW" 2>/dev/null \
  || fail "No authorship note on HEAD — post-commit hook did not fire (autter hooks may not be wired correctly)"
...
# ── 10. autter blame ──────────────────────────────────────────────────────────
if autter blame utils/math_utils.py > "$BLAME_OUT" 2>/dev/null; then
  if grep -q "fibonacci" "$BLAME_OUT" 2>/dev/null; then
    pass "autter blame output covers fibonacci function lines"
    ...
    if grep -qiE "ai-generated|${AGENT}|generated" "$BLAME_OUT" 2>/dev/null; then
      pass "AI attribution visible in autter blame output for fibonacci lines"
    else
      warn "autter blame does not show explicit AI attribution for fibonacci lines (agent_id may not be present in note)"
    fi
  else
    warn "fibonacci lines not visible in autter blame output — file may not be tracked yet"
  fi
else
  warn "autter blame command failed — blame verification skipped"
fi
