...
echo "$CHECKPOINT_JSON" | autter checkpoint agent-v1 --hook-input stdin \
  || fail "autter checkpoint agent-v1 command failed"
pass "Synthetic checkpoint accepted by autter"
...
if autter blame "$TEST_FILE" 2>/dev/null | grep -qiE "$AGENT|ai|attribution"; then
