#!/usr/bin/env python3
"""
Reproduce runaway memory patterns in autter.
...
It generates synthetic repos, runs autter commands, and samples peak RSS via `ps`.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Reproduce autter runaway memory behavior.",
    )
    parser.add_argument(
        "--autter-bin",
        default="target/debug/autter",
        help="Path to autter binary (default: target/debug/autter).",
    )
    parser.add_argument(
        "--scenario",
    ...
    parser.add_argument(
        "--work-root",
        default="/tmp/autter-memory-repro",
    ...
    autter_bin = Path(args.autter_bin).expanduser().resolve()
    if not autter_bin.exists():
        print(
            f"autter binary not found at {autter_bin}. "
            "Build first (example: cargo build --bin autter).",
            file=sys.stderr,
        )
        return 2
