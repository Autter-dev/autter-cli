...
usage() {
  cat <<'EOF'
Benchmark autter rebase/rewrite performance with large synthetic commit stacks on top of an OSS repo.
...
Options:
  ...
  --work-root <path>           Working directory (default: /tmp/autter-nasty-rebase-<timestamp>)
  ...
  --git-bin <path>             Git binary to use (default: wrapper next to autter, else PATH git)
  --autter-bin <path>          autter binary (default: PATH autter)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --autter-bin) AUTTER_BIN="$2"; shift 2 ;;
...
if [[ -z "$AUTTER_BIN" ]]; then
  AUTTER_BIN="$(command -v autter || true)"
fi

if [[ -z "$AUTTER_BIN" ]]; then
  echo "error: autter not found in PATH" >&2
  exit 1
fi

if [[ -z "$GIT_BIN" ]]; then
  CANDIDATE_WRAP_GIT="$(dirname "$AUTTER_BIN")/git"
...
if [[ -z "$WORK_ROOT" ]]; then
  WORK_ROOT="${TMPDIR:-/tmp}/autter-nasty-rebase-$(date +%Y%m%d-%H%M%S)"
fi
...
cleanup() {
  if [[ "$HOOK_MODE" == "daemon" && "$DAEMON_STARTED" -eq 1 ]]; then
    (
      ...
      AUTTER_DEBUG=0 AUTTER_DEBUG_PERFORMANCE=0 "$AUTTER_BIN" daemon shutdown >/dev/null 2>&1 || true
    )
  fi
}
trap cleanup EXIT
...
if [[ "$HOOK_MODE" == "daemon" ]]; then
  if [[ -z "${AUTTER_DAEMON_CONTROL_SOCKET:-}" ]]; then
    export AUTTER_DAEMON_CONTROL_SOCKET="$HOME/.autter/internal/daemon/control.sock"
  fi
  if [[ -z "${GIT_TRACE2_EVENT:-}" ]]; then
    export GIT_TRACE2_EVENT="af_unix:stream:$HOME/.autter/internal/daemon/trace2.sock"
  fi
  if [[ -z "${GIT_TRACE2_EVENT_NESTING:-}" ]]; then
    export GIT_TRACE2_EVENT_NESTING="10"
  fi
  export AUTTER_DAEMON_CHECKPOINT_DELEGATE="true"

  if ! AUTTER_DEBUG=0 AUTTER_DEBUG_PERFORMANCE=0 "$AUTTER_BIN" daemon start >/dev/null 2>&1; then
    echo "error: failed to start autter daemon for benchmark mode" >&2
    exit 1
  fi
  DAEMON_STARTED=1
fi
...
g() {
  AUTTER_DEBUG=0 AUTTER_DEBUG_PERFORMANCE=0 "$GIT_BIN" -C "$REPO_DIR" "$@"
}
...
run_ai_checkpoint() {
  if [[ "$HOOK_MODE" == "daemon" ]]; then
    (
      cd "$REPO_DIR"
      AUTTER_DEBUG=0 AUTTER_DEBUG_PERFORMANCE=0 \
      AUTTER_DAEMON_CHECKPOINT_DELEGATE=true \
      AUTTER_DAEMON_CONTROL_SOCKET="$AUTTER_DAEMON_CONTROL_SOCKET" \
      "$AUTTER_BIN" checkpoint mock_ai >/dev/null
    )
  else
    (
      cd "$REPO_DIR"
      AUTTER_DEBUG=0 AUTTER_DEBUG_PERFORMANCE=0 "$AUTTER_BIN" checkpoint mock_ai >/dev/null
    )
  fi
}
...
run_rebase_scenario() {
  ...
  local duration_s
  start_ns="$(now_ns)"
  if AUTTER_DEBUG=1 AUTTER_DEBUG_PERFORMANCE=1 "$GIT_BIN" -C "$REPO_DIR" "$@" >"$log_file" 2>&1; then
    status="ok"
  else
    status="fail"
  fi
...
echo "=== autter nasty rebase benchmark ==="
echo "repo_url=$REPO_URL"
echo "work_root=$WORK_ROOT"
echo "repo_dir=$REPO_DIR"
echo "git_bin=$GIT_BIN"
echo "autter_bin=$AUTTER_BIN"
...
g config user.name "autter bench"
g config user.email "bench@autter.local"
...
{
  echo "autter nasty rebase benchmark summary"
  echo "repo_url: $REPO_URL"
  echo "repo_dir: $REPO_DIR"
  echo "default_branch: $DEFAULT_BRANCH"
