#!/usr/bin/env python3
"""
Benchmark autter execution modes against main-branch wrapper mode.
...
def build_release_binary(
    ...
    run_cmd(
        ["cargo", "build", "--release", "--bin", "autter"],
        cwd=repo_dir,
        env=env,
        timeout_s=3600,
    )
    if os.name == "nt":
        binary = target_dir / "release" / "autter.exe"
    else:
        binary = target_dir / "release" / "autter"
...
def resolve_real_git_binary(repo_root: Path) -> Path:
    ...
    if (
        "autter" in fallback_path.name.lower()
        or str(repo_root / "target") in str(fallback_path)
    ):
        raise BenchmarkError(
            "Resolved `git` points to a autter wrapper, not the real git binary. "
...
class VariantRunner:
    def __init__(
        ...
        self.base_env["AUTTER_DEBUG"] = "0"
        self.base_env["AUTTER_DEBUG_PERFORMANCE"] = "0"
        ...
        self.daemon_socket_dir = self.home_dir / ".autter" / "internal" / "daemon"
        ...
        if self.variant.mode == "daemon":
            ...
            self.base_env["GIT_TRACE2_EVENT_NESTING"] = os.environ.get(
                "AUTTER_TEST_TRACE2_NESTING",
                "10",
            )
            self.base_env["AUTTER_DAEMON_CHECKPOINT_DELEGATE"] = "true"
            self.base_env["AUTTER_DAEMON_CONTROL_SOCKET"] = str(
    ...
    def run_autter(self, args: list[str], cwd: Path) -> subprocess.CompletedProcess[str]:
        return run_cmd(
    ...
    def init_repo(self, repo_dir: Path) -> None:
        ...
        self.run_git(["config", "user.email", "benchmark@autter.local"], cwd=repo_dir)


    def checkpoint_mock_ai(self, repo_dir: Path, files: list[str]) -> None:
        if not files:
            return
        self.run_autter(["checkpoint", "mock_ai", *files], cwd=repo_dir)
...
def render_report(
    ...
    lines: list[str] = []
    lines.append("# autter Mode Benchmark Report")
    lines.append("")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Use an existing current-branch autter binary (skip current build).",
    )
    parser.add_argument(
        "--main-bin",
        type=Path,
        default=None,
        help="Use an existing main-branch autter binary (skip main build/worktree).",
    )
    parser.add_argument(
        "--margin-pct",
        type=float,
...
def main() -> int:
    ...
    if args.work_root is None:
        work_root = Path(tempfile.mkdtemp(prefix="autter-modes-bench-"))
    else:
