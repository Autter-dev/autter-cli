...
def setup_variant_runtime(
    ...
    env["AUTTER_DEBUG"] = "0"
    env["AUTTER_DEBUG_PERFORMANCE"] = "0"
    env["PATH"] = f"{bin_dir}{os.pathsep}{env.get('PATH', '')}"

    daemon_proc: subprocess.Popen[str] | None = None
    if variant.mode == "daemon":
        daemon_dir = home_dir / ".autter" / "internal" / "daemon"
        ...
        env["AUTTER_DAEMON_CONTROL_SOCKET"] = str(control_socket)
...
def render_report(
    ...
    lines: list[str] = []
    lines.append("# autter Nasty Rebase Benchmark (Modes vs main)")
    lines.append("")
...
def main() -> int:
    ...
    if args.work_root is None:
        work_root = Path(tempfile.mkdtemp(prefix="autter-nasty-modes-"))
    else:
