...
4) running `autter checkpoint` (human) and measuring duration
"""
...
def resolve_autter_bin(repo_root: Path, explicit: str | None) -> Path:
    ...
    candidates = [
        repo_root / "target" / "release" / "autter",
        repo_root / "target" / "debug" / "autter",
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate

    print("Building autter binary (cargo build --bin autter)...")
    run(["cargo", "build", "--quiet", "--bin", "autter"], cwd=repo_root)
    built = repo_root / "target" / "debug" / "autter"
    if not built.exists():
        raise FileNotFoundError(
            f"Expected built binary not found at {built}; pass --autter-bin explicitly."
        )
    return built
...
def seed_ai_history(
    ...
    cmd = [str(autter_bin), "checkpoint", "mock_ai", "--", *ai_files]
...
def run_scenario(
    *,
    ...
    tmp_root = Path(
        tempfile.mkdtemp(prefix="autter-human-non-ai-checkpoint-", dir=str(tmp_parent))
    )
    ...
    try:
        ...
        for changed in changed_counts:
            ...
            for i in range(1, repeats + 1):
                ...
                seed_ai_history(repo_dir, autter_bin, ai_seed_files, base_env)
...
def main() -> None:
    ...
    parser.add_argument(
        "--autter-bin",
        default=None,
        help="Path to autter binary. If omitted, tries target/{release,debug}/autter.",
    )
    parser.add_argument(
        "--total-files",
        type=int,
