...
4) Stage all changes and run `autter commit`.
"""
...
def resolve_autter_bin(repo_root: Path, explicit: str | None) -> Path:
    if explicit:
        path = Path(explicit).expanduser()
        if not path.exists():
            raise FileNotFoundError(f"--autter-bin does not exist: {path}")
        return path

    local_debug = repo_root / "target" / "debug" / "autter"
    if local_debug.exists():
        return local_debug

    print("Building local debug binary (cargo build --bin autter)...")
    run(["cargo", "build", "--quiet", "--bin", "autter"], cwd=repo_root)
...
def parse_commit_perf_json(output: str) -> tuple[int | None, int | None, int | None, int | None]:
    ...
    for line in output.splitlines():
        if "[autter (perf-json)]" not in line:
            continue
...
def verify_binary_modes(autter_bin: Path, repo_root: Path) -> None:
    # autter mode
    autter_version = run([str(autter_bin), "--version"], cwd=repo_root).stdout.strip()

    # git-wrapper mode (debug-only escape hatch)
    git_version = run(
        [str(autter_bin), "--version"],
        cwd=repo_root,
        env={**dict(os.environ), "AUTTER": "git"},
    ).stdout.strip()

    if "git version " not in git_version:
        raise RuntimeError(
            "Local binary did not enter git-wrapper mode with AUTTER=git.\n"
            f"binary: {autter_bin}\n"
            f"autter mode output: {autter_version}\n"
            f"wrapper mode output: {git_version}\n"
        )

    print(f"preflight autter mode: {autter_version}")
...
def setup_template_repo(
    ...
    if ai_seed:
        ...
        run(
            [str(autter_bin), "checkpoint", "mock_ai", "--", *ai_seed],
            cwd=template_repo,
            env=base_env,
        )
        run(["git", "add", "."], cwd=template_repo)
        commit_env = {**base_env, "AUTTER": "git"}
        run(
            [str(autter_bin), "commit", "-m", "seed ai baseline"],
            cwd=template_repo,
            env=commit_env,
        )
...
def modify_files_for_run(
    ...
    *,
    ...
    if ai_files:
        run(
            [str(autter_bin), "checkpoint", "mock_ai", "--", *ai_files],
            cwd=run_repo,
            env=base_env,
        )
...
def benchmark_commit_once(
    ...
    proc = subprocess.run(
        [str(autter_bin), "commit", "-m", msg],
        cwd=str(run_repo),
        env={**perf_env, "AUTTER": "git"},
        text=True,
        check=True,
        capture_output=True,
    )
...
def run_scenario(
    *,
    ...
    root = Path(tempfile.mkdtemp(prefix="autter-commit-mostly-human-", dir=str(tmp_parent)))
    template_repo = root / "template"
    template_repo.mkdir(parents=True, exist_ok=True)

    base_env = dict(os.environ)
    perf_env = {**base_env, "AUTTER_DEBUG_PERFORMANCE": "2"}

    try:
        setup_template_repo(template_repo, autter_bin, total_files, ai_seed_files, base_env)
        ...
        for changed in changed_counts:
            for i in range(1, repeats + 1):
                ...
                modify_files_for_run(
                    ...
                    autter_bin=autter_bin,
                    base_env=base_env,
                )

                result = benchmark_commit_once(run_repo, autter_bin, changed, i, perf_env)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark `autter commit` with thousands of changed files, mostly human, "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to autter binary. Defaults to this repo's local debug binary "
            "(target/debug/autter)."
        ),
    )
    ...
    autter_bin = resolve_autter_bin(repo_root, args.autter_bin)
    changed_counts = parse_counts(args.changed_counts)

    print(f"autter binary: {autter_bin}")
    verify_binary_modes(autter_bin, repo_root)
    print(
