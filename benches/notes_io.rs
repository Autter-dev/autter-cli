...
//!
//!   - git_notes write  : `autter::git::refs::notes_add`
//!   - git_notes read   : `autter::git::refs::show_authorship_note`
//!   - git_notes batch  : `autter::git::refs::notes_add_batch`
//!   - git_notes commits_with_notes: `autter::git::refs::commits_with_authorship_notes`
...
//!
//! `NotesDatabase::global()` uses a `OnceLock`. We set `AUTTER_TEST_NOTES_DB_PATH`
...
//! iterations. `TmpRepo` owns a `tempfile::TempDir` and a `autter::git::Repository`
...
fn ensure_notes_db_initialized() {
    NOTES_DB_TMPFILE.get_or_init(|| {
        ...
        unsafe {
            std::env::set_var("AUTTER_TEST_NOTES_DB_PATH", &path);
            // Avoid polluting the production internal DB.
            std::env::set_var(
                "AUTTER_TEST_DB_PATH",
                std::env::temp_dir().join("autter-bench-internal-db"),
            );
        }
        path
    });
...
impl BenchRepo {
    fn new(count: usize) -> Self {
        ...
        for sha in &shas {
            autter::git::refs::notes_add(repo.autter_repo(), sha, NOTE_CONTENT)
...
fn bench_write_single(c: &mut Criterion) {
    ...
    group.bench_with_input(
        ...
        |b, sha| {
            b.iter(|| {
                autter::git::refs::notes_add(bench.repo.autter_repo(), sha, NOTE_CONTENT)
...
fn bench_write_batch_100(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("git_notes", "100_notes"), |b| {
        b.iter(|| {
            autter::git::refs::notes_add_batch(bench.repo.autter_repo(), &batch)
...
fn bench_read_single_hot(c: &mut Criterion) {
    ...
    group.bench_with_input(
        ...
        |b, sha| {
            b.iter(|| {
                let _ = autter::git::refs::show_authorship_note(bench.repo.autter_repo(), sha);
            });
        },
    );
...
fn bench_commits_with_notes_500(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("git_notes", "500_shas"), |b| {
        b.iter(|| {
            let _ = autter::git::refs::commits_with_authorship_notes(
                bench.repo.autter_repo(),
                &check_shas,
            )
...
// of git. To benchmark the true http path, set AUTTER_NOTES_BACKEND_KIND=http
// and rebuild.
// ---------------------------------------------------------------------------
