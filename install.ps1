...
function Start-DaemonIfRequested {
    if ($env:AUTTER_RESTART_DAEMON_AFTER_INSTALL -ne '1') {
        return
    }

    $daemonExe = Join-Path $HOME '.autter\bin\autter.exe'
    if (-not (Test-Path $daemonExe)) {
        Write-Warning 'Warning: Failed to locate autter.exe for daemon restart after install.'
        return
    }
    ...
    try {
        ...
        Write-Warning 'Warning: Failed to restart autter background service automatically.'
    }
}
...
function Stop-AutterBackgroundService {
    param(
        [Parameter(Mandatory = $true)][string]$AutterExe,
        [Parameter(Mandatory = $false)][switch]$Hard
    )

    if (-not (Test-Path -LiteralPath $AutterExe)) {
        return $false
    }
...
function Get-AutterManagedProcesses {
    param(
        [Parameter(Mandatory = $true)][string]$InstallDir
    )

    $targetPaths = @(
        (Normalize-PathString (Join-Path $InstallDir 'autter.exe')),
...
function Stop-AutterManagedProcesses {
    param(
        [Parameter(Mandatory = $true)][string]$InstallDir
    )

    $processes = @(Get-AutterManagedProcesses -InstallDir $InstallDir)
    if ($processes.Count -eq 0) {
        return $false
    }

    $pids = @($processes | Sort-Object ProcessId -Unique | Select-Object -ExpandProperty ProcessId)
    Write-Warning ("Stopping lingering autter processes: {0}" -f ($pids -join ', '))
...
function Wait-ForFileAvailable {
    ...
    [void](Stop-AutterBackgroundService -AutterExe $autterExe)
    ...
    while ($elapsed -lt $MaxWaitSeconds) {
        ...
        if ($elapsed -ge $ForceKillAfterSeconds) {
            [void](Stop-AutterBackgroundService -AutterExe $autterExe -Hard)
            [void](Stop-AutterManagedProcesses -InstallDir $InstallDir)
        }
...
} elseif (-not [string]::IsNullOrWhiteSpace($env:AUTTER_RELEASE_TAG) -and $env:AUTTER_RELEASE_TAG -ne 'latest') {
    # Environment variable override
    $releaseTag = $env:AUTTER_RELEASE_TAG
    $downloadUrlExe = "https://github.com/$Repo/releases/download/$releaseTag/$binaryName.exe"
    $downloadUrlNoExt = "https://github.com/$Repo/releases/download/$releaseTag/$binaryName"
} else {
    # Default to latest
...
public static class AutterElevation {
...
"@ -ErrorAction SilentlyContinue
    $isElevated = [AutterElevation]::IsElevated()
} catch { }

if ($isElevated -and $env:AUTTER_ALLOW_SUPERUSER -ne '1') {
    ...
    $isCi = $env:CI -or $env:GITHUB_ACTIONS -or $env:GITLAB_CI -or $env:JENKINS_URL `
        ...
        -or $env:AUTTER_DAEMON_UPGRADE -or $env:container

    if (-not $isCi) {
        Write-Host ''
        Write-Host 'Warning: installing autter as Administrator is not recommended.' -ForegroundColor Yellow
        Write-Host ''
        ...
        Write-Host '  - Set $env:AUTTER_ALLOW_SUPERUSER = "1"' -ForegroundColor Yellow
        Write-Host ''
    }
    # Propagate to child autter invocations (install-hooks, exchange-nonce, login)
    $env:AUTTER_ALLOW_SUPERUSER = '1'
}

# Install directory: %USERPROFILE%\.autter\bin
...
if (-not [string]::IsNullOrWhiteSpace($env:AUTTER_LOCAL_BINARY)) {
    if (-not (Test-Path -LiteralPath $env:AUTTER_LOCAL_BINARY)) {
        Remove-Item -Force -ErrorAction SilentlyContinue $tmpFile
        Write-ErrorAndExit "Local binary not found at $($env:AUTTER_LOCAL_BINARY)"
    }
    Copy-Item -Force -Path $env:AUTTER_LOCAL_BINARY -Destination $tmpFile
...
# Wait for autter.exe to be available if it exists and is in use
if (Test-Path -LiteralPath $finalExe) {
    if (-not (Wait-ForFileAvailable -Path $finalExe -InstallDir $installDir -MaxWaitSeconds 300 -RetryIntervalSeconds 5)) {
        Remove-Item -Force -ErrorAction SilentlyContinue $tmpFile
        Write-ErrorAndExit "Timeout waiting for $finalExe to be available. Please close any running autter processes and try again."
    }
}
...
$skipPathUpdate = $env:AUTTER_SKIP_PATH_UPDATE -eq '1'
if ($skipPathUpdate) {
    Write-Warning 'Skipping PATH updates because AUTTER_SKIP_PATH_UPDATE=1'
...
if ($pathUpdate.UserStatus -eq 'Updated') {
    Write-Success 'Successfully added autter to the user PATH.'
} elseif ($pathUpdate.UserStatus -eq 'AlreadyPresent') {
    Write-Success 'autter already present in the user PATH.'
...
# the Windows PATH entry we set above. Writing to ~/.bashrc ensures autter's
...
try {
    ...
    $pathCmd = 'export PATH="$HOME/.autter/bin:$PATH"'
    $markerString = '.autter/bin'
    ...
    if ($gitBashInstalled) {
        ...
        if ($alreadyPresent) {
            ...
            $appendContent = "`n# Added by autter installer on $timestamp`n$pathCmd`n"
